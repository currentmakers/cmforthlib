...
$00000080 constant CEC_CEC_CFGR_BRDNOGEN                            \ Avoid Error-Bit Generation in Broadcast The BRDNOGEN bit is set and cleared by software.
$00000100 constant CEC_CEC_CFGR_SFTOPT                              \ SFT Option Bit The SFTOPT bit is set and cleared by software.
...
$000000ff constant CEC_CEC_TXDR_TXD                                 \ Tx Data register. TXD is a write-only register containing the data byte to be transmitted. Note: TXD must be written when TXSTART=1
...
$000000ff constant CEC_CEC_RXDR_RXD                                 \ Rx Data register. RXD is read-only and contains the last data byte which has been received from the CEC line.
...
$40006c04 constant CEC_CEC_CFGR   \ offset: 0x04 : This register is used to configure the HDMI-CEC controller. It is mandatory to write CEC_CFGR only when CECEN=0.
