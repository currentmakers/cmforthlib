...
[ifndef] PSSI_DEF
  ...
  \ @brief Parallel synchronous slave interface
  \
  $00 constant PSSI_PSSI_CR             \ Parallel data clock polarity This bit configures the capture edge of the parallel clock or the edge used for driving outputs, depending on OUTEN.
  $04 constant PSSI_PSSI_SR             \ FIFO is ready to transfer four bytes
  $08 constant PSSI_PSSI_RIS            \ PSSI raw interrupt status register
  $0C constant PSSI_PSSI_IER            \ Data buffer overrun/underrun interrupt enable
