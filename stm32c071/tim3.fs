...
[ifndef] TIM3_DEF
  ...
  [ifdef] TIM3_TIM3_SMCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_SMS                       \ [0x00 : 3] SMS[2:0]: Slave mode selection When external signals are selected the active edge of the trigger signal (TRGI) is linked to the polarity selected on the external input (see Input Control register and Control Register description. reinitializes the counter, generates an update of the registers and starts the counter. Note: The gated mode must not be used if TI1F_ED is selected as the trigger input (TS=00100). Indeed, TI1F_ED outputs 1 pulse for each transition on TI1F, whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to receive events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly while triggers are received from the master timer.
    $03 constant TIM3_OCCS                      \ [0x03] OCREF clear selection This bit is used to select the OCREF clear source
    $04 constant TIM3_TS                        \ [0x04 : 3] TS[2:0]: Trigger selection This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See Table 77: TIM3 internal trigger connection on page 478 for more details on ITRx meaning for each Timer. Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition.
  ...
  [ifdef] TIM3_TIM3_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_CNT                       \ [0x00 : 32] counter value
  [then]
  ...
  [ifdef] TIM3_TIM3_CNT_ALTERNATE1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_CNT                       \ [0x00 : 31] counter value
  ...
  [ifdef] TIM3_TIM3_ARR_DEF
    ...
    \
    $00 constant TIM3_ARR                       \ [0x00 : 32] Auto-reload value ARR is the value to be loaded in the actual auto-reload register. Refer to the Section 18.3.1: Time-base unit on page 429 for more details about ARR update and behavior. The counter is blocked while the auto-reload value is null.
  [then]
