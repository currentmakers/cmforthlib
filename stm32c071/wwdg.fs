...
[ifndef] WWDG_DEF
  ...
  [ifdef] WWDG_WWDG_CR_DEF
    ...
    \
    $00 constant WWDG_T                         \ [0x00 : 7] 7-bit counter (MSB to LSB) These bits contain the value of the watchdog counter, decremented every (4096 x 2<sup>WDGTB[2:0]</sup>) PCLK cycles. A reset is produced when it is decremented from 0x40 to 0x3F (T6 becomes cleared).
  ...
  [ifdef] WWDG_WWDG_CFR_DEF
    ...
    $09 constant WWDG_EWI                       \ [0x09] Early wake-up interrupt enable Set by software and cleared by hardware after a reset. When set, an interrupt occurs whenever the counter reaches the value 0x40.
  ...
  [ifdef] WWDG_WWDG_SR_DEF
    ...
    \
    $00 constant WWDG_EWIF                      \ [0x00] Early wake-up interrupt flag This bit is set by hardware when the counter has reached the value 0x40. It must be cleared by software by writing 0. Writing 1 has no effect. This bit is also set if the interrupt is not enabled.
  [then]
