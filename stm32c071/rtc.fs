...
$00000010 constant RTC_RTC_ICSR_INITS                               \ Initialization status flag This bit is set by hardware when the calendar year field is different from 0 (Power-on reset state).
...
$00000010 constant RTC_RTC_CR_REFCKON                               \ RTC_REFIN reference clock detection enable (50 or 60 Hz) Note: PREDIV_S must be 0x00FF.
...
$00080000 constant RTC_RTC_CR_COSEL                                 \ Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768 kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to Section 24.3.14: Calibration clock output.
...
$00002000 constant RTC_RTC_CALR_CALW16                              \ Use a 16-second calibration cycle period When CALW16 is set to 1, the 16-second calibration cycle period is selected. This bit must not be set to 1 if CALW8 = 1. Note: CALM[0] is stuck at 0 when CALW16 = 1. Refer to Section 24.3.12: RTC smooth digital calibration.
$00004000 constant RTC_RTC_CALR_CALW8                               \ Use an 8-second calibration cycle period When CALW8 is set to 1, the 8-second calibration cycle period is selected. Note: CALM[1:0] are stuck at 00 when CALW8 = 1. Refer to Section 24.3.12: RTC smooth digital calibration.
...
$0f000000 constant RTC_RTC_ALRMASSR_MASKSS                          \ Mask the most-significant bits starting at this bit ... The overflow bits of the synchronous counter (bits 15) is never compared. This bit can be different from 0 only after a shift operation. Note: The overflow bits of the synchronous counter (bits 15) is never compared. This bit can be different from 0 only after a shift operation.
...
$00000008 constant RTC_RTC_SR_TSF                                   \ Timestamp flag This flag is set by hardware when a timestamp event occurs.
...
$00000008 constant RTC_RTC_MISR_TSMF                                \ Timestamp masked flag This flag is set by hardware when a timestamp interrupt occurs.
...
$00000008 constant RTC_RTC_SCR_CTSF                                 \ Clear timestamp flag Writing 1 in this bit clears the TSOVF bit in the RTC_SR register.
...
$40002808 constant RTC_RTC_SSR    \ offset: 0x08 : RTC sub second register
...
$40002838 constant RTC_RTC_TSSSR  \ offset: 0x38 : RTC timestamp sub second register
$40002840 constant RTC_RTC_ALRMAR  \ offset: 0x40 : RTC alarm A register
$40002844 constant RTC_RTC_ALRMASSR  \ offset: 0x44 : RTC alarm A sub second register
