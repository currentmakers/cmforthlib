...
[ifndef] RTC_DEF
  ...
  [ifdef] RTC_RTC_ICSR_DEF
    ...
    $04 constant RTC_INITS                      \ [0x04] Initialization status flag This bit is set by hardware when the calendar year field is different from 0 (Power-on reset state).
  ...
  [ifdef] RTC_RTC_CR_DEF
    ...
    $04 constant RTC_REFCKON                    \ [0x04] RTC_REFIN reference clock detection enable (50 or 60 Hz) Note: PREDIV_S must be 0x00FF.
    ...
    $13 constant RTC_COSEL                      \ [0x13] Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768 kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to Section 24.3.14: Calibration clock output.
  ...
  [ifdef] RTC_RTC_CALR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant RTC_CALM                       \ [0x00 : 9] Calibration minus The frequency of the calendar is reduced by masking CALM out of 2<sup>20</sup> RTCCLK pulses (32 seconds if the input frequency is 32768 Hz). This decreases the frequency of the calendar with a resolution of 0.9537 ppm. To increase the frequency of the calendar, this feature should be used in conjunction with CALP. See Section 24.3.12: RTC smooth digital calibration on page 606.
    $0d constant RTC_CALW16                     \ [0x0d] Use a 16-second calibration cycle period When CALW16 is set to 1, the 16-second calibration cycle period is selected. This bit must not be set to 1 if CALW8 = 1. Note: CALM[0] is stuck at 0 when CALW16 = 1. Refer to Section 24.3.12: RTC smooth digital calibration.
    $0e constant RTC_CALW8                      \ [0x0e] Use an 8-second calibration cycle period When CALW8 is set to 1, the 8-second calibration cycle period is selected. Note: CALM[1:0] are stuck at 00 when CALW8 = 1. Refer to Section 24.3.12: RTC smooth digital calibration.
  ...
  [ifdef] RTC_RTC_ALRMASSR_DEF
    ...
    $18 constant RTC_MASKSS                     \ [0x18 : 4] Mask the most-significant bits starting at this bit ... The overflow bits of the synchronous counter (bits 15) is never compared. This bit can be different from 0 only after a shift operation. Note: The overflow bits of the synchronous counter (bits 15) is never compared. This bit can be different from 0 only after a shift operation.
  [then]
  ...
  [ifdef] RTC_RTC_SR_DEF
    ...
    $03 constant RTC_TSF                        \ [0x03] Timestamp flag This flag is set by hardware when a timestamp event occurs.
  ...
  [ifdef] RTC_RTC_MISR_DEF
    ...
    $03 constant RTC_TSMF                       \ [0x03] Timestamp masked flag This flag is set by hardware when a timestamp interrupt occurs.
  ...
  [ifdef] RTC_RTC_SCR_DEF
    ...
    $03 constant RTC_CTSF                       \ [0x03] Clear timestamp flag Writing 1 in this bit clears the TSOVF bit in the RTC_SR register.
