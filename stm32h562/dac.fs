...
[ifndef] DAC_DEF
  ...
  [ifdef] DAC_DAC_CR_DEF
    ...
    $01 constant DAC_TEN1                       \ [0x01] DAC channel1 trigger enable This bit is set and cleared by software to enable/disable DAC channel1 trigger. Note: When software trigger is selected, the transfer from the DAC_DHR1 register to the DAC_DOR1 register takes only one dac_hclk clock cycle.
    ...
    $08 constant DAC_MAMP1                      \ [0x08 : 4] DAC channel1 mask/amplitude selector These bits are written by software to select mask in wave generation mode or amplitude in triangle generation mode. greater than or equal 1011: Unmask bits[11:0] of LFSR/ triangle amplitude equal to 4095
  ...
  [ifdef] DAC_DAC_SWTRGR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DAC_SWTRIG1                    \ [0x00] DAC channel1 software trigger This bit is set by software to trigger the DAC in software trigger mode. Note: This bit is cleared by hardware (one dac_hclk clock cycle later) once the DAC_DHR1 register value has been loaded into the DAC_DOR1 register.
  ...
  [ifdef] DAC_DAC_DHR12R1_DEF
    ...
    $10 constant DAC_DACC1DHRB                  \ [0x10 : 12] DAC channel1 12-bit right-aligned data B These bits are written by software. They specify 12-bit data for DAC channel1 when the DAC operates in Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DHR12L1_DEF
    ...
    $14 constant DAC_DACC1DHRB                  \ [0x14 : 12] DAC channel1 12-bit left-aligned data B These bits are written by software. They specify 12-bit data for DAC channel1 when the DAC operates in Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DHR8R1_DEF
    ...
    $08 constant DAC_DACC1DHRB                  \ [0x08 : 8] DAC channel1 8-bit right-aligned data These bits are written by software. They specify 8-bit data for DAC channel1 when the DAC operates in Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DHR12R2_DEF
    ...
    $10 constant DAC_DACC2DHRB                  \ [0x10 : 12] DAC channel2 12-bit right-aligned data These bits are written by software. They specify 12-bit data for DAC channel2 when the DAC operates in DMA Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DHR12L2_DEF
    ...
    $14 constant DAC_DACC2DHRB                  \ [0x14 : 12] DAC channel2 12-bit left-aligned data B These bits are written by software. They specify 12-bit data for DAC channel2 when the DAC operates in Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DHR8R2_DEF
    ...
    $08 constant DAC_DACC2DHRB                  \ [0x08 : 8] DAC channel2 8-bit right-aligned data These bits are written by software. They specify 8-bit data for DAC channel2 when the DAC operates in Double data mode.
  [then]
  ...
  [ifdef] DAC_DAC_DOR1_DEF
    ...
    $10 constant DAC_DACC1DORB                  \ [0x10 : 12] DAC channel1 data output These bits are read-only. They contain data output for DAC channel1 B.
  [then]
  ...
  [ifdef] DAC_DAC_DOR2_DEF
    ...
    $10 constant DAC_DACC2DORB                  \ [0x10 : 12] DAC channel2 data output These bits are read-only. They contain data output for DAC channel2 B.
  [then]
  ...
  [ifdef] DAC_DAC_SR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0b constant DAC_DAC1RDY                    \ [0x0b] DAC channel1 ready status bit This bit is set and cleared by hardware.
    $0c constant DAC_DORSTAT1                   \ [0x0c] DAC channel1 output register status bit This bit is set and cleared by hardware. It is applicable only when the DAC operates in Double data mode.
    ...
    $1b constant DAC_DAC2RDY                    \ [0x1b] DAC channel2 ready status bit This bit is set and cleared by hardware. Note: This bit is available only on dual-channel DACs. Refer to implementation.
    $1c constant DAC_DORSTAT2                   \ [0x1c] DAC channel2 output register status bit This bit is set and cleared by hardware. It is applicable only when the DAC operates in Double data mode. Note: This bit is available only on dual-channel DACs. Refer to implementation.
    ...
    $1f constant DAC_BWST2                      \ [0x1f] DAC channel2 busy writing sample time flag This bit is systematically set just after Sample and hold mode enable. It is set each time the software writes the register DAC_SHSR2, It is cleared by hardware when the write operation of DAC_SHSR2 is complete. (It takes about 3 LSI/LSE periods of synchronization). Note: This bit is available only on dual-channel DACs. Refer to implementation.
  [then]
  ...
  [ifdef] DAC_DAC_MCR_DEF
    ...
    $08 constant DAC_DMADOUBLE1                 \ [0x08] DAC channel1 DMA double data mode This bit is set and cleared by software.
    $09 constant DAC_SINFORMAT1                 \ [0x09] Enable signed format for DAC channel1 This bit is set and cleared by software.
    $0e constant DAC_HFSEL0                     \ [0x0e] High frequency interface mode selection
    $0f constant DAC_HFSEL1                     \ [0x0f] High frequency interface mode selection
    ...
    $18 constant DAC_DMADOUBLE2                 \ [0x18] DAC channel2 DMA double data mode This bit is set and cleared by software. Note: This bit is available only on dual-channel DACs. Refer to implementation.
    $19 constant DAC_SINFORMAT2                 \ [0x19] Enable signed format for DAC channel2 This bit is set and cleared by software. Note: This bit is available only on dual-channel DACs. Refer to implementation.
  [then]
  ...
  [ifdef] DAC_DAC_SHHR_DEF
    ...
    \
    $00 constant DAC_THOLD1                     \ [0x00 : 10] DAC channel1 hold time (only valid in Sample and hold mode) Hold time = (THOLD[9:0]) x LSI/LSE clock period Note: This register can be modified only when EN1 = 0.
    $10 constant DAC_THOLD2                     \ [0x10 : 10] DAC channel2 hold time (only valid in Sample and hold mode). Hold time = (THOLD[9:0]) x LSI/LSE clock period Note: This register can be modified only when EN2 = 0. These bits are available only on dual-channel DACs. Refer to implementation.
  [then]
  ...
  [ifdef] DAC_DAC_SHRR_DEF
    ...
    \ Reset value: 0x00010001
    \
    $00 constant DAC_TREFRESH1                  \ [0x00 : 8] DAC channel1 refresh time (only valid in Sample and hold mode) Refresh time = (TREFRESH[7:0]) x LSI/LSE clock period Note: This register can be modified only when EN1 = 0.
    $10 constant DAC_TREFRESH2                  \ [0x10 : 8] DAC channel2 refresh time (only valid in Sample and hold mode) Refresh time = (TREFRESH[7:0]) x LSI/LSE clock period Note: This register can be modified only when EN2 = 0. These bits are available only on dual-channel DACs. Refer to implementation.
  [then]
