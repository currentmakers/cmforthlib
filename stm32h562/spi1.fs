...
[ifndef] SPI1_DEF
  ...
  [ifdef] SPI1_SPI_CFG2_DEF
    ...
    $0f constant SPI1_IOSWP                     \ [0x0f] swap functionality of MISO and MOSI pins When this bit is set, the function of MISO and MOSI pins alternate functions are inverted. Original MISO pin becomes MOSI and original MOSI pin becomes MISO. Note: This bit can be also used in PCM and I2S modes to swap SDO and SDI pins.
    ...
    $17 constant SPI1_LSBFRST                   \ [0x17] data frame format Note: This bit can be also used in PCM and I2S modes.
  ...
  [ifdef] SPI1_SPI_RXDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant SPI1_RXDR                      \ [0x00 : 32] receive data register The register serves as an interface with RxFIFO. When it is read, RxFIFO is accessed. Note: In SPI mode, data is always right-aligned. Alignment of data at I2S mode depends on DATLEN and DATFMT setting. Unused bits are read as zero when the register is read. Writing to the register is ignored. Note: DR can be accessed byte-wise (8-bit access): in this case only one data-byte is read by single access halfword-wise (16 bit access) in this case 2 data-bytes or 1 halfword-data can be read by single access word-wise (32 bit access). In this case 4 data-bytes or 2 halfword-data or word-data can be read by single access. Read access of this register less than the configured data size is forbidden.
  [then]
  ...
  [ifdef] SPI1_SPI_I2SCFGR_DEF
    \
    \ @brief SPI/I2S configuration register
    \ Address offset: 0x50
    \ Reset value: 0x00000000
    \
    $00 constant SPI1_I2SMOD                    \ [0x00] I2S mode selection
    $01 constant SPI1_I2SCFG                    \ [0x01 : 3] I2S configuration mode others, not used
    $04 constant SPI1_I2SSTD                    \ [0x04 : 2] I2S standard selection For more details on I2S standards, refer to
    $07 constant SPI1_PCMSYNC                   \ [0x07] PCM frame synchronization
    $08 constant SPI1_DATLEN                    \ [0x08 : 2] data length to be transferred
    $0a constant SPI1_CHLEN                     \ [0x0a] channel length (number of bits per audio channel)
    $0b constant SPI1_CKPOL                     \ [0x0b] serial audio clock polarity
    $0c constant SPI1_FIXCH                     \ [0x0c] fixed channel length in slave
    $0d constant SPI1_WSINV                     \ [0x0d] word select inversion This bit is used to invert the default polarity of WS signal. WS is LOW. In PCM mode the start of frame is indicated by a rising edge. WS is HIGH. In PCM mode the start of frame is indicated by a falling edge.
    $0e constant SPI1_DATFMT                    \ [0x0e] data format
    $10 constant SPI1_I2SDIV                    \ [0x10 : 8] I2S linear prescaler I2SDIV can take any values except the value 1, when ODD is also equal to 1. Refer to for details
    $18 constant SPI1_ODD                       \ [0x18] odd factor for the prescaler Refer to for details
    $19 constant SPI1_MCKOE                     \ [0x19] master clock output enable
  [then]

  \
  \ @brief Serial peripheral interface
  \
  $00 constant SPI1_SPI_CR1             \ SPI/I2S control register 1
  $04 constant SPI1_SPI_CR2             \ SPI/I2S control register 2
  $08 constant SPI1_SPI_CFG1            \ SPI/I2S configuration register 1
  $0C constant SPI1_SPI_CFG2            \ SPI/I2S configuration register 2
  $10 constant SPI1_SPI_IER             \ SPI/I2S interrupt enable register
  $14 constant SPI1_SPI_SR              \ SPI/I2S status register
  $18 constant SPI1_SPI_IFCR            \ SPI/I2S interrupt/status flags clear register
  $20 constant SPI1_SPI_TXDR            \ SPI/I2S transmit data register
  $30 constant SPI1_SPI_RXDR            \ SPI/I2S receive data register
  $40 constant SPI1_SPI_CRCPOLY         \ SPI/I2S polynomial register
  $44 constant SPI1_SPI_TXCRC           \ SPI/I2S transmitter CRC register
  $48 constant SPI1_SPI_RXCRC           \ SPI/I2S receiver CRC register
  $4C constant SPI1_SPI_UDRDR           \ SPI/I2S underrun data register
  $50 constant SPI1_SPI_I2SCFGR         \ SPI/I2S configuration register
