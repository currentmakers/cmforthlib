...
$00001000 constant RNG_RNG_CR_NISTC                                 \ Non NIST compliant four conditioning loops are performed and 512 bits of noise source are used. Writing this bit is taken into account only if CONDRST bit is set to 1 in the same access, while CONFIGLOCK remains at 0. Writing to this bit is ignored if CONFIGLOCK=1.
...
$00000007 constant RNG_RNG_NSCR_EN_OSC1                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 1.
$00000038 constant RNG_RNG_NSCR_EN_OSC2                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 2.
$000001c0 constant RNG_RNG_NSCR_EN_OSC3                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 3.
$00000e00 constant RNG_RNG_NSCR_EN_OSC4                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 4.
$00007000 constant RNG_RNG_NSCR_EN_OSC5                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 5.
$00038000 constant RNG_RNG_NSCR_EN_OSC6                             \ When the RNG is enabled (RNGEN bit set), each bit of this bitfield enables one of the three inputs from the oscillator instance number 6.
...
$420c0808 constant RNG_RNG_DR     \ offset: 0x08 : RNG data register
$420c080c constant RNG_RNG_NSCR   \ offset: 0x0C : RNG noise source control register
$420c0810 constant RNG_RNG_HTCR   \ offset: 0x10 : RNG health test control register
