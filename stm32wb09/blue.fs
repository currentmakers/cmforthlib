...
[ifndef] BLUE_DEF
  ...
  [ifdef] BLUE_STATUS2REG_DEF
    \
    \ @brief STATUS2REG register
    \ Address offset: 0x7C
    \ Reset value: 0x00000000
    \
    $00 constant BLUE_IQSAMPLESREADY            \ [0x00] indicates if IQ samples have been received on the last reception.
    $01 constant BLUE_IQSAMPLESNUMBER           \ [0x01 : 7] indicate the number of IQ samples stored in the RAM buffer addressed by StatMach.
    $1d constant BLUE_IQSAMPLESMISSINGERROR     \ [0x1d] IQ sample internal buffer overflow error flag.
    $1e constant BLUE_ANTENNASWITCHINGPATTERNACCESSERROR     \ [0x1e] timing error flag related to Antenna Pattern not read on-time.
    $1f constant BLUE_ANTENNA_SWITCHING_PATTERN_ADDRESS_ERROR     \ [0x1f] AHB access error flag.
  [then]
  ...
  $7C constant BLUE_STATUS2REG          \ STATUS2REG register
