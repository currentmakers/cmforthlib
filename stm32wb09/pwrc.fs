...
[ifndef] PWRC_DEF
  ...
  [ifdef] PWRC_CR1_DEF
    ...
    $01 constant PWRC_ENSDNBOR                  \ [0x01] ENSDNBOR: Enable BOR supply monitoring during shutdown mode.
    ...
    $05 constant PWRC_ENBORH                    \ [0x05] ENBORH: enable BORH configuration
    $06 constant PWRC_SELBORH                   \ [0x06 : 2] SELBORH[1:0]: BORH selection of Vbor threshold
  ...
  [ifdef] PWRC_CR3_DEF
    ...
    $0e constant PWRC_EIWL2                     \ [0x0e] EIWL2: Enable wakeup on Internal event (LPUART). 0: Wakeup on internal line is disabled (default). 1: Wakeup on internal line is enabled.
  ...
  [ifdef] PWRC_SR1_DEF
    ...
    $0e constant PWRC_IWUF2                     \ [0x0e] IWUF2: Internal wakeup 2 flag (LPUART). 0: no wakeup from LPUART occurred since last clear. 1: a wakeup from LPUART occurred since last clear. Note: The user must clear the LPUART wakeup flag inside the LPUART IP to clear this bit (mirror of the LPUART wakeup line on the PWRC block).
  ...
  [ifdef] PWRC_CR5_DEF
    ...
    $07 constant PWRC_SMPSFRDY                  \ [0x07] SMPSFB Force ready check When this bit is set, the SMPS FSM will consider the SMPS ready .
    ...
    $0d constant PWRC_SMPS_PRECH_CUR_SEL        \ [0x0d : 2] SMPS_PRECH_CUR_SEL[1:0] Selection for SMPS PRECHARGE limit current
  [then]
  ...
  [ifdef] PWRC_PUCRA_DEF
    ...
    \
    $00 constant PWRC_PU                        \ [0x00 : 16] PU[x] : Pull Up Pull up activation on port A[i] pad when APC bit of PWRC CR3 is set
  [then]
  ...
  [ifdef] PWRC_PDCRA_DEF
    ...
    \
    $00 constant PWRC_PD                        \ [0x00 : 16] PD[x]: Pull Down Pull Down activation on port A[i] pad when APC bit of PWRC CR3 is set
  [then]
  ...
  [ifdef] PWRC_PUCRB_DEF
    ...
    \
    $00 constant PWRC_PU                        \ [0x00 : 16] PU[x] : Pull Up Pull up activation on port B[i] pad when APC bit of PWRC CR3 is set
  [then]
  ...
  [ifdef] PWRC_PDCRB_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant PWRC_PD                        \ [0x00 : 16] PD[x]: Pull Down Pull Down activation on port B[i] pad when APC bit of PWRC CR3 is set
  [then]
  ...
  [ifdef] PWRC_SR3_DEF
    ...
    $07 constant PWRC_WUF19                     \ [0x07] WUF19 WakeUp Flag 19 PB15 This bit is set when a wakeup is detected on wakeup line 19. It is cleared by a reset pad or by writing 1 in this bit field. writting this bit, clears the interrupt:
  [then]
  ...
  [ifdef] PWRC_DBGR_DEF
    ...
    $0d constant PWRC_DIS_PRECH                 \ [0x0d : 3] DIS_PRECH[2:0]: disable precharge during deepstop (debug) 111: precharge and SMPS monitoring are disabled (whatever CR5.SMPSLPOPEN) 101: precharge are activated only at deepstop exit (to be used only with CR5.SMPSLPOPEN=1) else: No effect (default 0x0)
  [then]
