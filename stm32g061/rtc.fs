...
$00000010 constant RTC_RTC_ICSR_INITS                               \ Initialization status flag This bit is set by hardware when the calendar year field is different from 0 (RTC domain reset state).
...
$00000007 constant RTC_RTC_CR_WUCKSEL                               \ ck_wut wakeup clock selection 10x: ck_spre (usually 1Hz) clock is selected 11x: ck_spre (usually 1Hz) clock is selected and 216is added to the WUT counter value
...
$00080000 constant RTC_RTC_CR_COSEL                                 \ Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to .
...
$80000000 constant RTC_RTC_CR_OUT2EN                                \ RTC_OUT2 output enable Setting this bit allows to remap the RTC outputs on RTC_OUT2
...
$000001ff constant RTC_RTC_CALR_CALM                                \ Calibration minus The frequency of the calendar is reduced by masking CALM out of 220 RTCCLK pulses (32 seconds if the input frequency is 32768Hz). This decreases the frequency of the calendar with a resolution of 0.9537ppm. To increase the frequency of the calendar, this feature should be used in conjunction with CALP. See .
...
$40002814 constant RTC_RTC_WUTR   \ offset: 0x14 : RTC wakeup timer register
...
$4000284c constant RTC_RTC_ALRMBSSR  \ offset: 0x4C : RTC alarm B sub second register
