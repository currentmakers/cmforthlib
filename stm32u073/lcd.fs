...
[ifndef] LCD_DEF
  ...
  [ifdef] LCD_LCD_CR_DEF
    ...
    $01 constant LCD_VSEL                       \ [0x01] Voltage source selection This bit determines the voltage source for the LCD.
    $02 constant LCD_DUTY                       \ [0x02 : 3] Duty selection These bits determine the duty cycle. Values 101, 110 and 111 are forbidden. Others: Reserved
    $05 constant LCD_BIAS                       \ [0x05 : 2] Bias selector These bits determine the bias used. Value 11 is forbidden.
    $07 constant LCD_MUX_SEG                    \ [0x07] Mux segment enable This bit is used to enable SEG pin remapping. Four SEG pins can be multiplexed with1SEG[31:28] or SEG[15:12]. See Section118.3.7.
  ...
  [ifdef] LCD_LCD_FCR_DEF
    ...
    $0a constant LCD_CC                         \ [0x0a : 3] Contrast control These bits specify one of the V<sub>LCD </sub>maximum voltages (independent of V<sub>DD</sub>). It ranges from12.60 V to 3.51V. Note: Refer to the datasheet for the V<sub>LCDx</sub> values.
    $0d constant LCD_BLINKF                     \ [0x0d : 3] Blink frequency selection
    $10 constant LCD_BLINK                      \ [0x10 : 2] Blink mode selection
    $12 constant LCD_DIV                        \ [0x12 : 4] DIV clock divider These bits are written by software to define the division factor of the DIV divider (see1Section118.3.2.) ...
    $16 constant LCD_PS                         \ [0x16 : 4] PS 16-bit prescaler These bits are written by software to define the division factor of the PS 16-bit prescaler. ck_ps = LCDCLK/(2<sup>PS[3:0]</sup>). See<sub> </sub>Section118.3.2. ...
  [then]
  ...
  [ifdef] LCD_LCD_SR_DEF
    ...
    \
    $00 constant LCD_ENS                        \ [0x00] LCD enabled status This bit is set and cleared by hardware. It indicates the LCD controller status. Note: This bit is set immediately when LCDEN in LCD_CR goes from 0 to 1. On deactivation, it reflects the real LCD status. It becomes 0 at the end of the last displayed frame.
    $01 constant LCD_SOF                        \ [0x01] Start-of-frame flag This bit is set by hardware at the beginning of a new frame, at the same time as the display data is updated. It is cleared by writing a 1 to SOFC in LCD_CLR. The bit clear has priority over the set.
    $02 constant LCD_UDR                        \ [0x02] Update display request Each time software modifies the LCD_RAM, it must set this bit to transfer the updated data to the second level buffer. This bit stays set until the end of the update. During this time, the LCD_RAM is write protected. When the display is disabled, the update is performed for all LCD_DISPLAY locations. When the display is enabled, the update is performed only for locations for which commons are active (depending on DUTY). For example if DUTY = 1/2, Note: only the LCD_DISPLAY of COM0 and COM1 are updated. Note: Writing 0 on this bit or writing 1 when it is already 1 has no effect. This bit can be cleared by hardware only. It can be cleared only when LCDEN = 1
    ...
    $04 constant LCD_RDY                        \ [0x04] Ready flag This bit is set and cleared by hardware. It indicates the status of the stepup converter.
  ...
  [ifdef] LCD_LCD_CLR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $01 constant LCD_SOFC                       \ [0x01] Start-of-frame flag clear This bit is written by software to clear SOF in LCD_SR.
    $03 constant LCD_UDDC                       \ [0x03] Update display done clear This bit is written by software to clear UDD in LCD_SR.
  [then]
  ...
  [ifdef] LCD_LCD_RAM1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LCD_SEGMENT_DATA               \ [0x00 : 20] Each bit corresponds to one pixel of the LCD display.
  [then]
