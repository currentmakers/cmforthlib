...
$00000001 constant TIM6_TIM6_CR1_CEN                                \ Counter enable CEN is cleared automatically in one-pulse mode, when an update event occurs.
...
$00001000 constant TIM6_TIM6_CR1_DITHEN                             \ Dithering enable Note: The DITHEN bit can only be modified when CEN bit is reset.
...
$00000070 constant TIM6_TIM6_CR2_MMS                                \ Master mode selection These bits are used to select the information to be sent in master mode to slave timers for synchronization (TRGO). The combination is as follows: Note: The clock of the slave timer or he peripheral receiving the tim_trgo must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
...
$0000ffff constant TIM6_TIM6_CNT_CNT                                \ Counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register only holds the non-dithered part in CNT[15:0]. The fractional part is not available.
...
$0000ffff constant TIM6_TIM6_PSC_PSC                                \ Prescaler value The counter clock frequency ftim_cnt_ck is equal to ftim_psc_ck / (PSC[15:0] + 1). PSC contains the value to be loaded into the active prescaler register at each update event. (including when the counter is cleared through UG bit of TIMx_EGR register.
