...
[ifndef] TIM7_DEF
  ...
  [ifdef] TIM7_TIM7_CR1_DEF
    ...
    \
    $00 constant TIM7_CEN                       \ [0x00] Counter enable CEN is cleared automatically in one-pulse mode, when an update event occurs.
    ...
    $0c constant TIM7_DITHEN                    \ [0x0c] Dithering enable Note: The DITHEN bit can only be modified when CEN bit is reset.
  [then]
  ...
  [ifdef] TIM7_TIM7_CR2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $04 constant TIM7_MMS                       \ [0x04 : 3] Master mode selection These bits are used to select the information to be sent in master mode to slave timers for synchronization (TRGO). The combination is as follows: Note: The clock of the slave timer or he peripheral receiving the tim_trgo must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
  [then]
  ...
  [ifdef] TIM7_TIM7_SR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM7_UIF                       \ [0x00] Update interrupt flag This bit is set by hardware on an update event. It is cleared by software. On counter overflow if UDIS = 0 in the TIMx_CR1 register. When CNT is reinitialized by software using the UG bit in the TIMx_EGR register, if URS = 0 and UDIS = 0 in the TIMx_CR1 register.
  [then]
  ...
  [ifdef] TIM7_TIM7_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM7_CNT                       \ [0x00 : 16] Counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register only holds the non-dithered part in CNT[15:0]. The fractional part is not available.
  ...
  [ifdef] TIM7_TIM7_PSC_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM7_PSC                       \ [0x00 : 16] Prescaler value The counter clock frequency ftim_cnt_ck is equal to ftim_psc_ck / (PSC[15:0] + 1). PSC contains the value to be loaded into the active prescaler register at each update event. (including when the counter is cleared through UG bit of TIMx_EGR register.
  [then]
