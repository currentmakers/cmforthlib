...
[ifndef] DBGMCU_DEF
  ...
  [ifdef] DBGMCU_DBGMCU_CR_DEF
    ...
    \
    $01 constant DBGMCU_DBG_STOP                \ [0x01] Allows debug in Stop mode
    $02 constant DBGMCU_DBG_STANDBY             \ [0x02] Allows debug in Standby mode
    ...
    $10 constant DBGMCU_DCRT                    \ [0x10] Debug credentials reset type
  [then]
  ...
  [ifdef] DBGMCU_DBGMCU_SR_DEF
    ...
    \
    $00 constant DBGMCU_AP_PRESENT              \ [0x00 : 16] Bit n identifies whether access port AP n is present in device
    $10 constant DBGMCU_AP_ENABLED              \ [0x10 : 16] Bit n identifies whether access port AP n is open (can be accessed via the debug port) or locked (debug access to the AP is blocked)
  [then]
  ...
  [ifdef] DBGMCU_DBGMCU_DBG_AUTH_HOST_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DBGMCU_MESSAGE                 \ [0x00 : 32] Debug host to device mailbox message.
  [then]
  ...
  [ifdef] DBGMCU_DBGMCU_DBG_AUTH_DEVICE_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DBGMCU_MESSAGE                 \ [0x00 : 32] Device to debug host mailbox message.
  [then]
  ...
  [ifdef] DBGMCU_DBGMCU_DBG_AUTH_ACK_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DBGMCU_HOST_ACK                \ [0x00] Host to device acknowledge.
    $01 constant DBGMCU_DEV_ACK                 \ [0x01] Device to host acknowledge.
  [then]
