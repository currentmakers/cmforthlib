...
[ifndef] TIM2_DEF
  ...
  [ifdef] TIM2_CR2_DEF
    ...
    $04 constant TIM2_MMS                       \ [0x04 : 3] Master mode selection These bits permit to select the information to be sent in master mode to slave timers for synchronization (TRGO). The combination is as follows: When the Counter Enable signal is controlled by the trigger input, there is a delay on TRGO, except if the master/slave mode is selected (see the MSM bit description in TIMx_SMCR register). Note: The clock of the slave timer or ADC must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
  ...
  [ifdef] TIM2_SMCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_SMS1                      \ [0x00 : 3] Slave mode selection When external signals are selected the active edge of the trigger signal (TRGI) is linked to the polarity selected on the external input (see Input Control register and Control Register description. reinitializes the counter, generates an update of the registers and starts the counter. Note: The gated mode must not be used if TI1F_ED is selected as the trigger input (TS=00100). Indeed, TI1F_ED outputs 1 pulse for each transition on TI1F, whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to receive events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly while triggers are received from the master timer.
    $03 constant TIM2_OCCS                      \ [0x03] OCREF clear selection This bit is used to select the OCREF clear source
    $04 constant TIM2_TS1                       \ [0x04 : 3] Trigger selection This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See for more details on ITRx meaning for each Timer. Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition.
    $07 constant TIM2_MSM                       \ [0x07] Master/Slave mode
    $08 constant TIM2_ETF                       \ [0x08 : 4] External trigger filter This bit-field then defines the frequency used to sample ETRP signal and the length of the digital filter applied to ETRP. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
    $0c constant TIM2_ETPS                      \ [0x0c : 2] External trigger prescaler External trigger signal ETRP frequency must be at most 1/4 of CK_INT frequency. A prescaler can be enabled to reduce ETRP frequency. It is useful when inputting fast external clocks.
    ...
    $0f constant TIM2_ETP                       \ [0x0f] External trigger polarity This bit selects whether ETR or ETR is used for trigger operations
    $10 constant TIM2_SMS2                      \ [0x10] Slave mode selection When external signals are selected the active edge of the trigger signal (TRGI) is linked to the polarity selected on the external input (see Input Control register and Control Register description. reinitializes the counter, generates an update of the registers and starts the counter. Note: The gated mode must not be used if TI1F_ED is selected as the trigger input (TS=00100). Indeed, TI1F_ED outputs 1 pulse for each transition on TI1F, whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals (timer, ADC, ...) receiving the TRGO or the TRGO2 signals must be enabled prior to receive events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly while triggers are received from the master timer.
    $14 constant TIM2_TS2                       \ [0x14 : 2] Trigger selection This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See for more details on ITRx meaning for each Timer. Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition.
  [then]
  ...
  [ifdef] TIM2_CCMR1_Output_DEF
    ...
    $10 constant TIM2_OC1M_3                    \ [0x10] Output Compare 1 mode - bit 3
    $18 constant TIM2_OC2M_3                    \ [0x18] Output Compare 2 mode - bit 3
  [then]
  ...
  [ifdef] TIM2_CCMR2_Output_DEF
    ...
    $10 constant TIM2_OC3M_3                    \ [0x10] Output Compare 3 mode - bit 3
    $18 constant TIM2_OC4M_3                    \ [0x18] Output Compare 4 mode - bit 3
  [then]
  ...
  [ifdef] TIM2_CCER_DEF
    ...
    $03 constant TIM2_CC1NP                     \ [0x03] Capture/Compare 1 output Polarity. CC1 channel configured as output: CC1NP must be kept cleared in this case. CC1 channel configured as input: This bit is used in conjunction with CC1P to define TI1FP1/TI2FP1 polarity. refer to CC1P description.
  ...
  [ifdef] TIM2_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_CNT_L                     \ [0x00 : 16] Low counter value
    $10 constant TIM2_CNT_H                     \ [0x10 : 16] High counter value (TIM2 only)
  [then]


  [ifdef] TIM2_CNT_ALTERNATE5_DEF
    \
    \ @brief counter
    \ Address offset: 0x24
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_CNT                       \ [0x00 : 31] Most significant part counter value (TIM2) nullLeast significant part of counter value
    $1f constant TIM2_UIFCPY                    \ [0x1f] UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
  [then]
  ...
  [ifdef] TIM2_ARR_DEF
    ...
    \
    $00 constant TIM2_ARR                       \ [0x00 : 32] High auto-reload value (TIM2) nullLow Auto-reload value ARR is the value to be loaded in the actual auto-reload register. Refer to the for more details about ARR update and behavior. The counter is blocked while the auto-reload value is null.
  [then]
  ...
  [ifdef] TIM2_DCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_DBA                       \ [0x00 : 5] DMA base address This 5-bit vector defines the base-address for DMA transfers (when read/write access are done through the TIMx_DMAR address). DBA is defined as an offset starting from the address of the TIMx_CR1 register. Example: ... Example: Let us consider the following transfer: DBL = 7 transfers & DBA = TIMx_CR1. In this case the transfer is done to/from 7 registers starting from the TIMx_CR1 address.
    $08 constant TIM2_DBL                       \ [0x08 : 5] DMA burst length This 5-bit vector defines the number of DMA transfers (the timer recognizes a burst transfer when a read or a write access is done to the TIMx_DMAR address). ...
  [then]
  ...
  [ifdef] TIM2_OR1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_OCREF_CLR                 \ [0x00] Ocref_clr source selection This bit selects the ocref_clr input source.
  [then]
  ...
  [ifdef] TIM2_AF1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant TIM2_ETRSEL                    \ [0x0e : 4] ETR source selection These bits select the ETR input source. Others: Reserved
  [then]
  ...
  [ifdef] TIM2_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_TI1SEL                    \ [0x00 : 4] TI1[0] to TI1[15] input selection These bits select the TI1[0] to TI1[15] input source. Others: Reserved
    $08 constant TIM2_TI2SEL                    \ [0x08 : 4] TI2[0] to TI2[15] input selection These bits select the TI2[0] to TI2[15] input source. Others: Reserved
  [then]
  ...
  $24 constant TIM2_CNT_ALTERNATE5      \ counter
  ...
  $50 constant TIM2_OR1                 \ TIM option register
  $60 constant TIM2_AF1                 \ TIM alternate function option register 1
  $68 constant TIM2_TISEL               \ TIM alternate function option register 1
