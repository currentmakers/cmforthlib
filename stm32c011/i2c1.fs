...
[ifndef] I2C1_DEF
  ...
  [ifdef] I2C1_I2C_CR1_DEF
    ...
    $06 constant I2C1_TCIE                      \ [0x06] Transfer Complete interrupt enable Note: Any of these events generate an interrupt: Transfer Complete (TC) Transfer Complete Reload (TCR)
    ...
    $12 constant I2C1_WUPEN                     \ [0x12] Wakeup from Stop mode enable Note: If the Wakeup from Stop mode feature is not supported, this bit is reserved and forced by hardware to 0. Refer to . Note: WUPEN can be set only when DNF = 0000
    $13 constant I2C1_GCEN                      \ [0x13] General call enable
    $14 constant I2C1_SMBHEN                    \ [0x14] SMBus host address enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
    $15 constant I2C1_SMBDEN                    \ [0x15] SMBus device default address enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
    $16 constant I2C1_ALERTEN                   \ [0x16] SMBus alert enable Note: When ALERTEN=0, the SMBA pin can be used as a standard GPIO. If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
    $17 constant I2C1_PECEN                     \ [0x17] PEC enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
  [then]
  ...
  [ifdef] I2C1_I2C_TIMINGR_DEF
    ...
    $1c constant I2C1_PRESC                     \ [0x1c : 4] Timing prescaler This field is used to prescale I2CCLK in order to generate the clock period tPRESC used for data setup and hold counters (refer to ) and for SCL high and low level counters (refer to ). tPRESC = (PRESC+1) x tI2CCLK
  [then]
  ...
  [ifdef] I2C1_I2C_ICR_DEF
    ...
    $0b constant I2C1_PECCF                     \ [0x0b] PEC Error flag clear Writing 1 to this bit clears the PECERR flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
    $0c constant I2C1_TIMOUTCF                  \ [0x0c] Timeout detection flag clear Writing 1 to this bit clears the TIMEOUT flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
    $0d constant I2C1_ALERTCF                   \ [0x0d] Alert flag clear Writing 1 to this bit clears the ALERT flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0. Refer to .
  [then]
