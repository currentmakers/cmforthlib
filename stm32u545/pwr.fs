...
[ifndef] PWR_DEF
  ...
  [ifdef] PWR_PWR_CR2_DEF
    ...
    $0c constant PWR_PKARAMPDS                  \ [0x0c] PKA32 SRAM power-down in Stop modes (Stop 0, 1, 2, 3)
  ...
  [ifdef] PWR_PWR_SVMCR_DEF
    ...
    $1c constant PWR_USV                        \ [0x1c] VDDUSB independent USB supply valid This bit is used to validate the VDDUSB supply for electrical and logical isolation purpose. Setting this bit is mandatory to use the USB peripheral. If VDDUSB is not always present in the application, the VDDUSB voltage monitor can be used to determine whether this supply is ready or not.
