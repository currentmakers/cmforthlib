...
[ifndef] FLASH_DEF
  ...
  [ifdef] FLASH_SR_DEF
    ...
    $07 constant FLASH_ERSERR                   \ [0x07] Erase Sequence Error
    $08 constant FLASH_RDERR                    \ [0x08] RDERR
  ...
  [ifdef] FLASH_CR_DEF
    ...
    $03 constant FLASH_SNB                      \ [0x03 : 4] Sector number
    ...
    $1a constant FLASH_RDERRIE                  \ [0x1a] PCROP error interrupt enable
  ...
  [ifdef] FLASH_OPTCR_DEF
    ...
    $05 constant FLASH_IWDG_SW                  \ [0x05] WDG_SW User option bytes
  ...
  [ifdef] FLASH_OPTCR2_DEF
    \
    \ @brief Flash option control register
    \ Address offset: 0x1C
    \ Reset value: 0x800000FF
    \
    $00 constant FLASH_PCROPi                   \ [0x00 : 8] PCROP option byte
    $1f constant FLASH_PCROP_RDP                \ [0x1f] PCROP zone preserved when RDP level decreased
  [then]
  ...
  $1C constant FLASH_OPTCR2             \ Flash option control register
