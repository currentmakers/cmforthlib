...
[ifndef] OTFDEC_DEF
  ...
  [ifdef] OTFDEC_OTFDEC_PRIVCFGR_DEF
    \
    \ @brief OTFDEC privileged access control configuration register
    \ Address offset: 0x10
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_PRIV                    \ [0x00] Privileged access protection.
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1CFGR_DEF
    ...
    $10 constant OTFDEC_REG_VERSION             \ [0x10 : 16] region firmware version
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1STARTADDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_START_ADDR          \ [0x00 : 32] Region AHB start address
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1ENDADDR_DEF
    ...
    \
    $00 constant OTFDEC_REG_END_ADDR            \ [0x00 : 32] Region AHB end address
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1NONCER0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_NONCE               \ [0x00 : 32] Region nonce, bits [31:0]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1NONCER1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_NONCE               \ [0x00 : 32] Region nonce, bits [63:32]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1KEYR0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_KEY                 \ [0x00 : 32] Region key, bits [31:0]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1KEYR1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_KEY                 \ [0x00 : 32] Region key, bits [63:32]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1KEYR2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_KEY                 \ [0x00 : 32] Region key, bits [95:64]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_R1KEYR3_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_REG_KEY                 \ [0x00 : 32] Region key, bits [127:96]
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_ICR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant OTFDEC_SEIF                    \ [0x00] Security error interrupt flag clear
    $01 constant OTFDEC_XONEIF                  \ [0x01] Execute-only execute-never error interrupt flag clear
    $02 constant OTFDEC_KEIF                    \ [0x02] Key error interrupt flag clear
  [then]
  ...
  [ifdef] OTFDEC_OTFDEC_IER_DEF
    ...
    $01 constant OTFDEC_XONEIE                  \ [0x01] Execute-only execute-never error interrupt enable
    $02 constant OTFDEC_KEIE                    \ [0x02] Key error interrupt enable
  [then]

  \
  \ @brief OTFDEC register bank
  \
  $00 constant OTFDEC_OTFDEC_CR         \ OTFDEC control register
  $10 constant OTFDEC_OTFDEC_PRIVCFGR   \ OTFDEC privileged access control configuration register
