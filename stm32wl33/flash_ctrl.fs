...
[ifndef] FLASH_CTRL_DEF
  ...
  [ifdef] FLASH_CTRL_CONFIG_DEF
    ...
    $06 constant FLASH_CTRL_SLEEP_SM            \ [0x06] Flash memory power-down mode enable in SLEEP mode This bit allows to have the Flash memory in power-down mode or in idle mode when the device is in Sleep mode. - 0: When the device is in Sleep mode, the NVM is in Idle mode. - 1: When the device is in Sleep mode, the NVM is in power-down mode.
  [then]
  ...
  [ifdef] FLASH_CTRL_SIZE_DEF
    ...
    \
    $00 constant FLASH_CTRL_FLASH_SIZE          \ [0x00 : 17] Maximum valid address for flash memory: - 00 : 0x03FFF (64kb) - 01 : 0x07FFF (128kb) - 10 : 0x0BFFF (192kb) - 11 : 0x0FFFF (256kb)
  ...
  [ifdef] FLASH_CTRL_UNLOCK012_DEF
    \
    \ @brief UNLOCK012 register
    \ Address offset: 0x50
    \ Reset value: 0xFFFFFFFF
    \
    $00 constant FLASH_CTRL_UNLOCK012           \ [0x00 : 32] (NOT TO BE DOCUMENTED) Remove read-write protection from IFR0, IFR1, IFR2 sectors
  [then]


  [ifdef] FLASH_CTRL_UNLOCK3_DEF
    \
    \ @brief UNLOCK3 register
    \ Address offset: 0x54
    \ Reset value: 0xFFFFFFFF
    \
    $00 constant FLASH_CTRL_UNLOCK3             \ [0x00 : 32] (NOT TO BE DOCUMENTED) Remove read-write protection from IFR3 sector
  [then]
  ...
  $50 constant FLASH_CTRL_UNLOCK012     \ UNLOCK012 register
  $54 constant FLASH_CTRL_UNLOCK3       \ UNLOCK3 register
