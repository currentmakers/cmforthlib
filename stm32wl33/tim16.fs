...
$00000007 constant TIM16_SMCR_SMS_2_0                               \ SMS[3:0]: Slave mode selection When external signals are selected the active edge of the trigger signal (TRGI) is linked to the polarity selected on the external input (see Input Control register and Control Register description.
$00000070 constant TIM16_SMCR_TS_2_0                                \ TS[4:0]: Trigger selection This bitfield selects the trigger input to be used to synchronize the counter. 00000: Internal Trigger 0 (ITR0) 00001: Internal Trigger 1 (ITR1) 00010: Internal Trigger 2 (ITR2) 00011: Internal Trigger 3 (ITR3) 00100: TI1 Edge Detector (TI1F_ED) 00101: Filtered Timer Input 1 (TI1FP1) Other codes: Reserved Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition. See Table 79 in IUM: TIM16 register map and reset values on page 469 for more details on ITRx meaning for each Timer.
$00000080 constant TIM16_SMCR_MSM                                   \ MSM: Master/slave mode 0: No action 1: The effect of an event on the trigger input (TRGI) is delayed to allow a perfect synchronization between the current timer and its slaves (through TRGO). It is useful if we want to synchronize several timers on a single external event.
$00010000 constant TIM16_SMCR_SMS_3                                 \ SMS[3:0]: Slave mode selection. See SMS_LSB description
$00300000 constant TIM16_SMCR_TS_4_3                                \ TS[4:0]: Trigger selection. See TS_LSB description
...
$00000040 constant TIM16_DIER_TIE                                   \ TIE: Trigger interrupt enable 0: Trigger interrupt disabled 1: Trigger interrupt enabled
...
$00004000 constant TIM16_DIER_TDE                                   \ TDE: Trigger DMA request enable 0: Trigger DMA request disabled 1: Trigger DMA request enabled
...
$00000040 constant TIM16_EGR_TG                                     \ TG: Trigger generation This bit is set by software in order to generate an event, it is automatically cleared by hardware. 0: No action 1: The TIF flag is set in TIMx_SR register. Related interrupt or DMA transfer can occur if enabled
...
$00000070 constant TIM16_CCMR1_OC1M_2_0                             \ OC1M[2:0]: Output Compare 1 mode (bits 2 to 0) These bits define the behavior of the output reference signal OC1REF from which OC1 and OC1N are derived. OC1REF is active high whereas OC1 and OC1N active level depends on CC1P and CC1NP bits.
...
$00000001 constant TIM16_OR1_OR1_0                                  \ Not used in Blue51. Not available in IUM
$00000006 constant TIM16_OR1_TI1_RMP                                \ TI1_RMP[1:0]: Timer 16 input 1 connection This bit is set and cleared by software. 00: TIM16 TI1 is connected to GPIO 01: TIM16 TI1 is connected to LCO 10: TIM16 TI1 is connected to COMP_OUT 11: TIM16 TI1 is connected to MCO
...
$0000000f constant TIM16_TISEL_TI1SEL                               \ TI1SEL[3:0]: selects TI1[0] to TI1[15] input 0000: TIMx_CH1 input Others: Reserved
...
$40005008 constant TIM16_SMCR     \ offset: 0x08 : SMCR register
...
$40005018 constant TIM16_CCMR1_IN  \ offset: 0x18 : CCMR1_in register
...
$40005050 constant TIM16_OR1      \ offset: 0x50 : OR1 register
