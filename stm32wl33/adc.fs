...
$000ff000 constant ADC_COMP_1_OFFSET1                               \ OFFSET1[7:0]: first calibration point
...
$000ff000 constant ADC_COMP_2_OFFSET2                               \ OFFSET2[7:0]: second calibration point
...
$000ff000 constant ADC_COMP_3_OFFSET3                               \ OFFSET3[7:0]: third calibration point
...
$000ff000 constant ADC_COMP_4_OFFSET4                               \ OFFSET4[7:0]: fourth calibration point
...
$00003000 constant ADC_COMP_SEL_OFFSET_GAIN6                        \ OFFSET_GAIN6[1:0]: gain / offset used in ADC single negative mode with Vinput range = 3.6V. This field also selects the gain/offset for VBAT input:: 00: OFFSET1 and GAIN1 from COMP_1 01: OFFSET2 and GAIN2 from COMP_2 10: OFFSET3 and GAIN3 from COMP_3 11: OFFSET4 and GAIN4 from COMP_4
...
$0000ffff constant ADC_TEST_CONF_ADC_SWITCH_EN                      \ ADC_SWITCH_EN[15:0]: enable individually each connection of the switching matrix at the ADC input. For each bit: 0: switch X is ON 1: switch X is OFF Bit mapping (corresponding to AUXADC_INSEL_1V2[15:0]): Bit 0: VINM[0] to ADC negative input Bit 1: VINM[1] to ADC negative input Bit 2: VINM[2] to ADC negative input Bit 3: VINM[3] to ADC negative input Bit4: GND to ADC negative input Bit5: VBAT to ADC negative input Bit6: GND to ADC negative input Bit7: VDDA to ADC negative input Bit8: VINP[0] to ADC positive input Bit9: VINP[1] to ADC positive input Bit10: VINP[2] to ADC positive input Bit11: VINP[3] to ADC positive input Bit12: VBAT to ADC positive input Bit13: TEMP to ADC positive input Bit14: GND to ADC positive input Bit15: VDDA to ADC positive input.
$000c0000 constant ADC_TEST_CONF_SEL_VIN_TYPE                       \ SEL_VIN_TYPE[1:0]: operation mode of the selected VIN 00: ADC single negative input 01: ADC single positive input 10: ADC differential input mode 11: reserved
$00200000 constant ADC_TEST_CONF_ADC_RUN                            \ ADC_RUN: Start/stop ADC conversion. 0: stop the ADC conversion, 1: starts the ADC conversion.
$00400000 constant ADC_TEST_CONF_ADC_ENABLE                         \ ADC_ENABLE: 0: disable the ADC (power OFF) 1: enable the ADC (power ON)
...
$0000000f constant ADC_DTB_CONF_ADC_DBG_CONF                        \ ADC_DBG_CONF[3:0]: use for debug purpose.
$00000300 constant ADC_DTB_CONF_ADC_DTB_CONF                        \ ADC_DTB_CONF[1:0]: configure the DTB output. 00: DTB bus is all 0 01: output the ADC_BUSY, ADC_EOC, offset compensation data[11:0] on the ADC_DTB 10: output the DS information on the ADC_DTB 11: select states of the FSM and enable ADC serial output Note: detailed DTB configurations are available in the Table 38 in IUM
$00000400 constant ADC_DTB_CONF_DTB_SER_SEL                         \ DTB_SER_SEL: DTB serial output selection when ADC_DB_CONF[1:0]=3d 0: pre down-sampler with offset compensation data 1: post down-sampler data
$00ff0000 constant ADC_DTB_CONF_FSM_STATE                           \ FSM_STATE[7:0]: show the state of the state machine. Bit 0: IDLE Bit 1: Reserved Bit 2: ADC setup phase Bit 3: Reserved Bit 4: ADC_START_CONV resynchronization Bit 5: Reserved Bit 6: ADC mode Bit 7: sequence mode
$07000000 constant ADC_DTB_CONF_FSM_CUR_STATE                       \ FSM_CUR_STATE[2:0]: show the last executed state by the state machine. 000: IDLE mode 001: Reserved 010: ADC setup phase 011: Reserved 100: ADC_START_CONV resynchronization 101: Reserved 110: ADC mode 111: sequence mode
...
$41006060 constant ADC_TEST_CONF  \ offset: 0x60 : TEST_CONF register
$41006064 constant ADC_DTB_CONF   \ offset: 0x64 : DTB_CONF register
