...
$00000001 constant ADC3_ADC_ISR_ADRDY                               \ ADC ready This bit is set by hardware after the ADC has been enabled (bit ADEN = 1) and when the ADC reaches a state where it is ready to accept conversion requests. It is cleared by software writing 1 to it.
...
$00000400 constant ADC3_ADC_ISR_JQOVF                               \ Injected context queue overflow This bit is set by hardware when an Overflow of the Injected Queue of Context occurs. It is cleared by software writing 1 to it. Refer to Section 16.4.21: Queue of context for injected conversions for more information.
...
$00200000 constant ADC3_ADC_CFGR_JQM                                \ JSQR queue mode This bit is set and cleared by software. It defines how an empty Queue is managed. Refer to Section 16.4.21: Queue of context for injected conversions for more information. Note: The software is allowed to write this bit only when JADSTART = 0 (which ensures that no injected conversion is ongoing). Note: When dual mode is enabled (DUAL bits in ADCx_CCR register are not equal to zero), the bit JQM of the slave ADC is no more writable and its content is equal to the bit JQM of the master ADC.
...
$0000ffff constant ADC3_ADC_DR_RDATA                                \ Regular data converted These bits are read-only. They contain the conversion result from the last converted regular channel. The data are left- or right-aligned as described in Section 16.4.26: Data management.
...
$0000003c constant ADC3_ADC_JSQR_JEXTSEL                            \ External Trigger Selection for injected group These bits select the external event used to trigger the start of conversion of an injected group: ... Note: The software is allowed to write these bits only when JADSTART = 0 (which ensures that no injected conversion is ongoing).
$000000c0 constant ADC3_ADC_JSQR_JEXTEN                             \ External Trigger Enable and Polarity Selection for injected channels These bits are set and cleared by software to select the external trigger polarity and enable the trigger of an injected group. Note: The software is allowed to write these bits only when JADSTART = 0 (which ensures that no injected conversion is ongoing). Note: If JQM = 1 and if the Queue of Context becomes empty, the software and hardware triggers of the injected sequence are both internally disabled (refer to Section 16.4.21: Queue of context for injected conversions)
$00001f00 constant ADC3_ADC_JSQR_JSQ1                               \ 1st conversion in the injected sequence These bits are written by software with the channel number (0 to 18) assigned as the 1st in the injected conversion sequence. Note: The software is allowed to write these bits only when JADSTART = 0 (which ensures that no injected conversion is ongoing).
$0007c000 constant ADC3_ADC_JSQR_JSQ2                               \ 2nd conversion in the injected sequence These bits are written by software with the channel number (0 to 18) assigned as the 2nd in the injected conversion sequence. Note: The software is allowed to write these bits only when JADSTART = 0 (which ensures that no injected conversion is ongoing).
$01f00000 constant ADC3_ADC_JSQR_JSQ3                               \ 3rd conversion in the injected sequence These bits are written by software with the channel number (0 to 18) assigned as the 3rd in the injected conversion sequence. Note: The software is allowed to write these bits only when JADSTART = 0 (which ensures that no injected conversion is ongoing).
...
$80000000 constant ADC3_ADC_OFR1_OFFSET_EN                          \ Offset y enable This bit is written by software to enable or disable the offset programmed into bits OFFSETy[11:0]. Note: The software is allowed to write this bit only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing).
...
$80000000 constant ADC3_ADC_OFR2_OFFSET_EN                          \ Offset y enable This bit is written by software to enable or disable the offset programmed into bits OFFSETy[11:0]. Note: The software is allowed to write this bit only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing).
...
$00000fff constant ADC3_ADC_OFR3_OFFSET                             \ Data offset y for the channel programmed into bits OFFSETy_CH[4:0] These bits are written by software to define the offset y to be subtracted from the raw converted data when converting a channel (can be regular or injected). The channel to which applies the data offset y must be programmed in the bits OFFSETy_CH[4:0]. The conversion result can be read from in the ADC_DR (regular conversion) or from in the ADC_JDRyi registers (injected conversion). Note: The software is allowed to write these bits only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing). If several offset (OFFSETy) point to the same channel, only the offset with the lowest x value is considered for the subtraction. Note: Ex: if OFFSET1_CH[4:0]=4 and OFFSET2_CH[4:0]=4, this is OFFSET1[11:0] which is subtracted when converting channel 4.
$7c000000 constant ADC3_ADC_OFR3_OFFSET_CH                          \ Channel selection for the data offset y These bits are written by software to define the channel to which the offset programmed into bits OFFSETy[11:0] applies. Note: The software is allowed to write these bits only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing). Note: Some channels are not connected physically and must not be selected for the data offset y.
$80000000 constant ADC3_ADC_OFR3_OFFSET_EN                          \ Offset y enable This bit is written by software to enable or disable the offset programmed into bits OFFSETy[11:0]. Note: The software is allowed to write this bit only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing).
...
$00000fff constant ADC3_ADC_OFR4_OFFSET                             \ Data offset y for the channel programmed into bits OFFSETy_CH[4:0] These bits are written by software to define the offset y to be subtracted from the raw converted data when converting a channel (can be regular or injected). The channel to which applies the data offset y must be programmed in the bits OFFSETy_CH[4:0]. The conversion result can be read from in the ADC_DR (regular conversion) or from in the ADC_JDRyi registers (injected conversion). Note: The software is allowed to write these bits only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing). If several offset (OFFSETy) point to the same channel, only the offset with the lowest x value is considered for the subtraction. Note: Ex: if OFFSET1_CH[4:0]=4 and OFFSET2_CH[4:0]=4, this is OFFSET1[11:0] which is subtracted when converting channel 4.
$7c000000 constant ADC3_ADC_OFR4_OFFSET_CH                          \ Channel selection for the data offset y These bits are written by software to define the channel to which the offset programmed into bits OFFSETy[11:0] applies. Note: The software is allowed to write these bits only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing). Note: Some channels are not connected physically and must not be selected for the data offset y.
$80000000 constant ADC3_ADC_OFR4_OFFSET_EN                          \ Offset y enable This bit is written by software to enable or disable the offset programmed into bits OFFSETy[11:0]. Note: The software is allowed to write this bit only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing).
...
$0000ffff constant ADC3_ADC_JDR1_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 16.4.26: Data management.
...
$0000ffff constant ADC3_ADC_JDR2_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 16.4.26: Data management.
...
$0000ffff constant ADC3_ADC_JDR3_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 16.4.26: Data management.
...
$0000ffff constant ADC3_ADC_JDR4_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 16.4.26: Data management.
...
$0007ffff constant ADC3_ADC_AWD2CR_AWD2CH                           \ Analog watchdog 2 channel selection These bits are set and cleared by software. They enable and select the input channels to be guarded by the analog watchdog 2. AWD2CH[i] = 0: ADC analog input channel i is not monitored by AWD2 AWD2CH[i] = 1: ADC analog input channel i is monitored by AWD2 When AWD2CH[18:0] = 000..0, the analog watchdog 2 is disabled Note: The channels selected by AWD2CH must be also selected into the SQRi or JSQRi registers. Note: The software is allowed to write these bits only when ADSTART = 0 and JADSTART = 0 (which ensures that no conversion is ongoing). Note: Some channels are not connected physically and must not be selected for the analog watchdog.
...
\ @brief ADC3 global Interrupt
\
$50040200 constant ADC3_ADC_ISR   \ offset: 0x00 : ADC interrupt and status register
$50040204 constant ADC3_ADC_IER   \ offset: 0x04 : ADC interrupt enable register
$50040208 constant ADC3_ADC_CR    \ offset: 0x08 : ADC control register
$5004020c constant ADC3_ADC_CFGR  \ offset: 0x0C : ADC configuration register
$50040210 constant ADC3_ADC_CFGR2  \ offset: 0x10 : ADC configuration register 2
$50040214 constant ADC3_ADC_SMPR1  \ offset: 0x14 : ADC sample time register 1
$50040218 constant ADC3_ADC_SMPR2  \ offset: 0x18 : ADC sample time register 2
$50040220 constant ADC3_ADC_TR1   \ offset: 0x20 : ADC watchdog threshold register 1
$50040224 constant ADC3_ADC_TR2   \ offset: 0x24 : ADC watchdog threshold register 2
$50040228 constant ADC3_ADC_TR3   \ offset: 0x28 : ADC watchdog threshold register 3
$50040230 constant ADC3_ADC_SQR1  \ offset: 0x30 : ADC regular sequence register 1
$50040234 constant ADC3_ADC_SQR2  \ offset: 0x34 : ADC regular sequence register 2
$50040238 constant ADC3_ADC_SQR3  \ offset: 0x38 : ADC regular sequence register 3
$5004023c constant ADC3_ADC_SQR4  \ offset: 0x3C : ADC regular sequence register 4
$50040240 constant ADC3_ADC_DR    \ offset: 0x40 : ADC regular data register
$5004024c constant ADC3_ADC_JSQR  \ offset: 0x4C : ADC injected sequence register
$50040260 constant ADC3_ADC_OFR1  \ offset: 0x60 : ADC offset 1 register
$50040264 constant ADC3_ADC_OFR2  \ offset: 0x64 : ADC offset 2 register
$50040268 constant ADC3_ADC_OFR3  \ offset: 0x68 : ADC offset 3 register
$5004026c constant ADC3_ADC_OFR4  \ offset: 0x6C : ADC offset 4 register
$50040280 constant ADC3_ADC_JDR1  \ offset: 0x80 : ADC injected channel 1 data register
$50040284 constant ADC3_ADC_JDR2  \ offset: 0x84 : ADC injected channel 2 data register
$50040288 constant ADC3_ADC_JDR3  \ offset: 0x88 : ADC injected channel 3 data register
$5004028c constant ADC3_ADC_JDR4  \ offset: 0x8C : ADC injected channel 4 data register
$500402a0 constant ADC3_ADC_AWD2CR  \ offset: 0xA0 : ADC analog watchdog 2 configuration register
$500402a4 constant ADC3_ADC_AWD3CR  \ offset: 0xA4 : ADC analog watchdog 3 configuration register
$500402b0 constant ADC3_ADC_DIFSEL  \ offset: 0xB0 : ADC differential mode selection register
$500402b4 constant ADC3_ADC_CALFACT  \ offset: 0xB4 : ADC calibration factors
