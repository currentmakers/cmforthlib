...
$00000003 constant SEC_SDMMC1_SDMMC_POWER_PWRCTRL                   \ SDMMC state control bits. These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL?11). These bits are used to define the functional state of the SDMMC signals: Any further write will be ignored, PWRCTRL value will keep 11.
...
$00010000 constant SEC_SDMMC1_SDMMC_CLKCR_NEGEDGE                   \ SDMMC_CK dephasing selection bit for data and Command. This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0). When clock division = 1 (CLKDIV = 0), this bit has no effect. Data and Command change on SDMMC_CK falling edge. When clock division &gt;1 (CLKDIV &gt; 0) &amp; DDR = 0: - SDMMC_CK edge occurs on SDMMCCLK rising edge. When clock division >1 (CLKDIV > 0) & DDR = 1: - Data changed on the SDMMCCLK falling edge succeeding a SDMMC_CK edge. - SDMMC_CK edge occurs on SDMMCCLK rising edge. - Data changed on the SDMMC_CK falling edge succeeding a SDMMC_CK edge. - SDMMC_CK edge occurs on SDMMCCLK rising edge.
...
$00080000 constant SEC_SDMMC1_SDMMC_CLKCR_BUSSPEED                  \ Bus speed mode selection between DS, HS, SDR12, SDR25 and SDR50, DDR50, SDR104. This bit can only be written when the CPSM and DPSM are not active (CPSMACT = 0 and DPSMACT = 0)
...
$00000400 constant SEC_SDMMC1_SDMMC_CMDR_WAITINT                    \ CPSM waits for interrupt request. If this bit is set, the CPSM disables command timeout and waits for an card interrupt request (Response). If this bit is cleared in the CPSM Wait state, will cause the abort of the interrupt mode.
...
$00002000 constant SEC_SDMMC1_SDMMC_CMDR_DTHOLD                     \ Hold new data block transmission and reception in the DPSM. If this bit is set, the DPSM will not move from the Wait_S state to the Send state or from the Wait_R state to the Receive state.
...
$0000003f constant SEC_SDMMC1_SDMMC_RESPCMDR_RESPCMD                \ Response command index
...
$00000000 constant SEC_SDMMC1_SDMMC_RESP1R_CARDSTATUS1              \ see Table 432
...
$00000000 constant SEC_SDMMC1_SDMMC_RESP2R_CARDSTATUS2              \ see Table404.
...
$00000000 constant SEC_SDMMC1_SDMMC_RESP3R_CARDSTATUS3              \ see Table404.
...
$00000000 constant SEC_SDMMC1_SDMMC_RESP4R_CARDSTATUS4              \ see Table404.
...
$00000200 constant SEC_SDMMC1_SDMMC_DCTRL_RWSTOP                    \ Read wait stop This bit is written by firmware and auto cleared by hardware when the DPSM moves from the READ_WAIT state to the WAIT_R or IDLE state.
...
$00002000 constant SEC_SDMMC1_SDMMC_DCTRL_FIFORST                   \ FIFO reset, will flush any remaining data. This bit can only be written by firmware when IDMAEN= 0 and DPSM is active (DPSMACT = 1). This bit will only take effect when a transfer error or transfer hold occurs.
...
$00000010 constant SEC_SDMMC1_SDMMC_STAR_TXUNDERR                   \ Transmit FIFO underrun error or IDMA read transfer error. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$00000020 constant SEC_SDMMC1_SDMMC_STAR_RXOVERR                    \ Received FIFO overrun error or IDMA write transfer error. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
...
$00000100 constant SEC_SDMMC1_SDMMC_STAR_DATAEND                    \ Data transfer ended correctly. (data counter, DATACOUNT is zero and no errors occur). Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
...
$00000400 constant SEC_SDMMC1_SDMMC_STAR_DBCKEND                    \ Data block sent/received. (CRC check passed) and DPSM moves to the READWAIT state. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
...
$00400000 constant SEC_SDMMC1_SDMMC_STAR_SDIOIT                     \ SDIO interrupt received. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$00800000 constant SEC_SDMMC1_SDMMC_STAR_ACKFAIL                    \ Boot acknowledgment received (boot acknowledgment check fail). Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$01000000 constant SEC_SDMMC1_SDMMC_STAR_ACKTIMEOUT                 \ Boot acknowledgment timeout. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$02000000 constant SEC_SDMMC1_SDMMC_STAR_VSWEND                     \ Voltage switch critical timing section completion. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$04000000 constant SEC_SDMMC1_SDMMC_STAR_CKSTOP                     \ SDMMC_CK stopped in Voltage switch procedure. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$08000000 constant SEC_SDMMC1_SDMMC_STAR_IDMATE                     \ IDMA transfer error. Interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
$10000000 constant SEC_SDMMC1_SDMMC_STAR_IDMABTC                    \ IDMA buffer transfer complete. interrupt flag is cleared by writing corresponding interrupt clear bit in SDMMC_ICR.
...
$00000004 constant SEC_SDMMC1_SDMMC_IDMACTRLR_IDMABACT              \ Double buffer mode active buffer indication This bit can only be written by firmware when DPSM is inactive (DPSMACT = 0). When IDMA is enabled this bit is toggled by hardware.
...
$00001fe0 constant SEC_SDMMC1_SDMMC_IDMABSIZER_IDMABNDT             \ Number of transfers per buffer. This 8-bit value shall be multiplied by 8 to get the size of the buffer in 32-bit words and by 32 to get the size of the buffer in bytes. Example: IDMABNDT = 0x01: buffer size = 8 words = 32 bytes. These bits can only be written by firmware when DPSM is inactive (DPSMACT = 0).
...
$00000000 constant SEC_SDMMC1_SDMMC_IDMABASE0R_IDMABASE0            \ Buffer 0 memory base address bits [31:2], shall be word aligned (bit [1:0] are always 0 and read only). This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1) and memory buffer 0 is inactive (IDMABACT = 1).
...
$00000000 constant SEC_SDMMC1_SDMMC_FIFOR_FIFODATA                  \ Receive and transmit FIFO data This register can only be read or written by firmware when the DPSM is active (DPSMACT=1). The FIFO data occupies 16 entries of 32-bit words.
...
$0000000f constant SEC_SDMMC1_SDMMC_VER_MINREV                      \ IP minor revision number.
$000000f0 constant SEC_SDMMC1_SDMMC_VER_MAJREV                      \ IP major revision number.
...
$00000000 constant SEC_SDMMC1_SDMMC_ID_IP_ID                        \ SDMMC IP identification.


\
\ @brief SDMMC1
\
$520c8000 constant SEC_SDMMC1_SDMMC_POWER  \ offset: 0x00 : SDMMC power control register
$520c8004 constant SEC_SDMMC1_SDMMC_CLKCR  \ offset: 0x04 : The SDMMC_CLKCR register controls the SDMMC_CK output clock, the SDMMC_RX_CLK receive clock, and the bus width.
$520c8008 constant SEC_SDMMC1_SDMMC_ARGR  \ offset: 0x08 : The SDMMC_ARGR register contains a 32-bit command argument, which is sent to a card as part of a command message.
$520c800c constant SEC_SDMMC1_SDMMC_CMDR  \ offset: 0x0C : The SDMMC_CMDR register contains the command index and command type bits. The command index is sent to a card as part of a command message. The command type bits control the command path state machine (CPSM).
$520c8010 constant SEC_SDMMC1_SDMMC_RESPCMDR  \ offset: 0x10 : SDMMC command response register
$520c8014 constant SEC_SDMMC1_SDMMC_RESP1R  \ offset: 0x14 : The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
$520c8018 constant SEC_SDMMC1_SDMMC_RESP2R  \ offset: 0x18 : The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
$520c801c constant SEC_SDMMC1_SDMMC_RESP3R  \ offset: 0x1C : The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
$520c8020 constant SEC_SDMMC1_SDMMC_RESP4R  \ offset: 0x20 : The SDMMC_RESP1/2/3/4R registers contain the status of a card, which is part of the received response.
$520c8024 constant SEC_SDMMC1_SDMMC_DTIMER  \ offset: 0x24 : The SDMMC_DTIMER register contains the data timeout period, in card bus clock periods. A counter loads the value from the SDMMC_DTIMER register, and starts decrementing when the data path state machine (DPSM) enters the Wait_R or Busy state. If the timer reaches 0 while the DPSM is in either of these states, the timeout status flag is set.
$520c8028 constant SEC_SDMMC1_SDMMC_DLENR  \ offset: 0x28 : The SDMMC_DLENR register contains the number of data bytes to be transferred. The value is loaded into the data counter when data transfer starts.
$520c802c constant SEC_SDMMC1_SDMMC_DCTRL  \ offset: 0x2C : The SDMMC_DCTRL register control the data path state machine (DPSM).
$520c8030 constant SEC_SDMMC1_SDMMC_DCNTR  \ offset: 0x30 : The SDMMC_DCNTR register loads the value from the data length register (see SDMMC_DLENR) when the DPSM moves from the Idle state to the Wait_R or Wait_S state. As data is transferred, the counter decrements the value until it reaches 0. The DPSM then moves to the Idle state and when there has been no error, the data status end flag (DATAEND) is set.
$520c8034 constant SEC_SDMMC1_SDMMC_STAR  \ offset: 0x34 : The SDMMC_STAR register is a read-only register. It contains two types of flag:Static flags (bits [29,21,11:0]): these bits remain asserted until they are cleared by writing to the SDMMC interrupt Clear register (see SDMMC_ICR)Dynamic flags (bits [20:12]): these bits change state depending on the state of the underlying logic (for example, FIFO full and empty flags are asserted and de-asserted as data while written to the FIFO)
$520c8038 constant SEC_SDMMC1_SDMMC_ICR  \ offset: 0x38 : The SDMMC_ICR register is a write-only register. Writing a bit with 1 clears the corresponding bit in the SDMMC_STAR status register.
$520c803c constant SEC_SDMMC1_SDMMC_MASKR  \ offset: 0x3C : The interrupt mask register determines which status flags generate an interrupt request by setting the corresponding bit to 1.
$520c8040 constant SEC_SDMMC1_SDMMC_ACKTIMER  \ offset: 0x40 : The SDMMC_ACKTIMER register contains the acknowledgment timeout period, in SDMMC_CK bus clock periods. A counter loads the value from the SDMMC_ACKTIMER register, and starts decrementing when the data path state machine (DPSM) enters the Wait_Ack state. If the timer reaches 0 while the DPSM is in this states, the acknowledgment timeout status flag is set.
$520c8050 constant SEC_SDMMC1_SDMMC_IDMACTRLR  \ offset: 0x50 : The receive and transmit FIFOs can be read or written as 32-bit wide registers. The FIFOs contain 32 entries on 32 sequential addresses. This allows the CPU to use its load and store multiple operands to read from/write to the FIFO.
$520c8054 constant SEC_SDMMC1_SDMMC_IDMABSIZER  \ offset: 0x54 : The SDMMC_IDMABSIZER register contains the buffers size when in double buffer configuration.
$520c8058 constant SEC_SDMMC1_SDMMC_IDMABASE0R  \ offset: 0x58 : The SDMMC_IDMABASE0R register contains the memory buffer base address in single buffer configuration and the buffer 0 base address in double buffer configuration.
$520c805c constant SEC_SDMMC1_SDMMC_IDMABASE1R  \ offset: 0x5C : The SDMMC_IDMABASE1R register contains the double buffer configuration second buffer memory base address.
$520c8080 constant SEC_SDMMC1_SDMMC_FIFOR  \ offset: 0x80 : The receive and transmit FIFOs can be only read or written as word (32-bit) wide registers. The FIFOs contain 16 entries on sequential addresses. This allows the CPU to use its load and store multiple operands to read from/write to the FIFO.When accessing SDMMC_FIFOR with half word or byte access an AHB bus fault is generated.
$520c83f4 constant SEC_SDMMC1_SDMMC_VER  \ offset: 0x3F4 : SDMMC IP version register
$520c83f8 constant SEC_SDMMC1_SDMMC_ID  \ offset: 0x3F8 : SDMMC IP identification register
