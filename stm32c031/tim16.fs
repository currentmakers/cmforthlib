...
[ifndef] TIM16_DEF
  ...
  [ifdef] TIM16_TIM16_CR1_DEF
    ...
    $08 constant TIM16_CKD                      \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (CK_INT) frequency and the dead-time and sampling clock (tDTS)used by the dead-time generators and the digital filters (TIx),
  ...
  [ifdef] TIM16_TIM16_CCER_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM16_CC1E                     \ [0x00] Capture/Compare 1 output enable When CC1 channel is configured as output, the OC1 level depends on MOE, OSSI, OSSR, OIS1, OIS1N and CC1NE bits, regardless of the CC1E bits state. Refer to for details.
  ...
  [ifdef] TIM16_TIM16_ARR_DEF
    ...
    \
    $00 constant TIM16_ARR                      \ [0x00 : 16] Auto-reload value ARR is the value to be loaded in the actual auto-reload register. Refer to the for more details about ARR update and behavior. The counter is blocked while the auto-reload value is null.
  [then]
  ...
  [ifdef] TIM16_TIM16_BDTR_DEF
    ...
    $0c constant TIM16_BKE                      \ [0x0c] Break enable 1; Break inputs (BRK and CCS clock failure event) enabled Note: This bit cannot be modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Any write operation to this bit takes a delay of 1 APB clock cycle to become effective.
    $0d constant TIM16_BKP                      \ [0x0d] Break polarity Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Any write operation to this bit takes a delay of 1 APB clock cycle to become effective.
  ...
  [ifdef] TIM16_TIM16_AF1_DEF
    ...
    \
    $00 constant TIM16_BKINE                    \ [0x00] BRK BKIN input enable This bit enables the BKIN alternate function input for the timers BRK input. BKIN input is ORed with the other BRK sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
  ...
  [ifdef] TIM16_TIM16_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM16_TI1SEL                   \ [0x00 : 4] selects TI1[0] to TI1[15] input Others: Reserved
  [then]
  ...
  $18 constant TIM16_TIM16_CCMR1_INPUT  \ TIM16 capture/compare mode register 1 [alternate]
