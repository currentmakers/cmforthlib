...
$00fe0000 constant I3C2_I3C_CR_ADD                                  \ 7-bit I3C dynamic / I less than sup>2 less than /sup>C static target address (when I3C acts as controller)
...
$00ff0000 constant I3C2_I3C_TIMINGR0_SCLL_OD                        \ SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles:
$ff000000 constant I3C2_I3C_TIMINGR0_SCLH_I2C                       \ SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles:
...
$10000000 constant I3C2_I3C_TIMINGR1_SDA_HD                         \ SDA hold time (when the I3C acts as controller), in number of kernel clocks cycles (refer to MIPI timing SDA hold time in push-pull t less than sub>HD_PP less than /sub>):
...
$00000001 constant I3C2_I3C_TIMINGR2_STALLT                         \ Controller clock stall enable on T-bit phase of data (and on the ACK/NACK phase of data byte of a legacy I less than sup>2 less than /sup>C read)
...
$01000000 constant I3C2_I3C_GETMXDSR_TSCO                           \ clock-to-data turnaround time (t less than sub>SCO less than /sub>)
