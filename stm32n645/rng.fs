...
[ifndef] RNG_DEF
  ...
  [ifdef] RNG_RNG_NSCR_DEF
    ...
    \
    $00 constant RNG_EN_OSC1                    \ [0x00 : 3] Each bit drives one oscillator enable signal input of instance number 1, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
    $03 constant RNG_EN_OSC2                    \ [0x03 : 3] Each bit drives one oscillator enable signal input of instance number 2, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
    $06 constant RNG_EN_OSC3                    \ [0x06 : 3] Each bit drives one oscillator enable signal input of instance number 3, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
    $09 constant RNG_EN_OSC4                    \ [0x09 : 3] Each bit drives one oscillator enable signal input of instance number 4, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
    $0c constant RNG_EN_OSC5                    \ [0x0c : 3] Each bit drives one oscillator enable signal input of instance number 5, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
    $0f constant RNG_EN_OSC6                    \ [0x0f : 3] Each bit drives one oscillator enable signal input of instance number 6, gated with the RNGEN bit in RNG_CR (set bit to enable the oscillator). Bit is ignored otherwise.
  [then]
