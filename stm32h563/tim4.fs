...
[ifndef] TIM4_DEF
  ...
  [ifdef] TIM4_TIM4_CR1_DEF
    ...
    $08 constant TIM4_CKD                       \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and sampling clock used by the digital filters (tim_etr_in, tim_tix),
    $0b constant TIM4_UIFREMAP                  \ [0x0b] UIF status bit remapping
    $0c constant TIM4_DITHEN                    \ [0x0c] Dithering Enable Note: The DITHEN bit can only be modified when CEN bit is reset.
  [then]
  ...
  [ifdef] TIM4_TIM4_CR2_DEF
    ...
    $04 constant TIM4_MMS1                      \ [0x04 : 3] Master mode selection These bits allow to select the information to be sent in master mode to slave timers for synchronization (tim_trgo). The combination is as follows: tim_trgo, except if the master/slave mode is selected (see the MSM bit description in TIMx_SMCR register). Others: Reserved Note: The clock of the slave timer or ADC must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
    $07 constant TIM4_TI1S                      \ [0x07] tim_ti1 selection
    $19 constant TIM4_MMS2                      \ [0x19] Master mode selection These bits allow to select the information to be sent in master mode to slave timers for synchronization (tim_trgo). The combination is as follows: tim_trgo, except if the master/slave mode is selected (see the MSM bit description in TIMx_SMCR register). Others: Reserved Note: The clock of the slave timer or ADC must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
  [then]
  ...
  [ifdef] TIM4_TIM4_SMCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_SMS                       \ [0x00 : 3] Slave mode selection When external signals are selected the active edge of the trigger signal (tim_trgi) is linked to the polarity selected on the external input (see Input Control register and Control Register description. Note: The gated mode must not be used if tim_ti1f_ed is selected as the trigger input (TS=00100). Indeed, tim_ti1f_ed outputs 1 pulse for each transition on tim_ti1f, whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals (timer, ADC, ...) receiving the tim_trgo signal must be enabled prior to receive events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly while triggers are received from the master timer.
    $03 constant TIM4_OCCS                      \ [0x03] OCREF clear selection This bit is used to select the OCREF clear source Note: If the OCREF clear selection feature is not supported, this bit is reserved and forced by hardware to '0'. .
    $04 constant TIM4_TS                        \ [0x04 : 3] Trigger selection (see bits 21:20 for TS[4:3]) This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See for product specific implementation details. Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition.
    $07 constant TIM4_MSM                       \ [0x07] Master/Slave mode
    $08 constant TIM4_ETF                       \ [0x08 : 4] External trigger filter This bit-field then defines the frequency used to sample tim_etrp signal and the length of the digital filter applied to tim_etrp. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
    $0c constant TIM4_ETPS                      \ [0x0c : 2] External trigger prescaler External trigger signal tim_etrp frequency must be at most 1/4 of tim_ker_ck frequency. A prescaler can be enabled to reduce tim_etrp frequency. It is useful when inputting fast external clocks on tim_etr_in.
    ...
    $0f constant TIM4_ETP                       \ [0x0f] External trigger polarity This bit selects whether tim_etr_in or tim_etr_in is used for trigger operations
    $10 constant TIM4_SMS_2                     \ [0x10] Slave mode selection When external signals are selected the active edge of the trigger signal (tim_trgi) is linked to the polarity selected on the external input (see Input Control register and Control Register description. Note: The gated mode must not be used if tim_ti1f_ed is selected as the trigger input (TS=00100). Indeed, tim_ti1f_ed outputs 1 pulse for each transition on tim_ti1f, whereas the gated mode checks the level of the trigger signal. Note: The clock of the slave peripherals (timer, ADC, ...) receiving the tim_trgo signal must be enabled prior to receive events from the master timer, and the clock frequency (prescaler) must not be changed on-the-fly while triggers are received from the master timer.
    $14 constant TIM4_TS_2                      \ [0x14 : 2] Trigger selection (see bits 21:20 for TS[4:3]) This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See for product specific implementation details. Note: These bits must be changed only when they are not used (e.g. when SMS=000) to avoid wrong edge detections at the transition.
    $18 constant TIM4_SMSPE                     \ [0x18] SMS preload enable This bit selects whether the SMS[3:0] bitfield is preloaded
    $19 constant TIM4_SMSPS                     \ [0x19] SMS preload source This bit selects whether the events that triggers the SMS[3:0] bitfield transfer from preload to active
  [then]
  ...
  [ifdef] TIM4_TIM4_SR_DEF
    ...
    $14 constant TIM4_IDXF                      \ [0x14] Index interrupt flag This flag is set by hardware when an index event is detected. It is cleared by software by writing it to '0'.
    $15 constant TIM4_DIRF                      \ [0x15] Direction change interrupt flag This flag is set by hardware when the direction changes in encoder mode (DIR bit value in TIMx_CR is changing). It is cleared by software by writing it to '0'.
    $16 constant TIM4_IERRF                     \ [0x16] Index error interrupt flag This flag is set by hardware when an index error is detected. It is cleared by software by writing it to '0'.
    $17 constant TIM4_TERRF                     \ [0x17] Transition error interrupt flag This flag is set by hardware when a transition error is detected in encoder mode. It is cleared by software by writing it to '0'.
  [then]
  ...
  [ifdef] TIM4_TIM4_CCMR1_Input_DEF
    ...
    $02 constant TIM4_IC1PSC                    \ [0x02 : 2] Input capture 1 prescaler This bit-field defines the ratio of the prescaler acting on CC1 input (tim_ic1). The prescaler is reset as soon as CC1E=0 (TIMx_CCER register).
    $04 constant TIM4_IC1F                      \ [0x04 : 4] Input capture 1 filter This bit-field defines the frequency used to sample tim_ti1 input and the length of the digital filter applied to tim_ti1. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
  ...
  [ifdef] TIM4_TIM4_CCMR1_Output_DEF
    ...
    $03 constant TIM4_OC1PE                     \ [0x03] Output compare 1 preload enable Note: The PWM mode can be used without validating the preload register only in one-pulse mode (OPM bit set in TIMx_CR1 register). Else the behavior is not guaranteed.
    $04 constant TIM4_OC1M1                     \ [0x04 : 3] Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Note: In PWM mode, the tim_ocref_clr level changes only when the result of the comparison changes or when the output compare mode switches from 'frozen' mode to 'PWM' mode.
    ...
    $0c constant TIM4_OC2M1                     \ [0x0c : 3] Output compare 2 mode refer to OC1M description on bits 6:4
    $0f constant TIM4_OC2CE                     \ [0x0f] Output compare 2 clear enable
    $10 constant TIM4_OC1M2                     \ [0x10] Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Note: In PWM mode, the tim_ocref_clr level changes only when the result of the comparison changes or when the output compare mode switches from 'frozen' mode to 'PWM' mode.
    $18 constant TIM4_OC2M2                     \ [0x18] Output compare 2 mode refer to OC1M description on bits 6:4
  [then]
  ...
  [ifdef] TIM4_TIM4_CCMR2_Output_DEF
    ...
    $04 constant TIM4_OC3M1                     \ [0x04 : 3] Output compare 3 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
    ...
    $0c constant TIM4_OC4M1                     \ [0x0c : 3] Output compare 4 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
    $0f constant TIM4_OC4CE                     \ [0x0f] Output compare 4 clear enable
    $10 constant TIM4_OC3M2                     \ [0x10] Output compare 3 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
    $18 constant TIM4_OC4M2                     \ [0x18] Output compare 4 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
  [then]
  ...
  [ifdef] TIM4_TIM4_CCER_DEF
    ...
    $03 constant TIM4_CC1NP                     \ [0x03] Capture/Compare 1 output Polarity. CC1 channel configured as output: CC1NP must be kept cleared in this case. CC1 channel configured as input: This bit is used in conjunction with CC1P to define tim_ti1fp1/tim_ti2fp1 polarity. refer to CC1P description.
  ...
  [ifdef] TIM4_TIM4_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_CNT                       \ [0x00 : 16] Counter value' Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register holds the non-dithered part in CNT[15:0]. The fractional part is not available.
    $1f constant TIM4_UIFCPY                    \ [0x1f] Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 Reserved If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
  [then]
  ...
  [ifdef] TIM4_TIM4_ECR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_IE                        \ [0x00] Index enable This bit indicates if the Index event resets the counter.
    $01 constant TIM4_IDIR                      \ [0x01 : 2] Index direction This bit indicates in which direction the Index event resets the counter. Note: The IDR[1:0] bitfield must be written when IE bit is reset (index disabled).
    $03 constant TIM4_IBLK                      \ [0x03 : 2] Index blanking This bit indicates if the Index event is conditioned by the tim_ti3 input
    $05 constant TIM4_FIDX                      \ [0x05] First index This bit indicates if the first index only is taken into account
    $06 constant TIM4_IPOS                      \ [0x06 : 2] Index positioning In quadrature encoder mode (SMS[3:0] = 0001, 0010, 0011, 1110, 1111), this bit indicates in which AB input configuration the Index event resets the counter. In directional clock mode or clock plus direction mode (SMS[3:0] = 1010, 1011, 1100, 1101), these bits indicates on which level the Index event resets the counter. In bidirectional clock mode, this applies for both clock inputs. x0: Index resets the counter when clock is 0 x1: Index resets the counter when clock is 1 Note: IPOS[1] bit is not significant
    $10 constant TIM4_PW                        \ [0x10 : 8] Pulse width This bitfield defines the pulse duration, as following: tPW = PW[7:0] x tPWG
    $18 constant TIM4_PWPRSC                    \ [0x18 : 3] Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tPWG = (2(PWPRSC[2:0])) x ttim_ker_ck
  [then]
  ...
  [ifdef] TIM4_TIM4_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_TI1SEL                    \ [0x00 : 4] Selects tim_ti1[0..15] input ... Refer to for product specific implementation.
    $08 constant TIM4_TI2SEL                    \ [0x08 : 4] Selects tim_ti2[0..15] input ... Refer to for product specific implementation.
    $10 constant TIM4_TI3SEL                    \ [0x10 : 4] Selects tim_ti3[0..15] input ... Refer to for product specific implementation.
    $18 constant TIM4_TI4SEL                    \ [0x18 : 4] Selects tim_ti4[0..15] input ... Refer to for product specific implementation.
  [then]
  ...
  [ifdef] TIM4_TIM4_AF1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant TIM4_ETRSEL                    \ [0x0e : 4] etr_in source selection These bits select the etr_in input source. ... Refer to for product specific implementation.
  [then]
  ...
  [ifdef] TIM4_TIM4_AF2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant TIM4_OCRSEL                    \ [0x10 : 3] ocref_clr source selection These bits select the ocref_clr input source. ... Refer to for product specific implementation.
  [then]
  ...
  $18 constant TIM4_TIM4_CCMR1_INPUT    \ TIM4 capture/compare mode register 1 [alternate]
  $18 constant TIM4_TIM4_CCMR1_OUTPUT   \ TIM4 capture/compare mode register 1 [alternate]
  $1C constant TIM4_TIM4_CCMR2_INPUT    \ TIM4 capture/compare mode register 2 [alternate]
  $1C constant TIM4_TIM4_CCMR2_OUTPUT   \ TIM4 capture/compare mode register 2 [alternate]
