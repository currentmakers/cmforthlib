...
[ifndef] ETH_DEF
  ...
  [ifdef] ETH_ETH_MACRWKPFR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_MACRWKPFR                  \ [0x00 : 32] Remote wakeup packet filter Refer to Table648, Table649 and Table650 for details on register content and programming sequence.
  [then]
  ...
  [ifdef] ETH_ETH_MACLCSR_DEF
    ...
    $01 constant ETH_TLPIEX                     \ [0x01] Transmit LPI Exit When this bit is set, it indicates that the MAC transmitter exited the LPI state after the application cleared the LPIEN bit and the LPI TW Timer has expired. This bit is cleared by a read into this register (or by writing it to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
  ...
  [ifdef] ETH_ETH_TX_LPI_TRAN_CNTR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TXLPITRC                   \ [0x00 : 32] Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the LPI control and status register (ETH_MACLCSR)), the counter is incremented.
  [then]
  ...
  [ifdef] ETH_ETH_MACTSCR_DEF
    ...
    $10 constant ETH_SNAPTYPSEL                 \ [0x10 : 2] Select PTP packets for Taking Snapshots These bits, along with Bits 15 and 14, define the set of PTP packet types for which snapshot needs to be taken. The encoding is given in Table634: Timestamp Snapshot Dependency on ETH_MACTSCR bits.
  ...
  [ifdef] ETH_ETH_MACSTSUR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TSS                        \ [0x00 : 32] Timestamp Seconds The value in this field is the seconds part of the update.
  [then]
  ...
  [ifdef] ETH_ETH_MACSTNUR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TSSS                       \ [0x00 : 31] Timestamp subseconds The value in this field is the subseconds part of the update.
  ...
  [ifdef] ETH_ETH_MACTSSR_DEF
    ...
    $01 constant ETH_TSTARGT0                   \ [0x01] Timestamp Target Time Reached When set, this bit indicates that the value of system time is greater than or equal to the value specified in the ETH_MACPPSTTSR and ETH_MACPPSTTSNR registers (see PPS target time seconds register (ETH_MACPPSTTSR) and PPS target time nanoseconds register (ETH_MACPPSTTNR)). This bit is cleared when the application reads this bit (or writes of 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set)
    $02 constant ETH_AUXTSTRIG                  \ [0x02] Auxiliary Timestamp Trigger Snapshot This bit is set high when the auxiliary snapshot is written to the FIFO. This bit is cleared when the application reads this bit (or writes it to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
    $03 constant ETH_TSTRGTERR0                 \ [0x03] Timestamp Target Time Error This bit is set when the latest target time programmed in the ETH_MACPPSTTSR and ETH_MACPPSTTSNR elapses (see PPS target time seconds register (ETH_MACPPSTTSR) and PPS target time nanoseconds register (ETH_MACPPSTTNR)). This bit is cleared when the application reads this bit (or writes it to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
    $0f constant ETH_TXTSSIS                    \ [0x0f] Tx Timestamp Status Interrupt Status When drop transmit status is enabled in MTL, this bit is set when the captured transmit timestamp is updated in the Tx timestamp status nanoseconds register (ETH_MACTXTSSNR) and Tx timestamp status seconds register (ETH_MACTXTSSSR). When PTP offload feature is enabled, this bit is set when the captured transmit timestamp is updated in the Tx timestamp status nanoseconds register (ETH_MACTXTSSNR) and Tx timestamp status seconds register (ETH_MACTXTSSSR), for PTO generated Delay Request and Pdelay request packets. This bit is cleared when the Tx timestamp status seconds register (ETH_MACTXTSSSR) is read (or written to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
  ...
  [ifdef] ETH_ETH_DMAMR_DEF
    ...
    $10 constant ETH_INTM                       \ [0x10 : 2] Interrupt Mode This field defines the interrupt mode of the Ethernet peripheral. The behavior of the interrupt signal and of the RI/TI bits in the ETH_DMACSR register changes depending on the INTM value (refer to Table651: Transfer complete interrupt behavior).
  [then]
