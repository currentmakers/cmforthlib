...
$00000000 constant ETH_ETH_MACRWKPFR_MACRWKPFR                      \ Remote wakeup packet filter Refer to Table648, Table649 and Table650 for details on register content and programming sequence.
...
$00000002 constant ETH_ETH_MACLCSR_TLPIEX                           \ Transmit LPI Exit When this bit is set, it indicates that the MAC transmitter exited the LPI state after the application cleared the LPIEN bit and the LPI TW Timer has expired. This bit is cleared by a read into this register (or by writing it to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
...
$00000fff constant ETH_ETH_MAC1USTCR_TIC_1US_CNTR                   \ 1s tick Counter The application must program this counter so that the number of clock cycles of CSR clock is 1s (subtract 1 from the value before programming). For example if the CSR clock is 100MHz then this field needs to be programmed to 1001 = 99 (which is 0x63). This is required to generate the 1s events that are used to update some of the EEE related counters.
...
$00000000 constant ETH_ETH_TX_LPI_TRAN_CNTR_TXLPITRC                \ Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the LPI control and status register (ETH_MACLCSR)), the counter is incremented.
...
$00030000 constant ETH_ETH_MACTSCR_SNAPTYPSEL                       \ Select PTP packets for Taking Snapshots These bits, along with Bits 15 and 14, define the set of PTP packet types for which snapshot needs to be taken. The encoding is given in Table634: Timestamp Snapshot Dependency on ETH_MACTSCR bits.
...
$00000000 constant ETH_ETH_MACSTSUR_TSS                             \ Timestamp Seconds The value in this field is the seconds part of the update.
...
$7fffffff constant ETH_ETH_MACSTNUR_TSSS                            \ Timestamp subseconds The value in this field is the subseconds part of the update.
...
$00000002 constant ETH_ETH_MACTSSR_TSTARGT0                         \ Timestamp Target Time Reached When set, this bit indicates that the value of system time is greater than or equal to the value specified in the ETH_MACPPSTTSR and ETH_MACPPSTTSNR registers (see PPS target time seconds register (ETH_MACPPSTTSR) and PPS target time nanoseconds register (ETH_MACPPSTTNR)). This bit is cleared when the application reads this bit (or writes of 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set)
$00000004 constant ETH_ETH_MACTSSR_AUXTSTRIG                        \ Auxiliary Timestamp Trigger Snapshot This bit is set high when the auxiliary snapshot is written to the FIFO. This bit is cleared when the application reads this bit (or writes it to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
...
$00008000 constant ETH_ETH_MACTSSR_TXTSSIS                          \ Tx Timestamp Status Interrupt Status When drop transmit status is enabled in MTL, this bit is set when the captured transmit timestamp is updated in the Tx timestamp status nanoseconds register (ETH_MACTXTSSNR) and Tx timestamp status seconds register (ETH_MACTXTSSSR). When PTP offload feature is enabled, this bit is set when the captured transmit timestamp is updated in the Tx timestamp status nanoseconds register (ETH_MACTXTSSNR) and Tx timestamp status seconds register (ETH_MACTXTSSSR), for PTO generated Delay Request and Pdelay request packets. This bit is cleared when the Tx timestamp status seconds register (ETH_MACTXTSSSR) is read (or written to 1 when RCWE bit in CSR software control register (ETH_MACCSRSWCR) is set).
...
$00000060 constant ETH_ETH_MACPPSCR_ALTERNATE_TRGTMODSEL0           \ Target Time Register Mode for PPS Output This field indicates the Target Time registers (PPS target time seconds register (ETH_MACPPSTTSR) and PPS target time nanoseconds register (ETH_MACPPSTTNR)) mode for PPS output signal:
...
$00030000 constant ETH_ETH_DMAMR_INTM                               \ Interrupt Mode This field defines the interrupt mode of the Ethernet peripheral. The behavior of the interrupt signal and of the RI/TI bits in the ETH_DMACSR register changes depending on the INTM value (refer to Table651: Transfer complete interrupt behavior).
