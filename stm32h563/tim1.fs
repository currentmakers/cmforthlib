...
[ifndef] TIM1_DEF
  ...
  [ifdef] TIM1_TIM1_CR1_DEF
    ...
    $08 constant TIM1_CKD                       \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and the dead-time and sampling clock (tsub DTS/sub )used by the dead-time generators and the digital filters (tim_etr_in, tim_tix),
  ...
  [ifdef] TIM1_TIM1_SR_DEF
    ...
    $05 constant TIM1_COMIF                     \ [0x05] COM interrupt flag This flag is set by hardware on COM event (when capture/compare Control bitsCCxE, CCxNE, OCxMhave been updated). It is cleared by software.
  ...
  [ifdef] TIM1_TIM1_ECR_DEF
    ...
    $10 constant TIM1_PW                        \ [0x10 : 8] Pulse width This bitfield defines the pulse duration, as following: tsub PW/sub = PW[7:0] x tsub PWG/sub
    $18 constant TIM1_PWPRSC                    \ [0x18 : 3] Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsub PWG/sub = (2sup (PWPRSC[2:0])/sup ) x tsub tim_ker_ck/sub
  [then]
