...
[ifndef] LTDC_DEF
  ...
  [ifdef] LTDC_LTDC_L1CR_DEF
    ...
    $04 constant LTDC_CLUTEN                    \ [0x04] color look-up table enable This bit is set and cleared by software. The CLUT is only meaningful for L8, AL44 and AL88 pixel format. Refer to Color look-up table (CLUT)
  [then]
  ...
  [ifdef] LTDC_LTDC_L1WHPCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LTDC_WHSTPOS                   \ [0x00 : 12] window horizontal start position These bits configure the first visible pixel of a line of the layer window. WHSTPOS[11:0] must be UNDER OR EQUAL AAW[11:0] bits (programmed in LTDC_AWCR register).
    $10 constant LTDC_WHSPPOS                   \ [0x10 : 12] window horizontal stop position These bits configure the last visible pixel of a line of the layer window. WHSPPOS[11:0] must be more or equal to AHBP[11:0] bits + 1 (programmed in LTDC_BPCR register).
  [then]
  ...
  [ifdef] LTDC_LTDC_L1WVPCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LTDC_WVSTPOS                   \ [0x00 : 11] window vertical start position These bits configure the first visible line of the layer window. WVSTPOS[10:0] must be UNDER OR EQUAL AAH[10:0] bits (programmed in LTDC_AWCR register).
    $10 constant LTDC_WVSPPOS                   \ [0x10 : 11] window vertical stop position These bits configure the last visible line of the layer window. WVSPPOS[10:0] must be more or equal to AVBP[10:0] bits + 1 (programmed in LTDC_BPCR register).
  [then]
  ...
  [ifdef] LTDC_LTDC_L1CFBLR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LTDC_CFBLL                     \ [0x00 : 13] color frame buffer line length These bits define the length of one line of pixels in bytes + 7. The line length is computed as follows: active high width * number of bytes per pixel + 7.
