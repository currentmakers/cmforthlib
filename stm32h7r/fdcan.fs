...
[ifndef] FDCAN_DEF
  ...
  [ifdef] FDCAN_FDCAN_DBTP_DEF
    ...
    \
    $00 constant FDCAN_DSJW                     \ [0x00 : 4] Synchronization jump width Must always be smaller than DTSEG2, valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1: t<sub>SJW</sub> = (DSJW + 1) x tq.
    $04 constant FDCAN_DTSEG2                   \ [0x04 : 4] Data time segment after sample point Valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1, i.e. t<sub>BS2</sub> = (DTSEG2 + 1) x tq.
    $08 constant FDCAN_DTSEG1                   \ [0x08 : 5] Data time segment before sample point Valid values are 0 to 31. The value used by the hardware is the one programmed, incremented by 1, i.e. t<sub>BS1</sub> = (DTSEG1 + 1) x tq.
  ...
  [ifdef] FDCAN_FDCAN_PSR_DEF
    ...
    $03 constant FDCAN_ACT                      \ [0x03 : 2] Activity Monitors the modules CAN communication state.
  ...
  [ifdef] FDCAN_FDCAN_ILS_DEF
    ...
    $05 constant FDCAN_BERR                     \ [0x05] Bit and line error grouping the following interruption EPL Error passive interrupt line ELOL: Error logging overflow interrupt line
  ...
  [ifdef] FDCAN_FDCAN_RXGFC_DEF
    ...
    $10 constant FDCAN_LSS                      \ [0x10 : 5] List size standard 1 to 28: Number of standard message ID filter elements >28: Values greater than 28 are interpreted as 28. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $18 constant FDCAN_LSE                      \ [0x18 : 4] List size extended 1 to 8: Number of extended message ID filter elements >8: Values greater than 8 are interpreted as 8. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
