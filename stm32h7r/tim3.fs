...
[ifndef] TIM3_DEF
  ...
  [ifdef] TIM3_TIM3_CR1_DEF
    ...
    $08 constant TIM3_CKD                       \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and sampling clock used by the digital filters (tim_etr_in, tim_tix),
    $0b constant TIM3_UIFREMAP                  \ [0x0b] UIF status bit remapping
    $0c constant TIM3_DITHEN                    \ [0x0c] Dithering Enable Note: The DITHEN bit can only be modified when CEN bit is reset.
  [then]
  ...
  [ifdef] TIM3_TIM3_CR2_DEF
    ...
    $04 constant TIM3_MMS                       \ [0x04 : 3] MMS[0]: Master mode selection
  ...
  [ifdef] TIM3_TIM3_SMCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_SMS                       \ [0x00 : 3] SMS[0]: Slave mode selection
    $03 constant TIM3_OCCS                      \ [0x03] OCREF clear selection This bit is used to select the OCREF clear source Note: If the OCREF clear selection feature is not supported, this bit is reserved and forced by hardware to 0. Section 65.3: TIM2/TIM3/TIM4/TIM5 implementation.
    $04 constant TIM3_TS                        \ [0x04 : 3] TS[0]: Trigger selection This bit-field selects the trigger input to be used to synchronize the counter. Others: Reserved See Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation details. Note: These bits must be changed only when they are not used (for example when SMS = 000) to avoid wrong edge detections at the transition.
    $07 constant TIM3_MSM                       \ [0x07] Master/Slave mode
    $08 constant TIM3_ETF                       \ [0x08 : 4] External trigger filter This bit-field then defines the frequency used to sample tim_etrp signal and the length of the digital filter applied to tim_etrp. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
    $0c constant TIM3_ETPS                      \ [0x0c : 2] External trigger prescaler External trigger signal tim_etrp frequency must be at most 1/4 of tim_ker_ck frequency. A prescaler can be enabled to reduce tim_etrp frequency. It is useful when inputting fast external clocks on tim_etr_in.
    $0e constant TIM3_ECE                       \ [0x0e] External clock enable This bit enables External clock mode 2. Setting the ECE bit has the same effect as selecting external clock mode 1 with tim_trgi connected to tim_etrf (SMS=111 and TS=00111). It is possible to simultaneously use external clock mode 2 with the following slave modes: reset mode, gated mode and trigger mode. Nevertheless, tim_trgi must not be connected to tim_etrf in this case (TS bits must not be 00111). Note: If external clock mode 1 and external clock mode 2 are enabled at the same time, the external clock input is tim_etrf.
    $0f constant TIM3_ETP                       \ [0x0f] External trigger polarity This bit selects whether tim_etr_in or tim_etr_in is used for trigger operations
    $10 constant TIM3_SMS_1                     \ [0x10] SMS[3]
    $14 constant TIM3_TS_1                      \ [0x14 : 2] TS[4:3]
    $18 constant TIM3_SMSPE                     \ [0x18] SMS preload enable This bit selects whether the SMS[3:0] bitfield is preloaded
    $19 constant TIM3_SMSPS                     \ [0x19] SMS preload source This bit selects whether the events that triggers the SMS[3:0] bitfield transfer from preload to active
  [then]
  ...
  [ifdef] TIM3_TIM3_SR_DEF
    ...
    $14 constant TIM3_IDXF                      \ [0x14] Index interrupt flag This flag is set by hardware when an index event is detected. It is cleared by software by writing it to 0.
    $15 constant TIM3_DIRF                      \ [0x15] Direction change interrupt flag This flag is set by hardware when the direction changes in encoder mode (DIR bit value in TIMx_CR is changing). It is cleared by software by writing it to 0.
    $16 constant TIM3_IERRF                     \ [0x16] Index error interrupt flag This flag is set by hardware when an index error is detected. It is cleared by software by writing it to 0.
    $17 constant TIM3_TERRF                     \ [0x17] Transition error interrupt flag This flag is set by hardware when a transition error is detected in encoder mode. It is cleared by software by writing it to 0.
  [then]
  ...
  [ifdef] TIM3_TIM3_CCMR1_INPUT_DEF
    ...
    $02 constant TIM3_IC1PSC                    \ [0x02 : 2] Input capture 1 prescaler This bit-field defines the ratio of the prescaler acting on CC1 input (tim_ic1). The prescaler is reset as soon as CC1E=0 (TIMx_CCER register).
    $04 constant TIM3_IC1F                      \ [0x04 : 4] Input capture 1 filter This bit-field defines the frequency used to sample tim_ti1 input and the length of the digital filter applied to tim_ti1. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
  ...
  [ifdef] TIM3_TIM3_CCMR1_OUTPUT_DEF
    ...
    $04 constant TIM3_OC1M                      \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Note: In PWM mode, the tim_ocref_clr level changes only when the result of the comparison changes or when the output compare mode switches from frozen mode to PWM mode.
    ...
    $0c constant TIM3_OC2M                      \ [0x0c : 3] OC2M[2:0]: Output compare 2 mode refer to OC1M description on bits 6:4
  ...
  [ifdef] TIM3_TIM2_CCMR3_OUTPUT_DEF
    \
    \ @brief TIM3 capture/compare mode register 2
    \ Address offset: 0x1C
    \ Reset value: 0x00000000
    ...
    $04 constant TIM3_OC3M                      \ [0x04 : 3] OC3M[2:0]: Output compare 3 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
    ...
    $0c constant TIM3_OC4M                      \ [0x0c : 3] OC4M[2:0]: Output compare 4 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
  ...
  [ifdef] TIM3_TIM3_CCER_DEF
    ...
    $03 constant TIM3_CC1NP                     \ [0x03] Capture/Compare 1 output Polarity. CC1 channel configured as output: CC1NP must be kept cleared in this case. CC1 channel configured as input: This bit is used in conjunction with CC1P to define tim_ti1fp1/tim_ti2fp1 polarity. refer to CC1P description.
  ...
  [ifdef] TIM3_TIM3_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_CNT                       \ [0x00 : 31] Least significant part of counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register holds the non-dithered part in CNT[30:0]. The fractional part is not available.
    $1f constant TIM3_UIFCPY_CNT                \ [0x1f] Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 CNT[31]: Most significant bit of counter value If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
  [then]
  ...
  [ifdef] TIM3_TIM3_ECR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_IE                        \ [0x00] Index enable This bit indicates if the Index event resets the counter.
    $01 constant TIM3_IDIR                      \ [0x01 : 2] Index direction This bit indicates in which direction the Index event resets the counter. Note: The IDR[1:0] bitfield must be written when IE bit is reset (index disabled).
    $03 constant TIM3_IBLK                      \ [0x03 : 2] Index blanking This bit indicates if the Index event is conditioned by the tim_ti3 input
    $05 constant TIM3_FIDX                      \ [0x05] First index This bit indicates if the first index only is taken into account
    $06 constant TIM3_IPOS                      \ [0x06 : 2] Index positioning
    $10 constant TIM3_PW                        \ [0x10 : 8] Pulse width This bitfield defines the pulse duration, as following: t<sub>PW</sub> = PW[7:0] x t<sub>PWG</sub>
    $18 constant TIM3_PWPRSC                    \ [0x18 : 3] Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: t<sub>PWG</sub> = (2<sup>(PWPRSC[2:0])</sup>) x t<sub>tim_ker_ck</sub>
  [then]
  ...
  [ifdef] TIM3_TIM3_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM3_TI1SEL                    \ [0x00 : 4] Selects tim_ti1[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $08 constant TIM3_TI2SEL                    \ [0x08 : 4] Selects tim_ti2[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $10 constant TIM3_TI3SEL                    \ [0x10 : 4] Selects tim_ti3[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $18 constant TIM3_TI4SEL                    \ [0x18 : 4] Selects tim_ti4[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
  ...
  [ifdef] TIM3_TIM3_AF1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant TIM3_ETRSEL                    \ [0x0e : 4] etr_in source selection These bits select the etr_in input source. ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
  ...
  [ifdef] TIM3_TIM3_AF2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant TIM3_OCRSEL                    \ [0x10 : 3] ocref_clr source selection These bits select the ocref_clr input source. ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
  ...
  $18 constant TIM3_TIM3_CCMR1_INPUT    \ TIM3 capture/compare mode register 1
  $18 constant TIM3_TIM3_CCMR1_OUTPUT   \ TIM3 capture/compare mode register 1
  $1C constant TIM3_TIM3_CCMR2_INPUT    \ TIM3 capture/compare mode register 2
  $1C constant TIM3_TIM2_CCMR3_OUTPUT   \ TIM3 capture/compare mode register 2
