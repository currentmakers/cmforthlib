...
$00000400 constant ADC1_ADC_ISR_JQOVF                               \ Injected context queue overflow This bit is set by hardware when an Overflow of the Injected Queue of Context occurs. It is cleared by software writing 1 to it. Refer to Section 25.4.21: Queue of context for injected conversions for more information.
...
$00000004 constant ADC1_ADC_CFGR_ADFCFG                             \ ADF mode configuration This bit is set and cleared by software to enable the ADF mode. It is effective only when DMAEN = 0. Note: To make sure no conversion is ongoing, the software is allowed to write this bit only when ADSTART = 0 and JADSTART = 0.
...
$00002000 constant ADC1_ADC_CFGR_CONT                               \ Single / Continuous conversion mode for regular conversions This bit is set and cleared by software. If it is set, regular conversion takes place continuously until it is cleared. Note: It is not possible to have both Discontinuous mode and Continuous mode enabled: it is forbidden to set both DISCEN = 1 and CONT = 1. Note: The software is allowed to write this bit only when ADSTART = 0 (which ensures that no regular conversion is ongoing). Note: When dual mode is enabled (DUAL bits in ADC_CCR register are not equal to zero), the bit CONT of the slave ADC is no more writable and its content is equal to the bit CONT of the master ADC.
...
$00200000 constant ADC1_ADC_CFGR_JQM                                \ JSQR queue mode This bit is set and cleared by software. It defines how an empty Queue is managed. Refer to Section 25.4.21: Queue of context for injected conversions for more information. Note: The software is allowed to write this bit only when JADSTART = 0 (which ensures that no injected conversion is ongoing). Note: When dual mode is enabled (DUAL bits in ADC_CCR register are not equal to zero), the bit JQM of the slave ADC is no more writable and its content is equal to the bit JQM of the master ADC.
...
$0000ffff constant ADC1_ADC_DR_RDATA                                \ Regular data converted These bits are read-only. They contain the conversion result from the last converted regular channel. The data are left- or right-aligned as described in Section 25.4.26: Data management.
...
$0000ffff constant ADC1_ADC_JDR1_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 25.4.26: Data management.
...
$0000ffff constant ADC1_ADC_JDR2_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 25.4.26: Data management.
...
$0000ffff constant ADC1_ADC_JDR3_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 25.4.26: Data management.
...
$0000ffff constant ADC1_ADC_JDR4_JDATA                              \ Injected data These bits are read-only. They contain the conversion result from injected channel y. The data are left -or right-aligned as described in Section 25.4.26: Data management.
...
$40022020 constant ADC1_ADC_TR1   \ offset: 0x20 : ADC watchdog threshold register 1
$40022024 constant ADC1_ADC_TR2   \ offset: 0x24 : ADC watchdog threshold register 2
$40022028 constant ADC1_ADC_TR3   \ offset: 0x28 : ADC watchdog threshold register 3
...
$40022060 constant ADC1_ADC_OFR1  \ offset: 0x60 : ADC offset 1 register
$40022064 constant ADC1_ADC_OFR2  \ offset: 0x64 : ADC offset 2 register
$40022068 constant ADC1_ADC_OFR3  \ offset: 0x68 : ADC offset 3 register
$4002206c constant ADC1_ADC_OFR4  \ offset: 0x6C : ADC offset 4 register
...
$400220b0 constant ADC1_ADC_DIFSEL  \ offset: 0xB0 : ADC Differential mode Selection Register
$400220b4 constant ADC1_ADC_CALFACT  \ offset: 0xB4 : ADC Calibration Factors
$400220c8 constant ADC1_ADC_OR    \ offset: 0xC8 : ADC option register
