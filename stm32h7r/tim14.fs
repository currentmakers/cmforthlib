...
[ifndef] TIM14_DEF
  ...
  [ifdef] TIM14_TIM14_CR1_DEF
    ...
    $08 constant TIM14_CKD                      \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and sampling clock used by the digital filters (tim_tix),
  ...
  [ifdef] TIM14_TIM14_CCMR1_OUTPUT_DEF
    ...
    $04 constant TIM14_OC1M                     \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode (refer to bit 16 for OC1M[3]) These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Others: Reserved Note: In PWM mode 1 or 2, the OCREF level changes when the result of the comparison changes or when the output compare mode switches from frozen to PWM mode.
  ...
  [ifdef] TIM14_TIM14_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM14_CNT                      \ [0x00 : 16] Counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register only holds the non-dithered part in CNT[15:0]. The fractional part is not available.
  ...
  [ifdef] TIM14_TIM14_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM14_TI1SEL                   \ [0x00 : 4] selects tim_ti1_in[15:0] input ... Refer to Table 656: Interconnect to the tim_ti1 input multiplexer for interconnects list.
  [then]
  ...
  $18 constant TIM14_TIM14_CCMR1_OUTPUT \ TIM14 capture/compare mode register 1
