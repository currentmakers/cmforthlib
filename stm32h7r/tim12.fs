...
[ifndef] TIM12_DEF
  ...
  [ifdef] TIM12_TIM12_CR1_DEF
    ...
    $08 constant TIM12_CKD                      \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and sampling clock used by the digital filters (tim_tix),
  ...
  [ifdef] TIM12_TIM12_CR2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $04 constant TIM12_MMS                      \ [0x04 : 3] Master mode selection These bits allow to select the information to be sent in master mode to slave timers for synchronization (tim_trgo). The combination is as follows:
  ...
  [ifdef] TIM12_TIM12_CCMR1_INPUT_DEF
    ...
    $02 constant TIM12_IC1PSC                   \ [0x02 : 2] Input capture 1 prescaler This bitfield defines the ratio of the prescaler acting on the CC1 input (tim_ic1). The prescaler is reset as soon as CC1E=0 (TIMx_CCER register).
    $04 constant TIM12_IC1F                     \ [0x04 : 4] Input capture 1 filter This bitfield defines the frequency used to sample the tim_ti1 input and the length of the digital filter applied to tim_ti1. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
  ...
  [ifdef] TIM12_TIM12_CCMR1_OUTPUT_DEF
    ...
    \
    $00 constant TIM12_CC1S                     \ [0x00 : 2] Capture/Compare 1 selection This bitfield defines the direction of the channel (input/output) as well as the used input. Note: The CC1S bits are writable only when the channel is OFF (CC1E = 0 in TIMx_CCER).
    $02 constant TIM12_OC1FE                    \ [0x02] Output compare 1 fast enable This bit is used to accelerate the effect of an event on the trigger in input on the CC output.
    $03 constant TIM12_OC1PE                    \ [0x03] Output compare 1 preload enable
    $04 constant TIM12_OC1M                     \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode (refer to bit 16 for OC1M[3]) These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas the active level of tim_oc1 depends on the CC1P. Note: In PWM mode 1 or 2, the OCREF level changes only when the result of the comparison changes or when the output compare mode switches from frozen mode to PWM mode.
    $08 constant TIM12_CC2S                     \ [0x08 : 2] Capture/Compare 2 selection This bitfield defines the direction of the channel (input/output) as well as the used input. Note: The CC2S bits are writable only when the channel is OFF (CC2E = 0 in TIMx_CCER).
    $0a constant TIM12_OC2FE                    \ [0x0a] Output compare 2 fast enable
    $0b constant TIM12_OC2PE                    \ [0x0b] Output compare 2 preload enable
    $0c constant TIM12_OC2M                     \ [0x0c : 3] OC2M[2:0]: Output compare 2 mode Refer to OC1M[3:0] for bit description.
  ...
  [ifdef] TIM12_TIM12_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM12_CNT                      \ [0x00 : 16] Counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register only holds the non-dithered part in CNT[15:0]. The fractional part is not available.
  ...
  [ifdef] TIM12_TIM12_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM12_TI1SEL                   \ [0x00 : 4] selects tim_ti1_in[15:0] input ... Refer to Table 656: Interconnect to the tim_ti1 input multiplexer for interconnects list.
    $08 constant TIM12_TI2SEL                   \ [0x08 : 4] selects tim_ti2_in[15:0] input ... Refer to Table 657: Interconnect to the tim_ti2 input multiplexer for interconnects list.
  [then]
