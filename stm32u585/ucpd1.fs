...
[ifndef] UCPD1_DEF
  ...
  [ifdef] UCPD1_UCPD_CFGR3_DEF
    \
    \ @brief UCPD configuration register 3
    \ Address offset: 0x08
    \ Reset value: 0x00000000
    \
    $00 constant UCPD1_TRIM1_NG_CCRPD           \ [0x00 : 4] SW trim value for RPD resistors on the CC1 line
    $09 constant UCPD1_TRIM1_NG_CC3A0           \ [0x09 : 4] SW trim value for Iref on the CC1 line
    $10 constant UCPD1_TRIM2_NG_CCRPD           \ [0x10 : 4] SW trim value for RPD resistors on the CC2 line
    $19 constant UCPD1_TRIM2_NG_CC3A0           \ [0x19 : 4] SW trim value for Iref on the CC2 line
  [then]
  ...
  [ifdef] UCPD1_UCPD_CR_DEF
    ...
    $0d constant UCPD1_CC1VCONNEN               \ [0x0d] VCONN switch enable for CC1
    $0e constant UCPD1_CC2VCONNEN               \ [0x0e] VCONN switch enable for CC2
  ...
  $08 constant UCPD1_UCPD_CFGR3         \ UCPD configuration register 3
  ...
  $28 constant UCPD1_UCPD_RX_ORDSETR    \ Rx ordered set code detected
  $2C constant UCPD1_UCPD_RX_PAYSZR     \ Rx payload size received This bitfield contains the number of bytes of a payload (including header but excluding CRC) received: each time a new data byte is received in the UCPD_RXDR register, the bitfield value increments and the RXMSGEND flag is set (and an interrupt generated if enabled). The bitfield may return a spurious value when a byte reception is ongoing (the RXMSGEND flag is low).
  $30 constant UCPD1_UCPD_RXDR          \ Data byte received
