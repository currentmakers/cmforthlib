...
[ifndef] DBGMCU_DEF
  ...
  [ifdef] DBGMCU_DBGMCU_SR_DEF
    ...
    \
    $00 constant DBGMCU_AP_PRESENT              \ [0x00 : 8] Bit n identifies whether access port AP n is present in device Bit n = 0: APn absent Bit n = 1: APn present
    $08 constant DBGMCU_AP_LOCKED               \ [0x08 : 8] DECLARATION TO BE CONFIRMED by PRODUCT OWNER! Bit n identifies whether access port AP n is open (can be accessed via the debug port) or locked (debug access to the AP is blocked) Bit n = 0: APn locked Bit n = 1: APn enabled
  [then]
