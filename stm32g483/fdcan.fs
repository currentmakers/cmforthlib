...
$00000000 constant FDCAN_FDCAN_ENDN_ETV                             \ Endianness test value The endianness test value is 0x8765 4321.
...
$0000000f constant FDCAN_FDCAN_DBTP_DSJW                            \ Synchronization jump width Must always be smaller than DTSEG2, valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1: t<sub>SJW</sub> = (DSJW + 1) x tq.
$000000f0 constant FDCAN_FDCAN_DBTP_DTSEG2                          \ Data time segment after sample point Valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1, i.e. t<sub>BS2</sub> = (DTSEG2 + 1) x tq.
$00001f00 constant FDCAN_FDCAN_DBTP_DTSEG1                          \ Data time segment before sample point Valid values are 0 to 31. The value used by the hardware is the one programmed, incremented by 1, i.e. t<sub>BS1</sub> = (DTSEG1 + 1) x tq.
$001f0000 constant FDCAN_FDCAN_DBTP_DBRP                            \ Data bit rate prescaler The value by which the oscillator frequency is divided to generate the bit time quanta. The bit time is built up from a multiple of this quanta. Valid values for the Baud Rate Prescaler are 0 to 31. The hardware interpreters this value as the value programmed plus 1.
...
$00000080 constant FDCAN_FDCAN_TEST_RX                              \ Receive pin Monitors the actual value of pin FDCANx_RX
...
$000000ff constant FDCAN_FDCAN_RWD_WDC                              \ Watchdog configuration Start value of the message RAM watchdog counter. With the reset value of 00, the counter is disabled. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of FDCAN_CCCR register are set to 1.
$0000ff00 constant FDCAN_FDCAN_RWD_WDV                              \ Watchdog value Actual message RAM watchdog counter value.
...
$00000004 constant FDCAN_FDCAN_CCCR_ASM                             \ ASM restricted operation mode The restricted operation mode is intended for applications that adapt themselves to different CAN bit rates. The application tests different bit rates and leaves the Restricted operation Mode after it has received a valid frame. In the optional Restricted operation Mode the node is able to transmit and receive data and remote frames and it gives acknowledge to valid frames, but it does not send active error frames or overload frames. In case of an error condition or overload condition, it does not send dominant bits, instead it waits for the occurrence of bus idle condition to resynchronize itself to the CAN communication. The error counters are not incremented. Bit ASM can only be set by software when both CCE and INIT are set to 1. The bit can be reset by the software at any time.
$00000008 constant FDCAN_FDCAN_CCCR_CSA                             \ Clock stop acknowledge
$00000010 constant FDCAN_FDCAN_CCCR_CSR                             \ Clock stop request
$00000020 constant FDCAN_FDCAN_CCCR_MON                             \ Bus monitoring mode Bit MON can only be set by software when both CCE and INIT are set to 1. The bit can be reset by the Host at any time.
...
$00008000 constant FDCAN_FDCAN_CCCR_NISO                            \ Non ISO operation If this bit is set, the FDCAN uses the CAN FD frame format as specified by the Bosch CAN FD Specification V1.0.
...
$0000007f constant FDCAN_FDCAN_NBTP_NTSEG2                          \ Nominal time segment after sample point Valid values are 0 to 127. The actual interpretation by the hardware of this value is such that one more than the programmed value is used.
$0000ff00 constant FDCAN_FDCAN_NBTP_NTSEG1                          \ Nominal time segment before sample point Valid values are 0 to 255. The actual interpretation by the hardware of this value is such that one more than the programmed value is used. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$01ff0000 constant FDCAN_FDCAN_NBTP_NBRP                            \ Bit rate prescaler Value by which the oscillator frequency is divided for generating the bit time quanta. The bit time is built up from a multiple of this quanta. Valid values are 0 to 511. The actual interpretation by the hardware of this value is such that one more than the value programmed here is used. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$fe000000 constant FDCAN_FDCAN_NBTP_NSJW                            \ Nominal (re)synchronization jump width Valid values are 0 to 127. The actual interpretation by the hardware of this value is such that the used value is the one programmed incremented by one. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
...
$00000003 constant FDCAN_FDCAN_TSCC_TSS                             \ Timestamp select These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
...
$00000001 constant FDCAN_FDCAN_TOCC_ETOC                            \ Timeout counter enable This is a protected write (P) bit, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00000006 constant FDCAN_FDCAN_TOCC_TOS                             \ Timeout select When operating in Continuous mode, a write to TOCV presets the counter to the value configured by TOCC[TOP] and continues down-counting. When the timeout counter is controlled by one of the FIFOs, an empty FIFO presets the counter to the value configured by TOCC[TOP]. Down-counting is started when the first FIFO element is stored. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$ffff0000 constant FDCAN_FDCAN_TOCC_TOP                             \ Timeout period Start value of the timeout counter (down-counter). Configures the timeout period.
...
$000000ff constant FDCAN_FDCAN_ECR_TEC                              \ Transmit error counter Actual state of the transmit error counter, values between 0 and 255. When CCCR.ASM is set, the CAN protocol controller does not increment TEC and REC when a CAN protocol error is detected, but CEL is still incremented.
$00007f00 constant FDCAN_FDCAN_ECR_REC                              \ Receive error counter Actual state of the receive error counter, values between 0 and 127.
$00008000 constant FDCAN_FDCAN_ECR_RP                               \ Receive error passive
$00ff0000 constant FDCAN_FDCAN_ECR_CEL                              \ CAN error logging The counter is incremented each time when a CAN protocol error causes the transmit error counter or the receive error counter to be incremented. It is reset by read access to CEL. The counter stops at 0xFF; the next increment of TEC or REC sets interrupt flag IR[ELO]. Access type is RX: reset on read.
...
$00000007 constant FDCAN_FDCAN_PSR_LEC                              \ Last error code The LEC indicates the type of the last error to occur on the CAN bus. This field is cleared to 0 when a message has been transferred (reception or transmission) without error. Access type is RS: set on read.
$00000018 constant FDCAN_FDCAN_PSR_ACT                              \ Activity Monitors the modules CAN communication state.
...
$00000700 constant FDCAN_FDCAN_PSR_DLEC                             \ Data last error code Type of last error that occurred in the data phase of a FDCAN format frame with its BRS flag set. Coding is the same as for LEC. This field is cleared to 0 when a FDCAN format frame with its BRS flag set has been transferred (reception or transmission) without error. Access type is RS: set on read.
$00000800 constant FDCAN_FDCAN_PSR_RESI                             \ ESI flag of last received FDCAN message This bit is set together with REDL, independent of acceptance filtering. Access type is RX: reset on read.
$00001000 constant FDCAN_FDCAN_PSR_RBRS                             \ BRS flag of last received FDCAN message This bit is set together with REDL, independent of acceptance filtering. Access type is RX: reset on read.
$00002000 constant FDCAN_FDCAN_PSR_REDL                             \ Received FDCAN message This bit is set independent of acceptance filtering. Access type is RX: reset on read.
$00004000 constant FDCAN_FDCAN_PSR_PXE                              \ Protocol exception event
$007f0000 constant FDCAN_FDCAN_PSR_TDCV                             \ Transmitter delay compensation value Position of the secondary sample point, defined by the sum of the measured delay from FDCAN_TX to FDCAN_RX and TDCR.TDCO. The SSP position is, in the data phase, the number of minimum time quanta (mtq) between the start of the transmitted bit and the secondary sample point. Valid values are 0 to 127 mtq.
...
$0000007f constant FDCAN_FDCAN_TDCR_TDCF                            \ Transmitter delay compensation filter window length Defines the minimum value for the SSP position, dominant edges on FDCAN_RX that would result in an earlier SSP position are ignored for transmitter delay measurements. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00007f00 constant FDCAN_FDCAN_TDCR_TDCO                            \ Transmitter delay compensation offset Offset value defining the distance between the measured delay from FDCAN_TX to FDCAN_RX and the secondary sample point. Valid values are 0 to 127 mtq. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
...
$00000001 constant FDCAN_FDCAN_ILS_RXFIFO0                          \ RX FIFO bit grouping the following interruption RF0LL: Rx FIFO 0 message lost interrupt line RF0FL: Rx FIFO 0 full interrupt line RF0NL: Rx FIFO 0 new message interrupt line
$00000002 constant FDCAN_FDCAN_ILS_RXFIFO1                          \ RX FIFO bit grouping the following interruption RF1LL: Rx FIFO 1 message lost interrupt line RF1FL: Rx FIFO 1 full interrupt line RF1NL: Rx FIFO 1 new message interrupt line
$00000004 constant FDCAN_FDCAN_ILS_SMSG                             \ Status message bit grouping the following interruption TCFL: Transmission cancellation finished interrupt line TCL: Transmission completed interrupt line HPML: High-priority message interrupt line
$00000008 constant FDCAN_FDCAN_ILS_TFERR                            \ Tx FIFO ERROR grouping the following interruption TEFLL: Tx event FIFO element lost interrupt line TEFFL: Tx event FIFO full interrupt line TEFNL: Tx event FIFO new entry interrupt line TFEL: Tx FIFO empty interrupt line
$00000010 constant FDCAN_FDCAN_ILS_MISC                             \ Interrupt regrouping the following interruption TOOL: Timeout occurred interrupt line MRAFL: Message RAM access failure interrupt line TSWL: Timestamp wraparound interrupt line
$00000020 constant FDCAN_FDCAN_ILS_BERR                             \ Bit and line error grouping the following interruption EPL Error passive interrupt line ELOL: Error logging overflow interrupt line
$00000040 constant FDCAN_FDCAN_ILS_PERR                             \ Protocol error grouping the following interruption ARAL: Access to reserved address line PEDL: Protocol error in data phase line PEAL: Protocol error in arbitration phase line WDIL: Watchdog interrupt line BOL: Bus_Off status EWL: Warning status interrupt line
...
$00000001 constant FDCAN_FDCAN_RXGFC_RRFE                           \ Reject remote frames extended These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00000002 constant FDCAN_FDCAN_RXGFC_RRFS                           \ Reject remote frames standard These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$0000000c constant FDCAN_FDCAN_RXGFC_ANFE                           \ Accept non-matching frames extended Defines how received messages with 29-bit IDs that do not match any element of the filter list are treated. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00000030 constant FDCAN_FDCAN_RXGFC_ANFS                           \ Accept Non-matching frames standard Defines how received messages with 11-bit IDs that do not match any element of the filter list are treated. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00000100 constant FDCAN_FDCAN_RXGFC_F1OM                           \ FIFO 1 operation mode (overwrite or blocking) This is a protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$00000200 constant FDCAN_FDCAN_RXGFC_F0OM                           \ FIFO 0 operation mode (overwrite or blocking) This is protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$001f0000 constant FDCAN_FDCAN_RXGFC_LSS                            \ List size standard 1 to 28: Number of standard message ID filter elements >28: Values greater than 28 are interpreted as 28. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
$0f000000 constant FDCAN_FDCAN_RXGFC_LSE                            \ List size extended 1 to 8: Number of extended message ID filter elements >8: Values greater than 8 are interpreted as 8. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
...
$00000007 constant FDCAN_FDCAN_HPMS_BIDX                            \ Buffer index Index of Rx FIFO element to which the message was stored. Only valid when MSI[1] = 1.
$000000c0 constant FDCAN_FDCAN_HPMS_MSI                             \ Message storage indicator
$00001f00 constant FDCAN_FDCAN_HPMS_FIDX                            \ Filter index Index of matching filter element. Range is 0 to RXGFC[LSS] - 1 or RXGFC[LSE] - 1.
$00008000 constant FDCAN_FDCAN_HPMS_FLST                            \ Filter list Indicates the filter list of the matching filter element.
...
$0000000f constant FDCAN_FDCAN_RXF0S_F0FL                           \ Rx FIFO 0 fill level Number of elements stored in Rx FIFO 0, range 0 to 3.
$00000300 constant FDCAN_FDCAN_RXF0S_F0GI                           \ Rx FIFO 0 get index Rx FIFO 0 read index pointer, range 0 to 2.
$00030000 constant FDCAN_FDCAN_RXF0S_F0PI                           \ Rx FIFO 0 put index Rx FIFO 0 write index pointer, range 0 to 2.
$01000000 constant FDCAN_FDCAN_RXF0S_F0F                            \ Rx FIFO 0 full
$02000000 constant FDCAN_FDCAN_RXF0S_RF0L                           \ Rx FIFO 0 message lost This bit is a copy of interrupt flag IR[RF0L]. When IR[RF0L] is reset, this bit is also reset.
...
$0000000f constant FDCAN_FDCAN_RXF1S_F1FL                           \ Rx FIFO 1 fill level Number of elements stored in Rx FIFO 1, range 0 to 3.
$00000300 constant FDCAN_FDCAN_RXF1S_F1GI                           \ Rx FIFO 1 get index Rx FIFO 1 read index pointer, range 0 to 2.
$00030000 constant FDCAN_FDCAN_RXF1S_F1PI                           \ Rx FIFO 1 put index Rx FIFO 1 write index pointer, range 0 to 2.
$01000000 constant FDCAN_FDCAN_RXF1S_F1F                            \ Rx FIFO 1 full
$02000000 constant FDCAN_FDCAN_RXF1S_RF1L                           \ Rx FIFO 1 message lost This bit is a copy of interrupt flag IR[RF1L]. When IR[RF1L] is reset, this bit is also reset.
...
$01000000 constant FDCAN_FDCAN_TXBC_TFQM                            \ Tx FIFO/queue mode This is a protected write (P) bit, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
...
$00000007 constant FDCAN_FDCAN_TXFQS_TFFL                           \ Tx FIFO free level Number of consecutive free Tx FIFO elements starting from TFGI, range 0 to 3. Read as 0 when Tx queue operation is configured (TXBC[TFQM] = 1).
$00000300 constant FDCAN_FDCAN_TXFQS_TFGI                           \ Tx FIFO get index Tx FIFO read index pointer, range 0 to 3. Read as 0 when Tx queue operation is configured (TXBC.TFQM = 1)
$00030000 constant FDCAN_FDCAN_TXFQS_TFQPI                          \ Tx FIFO/queue put index Tx FIFO/queue write index pointer, range 0 to 3
...
$00000007 constant FDCAN_FDCAN_TXBCF_CF                             \ Cancellation finished Each Tx buffer has its own CF bit. The bits are set when the corresponding TXBRP bit is cleared after a cancellation was requested via TXBCR. In case the corresponding TXBRP bit was not set at the point of cancellation, CF is set immediately. The bits are reset when a new transmission is requested by writing a 1 to the corresponding bit of register TXBAR.
...
$00000007 constant FDCAN_FDCAN_TXBTIE_TIE                           \ Transmission interrupt enable Each Tx buffer has its own TIE bit.
...
$00000007 constant FDCAN_FDCAN_TXBCIE_CFIE                          \ Cancellation finished interrupt enable. Each Tx buffer has its own CFIE bit.
...
$00000007 constant FDCAN_FDCAN_TXEFS_EFFL                           \ Event FIFO fill level Number of elements stored in Tx event FIFO, range 0 to 3.
$00000300 constant FDCAN_FDCAN_TXEFS_EFGI                           \ Event FIFO get index Tx event FIFO read index pointer, range 0 to 3.
$00030000 constant FDCAN_FDCAN_TXEFS_EFPI                           \ Event FIFO put index Tx event FIFO write index pointer, range 0 to 3.
$01000000 constant FDCAN_FDCAN_TXEFS_EFF                            \ Event FIFO full
$02000000 constant FDCAN_FDCAN_TXEFS_TEFL                           \ Tx event FIFO element lost This bit is a copy of interrupt flag IR[TEFL]. When IR[TEFL] is reset, this bit is also reset. 0 No Tx event FIFO element lost 1 Tx event FIFO element lost, also set after write attempt to Tx event FIFO of size 0.
