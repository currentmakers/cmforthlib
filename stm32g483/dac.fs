...
[ifndef] DAC_DEF
  ...
  [ifdef] DAC_DAC_CR_DEF
    ...
    $02 constant DAC_TSEL1                      \ [0x02 : 4] DAC channel1 trigger selection These bits select the external event used to trigger DAC channel1. Note: Only used if bit TEN1 = 1 (DAC channel1 trigger enabled).
    ...
    $12 constant DAC_TSEL2                      \ [0x12 : 4] DAC channel2 trigger selection These bits select the external event used to trigger DAC channel2 Note: Only used if bit TEN2 = 1 (DAC channel2 trigger enabled).
  ...
  [ifdef] DAC_DAC_SWTRGR_DEF
    ...
    $10 constant DAC_SWTRIGB1                   \ [0x10] DAC channel1 software trigger B
    $11 constant DAC_SWTRIGB2                   \ [0x11] DAC channel2 software trigger B
  [then]
  ...
  [ifdef] DAC_DAC_SR_DEF
    ...
    $1b constant DAC_DAC2RDY                    \ [0x1b] DAC channel 2 ready status bit
    $1c constant DAC_DORSTAT2                   \ [0x1c] DAC channel 2 output register status bit
  ...
  [ifdef] DAC_DAC_STR1_DEF
    \
    \ @brief Sawtooth register
    \ Address offset: 0x58
    \ Reset value: 0x00000000
    \
    $00 constant DAC_STRSTDATA1                 \ [0x00 : 12] DAC Channel 1 Sawtooth reset value
    $0c constant DAC_STDIR1                     \ [0x0c] DAC Channel1 Sawtooth direction setting
    $10 constant DAC_STINCDATA1                 \ [0x10 : 16] DAC CH1 Sawtooth increment value (12.4 bit format)
  [then]


  [ifdef] DAC_DAC_STR2_DEF
    \
    \ @brief Sawtooth register
    \ Address offset: 0x5C
    \ Reset value: 0x00000000
    \
    $00 constant DAC_STRSTDATA2                 \ [0x00 : 12] DAC Channel 2 Sawtooth reset value
    $0c constant DAC_STDIR2                     \ [0x0c] DAC Channel2 Sawtooth direction setting
    $10 constant DAC_STINCDATA2                 \ [0x10 : 16] DAC CH2 Sawtooth increment value (12.4 bit format)
  [then]


  [ifdef] DAC_DAC_STMODR_DEF
    \
    \ @brief Sawtooth Mode register
    \ Address offset: 0x60
    \ Reset value: 0x00000000
    \
    $00 constant DAC_STRSTTRIGSEL1              \ [0x00 : 4] DAC Channel 1 Sawtooth Reset trigger selection
    $08 constant DAC_STINCTRIGSEL1              \ [0x08 : 4] DAC Channel 1 Sawtooth Increment trigger selection
    $10 constant DAC_STRSTTRIGSEL2              \ [0x10 : 4] DAC Channel 1 Sawtooth Reset trigger selection
    $18 constant DAC_STINCTRIGSEL2              \ [0x18 : 4] DAC Channel 2 Sawtooth Increment trigger selection
  [then]
  ...
  $58 constant DAC_DAC_STR1             \ Sawtooth register
  $5C constant DAC_DAC_STR2             \ Sawtooth register
  $60 constant DAC_DAC_STMODR           \ Sawtooth Mode register
