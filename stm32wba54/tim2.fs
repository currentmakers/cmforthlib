...
[ifndef] TIM2_DEF
  ...
  [ifdef] TIM2_TIM_CCMR1_ALTERNATE_DEF
    \
    \ @brief TIM capture/compare mode register 1 [alternate]
    \ Address offset: 0x18
    \ Reset value: 0x00000000
    ...
    $04 constant TIM2_OC1M                      \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Note: In PWM mode, the tim_ocref_clr level changes only when the result of the comparison changes or when the output compare mode switches from 'frozen' mode to 'PWM' mode.
    ...
    $0c constant TIM2_OC2M                      \ [0x0c : 3] OC2M[2:0]: Output compare 2 mode refer to OC1M description on bits 6:4
  ...
  [ifdef] TIM2_TIM_CCMR2_ALTERNATE_DEF
    \
    \ @brief TIM capture/compare mode register 2 [alternate]
    \ Address offset: 0x1C
    \ Reset value: 0x00000000
    ...
    $04 constant TIM2_OC3M                      \ [0x04 : 3] OC3M[2:0]: Output compare 3 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
    ...
    $0c constant TIM2_OC4M                      \ [0x0c : 3] OC4M[2:0]: Output compare 4 mode Refer to OC1M description (bits 6:4 in TIMx_CCMR1 register)
  ...
  [ifdef] TIM2_TIM_CNT_DEF
    \
    \ @brief TIM counter
    \ Address offset: 0x24
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_CNT                       \ [0x00 : 31] Least significant part of counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register holds the non-dithered part in CNT[30:0]. The fractional part is not available.
    $1f constant TIM2_UIFCPY                    \ [0x1f] UIF copy or CNT bit 31 If TIMx_CR1.UIFREMAP = 0, thisbit means CNT[31], the most significant bit of counter value If TIMx_CRT1.UIFREMAP = 1, this bit means UIF Copy, and is a read-only copy of the UIF bit of the TIMx_ISR register
  [then]


  [ifdef] TIM2_TIM_PSC_DEF
    \
    \ @brief TIM prescaler
    \ Address offset: 0x28
    \ Reset value: 0x00000000
  ...
  [ifdef] TIM2_TIM_ECR_DEF
    ...
    $10 constant TIM2_PW                        \ [0x10 : 8] Pulse width This bitfield defines the pulse duration, as following: tsubPW/sub = PW[7:0] x tsubPWG/sub
    $18 constant TIM2_PWPRSC                    \ [0x18 : 3] Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsubPWG/sub = (2sup(PWPRSC[2:0])/sup) x tsubtim_ker_ck/sub
  [then]


  [ifdef] TIM2_TIM_TISEL_DEF
    \
    \ @brief TIM timer input selection register
    \ Address offset: 0x5C
    \ Reset value: 0x00000000
    \
    $00 constant TIM2_TI1SEL                    \ [0x00 : 4] Selects tim_ti1[15:0] input ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
    $08 constant TIM2_TI2SEL                    \ [0x08 : 4] Selects tim_ti2[15:0] input ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
    $10 constant TIM2_TI3SEL                    \ [0x10 : 4] Selects tim_ti3[15:0] input ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
    $18 constant TIM2_TI4SEL                    \ [0x18 : 4] Selects tim_ti4[15:0] input ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
  [then]


  [ifdef] TIM2_TIM_AF1_DEF
    \
    \ @brief TIM alternate function register 1
    \ Address offset: 0x60
    \ Reset value: 0x00000000
    \
    $0e constant TIM2_ETRSEL                    \ [0x0e : 4] etr_in source selection These bits select the etr_in input source. ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
  [then]


  [ifdef] TIM2_TIM_AF2_DEF
    \
    \ @brief TIM alternate function register 2
    \ Address offset: 0x64
    \ Reset value: 0x00000000
    \
    $10 constant TIM2_OCRSEL                    \ [0x10 : 3] ocref_clr source selection These bits select the ocref_clr input source. ... Refer to Section 29.4.2: TIM2/TIM3 pins and internal signals for product specific implementation.
  [then]


  [ifdef] TIM2_TIM_DCR_DEF
    \
  ...
  \ @brief TIM2 address block description
  \
  $00 constant TIM2_TIM_CR1             \ TIM control register 1
  $04 constant TIM2_TIM_CR2             \ TIM control register 2
  $08 constant TIM2_TIM_SMCR            \ TIM slave mode control register
  $0C constant TIM2_TIM_DIER            \ TIM DMA/Interrupt enable register
  $10 constant TIM2_TIM_SR              \ TIM status register
  $14 constant TIM2_TIM_EGR             \ TIM event generation register
  $18 constant TIM2_TIM_CCMR1           \ TIM capture/compare mode register 1 [alternate]
  $18 constant TIM2_TIM_CCMR1_ALTERNATE \ TIM capture/compare mode register 1 [alternate]
  $1C constant TIM2_TIM_CCMR2           \ TIM capture/compare mode register 2 [alternate]
  $1C constant TIM2_TIM_CCMR2_ALTERNATE \ TIM capture/compare mode register 2 [alternate]
  $20 constant TIM2_TIM_CCER            \ TIM capture/compare enable register
  $24 constant TIM2_TIM_CNT             \ TIM counter
  $28 constant TIM2_TIM_PSC             \ TIM prescaler
  $2C constant TIM2_TIM_ARR             \ TIM auto-reload register
  $34 constant TIM2_TIM_CCR1            \ TIM capture/compare register 1
  $38 constant TIM2_TIM_CCR2            \ TIM capture/compare register 2
  $3C constant TIM2_TIM_CCR3            \ TIM capture/compare register 3
  $40 constant TIM2_TIM_CCR4            \ TIM capture/compare register 4
  $58 constant TIM2_TIM_ECR             \ TIM timer encoder control register
  $5C constant TIM2_TIM_TISEL           \ TIM timer input selection register
  $60 constant TIM2_TIM_AF1             \ TIM alternate function register 1
  $64 constant TIM2_TIM_AF2             \ TIM alternate function register 2
  $3DC constant TIM2_TIM_DCR            \ TIM DMA control register
  $3E0 constant TIM2_TIM_DMAR           \ TIM DMA address for full transfer
