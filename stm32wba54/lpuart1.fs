...
[ifndef] LPUART1_DEF
  ...
  [ifdef] LPUART1_LPUART_CR1_DEF
    ...
    $09 constant LPUART1_PS                     \ [0x09] Parity selection This bit selects the odd or even parity when the parity generation/detection is enabled (PCE bit set). It is set and cleared by software. The parity is selected after the current byte. This bitfield can only be written when the LPUART is disabled (UE=0).
    ...
    $0b constant LPUART1_WAKE                   \ [0x0b] Receiver wakeup method This bit determines the LPUART wakeup method from Mute mode. It is set or cleared by software. This bitfield can only be written when the LPUART is disabled (UE=0).
    $0c constant LPUART1_M0                     \ [0x0c] Word length This bit is used in conjunction with bit 28 (M1) to determine the word length. It is set or cleared by software (refer to bit 28 (M1) description). This bit can only be written when the LPUART is disabled (UE=0).
    $0d constant LPUART1_MME                    \ [0x0d] Mute mode enable This bit activates the Mute mode function of the LPUART. When set, the LPUART can switch between the active and Mute modes, as defined by the WAKE bit. It is set and cleared by software.
    ...
    $1d constant LPUART1_FIFOEN                 \ [0x1d] FIFO mode enable This bit is set and cleared by software.
  ...
  [ifdef] LPUART1_LPUART_CR1_ALTERNATE1_DEF
    \
    ...
    $01 constant LPUART1_UESM                   \ [0x01] LPUART enable in low-power mode When this bit is cleared, the LPUART cannot request its kernel clock and is not functional in low-power mode. When this bit is set, the LPUART can wake up the MCU from low-power mode. This bit is set and cleared by software. Note: The UESM bit must be set at the initialization phase. Note: If the LPUART does not support the Wakeup from low-power mode, this bit is reserved and must be kept at reset value. Refer to Section 79.3: LPUART implementation on page 4652.
    $02 constant LPUART1_RE                     \ [0x02] Receiver enable This bit enables the receiver. It is set and cleared by software.
    $03 constant LPUART1_TE                     \ [0x03] Transmitter enable This bit enables the transmitter. When the Autonomous mode is disabled, TE bit is set and cleared by software. When the Autonomous mode is enabled, TE bit becomes a status bit, which is set and cleared by hardware. Note: During transmission, a low pulse on the TE bit (0 followed by 1) sends a preamble (idle line) after the current word, except in Smartcard mode. In order to generate an idle character, the TE must not be immediately written to 1. To ensure the required duration, the software can poll the TEACK bit in the LPUART_ISR register. Note: In Smartcard mode, when TE is set, there is a 1 bit-time delay before the transmission starts.
    $04 constant LPUART1_IDLEIE                 \ [0x04] IDLE interrupt enable This bit is set and cleared by software.
  ...
  [ifdef] LPUART1_LPUART_BRR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART1_BRR                    \ [0x00 : 20] LPUART baud rate division (LPUARTDIV)
  [then]
  ...
  [ifdef] LPUART1_LPUART_ISR_DEF
    ...
    $0a constant LPUART1_CTS                    \ [0x0a] CTS flag This bit is set/reset by hardware. It is an inverted copy of the status of the CTS input pin. Note: If the hardware flow control feature is not supported, this bit is reserved and kept at reset value.
    ...
    $11 constant LPUART1_CMF                    \ [0x11] Character match flag This bit is set by hardware, when a the character defined by ADD[7:0] is received. It is cleared by software, writing 1 to the CMCF in the LPUART_ICR register. An interrupt is generated if CMIE=1in the LPUART_CR1 register.
    $12 constant LPUART1_SBKF                   \ [0x12] Send break flag This bit indicates that a send break character was requested. It is set by software, by writing 1 to the SBKRQ bit in the LPUART_CR3 register. It is automatically reset by hardware during the stop bit of break transmission.
    ...
    $18 constant LPUART1_RXFF                   \ [0x18] RXFIFO Full This bit is set by hardware when the number of received data corresponds to RXFIFO size + 1 (RXFIFO full + 1 data in the LPUART_RDR register. An interrupt is generated if the RXFFIE bit =1 in the LPUART_CR1 register.
    $1a constant LPUART1_RXFT                   \ [0x1a] RXFIFO threshold flag This bit is set by hardware when the RXFIFO reaches the threshold programmed in RXFTCFG in LPUART_CR3 register i.e. the Receive FIFO contains RXFTCFG data. An interrupt is generated if the RXFTIE bit =1 (bit 27) in the LPUART_CR3 register.
    $1b constant LPUART1_TXFT                   \ [0x1b] TXFIFO threshold flag This bit is set by hardware when the TXFIFO reaches the threshold programmed in TXFTCFG in LPUART_CR3 register i.e. the TXFIFO contains TXFTCFG empty locations. An interrupt is generated if the TXFTIE bit =1 (bit 31) in the LPUART_CR3 register.
  [then]


  [ifdef] LPUART1_LPUART_ISR_ALTERNATE1_DEF
    \
  ...
  [ifdef] LPUART1_LPUART_RDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART1_RDR                    \ [0x00 : 9] Receive data value Contains the received data character. The RDR register provides the parallel interface between the input shift register and the internal bus (see Figure 952). When receiving with the parity enabled, the value read in the MSB bit is the received parity bit.
  [then]
  ...
  [ifdef] LPUART1_LPUART_PRESC_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART1_PRESCALER              \ [0x00 : 4] Clock prescaler The LPUART input clock can be divided by a prescaler: Remaining combinations: Reserved. Note: When PRESCALER is programmed with a value different of the allowed ones, programmed prescaler value is equal to 1011 i.e. input clock divided by 256.
  [then]


  [ifdef] LPUART1_LPUART_AUTOCR_DEF
    \
    \ @brief LPUART Autonomous mode control register
    \ Address offset: 0x30
    \ Reset value: 0x80000000
    \
    $00 constant LPUART1_TDN                    \ [0x00 : 16] TDC transmission data number This bitfield enables the programming of the number of data to be transmitted. It can be written only when UE is cleared in LPUART_CR1.
    $10 constant LPUART1_TRIGPOL                \ [0x10] Trigger polarity bit This bitfield can be written only when the UE bit is cleared in LPUART_CR1 register.
    $11 constant LPUART1_TRIGEN                 \ [0x11] Trigger enable bit Note: This bitfield can be written only when the UE bit of USART_CR1 register is cleared. Note: When a trigger is detected, TE is set to 1 in LPUART_CR1 and the data transfer is launched.
    $12 constant LPUART1_IDLEDIS                \ [0x12] Idle frame transmission disable bit after enabling the transmitter Note: This bitfield can be written only when the UE bit of USART_CR1 register is cleared.
    $13 constant LPUART1_TRIGSEL                \ [0x13 : 4] Trigger selection bits Refer to Section : Description LPUART interconnections. This bitfield can be written only when the UE bit is cleared in LPUART_CR1 register. ... Note: This bitfield can be written only when the UE bit of USART_CR1 register is cleared.
  [then]

  \
  \ @brief LPUART address block description
  \
  $00 constant LPUART1_LPUART_CR1       \ LPUART control register 1
  $00 constant LPUART1_LPUART_CR1_ALTERNATE1    \ LPUART control register 1 [alternate]
  ...
  $1C constant LPUART1_LPUART_ISR       \ LPUART interrupt and status register
  $1C constant LPUART1_LPUART_ISR_ALTERNATE1    \ LPUART interrupt and status register [alternate]
  ...
  $30 constant LPUART1_LPUART_AUTOCR    \ LPUART Autonomous mode control register
