...
$0000001f constant SEC_SPI1_SPI_CFG1_DSIZE                          \ number of bits in at single SPI data frame ..... Maximum data size can be limited up to 16-bits at some instances. At instances with limited set of features, DSIZE2:0] bits are reserved and must be kept at reset state. DSIZE[4:3] bits then control next settings of data size: 00xxx: 8-bits 01xxx: 16-bits 10xxx: 24-bits Note: 11xxx: 32-bits.
...
$00000200 constant SEC_SPI1_SPI_CFG1_UDRCFG                         \ behavior of slave transmitter at underrun condition For more details see Figure977: Optional configurations of slave detecting underrun condition.
...
$70000000 constant SEC_SPI1_SPI_CFG1_MBR                            \ master baud rate prescaler setting Note: MBR setting is considered at slave working at TI mode, too (see Section80.5.1: TI mode).
...
$00002000 constant SEC_SPI1_SPI_CFG2_RDIOM                          \ RDY signal input/output management Note: When DSIZE at the SPI_CFG1 register is configured shorter than 8-bit, the RDIOM bit must be kept at zero.
...
$00800000 constant SEC_SPI1_SPI_CFG2_LSBFRST                        \ data frame format Note: 1: LSB transmitted first
...
$000f0000 constant SEC_SPI1_SPI_AUTOCR_TRIGSEL                      \ trigger selection (refer Section: Description of SPI interconnections). ... Note: these bits can be written only when SPE=0.
$00100000 constant SEC_SPI1_SPI_AUTOCR_TRIGPOL                      \ trigger polarity Note: This bit can be written only when SPE=0.
$00200000 constant SEC_SPI1_SPI_AUTOCR_TRIGEN                       \ HW control of CSTART triggering enable Note: if user cannot prevent trigger event during write, the TRIGEN must be changed when SPI is disabled
...
$00000000 constant SEC_SPI1_SPI_RXDR_RXDR                           \ receive data register The register serves as an interface with RxFIFO. When it is read, RxFIFO is accessed. Note: data is always right-aligned. Unused bits are read as zero when the register is read. Writing to the register is ignored. Note: DR can be accessed byte-wise (8-bit access): in this case only one data-byte is read by single access Note: halfword-wise (16 bit access) in this case 2 data-bytes or 1 halfword-data can be read by single access Note: word-wise (32 bit access). In this case 4 data-bytes or 2 halfword-data or word-data can be read by single access. Note: Read access of this register less than the configured data size is forbidden.
...
\ @brief Serial peripheral interface
\
$50013000 constant SEC_SPI1_SPI_CR1  \ offset: 0x00 : SPI control register 1
$50013004 constant SEC_SPI1_SPI_CR2  \ offset: 0x04 : SPI control register 2
...
$50013010 constant SEC_SPI1_SPI_IER  \ offset: 0x10 : SPI interrupt enable register
$50013014 constant SEC_SPI1_SPI_SR  \ offset: 0x14 : SPI status register
$50013018 constant SEC_SPI1_SPI_IFCR  \ offset: 0x18 : SPI interrupt/status flags clear register
$5001301c constant SEC_SPI1_SPI_AUTOCR  \ offset: 0x1C : SPI autonomous mode control register
$50013020 constant SEC_SPI1_SPI_TXDR  \ offset: 0x20 : SPI transmit data register
$50013030 constant SEC_SPI1_SPI_RXDR  \ offset: 0x30 : SPI receive data register
$50013040 constant SEC_SPI1_SPI_CRCPOLY  \ offset: 0x40 : SPI polynomial register
$50013044 constant SEC_SPI1_SPI_TXCRC  \ offset: 0x44 : SPI transmitter CRC register
$50013048 constant SEC_SPI1_SPI_RXCRC  \ offset: 0x48 : SPI receiver CRC register
