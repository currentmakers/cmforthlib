...
$00000200 constant ETH_ETH_MACCR_DCRS                               \ Disable Carrier Sense During Transmission When this bit is set, the MAC transmitter ignores the (G)MII CRS signal during packet transmission in the half-duplex mode. As a result, no errors are generated because of Loss of Carrier or No Carrier during transmission. When this bit is reset, the MAC transmitter generates errors because of Carrier Sense. The MAC can even abort the transmission.
...
$00020000 constant ETH_ETH_MACECR_SPEN                              \ Slow Protocol Detection Enable When this bit is set, MAC processes the Slow Protocol packets (Ether Type 0x8809) and provides the Rx status. The MAC discards the Slow Protocol packets with invalid sub-types. When this bit is reset, the MAC forwards all error-free Slow Protocol packets to the application. The MAC considers such packets as normal Type packets.
$00040000 constant ETH_ETH_MACECR_USP                               \ Unicast Slow Protocol Packet Detect When this bit is set, the MAC detects the Slow Protocol packets with unicast address of the station specified in the and MAC Address 0 low register . The MAC also detects the Slow Protocol packets with the Slow Protocols multicast address (01-80-C2-00-00-02). When this bit is reset, the MAC detects only Slow Protocol packets with the Slow Protocol multicast address specified in the IEEE 802.3-2008, Section 5.
...
$3e000000 constant ETH_ETH_MACECR_EIPG                              \ Extended Inter-Packet Gap The value in this field is applicable when the EIPGEN bit is set. This field (as Most Significant bits) along with IPG field in , gives the minimum IPG greater than 96 bit times in steps of 8 bit times. For example: EIPG = 0 and IPG = 0 give 104 bit times EIPG = 0 and IPG = 1 give 112 bit times EIPG = 0 and IPG = 2 give 120 bit times .. EIPG = 7 and IPG = 31 give 2144 bit times
...
$00200000 constant ETH_ETH_MACVIR_CBTI                              \ Channel based tag insertion When this bit is set, outer VLAN tag is inserted for every packets transmitted by the MAC. The tag value is taken from the queue/channel specific VLAN tag register. This will override VLTI, VLP, VLC and VLT fields of this register When this bit is set, a write operation to byte 3 of this register initiates the read/write access to the indirect register. When reset outer VLAN operation is based on the setting of VLTI, VLP, VLC and VLT fields of this register
$01000000 constant ETH_ETH_MACVIR_ADDR                              \ Address This field selects one of the queue/channel specific VLAN Inclusion register for read/write access This does not have any effect when CBTI is reset
$40000000 constant ETH_ETH_MACVIR_RDWR                              \ Read write control This bit controls the read or write operation for indirectly accessing the queue/channel specific VLAN Inclusion register. When set indicates write operation and when reset indicates read operation This has no effect when CBTI is reset
$80000000 constant ETH_ETH_MACVIR_BUSY                              \ Busy This bit indicates the status of the read/write operation of indirect access to the queue/channel specific VLAN inclusion register. For write operation write to a register is complete when this bit is reset. For read operation the read data is valid when the bit is reset. No further writes are allowed to this register when this bit is set. This does not have any effect when CBTI is reset
...
$0000ffff constant ETH_ETH_MACVIRALTERNATE_VLT                      \ VLAN Tag for Transmit Packets This field contains the value of the VLAN tag to be inserted or replaced. The value must only be changed when the transmit lines are inactive or during the initialization phase. The following list describes the bits of this field: Bits[15:13]: User Priority Bit 12: Canonical Format Indicator (CFI) or Drop Eligible Indicator (DEI) Bits[11:0]: VLAN Identifier (VID) field of VLAN tag
$00080000 constant ETH_ETH_MACVIRALTERNATE_CSVL                     \ C-VLAN or S-VLAN When this bit is set, S-VLAN type (0x88A8) is inserted or replaced in the 13th and 14th bytes of transmitted packets. When this bit is reset, C-VLAN type (0x8100) is inserted or replaced in the 13th and 14th bytes of transmitted packets.
...
$00000010 constant ETH_ETH_MACISR_PMTIS                             \ PMT Interrupt Status This bit is set when a Magic packet or Wake-on-LAN packet is received in the power-down mode (RWKPRCVD and MGKPRCVD bits in ETH_MACPCSR register). This bit is cleared when Bits[6:5] are cleared because of a Read operation to the (ETH_MACPCSR).
$00000020 constant ETH_ETH_MACISR_LPIIS                             \ LPI Interrupt Status This bit is set for any LPI state entry or exit in the MAC Transmitter or Receiver. This bit is cleared when the TLPIEN bit of is read.
...
$00000200 constant ETH_ETH_MACISR_MMCRXIS                           \ MMC Receive Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_RX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
$00000400 constant ETH_ETH_MACISR_MMCTXIS                           \ MMC Transmit Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_TX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
...
$00004000 constant ETH_ETH_MACISR_RXSTSIS                           \ Receive Status Interrupt This bit indicates the status of received packets. This bit is set when the RWT bit is set in the . This bit is cleared when the corresponding interrupt source bit is read (or corresponding interrupt source bit is written to 1 when RCWE bit of is set) in the ETH_MACISR register.
...
$00000008 constant ETH_ETH_MACIER_PHYIE                             \ PHY Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of PHYIS bit in .
$00000010 constant ETH_ETH_MACIER_PMTIE                             \ PMT Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of PMTIS bit in .
$00000020 constant ETH_ETH_MACIER_LPIIE                             \ LPI Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of LPIIS bit in .
$00001000 constant ETH_ETH_MACIER_TSIE                              \ Timestamp Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of TSIS bit in .
$00002000 constant ETH_ETH_MACIER_TXSTSIE                           \ Transmit Status Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of TXSTSIS bit in the .
$00004000 constant ETH_ETH_MACIER_RXSTSIE                           \ Receive Status Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of RXSTSIS bit in the .
...
$00000002 constant ETH_ETH_MACRXTXSR_NCARR                          \ No Carrier When the DTXSTS bit is set in the , this bit indicates that the carrier signal from the PHY is not present at the end of preamble transmission. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
$00000004 constant ETH_ETH_MACRXTXSR_LCARR                          \ Loss of Carrier When the DTXSTS bit is set in the , this bit indicates that the loss of carrier occurred during packet transmission, that is, the ETH_CRS signal was inactive for one or more transmission clock periods during packet transmission. This bit is valid only for packets transmitted without collision. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
...
$00000010 constant ETH_ETH_MACRXTXSR_LCOL                           \ Late Collision When the DTXSTS bit is set in the , this bit indicates that the packet transmission aborted because a collision occurred after the collision window (64 bytes including Preamble in MII mode. This bit is not valid if the Underflow error occurs. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
...
$00000020 constant ETH_ETH_MACPCSR_MGKPRCVD                         \ Magic Packet Received When this bit is set, it indicates that the power management event is generated because of the reception of a magic packet. This bit is cleared when this register is read (or write of 1 when RCWE bit in is set).
...
$00000000 constant ETH_ETH_MACRWKPFR_MACRWKPFR                      \ Remote wakeup packet filter Refer to , and for details on register content and programming sequence.
...
$00000001 constant ETH_ETH_MACLCSR_TLPIEN                           \ Transmit LPI Entry When this bit is set, it indicates that the MAC Transmitter has entered the LPI state because of the setting of the LPIEN bit. This bit is cleared by a read into this register (or write of 1 when RCWE bit in is set).
$00000002 constant ETH_ETH_MACLCSR_TLPIEX                           \ Transmit LPI Exit When this bit is set, it indicates that the MAC transmitter exited the LPI state after the application cleared the LPIEN bit and the LPI TW Timer has expired. This bit is cleared by a read into this register (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
...
$000fffff constant ETH_ETH_MACLETR_LPIET                            \ LPI Entry Timer This field specifies the time in microseconds the MAC will wait to enter LPI mode, after it has transmitted all the frames. This field is valid and used only when LPITE and LPITXA are set to 1. Bits [2:0] are read-only so that the granularity of this timer is in steps of 8 micro-seconds.
...
$0000001f constant ETH_ETH_MACHWF1R_RXFIFOSIZE                      \ MTL Receive FIFO Size This field contains the configured value of MTL Rx FIFO in bytes expressed as Log to base 2 minus 7, that is, Log2(RXFIFO_SIZE) -7:
$000007c0 constant ETH_ETH_MACHWF1R_TXFIFOSIZE                      \ MTL Transmit FIFO Size This field contains the configured value of MTL Tx FIFO in bytes expressed as Log to base 2 minus 7, that is, Log2(TXFIFO_SIZE) -7:
...
$07000000 constant ETH_ETH_MACHWF2R_PPSOUTNUM                       \ Number of PPS Outputs This field indicates the number of PPS outputs:
$70000000 constant ETH_ETH_MACHWF2R_AUXSNAPNUM                      \ Number of Auxiliary Snapshot Inputs This field indicates the number of auxiliary snapshot inputs:
...
$00000007 constant ETH_ETH_MACHWF3R_NRVF                            \ Number of Extended VLAN Tag Filters Enabled This field indicates the Number of Extended VLAN Tag Filters selected:
...
$08000000 constant ETH_ETH_MACMDIOAR_PSE                            \ Preamble Suppression Enable When this bit is set, the SMA will suppress the 32-bit preamble and transmit MDIO frames with only 1 preamble bit. When this bit is 0, the MDIO frame always has 32 bits of preamble as defined in the IEEE specifications.
...
$00000000 constant ETH_ETH_MACA0LR_ADDRLO                           \ MAC Address x [31:0] (x = 0 to 3) This field contains the lower 32 bits of the first 6-byte MAC address. The MAC uses this field for filtering the received packets and inserting the MAC address in the Transmit Flow Control (Pause) Packets.
...
$40000000 constant ETH_ETH_MACA1HR_SA                               \ Source Address When this bit is set, the MAC Address1[47:0] is used to compare with the SA fields of the received packet. When this bit is reset, the MAC Address x[47:0] is used to compare with the DA fields of the received packet.
...
$00000000 constant ETH_ETH_MACA1LR_ADDRLO                           \ MAC Address x [31:0] (x = 0 to 3) This field contains the lower 32 bits of the first 6-byte MAC address. The MAC uses this field for filtering the received packets and inserting the MAC address in the Transmit Flow Control (Pause) Packets.
...
$40000000 constant ETH_ETH_MACA2HR_SA                               \ Source Address When this bit is set, the MAC Address1[47:0] is used to compare with the SA fields of the received packet. When this bit is reset, the MAC Address x[47:0] is used to compare with the DA fields of the received packet.
...
$00000000 constant ETH_ETH_MACA2LR_ADDRLO                           \ MAC Address x [31:0] (x = 0 to 3) This field contains the lower 32 bits of the first 6-byte MAC address. The MAC uses this field for filtering the received packets and inserting the MAC address in the Transmit Flow Control (Pause) Packets.
...
$40000000 constant ETH_ETH_MACA3HR_SA                               \ Source Address When this bit is set, the MAC Address1[47:0] is used to compare with the SA fields of the received packet. When this bit is reset, the MAC Address x[47:0] is used to compare with the DA fields of the received packet.
...
$00000000 constant ETH_ETH_MACA3LR_ADDRLO                           \ MAC Address x [31:0] (x = 0 to 3) This field contains the lower 32 bits of the first 6-byte MAC address. The MAC uses this field for filtering the received packets and inserting the MAC address in the Transmit Flow Control (Pause) Packets.
...
$00000020 constant ETH_ETH_MMC_RX_INTERRUPT_RXCRCERPIS              \ MMC Receive CRC Error Packet Counter Interrupt Status This bit is set when the counter reaches half of the maximum value or the maximum value.
$00000040 constant ETH_ETH_MMC_RX_INTERRUPT_RXALGNERPIS             \ MMC Receive Alignment Error Packet Counter Interrupt Status This bit is set when the (ETH_RX_ALIGNMENT_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
$00020000 constant ETH_ETH_MMC_RX_INTERRUPT_RXUCGPIS                \ MMC Receive Unicast Good Packet Counter Interrupt Status This bit is set when the (ETH_RX_UNICAST_PACKETS_GOOD) counter reaches half of the maximum value or the maximum value.
$04000000 constant ETH_ETH_MMC_RX_INTERRUPT_RXLPIUSCIS              \ MMC Receive LPI microsecond counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
$08000000 constant ETH_ETH_MMC_RX_INTERRUPT_RXLPITRCIS              \ MMC Receive LPI transition counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
...
$00004000 constant ETH_ETH_MMC_TX_INTERRUPT_TXSCOLGPIS              \ MMC Transmit Single Collision Good Packet Counter Interrupt Status This bit is set when the (ETH_TX_SINGLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
$00008000 constant ETH_ETH_MMC_TX_INTERRUPT_TXMCOLGPIS              \ MMC Transmit Multiple Collision Good Packet Counter Interrupt Status This bit is set when the (ETH_TX_MULTIPLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
$00200000 constant ETH_ETH_MMC_TX_INTERRUPT_TXGPKTIS                \ MMC Transmit Good Packet Counter Interrupt Status This bit is set when the counter reaches half of the maximum value or the maximum value.
$04000000 constant ETH_ETH_MMC_TX_INTERRUPT_TXLPIUSCIS              \ MMC Transmit LPI microsecond counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
$08000000 constant ETH_ETH_MMC_TX_INTERRUPT_TXLPITRCIS              \ MMC Transmit LPI transition counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
...
$00000020 constant ETH_ETH_MMC_RX_INTERRUPT_MASK_RXCRCERPIM         \ MMC Receive CRC Error Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_CRC_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
$00000040 constant ETH_ETH_MMC_RX_INTERRUPT_MASK_RXALGNERPIM        \ MMC Receive Alignment Error Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_ALIGNMENT_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
$00020000 constant ETH_ETH_MMC_RX_INTERRUPT_MASK_RXUCGPIM           \ MMC Receive Unicast Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_UNICAST_PACKETS_GOOD) counter reaches half of the maximum value or the maximum value.
$04000000 constant ETH_ETH_MMC_RX_INTERRUPT_MASK_RXLPIUSCIM         \ MMC Receive LPI microsecond counter interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_LPI_USEC_CNTR) counter reaches half of the maximum value or the maximum value.
$08000000 constant ETH_ETH_MMC_RX_INTERRUPT_MASK_RXLPITRCIM         \ MMC Receive LPI transition counter interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_LPI_TRAN_CNTR) counter reaches half of the maximum value or the maximum value.
...
$00004000 constant ETH_ETH_MMC_TX_INTERRUPT_MASK_TXSCOLGPIM         \ MMC Transmit Single Collision Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_SINGLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
$00008000 constant ETH_ETH_MMC_TX_INTERRUPT_MASK_TXMCOLGPIM         \ MMC Transmit Multiple Collision Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_MULTIPLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
$00200000 constant ETH_ETH_MMC_TX_INTERRUPT_MASK_TXGPKTIM           \ MMC Transmit Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_PACKET_COUNT_GOOD) counter reaches half of the maximum value or the maximum value.
$04000000 constant ETH_ETH_MMC_TX_INTERRUPT_MASK_TXLPIUSCIM         \ MMC Transmit LPI microsecond counter interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_LPI_USEC_CNTR) counter reaches half of the maximum value or the maximum value.
$08000000 constant ETH_ETH_MMC_TX_INTERRUPT_MASK_TXLPITRCIM         \ MMC Transmit LPI transition counter interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_LPI_TRAN_CNTR) counter reaches half of the maximum value or the maximum value.
...
$00000000 constant ETH_ETH_TX_LPI_TRAN_CNTR_TXLPITRC                \ Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the (ETH_MACLCSR)), the counter will increment.
...
$00000004 constant ETH_ETH_MACTSCR_TSINIT                           \ Initialize Timestamp When this bit is set, the system time is initialized (overwritten) with the value specified in the and update register (ETH_MACSTNUR). This bit should be zero before it is updated. This bit is reset when the initialization is complete.
$00000008 constant ETH_ETH_MACTSCR_TSUPDT                           \ Update Timestamp When this bit is set, the system time is updated (added or subtracted) with the value specified in and update register (ETH_MACSTNUR). This bit should be zero before updating it. This bit is reset when the update is complete in hardware.
...
$00030000 constant ETH_ETH_MACTSCR_SNAPTYPSEL                       \ Select PTP packets for Taking Snapshots These bits, along with Bits 15 and 14, define the set of PTP packet types for which snapshot needs to be taken. The encoding is given in Register Bits.
...
$00ff0000 constant ETH_ETH_MACSSIR_SSINC                            \ Sub-second Increment Value The value programmed in this field is accumulated every clock cycle (of clk_ptp_i) with the contents of the sub-second register. For example, when the PTP clock is 50 MHz (period is 20 ns), you should program 20 (0x14) when the System Time Nanoseconds register has an accuracy of 1 ns [TSCTRLSSR bit is set in (ETH_MACTSCR)]. When TSCTRLSSR is cleared, the Nanoseconds register has a resolution of ~0.465 ns. In this case, you should program a value of 43 (0x2B) which is derived by 20 ns/0.465.
...
$7fffffff constant ETH_ETH_MACSTNR_TSSS                             \ Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When TSCTRLSSR is set in , each bit represents 1 ns. The maximum value is 0x3B9A_C9FF after which it rolls-over to zero.
...
$7fffffff constant ETH_ETH_MACSTNUR_TSSS                            \ Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When the TSCTRLSSR bit is set in the , each bit represents 1 ns and the programmed value should not exceed 0x3B9A_C9FF.
...
$00000002 constant ETH_ETH_MACTSSR_TSTARGT0                         \ Timestamp Target Time Reached When set, this bit indicates that the value of system time is greater than or equal to the value specified in the and nanoseconds register (ETH_MACPPSTTNR). This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in software control register (ETH_MACCSRSWCR) is set)
$00000004 constant ETH_ETH_MACTSSR_AUXTSTRIG                        \ Auxiliary Timestamp Trigger Snapshot This bit is set high when the auxiliary snapshot is written to the FIFO. This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in software control register (ETH_MACCSRSWCR) is set).
$00000008 constant ETH_ETH_MACTSSR_TSTRGTERR0                       \ Timestamp Target Time Error This bit is set when the latest target time programmed in the register (ETH_MACPPSTTSR) and (ETH_MACPPSTTNR) elapses. This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in is set).
$00008000 constant ETH_ETH_MACTSSR_TXTSSIS                          \ Tx Timestamp Status Interrupt Status When drop transmit status is enabled in MTL, this bit is set when the captured transmit timestamp is updated in the (ETH_MACTXTSSNR) and . When PTP offload feature is enabled, this bit is set when the captured transmit timestamp is updated in the and timestamp status seconds register (ETH_MACTXTSSSR), for PTO generated Delay Request and Pdelay request packets. This bit is cleared when the is read (or write of 1 when RCWE bit in is set).
...
$00000010 constant ETH_ETH_MACPPSCR_PPSEN0                          \ Flexible PPS Output Mode Enable When this bit is set, PPSCTRL[3:0] function as PPSCMD. When this bit is reset, PPSCTRL[3:0] function as PPSCTRL (Fixed PPS mode).
$00000060 constant ETH_ETH_MACPPSCR_TRGTMODSEL0                     \ Target Time Register Mode for PPS Output This field indicates the Target Time registers ((ETH_MACPPSTTSR) and ) mode for PPS output signal:
...
$00000040 constant ETH_ETH_MACPOCR_DRRDIS                           \ Disable PTO Delay Request/Response response generation When this bit is set, the Delay Request and Delay response will not be generated for received SYNC and Delay request packet respectively, as required by the programmed mode.
...
$00000100 constant ETH_ETH_MTLOMR_CNTPRST                           \ Counters Preset When this bit is set: is initialized/preset to 0x7F0. Missed Packet and Overflow Packet counters in overflow counter register (ETH_MTLRXQMPOCR) is initialized/preset to 0x7F0 This bit is cleared automatically.
...
$00070000 constant ETH_ETH_MTLTXQDR_PTXQ                            \ Number of Packets in the Transmit Queue This field indicates the current number of packets in the Tx queue. When the DTXSTS bit of register is set to 1, this field does not reflect the number of packets in the Transmit queue.
...
$00030000 constant ETH_ETH_DMAMR_INTM                               \ Interrupt Mode This field defines the interrupt mode of the Ethernet peripheral. The behavior of the interrupt signal and of the RI/TI bits in the ETH_DMACSR register changes depending on the INTM value (refer to behavior).
...
$00010000 constant ETH_ETH_DMACCR_PBLX8                             \ 8xPBL mode When this bit is set, the PBL value programmed in Bits[21:16] in register (ETH_DMACTXCR) is multiplied eight times. Therefore, the DMA transfers the data in 8, 16, 32, 64, 128, and 256 beats depending on the PBL value.
...
$00ff0000 constant ETH_ETH_DMACRXRLR_ARBS                           \ Alternate Receive Buffer Size Indicates size in bytes for Buffer 1 when ARBS[7:0] is programmed to a non-zero value. When ARBS[7:0] = 0, Rx Buffer1 and Rx Buffer2 sizes are based on RBSZ[13:0] field of .
...
$00000004 constant ETH_ETH_DMACSR_TBU                               \ Transmit Buffer Unavailable This bit indicates that the application owns the next descriptor in the Transmit list, and the DMA cannot acquire it. Transmission is suspended. The TPSi field of the register (ETH_DMADSR) register explains the Transmit Process state transitions. To resume processing the Transmit descriptors, the application should do the following: 1. Change the ownership of the descriptor by setting Bit 31 of TDES3. 2. Issue a Transmit Poll Demand command. For ring mode, the application should advance the Transmit Descriptor Tail Pointer register of a channel.
...
$000007ff constant ETH_ETH_DMACMFCR_MFC                             \ Dropped Packet Counters This counter indicates the number of packet counters that are dropped by the DMA either because of bus error or because of programing RPF field in register (ETH_DMACRXCR). The counter gets cleared when this register is read.
...
$40028060 constant ETH_ETH_MACVIRALTERNATE  \ offset: 0x60 : VLAN inclusion register
...
$400280c0 constant ETH_ETH_MACPCSR  \ offset: 0xC0 : Power Down When this bit is set, the MAC receiver drops all received packets until it receives the expected magic packet or remote wakeup packet. This bit is then self-cleared and the power-down mode is disabled. The software can clear this bit before the expected magic packet or remote wakeup packet is received. The packets received by the MAC after this bit is cleared are forwarded to the application. This bit must only be set when the Magic Packet Enable, Global Unicast, or Remote wakeup Packet Enable bit is set high. Note: You can gate-off the CSR clock during the power-down mode. However, when the CSR clock is gated-off, you cannot perform any read or write operations on this register. Therefore, the Software cannot clear this bit.
$400280c4 constant ETH_ETH_MACRWKPFR  \ offset: 0xC4 : Remote wakeup packet filter Refer to , and for details on register content and programming sequence.
...
$40028904 constant ETH_ETH_MACL4A0R  \ offset: 0x904 : Layer 4 Source Port Number Field When the L4PEN0 bit is reset and the L4DPM0 bit is set in the (ETH_MACL3L4C0R), this field contains the value to be matched with the TCP Source Port Number field in the IPv4 or IPv6 packets. When the L4PEN0 and L4DPM0 bits are set in (ETH_MACL3L4C0R), this field contains the value to be matched with the UDP Source Port Number field in the IPv4 or IPv6 packets.
...
$40028934 constant ETH_ETH_MACL4A1R  \ offset: 0x934 : Layer 4 Source Port Number Field When the L4PEN1 bit is reset and the L4DPM1 bit is set in the (ETH_MACL3L4C1R), this field contains the value to be matched with the TCP Source Port Number field in the IPv4 or IPv6 packets. When the L4PEN1 and L4DPM1 bits are set in (ETH_MACL3L4C1R), this field contains the value to be matched with the UDP Source Port Number field in the IPv4 or IPv6 packets.
...
$40028b04 constant ETH_ETH_MACSSIR  \ offset: 0xB04 : Sub-second increment register
...
$40028b14 constant ETH_ETH_MACSTNUR  \ offset: 0xB14 : Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When the TSCTRLSSR bit is set in the , each bit represents 1 ns and the programmed value should not exceed 0x3B9A_C9FF.
...
$40028b70 constant ETH_ETH_MACPPSCR  \ offset: 0xB70 : ETH_MACPPSCR
$40028b70 constant ETH_ETH_MACPPSCRALTERNATE  \ offset: 0xB70 : ETH_MACPPSCRALTERNATE
