...
[ifndef] ETH_DEF
  ...
  [ifdef] ETH_ETH_MACECR_DEF
    ...
    $12 constant ETH_USP                        \ [0x12] Unicast Slow Protocol Packet Detect When this bit is set, the MAC detects the Slow Protocol packets with unicast address of the station specified in the and MAC Address 0 low register . The MAC also detects the Slow Protocol packets with the Slow Protocols multicast address (01-80-C2-00-00-02). When this bit is reset, the MAC detects only Slow Protocol packets with the Slow Protocol multicast address specified in the IEEE 802.3-2008, Section 5.
    ...
    $19 constant ETH_EIPG                       \ [0x19 : 5] Extended Inter-Packet Gap The value in this field is applicable when the EIPGEN bit is set. This field (as Most Significant bits) along with IPG field in , gives the minimum IPG greater than 96 bit times in steps of 8 bit times. For example: EIPG = 0 and IPG = 0 give 104 bit times EIPG = 0 and IPG = 1 give 112 bit times EIPG = 0 and IPG = 2 give 120 bit times .. EIPG = 7 and IPG = 31 give 2144 bit times
  [then]
  ...
  [ifdef] ETH_ETH_MACVIR_DEF
    ...
    $15 constant ETH_CBTI                       \ [0x15] Channel based tag insertion When this bit is set, outer VLAN tag is inserted for every packets transmitted by the MAC. The tag value is taken from the queue/channel specific VLAN tag register. This will override VLTI, VLP, VLC and VLT fields of this register When this bit is set, a write operation to byte 3 of this register initiates the read/write access to the indirect register. When reset outer VLAN operation is based on the setting of VLTI, VLP, VLC and VLT fields of this register
    $18 constant ETH_ADDR                       \ [0x18] Address This field selects one of the queue/channel specific VLAN Inclusion register for read/write access This does not have any effect when CBTI is reset
    $1e constant ETH_RDWR                       \ [0x1e] Read write control This bit controls the read or write operation for indirectly accessing the queue/channel specific VLAN Inclusion register. When set indicates write operation and when reset indicates read operation This has no effect when CBTI is reset
    $1f constant ETH_BUSY                       \ [0x1f] Busy This bit indicates the status of the read/write operation of indirect access to the queue/channel specific VLAN inclusion register. For write operation write to a register is complete when this bit is reset. For read operation the read data is valid when the bit is reset. No further writes are allowed to this register when this bit is set. This does not have any effect when CBTI is reset
  [then]


  [ifdef] ETH_ETH_MACVIRALTERNATE_DEF
    \
    \ @brief VLAN inclusion register
    \ Address offset: 0x60
    \ Reset value: 0x00000000
  ...
  [ifdef] ETH_ETH_MACISR_DEF
    ...
    $04 constant ETH_PMTIS                      \ [0x04] PMT Interrupt Status This bit is set when a Magic packet or Wake-on-LAN packet is received in the power-down mode (RWKPRCVD and MGKPRCVD bits in ETH_MACPCSR register). This bit is cleared when Bits[6:5] are cleared because of a Read operation to the (ETH_MACPCSR).
    $05 constant ETH_LPIIS                      \ [0x05] LPI Interrupt Status This bit is set for any LPI state entry or exit in the MAC Transmitter or Receiver. This bit is cleared when the TLPIEN bit of is read.
    ...
    $09 constant ETH_MMCRXIS                    \ [0x09] MMC Receive Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_RX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
    $0a constant ETH_MMCTXIS                    \ [0x0a] MMC Transmit Interrupt Status This bit is set high when an interrupt is generated in the (ETH_MMC_TX_INTERRUPT). This bit is cleared when all bits in this interrupt register are cleared.
    ...
    $0e constant ETH_RXSTSIS                    \ [0x0e] Receive Status Interrupt This bit indicates the status of received packets. This bit is set when the RWT bit is set in the . This bit is cleared when the corresponding interrupt source bit is read (or corresponding interrupt source bit is written to 1 when RCWE bit of is set) in the ETH_MACISR register.
  [then]
  ...
  [ifdef] ETH_ETH_MACIER_DEF
    ...
    \ Reset value: 0x00000000
    \
    $03 constant ETH_PHYIE                      \ [0x03] PHY Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of PHYIS bit in .
    $04 constant ETH_PMTIE                      \ [0x04] PMT Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of PMTIS bit in .
    $05 constant ETH_LPIIE                      \ [0x05] LPI Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of LPIIS bit in .
    $0c constant ETH_TSIE                       \ [0x0c] Timestamp Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of TSIS bit in .
    $0d constant ETH_TXSTSIE                    \ [0x0d] Transmit Status Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of TXSTSIS bit in the .
    $0e constant ETH_RXSTSIE                    \ [0x0e] Receive Status Interrupt Enable When this bit is set, it enables the assertion of the interrupt signal because of the setting of RXSTSIS bit in the .
  [then]
  ...
  [ifdef] ETH_ETH_MACRXTXSR_DEF
    ...
    $01 constant ETH_NCARR                      \ [0x01] No Carrier When the DTXSTS bit is set in the , this bit indicates that the carrier signal from the PHY is not present at the end of preamble transmission. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
    $02 constant ETH_LCARR                      \ [0x02] Loss of Carrier When the DTXSTS bit is set in the , this bit indicates that the loss of carrier occurred during packet transmission, that is, the ETH_CRS signal was inactive for one or more transmission clock periods during packet transmission. This bit is valid only for packets transmitted without collision. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
    ...
    $04 constant ETH_LCOL                       \ [0x04] Late Collision When the DTXSTS bit is set in the , this bit indicates that the packet transmission aborted because a collision occurred after the collision window (64 bytes including Preamble in MII mode. This bit is not valid if the Underflow error occurs. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
    $05 constant ETH_EXCOL                      \ [0x05] Excessive Collisions When the DTXSTS bit is set in the , this bit indicates that the transmission aborted after 16 successive collisions while attempting to transmit the current packet. If the DR bit is set in the (ETH_MACCR), this bit is set after the first collision and the packet transmission is aborted. Cleared on read (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
  ...
  [ifdef] ETH_ETH_MACPCSR_DEF
    ...
    $05 constant ETH_MGKPRCVD                   \ [0x05] Magic Packet Received When this bit is set, it indicates that the power management event is generated because of the reception of a magic packet. This bit is cleared when this register is read (or write of 1 when RCWE bit in is set).
  ...
  [ifdef] ETH_ETH_MACLCSR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TLPIEN                     \ [0x00] Transmit LPI Entry When this bit is set, it indicates that the MAC Transmitter has entered the LPI state because of the setting of the LPIEN bit. This bit is cleared by a read into this register (or write of 1 when RCWE bit in is set).
    $01 constant ETH_TLPIEX                     \ [0x01] Transmit LPI Exit When this bit is set, it indicates that the MAC transmitter exited the LPI state after the application cleared the LPIEN bit and the LPI TW Timer has expired. This bit is cleared by a read into this register (or write of 1 when RCWE bit in (ETH_MACCSRSWCR) is set).
  ...
  [ifdef] ETH_ETH_MACLETR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_LPIET                      \ [0x00 : 20] LPI Entry Timer This field specifies the time in microseconds the MAC will wait to enter LPI mode, after it has transmitted all the frames. This field is valid and used only when LPITE and LPITXA are set to 1. Bits [2:0] are read-only so that the granularity of this timer is in steps of 8 micro-seconds.
  [then]
  ...
  [ifdef] ETH_ETH_MMC_RX_INTERRUPT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $05 constant ETH_RXCRCERPIS                 \ [0x05] MMC Receive CRC Error Packet Counter Interrupt Status This bit is set when the counter reaches half of the maximum value or the maximum value.
    $06 constant ETH_RXALGNERPIS                \ [0x06] MMC Receive Alignment Error Packet Counter Interrupt Status This bit is set when the (ETH_RX_ALIGNMENT_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
    $11 constant ETH_RXUCGPIS                   \ [0x11] MMC Receive Unicast Good Packet Counter Interrupt Status This bit is set when the (ETH_RX_UNICAST_PACKETS_GOOD) counter reaches half of the maximum value or the maximum value.
    $1a constant ETH_RXLPIUSCIS                 \ [0x1a] MMC Receive LPI microsecond counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
    $1b constant ETH_RXLPITRCIS                 \ [0x1b] MMC Receive LPI transition counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
  [then]
  ...
  [ifdef] ETH_ETH_MMC_TX_INTERRUPT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant ETH_TXSCOLGPIS                 \ [0x0e] MMC Transmit Single Collision Good Packet Counter Interrupt Status This bit is set when the (ETH_TX_SINGLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
    $0f constant ETH_TXMCOLGPIS                 \ [0x0f] MMC Transmit Multiple Collision Good Packet Counter Interrupt Status This bit is set when the (ETH_TX_MULTIPLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
    $15 constant ETH_TXGPKTIS                   \ [0x15] MMC Transmit Good Packet Counter Interrupt Status This bit is set when the counter reaches half of the maximum value or the maximum value.
    $1a constant ETH_TXLPIUSCIS                 \ [0x1a] MMC Transmit LPI microsecond counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
    $1b constant ETH_TXLPITRCIS                 \ [0x1b] MMC Transmit LPI transition counter interrupt status This bit is set when the counter reaches half of the maximum value or the maximum value.
  [then]
  ...
  [ifdef] ETH_ETH_MMC_RX_INTERRUPT_MASK_DEF
    ...
    \ Reset value: 0x00000000
    \
    $05 constant ETH_RXCRCERPIM                 \ [0x05] MMC Receive CRC Error Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_CRC_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
    $06 constant ETH_RXALGNERPIM                \ [0x06] MMC Receive Alignment Error Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_ALIGNMENT_ERROR_PACKETS) counter reaches half of the maximum value or the maximum value.
    $11 constant ETH_RXUCGPIM                   \ [0x11] MMC Receive Unicast Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_UNICAST_PACKETS_GOOD) counter reaches half of the maximum value or the maximum value.
    $1a constant ETH_RXLPIUSCIM                 \ [0x1a] MMC Receive LPI microsecond counter interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_LPI_USEC_CNTR) counter reaches half of the maximum value or the maximum value.
    $1b constant ETH_RXLPITRCIM                 \ [0x1b] MMC Receive LPI transition counter interrupt Mask Setting this bit masks the interrupt when the (ETH_RX_LPI_TRAN_CNTR) counter reaches half of the maximum value or the maximum value.
  [then]
  ...
  [ifdef] ETH_ETH_MMC_TX_INTERRUPT_MASK_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant ETH_TXSCOLGPIM                 \ [0x0e] MMC Transmit Single Collision Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_SINGLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
    $0f constant ETH_TXMCOLGPIM                 \ [0x0f] MMC Transmit Multiple Collision Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_MULTIPLE_COLLISION_GOOD_PACKETS) counter reaches half of the maximum value or the maximum value.
    $15 constant ETH_TXGPKTIM                   \ [0x15] MMC Transmit Good Packet Counter Interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_PACKET_COUNT_GOOD) counter reaches half of the maximum value or the maximum value.
    $1a constant ETH_TXLPIUSCIM                 \ [0x1a] MMC Transmit LPI microsecond counter interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_LPI_USEC_CNTR) counter reaches half of the maximum value or the maximum value.
    $1b constant ETH_TXLPITRCIM                 \ [0x1b] MMC Transmit LPI transition counter interrupt Mask Setting this bit masks the interrupt when the (ETH_TX_LPI_TRAN_CNTR) counter reaches half of the maximum value or the maximum value.
  [then]
  ...
  [ifdef] ETH_ETH_TX_LPI_TRAN_CNTR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TXLPITRC                   \ [0x00 : 32] Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the (ETH_MACLCSR)), the counter will increment.
  [then]
  ...
  [ifdef] ETH_ETH_MACTSCR_DEF
    ...
    $02 constant ETH_TSINIT                     \ [0x02] Initialize Timestamp When this bit is set, the system time is initialized (overwritten) with the value specified in the and update register (ETH_MACSTNUR). This bit should be zero before it is updated. This bit is reset when the initialization is complete.
    $03 constant ETH_TSUPDT                     \ [0x03] Update Timestamp When this bit is set, the system time is updated (added or subtracted) with the value specified in and update register (ETH_MACSTNUR). This bit should be zero before updating it. This bit is reset when the update is complete in hardware.
    ...
    $10 constant ETH_SNAPTYPSEL                 \ [0x10 : 2] Select PTP packets for Taking Snapshots These bits, along with Bits 15 and 14, define the set of PTP packet types for which snapshot needs to be taken. The encoding is given in Register Bits.
  ...
  [ifdef] ETH_ETH_MACSSIR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant ETH_SSINC                      \ [0x10 : 8] Sub-second Increment Value The value programmed in this field is accumulated every clock cycle (of clk_ptp_i) with the contents of the sub-second register. For example, when the PTP clock is 50 MHz (period is 20 ns), you should program 20 (0x14) when the System Time Nanoseconds register has an accuracy of 1 ns [TSCTRLSSR bit is set in (ETH_MACTSCR)]. When TSCTRLSSR is cleared, the Nanoseconds register has a resolution of ~0.465 ns. In this case, you should program a value of 43 (0x2B) which is derived by 20 ns/0.465.
  [then]
  ...
  [ifdef] ETH_ETH_MACSTNR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TSSS                       \ [0x00 : 31] Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When TSCTRLSSR is set in , each bit represents 1 ns. The maximum value is 0x3B9A_C9FF after which it rolls-over to zero.
  [then]
  ...
  [ifdef] ETH_ETH_MACSTNUR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_TSSS                       \ [0x00 : 31] Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When the TSCTRLSSR bit is set in the , each bit represents 1 ns and the programmed value should not exceed 0x3B9A_C9FF.
  ...
  [ifdef] ETH_ETH_MACTSSR_DEF
    ...
    $01 constant ETH_TSTARGT0                   \ [0x01] Timestamp Target Time Reached When set, this bit indicates that the value of system time is greater than or equal to the value specified in the and nanoseconds register (ETH_MACPPSTTNR). This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in software control register (ETH_MACCSRSWCR) is set)
    $02 constant ETH_AUXTSTRIG                  \ [0x02] Auxiliary Timestamp Trigger Snapshot This bit is set high when the auxiliary snapshot is written to the FIFO. This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in software control register (ETH_MACCSRSWCR) is set).
    $03 constant ETH_TSTRGTERR0                 \ [0x03] Timestamp Target Time Error This bit is set when the latest target time programmed in the register (ETH_MACPPSTTSR) and (ETH_MACPPSTTNR) elapses. This bit is cleared when the application reads this bit (or write of 1 when RCWE bit in is set).
    $0f constant ETH_TXTSSIS                    \ [0x0f] Tx Timestamp Status Interrupt Status When drop transmit status is enabled in MTL, this bit is set when the captured transmit timestamp is updated in the (ETH_MACTXTSSNR) and . When PTP offload feature is enabled, this bit is set when the captured transmit timestamp is updated in the and timestamp status seconds register (ETH_MACTXTSSSR), for PTO generated Delay Request and Pdelay request packets. This bit is cleared when the is read (or write of 1 when RCWE bit in is set).
  ...
  [ifdef] ETH_ETH_MACPPSCR_DEF
    ...
    $04 constant ETH_PPSEN0                     \ [0x04] Flexible PPS Output Mode Enable When this bit is set, PPSCTRL[3:0] function as PPSCMD. When this bit is reset, PPSCTRL[3:0] function as PPSCTRL (Fixed PPS mode).
    $05 constant ETH_TRGTMODSEL0                \ [0x05 : 2] Target Time Register Mode for PPS Output This field indicates the Target Time registers ((ETH_MACPPSTTSR) and ) mode for PPS output signal:
  [then]


  [ifdef] ETH_ETH_MACPPSCRALTERNATE_DEF
    \
  ...
  [ifdef] ETH_ETH_MTLOMR_DEF
    ...
    $08 constant ETH_CNTPRST                    \ [0x08] Counters Preset When this bit is set: is initialized/preset to 0x7F0. Missed Packet and Overflow Packet counters in overflow counter register (ETH_MTLRXQMPOCR) is initialized/preset to 0x7F0 This bit is cleared automatically.
  ...
  [ifdef] ETH_ETH_MTLTXQDR_DEF
    ...
    $10 constant ETH_PTXQ                       \ [0x10 : 3] Number of Packets in the Transmit Queue This field indicates the current number of packets in the Tx queue. When the DTXSTS bit of register is set to 1, this field does not reflect the number of packets in the Transmit queue.
  ...
  [ifdef] ETH_ETH_DMACCR_DEF
    ...
    $10 constant ETH_PBLX8                      \ [0x10] 8xPBL mode When this bit is set, the PBL value programmed in Bits[21:16] in register (ETH_DMACTXCR) is multiplied eight times. Therefore, the DMA transfers the data in 8, 16, 32, 64, 128, and 256 beats depending on the PBL value.
  ...
  [ifdef] ETH_ETH_DMACRXRLR_DEF
    ...
    $10 constant ETH_ARBS                       \ [0x10 : 8] Alternate Receive Buffer Size Indicates size in bytes for Buffer 1 when ARBS[7:0] is programmed to a non-zero value. When ARBS[7:0] = 0, Rx Buffer1 and Rx Buffer2 sizes are based on RBSZ[13:0] field of .
  [then]
  ...
  [ifdef] ETH_ETH_DMACSR_DEF
    ...
    $02 constant ETH_TBU                        \ [0x02] Transmit Buffer Unavailable This bit indicates that the application owns the next descriptor in the Transmit list, and the DMA cannot acquire it. Transmission is suspended. The TPSi field of the register (ETH_DMADSR) register explains the Transmit Process state transitions. To resume processing the Transmit descriptors, the application should do the following: 1. Change the ownership of the descriptor by setting Bit 31 of TDES3. 2. Issue a Transmit Poll Demand command. For ring mode, the application should advance the Transmit Descriptor Tail Pointer register of a channel.
  ...
  [ifdef] ETH_ETH_DMACMFCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ETH_MFC                        \ [0x00 : 11] Dropped Packet Counters This counter indicates the number of packet counters that are dropped by the DMA either because of bus error or because of programing RPF field in register (ETH_DMACRXCR). The counter gets cleared when this register is read.
  ...
  $60 constant ETH_ETH_MACVIRALTERNATE  \ VLAN inclusion register
  ...
  $934 constant ETH_ETH_MACL4A1R        \ Layer 4 Source Port Number Field When the L4PEN1 bit is reset and the L4DPM1 bit is set in the (ETH_MACL3L4C1R), this field contains the value to be matched with the TCP Source Port Number field in the IPv4 or IPv6 packets. When the L4PEN1 and L4DPM1 bits are set in (ETH_MACL3L4C1R), this field contains the value to be matched with the UDP Source Port Number field in the IPv4 or IPv6 packets.
  ...
  $B14 constant ETH_ETH_MACSTNUR        \ Timestamp Sub-seconds The value in this field has the sub-second representation of time, with an accuracy of 0.46 ns. When the TSCTRLSSR bit is set in the , each bit represents 1 ns and the programmed value should not exceed 0x3B9A_C9FF.
  ...
  $B70 constant ETH_ETH_MACPPSCR        \ ETH_MACPPSCR
  $B70 constant ETH_ETH_MACPPSCRALTERNATE    \ ETH_MACPPSCRALTERNATE
