...
[ifndef] FLASH_DEF
  ...
  [ifdef] FLASH_KEYR1_DEF
    ...
    \
    $00 constant FLASH_KEY1R                    \ [0x00 : 32] Bank 1 access configuration unlock key
  [then]
  ...
  [ifdef] FLASH_CR1_DEF
    ...
    $19 constant FLASH_SNECCERR1                \ [0x19] Bank 1 ECC single correction error interrupt enable bit
  ...
  [ifdef] FLASH_SR1_DEF
    ...
    $19 constant FLASH_SNECCERR                 \ [0x19] Bank 1 single correction error flag
  ...
  [ifdef] FLASH_CR2_DEF
    ...
    $0e constant FLASH_SPSS2                    \ [0x0e] Bank 2 special sector selection bit
    ...
    $1c constant FLASH_CRCRDERRIE2              \ [0x1c] Bank 2 CRC read error interrupt enable bit
  [then]
  ...
  [ifdef] FLASH_CRCSADD2R_DEF
    ...
    \ Reset value: 0x00000000
    \
    $02 constant FLASH_CRC_START_ADDR           \ [0x02 : 18] CRC start address on bank 2
  [then]
  ...
  [ifdef] FLASH_CRCEADD2R_DEF
    ...
    \ Reset value: 0x00000000
    \
    $02 constant FLASH_CRC_END_ADDR             \ [0x02 : 18] CRC end address on bank 2
  [then]


  [ifdef] FLASH_CRCDATAR__DEF
    \
    \ @brief FLASH CRC data register
  ...
  $12C constant FLASH_PRAR_PRG2         \ FLASH protection address for bank 2
  ...
  $140 constant FLASH_BOOT_CURR_        \ FLASH register with boot address
  $144 constant FLASH_BOOT_PRGR_        \ FLASH register with boot address
  ...
  $15C constant FLASH_CRCDATAR_         \ FLASH CRC data register
