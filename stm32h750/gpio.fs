...
[ifndef] GPIO_DEF
  ...
  \ @brief GPIO
  \
  $00 constant GPIO_GPIO_MODER          \ GPIO port mode register
  $04 constant GPIO_GPIO_OTYPER         \ GPIO port output type register
  $08 constant GPIO_GPIO_OSPEEDR        \ GPIO port output speed register
  $0C constant GPIO_GPIO_PUPDR          \ GPIO port pull-up/pull-down register
  $10 constant GPIO_GPIO_IDR            \ GPIO port input data register
  $14 constant GPIO_GPIO_ODR            \ GPIO port output data register
  $18 constant GPIO_GPIO_BSRR           \ GPIO port bit set/reset register
  $1C constant GPIO_GPIO_LCKR           \ This register is used to lock the configuration of the port bits when a correct write sequence is applied to bit 16 (LCKK). The value of bits [15:0] is used to lock the configuration of the GPIO. During the write sequence, the value of LCKR[15:0] must not change. When the LOCK sequence has been applied on a port bit, the value of this port bit can no longer be modified until the next MCU reset or peripheral reset.A specific write sequence is used to write to the GPIOx_LCKR register. Only word access (32-bit long) is allowed during this locking sequence.Each lock bit freezes a specific configuration register (control and alternate function registers).
  $20 constant GPIO_GPIO_AFRL           \ GPIO alternate function low register
  $24 constant GPIO_GPIO_AFRH           \ GPIO alternate function high register
