...
[ifndef] ADC_DEF
  ...
  [ifdef] ADC_CR2_DEF
    ...
    $08 constant ADC_DMA                        \ [0x08] Direct memory access mode (for single ADC mode)
    $09 constant ADC_DDS                        \ [0x09] DMA disable selection (for single ADC mode)
  ...
  [ifdef] ADC_SMPR1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ADC_SMPx_x                     \ [0x00 : 32] Sample time bits
  [then]
