...
[ifndef] TAMP_DEF
  ...
  [ifdef] TAMP_TAMP_SECCFGR_DEF
    ...
    $10 constant TAMP_BKPWSEC                   \ [0x10 : 8] Backup registers write protection offset Protection zone 2 is defined for backup registers from TAMP_BKPyR (y = BKPRWSEC, from 0 to 128) to TAMP_BKPzR (z = BKPWSEC-1, from 0 to 128, BKPWSEC greater than or equal BKPRWSEC): if TZEN=1, these backup registers can be written only with secure access. They can be read with secure or non-secure access. Protection zone 3 defined for backup registers from TAMP_BKPtR (t = BKPWSEC, from 0 to 127). They can be read or written with secure or non-secure access. If TZEN=0: the protection zone 2 can be read and written with non-secure access. If BKPWSEC = 0 or if BKPWSEC less than or equal BKPRWSEC: there is no protection zone 2. If BKPWPRIV is set, BKPRWSEC[7:0] can be written only in privileged mode.
  ...
  [ifdef] TAMP_TAMP_MISR_DEF
    ...
    $13 constant TAMP_ITAMP4MF                  \ [0x13] Internal tamper 4 non-secure interrupt masked flag This flag is set by hardware when the internal tamper 4 non-secure interrupt is raised.
    ...
    $1e constant TAMP_ITAMP15MF                 \ [0x1e] internal tamper 15 non-secure interrupt masked flag This flag is set by hardware when the internal tamper 15 non-secure interrupt is raised.
  [then]
  ...
  [ifdef] TAMP_TAMP_SMISR_DEF
    ...
    $13 constant TAMP_ITAMP4MF                  \ [0x13] Internal tamper 4 secure interrupt masked flag This flag is set by hardware when the internal tamper 4 secure interrupt is raised.
    ...
    $1e constant TAMP_ITAMP15MF                 \ [0x1e] internal tamper 15 secure interrupt masked flag This flag is set by hardware when the internal tamper 15 secure interrupt is raised.
  [then]
  ...
  $24 constant TAMP_TAMP_PRIVCFGR       \ TAMP privilege mode control register
