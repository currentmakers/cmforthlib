...
[ifndef] TIM16_DEF
  ...
  [ifdef] TIM16_TIM16_CR1_DEF
    ...
    $08 constant TIM16_CKD                      \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and the dead-time and sampling clock (tDTS)used by the dead-time generators and the digital filters (tim_tix),
  ...
  [ifdef] TIM16_TIM16_SR_DEF
    ...
    $05 constant TIM16_COMIF                    \ [0x05] COM interrupt flag This flag is set by hardware on a COM event (once the capture/compare control bits CCxE, CCxNE, OCxMhave been updated). It is cleared by software.
  ...
  [ifdef] TIM16_TIM16_BDTR_DEF
    ...
    $0c constant TIM16_BKE                      \ [0x0c] Break enable 1; Break inputs (tim_brk and tim_sys_brk event) enabled Note: This bit cannot be modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Any write operation to this bit takes a delay of 1 APB clock cycle to become effective.
  ...
  [ifdef] TIM16_TIM16_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM16_TI1SEL                   \ [0x00 : 4] selects tim_ti1_in[0..15] input ... Refer to for interconnects list.
  [then]
  ...
  [ifdef] TIM16_TIM16_AF2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant TIM16_OCRSEL                   \ [0x10 : 3] tim_ocref_clr source selection These bits select the tim_ocref_clr input source. Refer to for product specific implementation. Note: These bits can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
  [then]
