...
$00000001 constant FMC_FMC_BCR1_MBKEN                               \ Memory bank enable bit Enables the memory bank. After reset Bank1 is enabled, all others are disabled. Accessing a disabled bank causes an ERROR on AHB bus.
...
$0000000c constant FMC_FMC_BCR1_MTYP                                \ Memory type Defines the type of external memory attached to the corresponding memory bank.
...
$00000040 constant FMC_FMC_BCR1_FACCEN                              \ Flash access enable Enables NOR Flash memory access operations.
...
$00000200 constant FMC_FMC_BCR1_WAITPOL                             \ Wait signal polarity bit Defines the polarity of the wait signal from memory used for either in Synchronous or Asynchronous mode.
...
$00070000 constant FMC_FMC_BCR1_CPSIZE                              \ CRAM page size These are used for CellularRAM 1.5 which does not allow burst access to cross the address boundaries between pages. When these bits are configured, the FMC controller splits automatically the burst access when the memory page size is reached (refer to memory datasheet for page size). Others: reserved
...
$00200000 constant FMC_FMC_BCR1_WFDIS                               \ Write FIFO disable This bit disables the Write FIFO used by the FMC controller. Note: The WFDIS bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
$00c00000 constant FMC_FMC_BCR1_NBLSET                              \ Byte lane (NBL) setup These bits configure the NBL setup timing from NBLx low to chip select NEx low.
$80000000 constant FMC_FMC_BCR1_FMCEN                               \ FMC controller enable This bit enables or disables the FMC controller. Note: The FMCEN bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
...
$30000000 constant FMC_FMC_BTR1_ACCMOD                              \ Access mode Specifies the asynchronous access modes as shown in the timing diagrams. These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BTR1_DATAHLD                             \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous accesses: For read accesses For write accesses
...
$00000001 constant FMC_FMC_BCR2_MBKEN                               \ Memory bank enable bit Enables the memory bank. After reset Bank1 is enabled, all others are disabled. Accessing a disabled bank causes an ERROR on AHB bus.
...
$0000000c constant FMC_FMC_BCR2_MTYP                                \ Memory type Defines the type of external memory attached to the corresponding memory bank.
...
$00000040 constant FMC_FMC_BCR2_FACCEN                              \ Flash access enable Enables NOR Flash memory access operations.
...
$00000200 constant FMC_FMC_BCR2_WAITPOL                             \ Wait signal polarity bit Defines the polarity of the wait signal from memory used for either in Synchronous or Asynchronous mode.
...
$00070000 constant FMC_FMC_BCR2_CPSIZE                              \ CRAM page size These are used for CellularRAM 1.5 which does not allow burst access to cross the address boundaries between pages. When these bits are configured, the FMC controller splits automatically the burst access when the memory page size is reached (refer to memory datasheet for page size). Others: reserved
...
$00200000 constant FMC_FMC_BCR2_WFDIS                               \ Write FIFO disable This bit disables the Write FIFO used by the FMC controller. Note: The WFDIS bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
$00c00000 constant FMC_FMC_BCR2_NBLSET                              \ Byte lane (NBL) setup These bits configure the NBL setup timing from NBLx low to chip select NEx low.
$80000000 constant FMC_FMC_BCR2_FMCEN                               \ FMC controller enable This bit enables or disables the FMC controller. Note: The FMCEN bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
...
$00f00000 constant FMC_FMC_BTR2_CLKDIV                              \ Clock divide ratio (for FMC_CLK signal) Defines the period of FMC_CLK clock output signal, expressed in number of HCLK cycles: In asynchronous NOR Flash, SRAM or PSRAM accesses, this value is don't care. Note: Refer to Section 5.6.5: Synchronous transactions for FMC_CLK divider ratio formula)
$0f000000 constant FMC_FMC_BTR2_DATLAT                              \ (see note below bit descriptions): Data latency for synchronous memory For synchronous access with read/write Burst mode enabled (BURSTEN / CBURSTRW bits set), defines the number of memory clock cycles (+2) to issue to the memory before reading/writing the first data: This timing parameter is not expressed in HCLK periods, but in FMC_CLK periods. For asynchronous access, this value is don't care.
$30000000 constant FMC_FMC_BTR2_ACCMOD                              \ Access mode Specifies the asynchronous access modes as shown in the timing diagrams. These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BTR2_DATAHLD                             \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous accesses: For read accesses For write accesses
...
$00000001 constant FMC_FMC_BCR3_MBKEN                               \ Memory bank enable bit Enables the memory bank. After reset Bank1 is enabled, all others are disabled. Accessing a disabled bank causes an ERROR on AHB bus.
...
$0000000c constant FMC_FMC_BCR3_MTYP                                \ Memory type Defines the type of external memory attached to the corresponding memory bank.
...
$00000040 constant FMC_FMC_BCR3_FACCEN                              \ Flash access enable Enables NOR Flash memory access operations.
...
$00000200 constant FMC_FMC_BCR3_WAITPOL                             \ Wait signal polarity bit Defines the polarity of the wait signal from memory used for either in Synchronous or Asynchronous mode.
...
$00070000 constant FMC_FMC_BCR3_CPSIZE                              \ CRAM page size These are used for CellularRAM 1.5 which does not allow burst access to cross the address boundaries between pages. When these bits are configured, the FMC controller splits automatically the burst access when the memory page size is reached (refer to memory datasheet for page size). Others: reserved
...
$00200000 constant FMC_FMC_BCR3_WFDIS                               \ Write FIFO disable This bit disables the Write FIFO used by the FMC controller. Note: The WFDIS bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
$00c00000 constant FMC_FMC_BCR3_NBLSET                              \ Byte lane (NBL) setup These bits configure the NBL setup timing from NBLx low to chip select NEx low.
$80000000 constant FMC_FMC_BCR3_FMCEN                               \ FMC controller enable This bit enables or disables the FMC controller. Note: The FMCEN bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
...
$00f00000 constant FMC_FMC_BTR3_CLKDIV                              \ Clock divide ratio (for FMC_CLK signal) Defines the period of FMC_CLK clock output signal, expressed in number of HCLK cycles: In asynchronous NOR Flash, SRAM or PSRAM accesses, this value is don't care. Note: Refer to Section 5.6.5: Synchronous transactions for FMC_CLK divider ratio formula)
$0f000000 constant FMC_FMC_BTR3_DATLAT                              \ (see note below bit descriptions): Data latency for synchronous memory For synchronous access with read/write Burst mode enabled (BURSTEN / CBURSTRW bits set), defines the number of memory clock cycles (+2) to issue to the memory before reading/writing the first data: This timing parameter is not expressed in HCLK periods, but in FMC_CLK periods. For asynchronous access, this value is don't care.
$30000000 constant FMC_FMC_BTR3_ACCMOD                              \ Access mode Specifies the asynchronous access modes as shown in the timing diagrams. These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BTR3_DATAHLD                             \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous accesses: For read accesses For write accesses
...
$00000001 constant FMC_FMC_BCR4_MBKEN                               \ Memory bank enable bit Enables the memory bank. After reset Bank1 is enabled, all others are disabled. Accessing a disabled bank causes an ERROR on AHB bus.
...
$0000000c constant FMC_FMC_BCR4_MTYP                                \ Memory type Defines the type of external memory attached to the corresponding memory bank.
...
$00000040 constant FMC_FMC_BCR4_FACCEN                              \ Flash access enable Enables NOR Flash memory access operations.
...
$00000200 constant FMC_FMC_BCR4_WAITPOL                             \ Wait signal polarity bit Defines the polarity of the wait signal from memory used for either in Synchronous or Asynchronous mode.
...
$00070000 constant FMC_FMC_BCR4_CPSIZE                              \ CRAM page size These are used for CellularRAM 1.5 which does not allow burst access to cross the address boundaries between pages. When these bits are configured, the FMC controller splits automatically the burst access when the memory page size is reached (refer to memory datasheet for page size). Others: reserved
...
$00200000 constant FMC_FMC_BCR4_WFDIS                               \ Write FIFO disable This bit disables the Write FIFO used by the FMC controller. Note: The WFDIS bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
$00c00000 constant FMC_FMC_BCR4_NBLSET                              \ Byte lane (NBL) setup These bits configure the NBL setup timing from NBLx low to chip select NEx low.
$80000000 constant FMC_FMC_BCR4_FMCEN                               \ FMC controller enable This bit enables or disables the FMC controller. Note: The FMCEN bit of the FMC_BCR2..4 registers is don't care. It is only enabled through the FMC_BCR1 register.
...
$30000000 constant FMC_FMC_BTR4_ACCMOD                              \ Access mode Specifies the asynchronous access modes as shown in the timing diagrams. These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BTR4_DATAHLD                             \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous accesses: For read accesses For write accesses
...
$0000ffff constant FMC_FMC_PCSCNTR_CSCOUNT                          \ Chip select counter. These bits are written by software to define the maximum chip select low pulse duration. It is expressed in FMC_CLK cycles for synchronous accesses and in HCLK cycles for asynchronous accesses. The counter is disabled if the programmed value is 0.
$00010000 constant FMC_FMC_PCSCNTR_CNTB1EN                          \ Counter Bank 1 enable This bit enables the chip select counter for PSRAM/NOR Bank 1.
$00020000 constant FMC_FMC_PCSCNTR_CNTB2EN                          \ Counter Bank 2 enable This bit enables the chip select counter for PSRAM/NOR Bank 2.
$00040000 constant FMC_FMC_PCSCNTR_CNTB3EN                          \ Counter Bank 3 enable This bit enables the chip select counter for PSRAM/NOR Bank 3.
$00080000 constant FMC_FMC_PCSCNTR_CNTB4EN                          \ Counter Bank 4 enable This bit enables the chip select counter for PSRAM/NOR Bank 4.
...
$00000002 constant FMC_FMC_PCR_PWAITEN                              \ Wait feature enable bit Enables the Wait feature for the NAND Flash memory bank:
$00000004 constant FMC_FMC_PCR_PBKEN                                \ NAND Flash memory bank enable bit Enables the memory bank. Accessing a disabled memory bank causes an ERROR on AHB bus
$00000008 constant FMC_FMC_PCR_PTYP                                 \ Memory type Defines the type of device attached to the corresponding memory bank:
$00000030 constant FMC_FMC_PCR_PWID                                 \ Data bus width Defines the external memory device width.
$00000040 constant FMC_FMC_PCR_ECCEN                                \ ECC computation logic enable bit
$00001e00 constant FMC_FMC_PCR_TCLR                                 \ CLE to RE delay Sets time from CLE low to RE low in number of AHB clock cycles (HCLK). Time is t_clr = (TCLR + SET + 2) THCLK where THCLK is the HCLK clock period Note: SET is MEMSET or ATTSET according to the addressed space.
$0000e000 constant FMC_FMC_PCR_TAR                                  \ ALE to RE delay Sets time from ALE low to RE low in number of AHB clock cycles (HCLK). Time is: t_ar = (TAR + SET + 2) THCLK where THCLK is the HCLK clock period Note: SET is MEMSET or ATTSET according to the addressed space.
$00010000 constant FMC_FMC_PCR_TAR3                                 \ ALE to RE delay Sets time from ALE low to RE low in number of AHB clock cycles (HCLK). Time is: t_ar = (TAR + SET + 2) THCLK where THCLK is the HCLK clock period Note: SET is MEMSET or ATTSET according to the addressed space.
$000e0000 constant FMC_FMC_PCR_ECCPS                                \ ECC page size Defines the page size for the extended ECC:
...
$00000001 constant FMC_FMC_SR_IRS                                   \ Interrupt rising edge status The flag is set by hardware and reset by software. Note: If this bit is written by software to 1 it is set.
$00000002 constant FMC_FMC_SR_ILS                                   \ Interrupt high-level status The flag is set by hardware and reset by software.
$00000004 constant FMC_FMC_SR_IFS                                   \ Interrupt falling edge status The flag is set by hardware and reset by software. Note: If this bit is written by software to 1 it is set.
...
$000000ff constant FMC_FMC_PMEM_MEMSET                              \ Common memory x setup time Defines the number of HCLK (+1) clock cycles to set up the address before the command assertion (NWE, NOE), for NAND Flash read or write access to common memory space on socket x:
$0000ff00 constant FMC_FMC_PMEM_MEMWAIT                             \ Common memory wait time Defines the minimum number of HCLK (+1) clock cycles to assert the command (NWE, NOE), for NAND Flash read or write access to common memory space on socket. The duration of command assertion is extended if the wait signal (NWAIT) is active (low) at the end of the programmed value of HCLK:
$00ff0000 constant FMC_FMC_PMEM_MEMHOLD                             \ Common memory hold time Defines the number of HCLK clock cycles for write access and HCLK (+2) clock cycles for read access during which the address is held (and data for write accesses) after the command is deasserted (NWE, NOE), for NAND Flash read or write access to common memory space on socket x:
$ff000000 constant FMC_FMC_PMEM_MEMHIZ                              \ Common memory x data bus Hi-Z time Defines the number of HCLK clock cycles during which the data bus is kept Hi-Z after the start of a NAND Flash write access to common memory space on socket. This is only valid for write transactions:
...
$000000ff constant FMC_FMC_PATT_ATTSET                              \ Attribute memory setup time Defines the number of HCLK (+1) clock cycles to set up address before the command assertion (NWE, NOE), for NAND Flash read or write access to attribute memory space on socket:
$0000ff00 constant FMC_FMC_PATT_ATTWAIT                             \ Attribute memory wait time Defines the minimum number of HCLK (+1) clock cycles to assert the command (NWE, NOE), for NAND Flash read or write access to attribute memory space on socket x. The duration for command assertion is extended if the wait signal (NWAIT) is active (low) at the end of the programmed value of HCLK:
$00ff0000 constant FMC_FMC_PATT_ATTHOLD                             \ Attribute memory hold time Defines the number of HCLK clock cycles for write access and HCLK (+2) clock cycles for read access during which the address is held (and data for write access) after the command deassertion (NWE, NOE), for NAND Flash read or write access to attribute memory space on socket:
$ff000000 constant FMC_FMC_PATT_ATTHIZ                              \ Attribute memory data bus Hi-Z time Defines the number of HCLK clock cycles during which the data bus is kept in Hi-Z after the start of a NAND Flash write access to attribute memory space on socket. Only valid for writ transaction:
...
$00000000 constant FMC_FMC_ECCR_ECC                                 \ ECC result This field contains the value computed by the ECC computation logic. Table 99 describes the contents of these bitfields.
...
$0000ff00 constant FMC_FMC_BWTR1_DATAST                             \ Data-phase duration. These bits are written by software to define the duration of the data phase (refer to Figure 21 to Figure 33), used in asynchronous SRAM, PSRAM and NOR Flash memory accesses: ...
...
$30000000 constant FMC_FMC_BWTR1_ACCMOD                             \ Access mode. Specifies the asynchronous access modes as shown in the next timing diagrams.These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BWTR1_DATAHLD                            \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous write accesses:
...
$0000ff00 constant FMC_FMC_BWTR2_DATAST                             \ Data-phase duration. These bits are written by software to define the duration of the data phase (refer to Figure 21 to Figure 33), used in asynchronous SRAM, PSRAM and NOR Flash memory accesses: ...
$000f0000 constant FMC_FMC_BWTR2_BUSTURN                            \ Bus turnaround phase duration These bits are written by software to add a delay at the end of current write transaction to next transaction on the same bank. For FRAM memories, the bus turnaround delay should be configured to match the minimum t sub PC /sub (precharge time) timings. The bus turnaround delay is inserted between any consecutive transactions on the same bank (read/read, write/write, read/write and write/read). The chip select is toggling between any consecutive accesses. (BUSTURN + 1)HCLK period greater than or equal tPC min ...
$30000000 constant FMC_FMC_BWTR2_ACCMOD                             \ Access mode. Specifies the asynchronous access modes as shown in the next timing diagrams.These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BWTR2_DATAHLD                            \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous write accesses:
...
$0000ff00 constant FMC_FMC_BWTR3_DATAST                             \ Data-phase duration. These bits are written by software to define the duration of the data phase (refer to Figure 21 to Figure 33), used in asynchronous SRAM, PSRAM and NOR Flash memory accesses: ...
$000f0000 constant FMC_FMC_BWTR3_BUSTURN                            \ Bus turnaround phase duration These bits are written by software to add a delay at the end of current write transaction to next transaction on the same bank. For FRAM memories, the bus turnaround delay should be configured to match the minimum t sub PC /sub (precharge time) timings. The bus turnaround delay is inserted between any consecutive transactions on the same bank (read/read, write/write, read/write and write/read). The chip select is toggling between any consecutive accesses. (BUSTURN + 1)HCLK period greater than or equal tPC min ...
$30000000 constant FMC_FMC_BWTR3_ACCMOD                             \ Access mode. Specifies the asynchronous access modes as shown in the next timing diagrams.These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BWTR3_DATAHLD                            \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous write accesses:
...
$0000ff00 constant FMC_FMC_BWTR4_DATAST                             \ Data-phase duration. These bits are written by software to define the duration of the data phase (refer to Figure 21 to Figure 33), used in asynchronous SRAM, PSRAM and NOR Flash memory accesses: ...
$000f0000 constant FMC_FMC_BWTR4_BUSTURN                            \ Bus turnaround phase duration These bits are written by software to add a delay at the end of current write transaction to next transaction on the same bank. For FRAM memories, the bus turnaround delay should be configured to match the minimum t sub PC /sub (precharge time) timings. The bus turnaround delay is inserted between any consecutive transactions on the same bank (read/read, write/write, read/write and write/read). The chip select is toggling between any consecutive accesses. (BUSTURN + 1)HCLK period greater than or equal tPC min ...
$30000000 constant FMC_FMC_BWTR4_ACCMOD                             \ Access mode. Specifies the asynchronous access modes as shown in the next timing diagrams.These bits are taken into account only when the EXTMOD bit in the FMC_BCRx register is 1.
$c0000000 constant FMC_FMC_BWTR4_DATAHLD                            \ Data hold phase duration These bits are written by software to define the duration of the data hold phase in HCLK cycles (refer to Figure 21 to Figure 33), used in asynchronous write accesses:
...
$00000c00 constant FMC_FMC_SDCR1_SDCLK                              \ SDRAM clock configuration These bits define the SDRAM clock period for both SDRAM banks and allow disabling the clock before changing the frequency. In this case the SDRAM must be re-initialized. Note: The corresponding bits in the FMC_SDCR2 register are don't care.
$00001000 constant FMC_FMC_SDCR1_RBURST                             \ Burst read This bit enables Burst read mode. The SDRAM controller anticipates the next read commands during the CAS latency and stores data in the Read FIFO. Note: The corresponding bit in the FMC_SDCR2 register is don't care.
$00006000 constant FMC_FMC_SDCR1_RPIPE                              \ Read pipe These bits define the delay, in clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
...
$00000c00 constant FMC_FMC_SDCR2_SDCLK                              \ SDRAM clock configuration These bits define the SDRAM clock period for both SDRAM banks and allow disabling the clock before changing the frequency. In this case the SDRAM must be re-initialized. Note: The corresponding bits in the FMC_SDCR2 register are don't care.
$00001000 constant FMC_FMC_SDCR2_RBURST                             \ Burst read This bit enables Burst read mode. The SDRAM controller anticipates the next read commands during the CAS latency and stores data in the Read FIFO. Note: The corresponding bit in the FMC_SDCR2 register is don't care.
$00006000 constant FMC_FMC_SDCR2_RPIPE                              \ Read pipe These bits define the delay, in clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
...
$000f0000 constant FMC_FMC_SDTR1_TWR                                \ Recovery delay These bits define the delay between a Write and a Precharge command in number of memory clock cycles. .... Note: TWR must be programmed to match the write recovery time (t sub WR /sub ) defined in the SDRAM datasheet, and to guarantee that: Note: TWR greater than or equal TRAS TRCD and TWR greater than or equalTRC TRCD TRP Note: Example: TRAS= 4 cycles, TRCD= 2 cycles. So, TWR = 2 cycles. TWR must be programmed to 0x1. Note: If two SDRAM devices are used, the FMC_SDTR1 and FMC_SDTR2 must be programmed with the same TWR timing corresponding to the slowest SDRAM device. Note: If only one SDRAM device is used, the TWR timing must be kept at reset value (0xF) for the not used bank.
$00f00000 constant FMC_FMC_SDTR1_TRP                                \ Row precharge delay These bits define the delay between a Precharge command and another command in number of memory clock cycles. The TRP timing is only configured in the FMC_SDTR1 register. If two SDRAM devices are used, the TRP must be programmed with the timing of the slowest device. .... Note: The corresponding bits in the FMC_SDTR2 register are don't care.
...
$0000f000 constant FMC_FMC_SDTR2_TRC                                \ Row cycle delay These bits define the delay between the Refresh command and the Activate command, as well as the delay between two consecutive Refresh commands. It is expressed in number of memory clock cycles. The TRC timing is only configured in the FMC_SDTR1 register. If two SDRAM devices are used, the TRC must be programmed with the timings of the slowest device. .... Note: TRC must match the TRC and TRFC (Auto Refresh period) timings defined in the SDRAM device datasheet. Note: The corresponding bits in the FMC_SDTR2 register are don't care.
$000f0000 constant FMC_FMC_SDTR2_TWR                                \ Recovery delay These bits define the delay between a Write and a Precharge command in number of memory clock cycles. .... Note: TWR must be programmed to match the write recovery time (t sub WR /sub ) defined in the SDRAM datasheet, and to guarantee that: Note: TWR greater than or equal TRAS TRCD and TWR greater than or equalTRC TRCD TRP Note: Example: TRAS= 4 cycles, TRCD= 2 cycles. So, TWR = 2 cycles. TWR must be programmed to 0x1. Note: If two SDRAM devices are used, the FMC_SDTR1 and FMC_SDTR2 must be programmed with the same TWR timing corresponding to the slowest SDRAM device. Note: If only one SDRAM device is used, the TWR timing must be kept at reset value (0xF) for the not used bank.
...
$003ffe00 constant FMC_FMC_SDCMR_MRD                                \ Mode Register definition This 13-bit field defines the SDRAM Mode Register content. The Mode Register is programmed using the Load Mode Register command.
...
$00000006 constant FMC_FMC_SDSR_MODES1                              \ Status Mode for Bank 1 This bit defines the Status Mode of SDRAM Bank 1.
$00000018 constant FMC_FMC_SDSR_MODES2                              \ Status Mode for Bank 2 This bit defines the Status Mode of SDRAM Bank 2.
$00000020 constant FMC_FMC_SDSR_BUSY                                \ Busy status This bit defines the status of the SDRAM controller after a Command Mode request 1; SDRAM Controller is not ready to accept a new request


\
\ @brief Flexible memory controller
\
$47000400 constant FMC_FMC_BCR1   \ offset: 0x00 : SRAM/NOR-Flash chip-select control register for bank 1
$47000404 constant FMC_FMC_BTR1   \ offset: 0x04 : SRAM/NOR-Flash chip-select timing register for bank 1
$47000408 constant FMC_FMC_BCR2   \ offset: 0x08 : SRAM/NOR-Flash chip-select control register for bank 2
$4700040c constant FMC_FMC_BTR2   \ offset: 0x0C : SRAM/NOR-Flash chip-select timing register for bank 2
$47000410 constant FMC_FMC_BCR3   \ offset: 0x10 : SRAM/NOR-Flash chip-select control register for bank 3
$47000414 constant FMC_FMC_BTR3   \ offset: 0x14 : SRAM/NOR-Flash chip-select timing register for bank 3
$47000418 constant FMC_FMC_BCR4   \ offset: 0x18 : SRAM/NOR-Flash chip-select control register for bank 4
$4700041c constant FMC_FMC_BTR4   \ offset: 0x1C : SRAM/NOR-Flash chip-select timing register for bank 4
$47000420 constant FMC_FMC_PCSCNTR  \ offset: 0x20 : PSRAM chip select counter register
$47000480 constant FMC_FMC_PCR    \ offset: 0x80 : NAND Flash control registers
$47000484 constant FMC_FMC_SR     \ offset: 0x84 : FIFO status and interrupt register
$47000488 constant FMC_FMC_PMEM   \ offset: 0x88 : Common memory space timing register
$4700048c constant FMC_FMC_PATT   \ offset: 0x8C : Attribute memory space timing register
$47000494 constant FMC_FMC_ECCR   \ offset: 0x94 : ECC result registers
$47000504 constant FMC_FMC_BWTR1  \ offset: 0x104 : SRAM/NOR-Flash write timing registers 1
$4700050c constant FMC_FMC_BWTR2  \ offset: 0x10C : SRAM/NOR-Flash write timing registers 2
$47000514 constant FMC_FMC_BWTR3  \ offset: 0x114 : SRAM/NOR-Flash write timing registers 3
$4700051c constant FMC_FMC_BWTR4  \ offset: 0x11C : SRAM/NOR-Flash write timing registers 4
$47000540 constant FMC_FMC_SDCR1  \ offset: 0x140 : SDRAM control registers 1
$47000544 constant FMC_FMC_SDCR2  \ offset: 0x144 : SDRAM control registers 2
$47000548 constant FMC_FMC_SDTR1  \ offset: 0x148 : SDRAM timing registers 1
$4700054c constant FMC_FMC_SDTR2  \ offset: 0x14C : SDRAM timing registers 2
$47000550 constant FMC_FMC_SDCMR  \ offset: 0x150 : SDRAM Command Mode register
$47000554 constant FMC_FMC_SDRTR  \ offset: 0x154 : SDRAM refresh timer register
$47000558 constant FMC_FMC_SDSR   \ offset: 0x158 : SDRAM status register
