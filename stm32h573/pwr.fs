...
$0000000c constant PWR_PWR_PMCR_SVOS                                \ system Stop mode voltage scaling selection These bits control the V sub CORE /sub voltage level in system Stop mode, to obtain the best trade-off between power consumption and performance.
...
$00001000 constant PWR_PWR_PMCR_BOOSTE                              \ analog switch V sub BOOST /sub control This bit enables the booster to guarantee the analog switch AC performance when the V sub DD /sub supply voltage is below 2.7 V (reduction of the total harmonic distortion to have the same switch performance over the full supply voltage range) The V sub DD /sub supply voltage can be monitored through the PVD and the PLS bits.
...
$00000008 constant PWR_PWR_VOSSR_VOSRDY                             \ Ready bit for V sub CORE /sub voltage scaling output selection.
$00002000 constant PWR_PWR_VOSSR_ACTVOSRDY                          \ Voltage level ready for currently used VOS
$0000c000 constant PWR_PWR_VOSSR_ACTVOS                             \ voltage output scaling currently applied to V sub CORE /sub This field provides the last VOS value.
...
$00100000 constant PWR_PWR_BDSR_VBATL                               \ V sub BAT /sub level monitoring versus low threshold
$00200000 constant PWR_PWR_BDSR_VBATH                               \ V sub BAT /sub level monitoring versus high threshold
...
$00000100 constant PWR_PWR_VMCR_AVDEN                               \ peripheral voltage monitor on V sub DDA /sub enable
...
$01000000 constant PWR_PWR_USBSCR_USB33DEN                          \ V sub DDUSB /sub voltage level detector enable
$02000000 constant PWR_PWR_USBSCR_USB33SV                           \ independent USB supply valid This bit is used to validate the V sub DDUSB /sub supply for electrical and logical isolation purpose. Setting this bit is mandatory to use the USBFS peripheral. If V sub DDUSB /sub is not always present in the application, the V sub DDUSB /sub voltage monitor can be used to determine whether this supply is ready or not.
...
$00080000 constant PWR_PWR_VMSR_AVDO                                \ analog voltage detector output on V sub DDA /sub This bit is set and cleared by hardware. It is valid only if AVD on VDDA is enabled by the AVDEN bit. Note: Since the AVD is disabled in Standby mode, this bit is equal to 0 after standby or reset until the AVDEN bit is set.
$00100000 constant PWR_PWR_VMSR_VDDIO2RDY                           \ voltage detector output on V sub DDIO2 /sub This bit is set and cleared by hardware.
...
$01000000 constant PWR_PWR_VMSR_USB33RDY                            \ V sub DDUSB /sub ready
