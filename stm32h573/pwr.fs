...
[ifndef] PWR_DEF
  ...
  [ifdef] PWR_PWR_PMCR_DEF
    ...
    $02 constant PWR_SVOS                       \ [0x02 : 2] system Stop mode voltage scaling selection These bits control the V sub CORE /sub voltage level in system Stop mode, to obtain the best trade-off between power consumption and performance.
    ...
    $0c constant PWR_BOOSTE                     \ [0x0c] analog switch V sub BOOST /sub control This bit enables the booster to guarantee the analog switch AC performance when the V sub DD /sub supply voltage is below 2.7 V (reduction of the total harmonic distortion to have the same switch performance over the full supply voltage range) The V sub DD /sub supply voltage can be monitored through the PVD and the PLS bits.
  ...
  [ifdef] PWR_PWR_VOSSR_DEF
    ...
    \
    $03 constant PWR_VOSRDY                     \ [0x03] Ready bit for V sub CORE /sub voltage scaling output selection.
    $0d constant PWR_ACTVOSRDY                  \ [0x0d] Voltage level ready for currently used VOS
    $0e constant PWR_ACTVOS                     \ [0x0e : 2] voltage output scaling currently applied to V sub CORE /sub This field provides the last VOS value.
  [then]
  ...
  [ifdef] PWR_PWR_VMCR_DEF
    ...
    $08 constant PWR_AVDEN                      \ [0x08] peripheral voltage monitor on V sub DDA /sub enable
  ...
  [ifdef] PWR_PWR_USBSCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $18 constant PWR_USB33DEN                   \ [0x18] V sub DDUSB /sub voltage level detector enable
    $19 constant PWR_USB33SV                    \ [0x19] independent USB supply valid This bit is used to validate the V sub DDUSB /sub supply for electrical and logical isolation purpose. Setting this bit is mandatory to use the USBFS peripheral. If V sub DDUSB /sub is not always present in the application, the V sub DDUSB /sub voltage monitor can be used to determine whether this supply is ready or not.
  [then]
  ...
  [ifdef] PWR_PWR_VMSR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $13 constant PWR_AVDO                       \ [0x13] analog voltage detector output on V sub DDA /sub This bit is set and cleared by hardware. It is valid only if AVD on VDDA is enabled by the AVDEN bit. Note: Since the AVD is disabled in Standby mode, this bit is equal to 0 after standby or reset until the AVDEN bit is set.
    $14 constant PWR_VDDIO2RDY                  \ [0x14] voltage detector output on V sub DDIO2 /sub This bit is set and cleared by hardware.
    ...
    $18 constant PWR_USB33RDY                   \ [0x18] V sub DDUSB /sub ready
  [then]
