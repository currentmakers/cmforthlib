...
$00000002 constant DCACHE_DCACHE_CR_CACHEINV                        \ full cache invalidation Can be set by software, only when EN = 1. Cleared by hardware when the BUSYF flag is set (during full cache invalidation operation). Writing 0 has no effect.
$00000700 constant DCACHE_DCACHE_CR_CACHECMD                        \ cache command maintenance operation (cleans and/or invalidates an address range) Can be set and cleared by software, only when no maintenance command is ongoing (BUSYCMDF = 0). others: reserved
$00000800 constant DCACHE_DCACHE_CR_STARTCMD                        \ starts maintenance command (maintenance operation defined in CACHECMD). Can be set by software, only when EN = 1, BUSYCMDF = 0, BUSYF = 0 and CACHECMD = 0b001, 0b010 or 0b011. Cleared by hardware when the BUSYCMDF flag is set (during cache maintenance operation). Writing 0 has no effect.
...
$80000000 constant DCACHE_DCACHE_CR_HBURST                          \ output burst type for cache master port read accesses Write access is always done in INCR burst type.
...
$00000002 constant DCACHE_DCACHE_SR_BSYENDF                         \ full invalidate busy end flag Cleared by writing DCACHE_FCR.CBSYENDF = 1.
$00000004 constant DCACHE_DCACHE_SR_ERRF                            \ cache error flag Cleared by writing DCACHE_FCR.CERRF = 1.
$00000008 constant DCACHE_DCACHE_SR_BUSYCMDF                        \ command busy flag
$00000010 constant DCACHE_DCACHE_SR_CMDENDF                         \ command end flag Cleared by writing DCACHE_FCR.CCMDENDF = 1.
...
$00000002 constant DCACHE_DCACHE_IER_BSYENDIE                       \ interrupt enable on busy end Set by SW to enable an interrupt generation at the end of a cache full invalidate operation.
$00000004 constant DCACHE_DCACHE_IER_ERRIE                          \ interrupt enable on cache error Set by software to enable an interrupt generation in case of cache functional error (eviction or clean operation write-back error)
$00000010 constant DCACHE_DCACHE_IER_CMDENDIE                       \ interrupt enable on command end Set by software to enable an interrupt generation at the end of a cache command (clean and/or invalidate an address range)
...
$00000002 constant DCACHE_DCACHE_FCR_CBSYENDF                       \ clear full invalidate busy end flag Set by software.
$00000004 constant DCACHE_DCACHE_FCR_CERRF                          \ clear cache error flag Set by software.
$00000010 constant DCACHE_DCACHE_FCR_CCMDENDF                       \ clear command end flag Set by software.
...
$fffffff0 constant DCACHE_DCACHE_CMDRSADDRR_CMDSTARTADDR            \ start address of range to which the cache maintenance command specified in DCACHE_CR.CACHECMD field applies This register must be set before DCACHE_CR.CACHECMD is written. .
...
$fffffff0 constant DCACHE_DCACHE_CMDREADDRR_CMDENDADDR              \ end address of range to which the cache maintenance command specified in DCACHE_CR.CACHECMD field applies This register must be set before DCACHE_CR.CACHECMD is written.
