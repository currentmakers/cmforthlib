...
$0000001f constant SPI4_SPI_CFG1_DSIZE                              \ number of bits in at single SPI data frame ..... Note: Maximum data size can be limited up to 16-bits at some instances. At instances with limited set of features, DSIZE2:0] bits are reserved and must be kept at reset state. DSIZE[4:3] bits then control next settings of data size: 00xxx: 8-bits 01xxx: 16-bits 10xxx: 24-bits 11xxx: 32-bits.
$000001e0 constant SPI4_SPI_CFG1_FTHLV                              \ FIFO threshold level Defines number of data frames at single data packet. Size of the packet should not exceed 1/2 of FIFO space. SPI interface is more efficient if configured packet sizes are aligned with data register access parallelism: If SPI data register is accessed as a 16-bit register and DSIZE less than or equal 8 bit, better to select FTHLV = 2, 4, 6. If SPI data register is accessed as a 32-bit register and DSIZE 8 bit, better to select FTHLV = 2, 4, 6, while if DSIZE less than or equal 8bit, better to select FTHLV = 4, 8, 12. Note: FTHLV[3:2] bits are reserved at instances with limited set of features
$00000200 constant SPI4_SPI_CFG1_UDRCFG                             \ behavior of slave transmitter at underrun condition For more details see underrun condition.
...
$70000000 constant SPI4_SPI_CFG1_MBR                                \ master baud rate prescaler setting Note: MBR setting is considered at slave working at TI mode, too (see mode).
...
$00002000 constant SPI4_SPI_CFG2_RDIOM                              \ RDY signal input/output management Note: When DSIZE at the SPI_CFG1 register is configured shorter than 8-bit, the RDIOM bit has to be kept at zero.
...
$00000200 constant SPI4_SPI_SR_MODF                                 \ mode fault This bit is cleared by writing 1 to MODFC bit of SPI_IFCR exclusively.
...
$ffff0000 constant SPI4_SPI_SR_CTSIZE                               \ number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus or during autonomous operation in low-power mode. Note: CTSIZE[15:0] bits are not available in instances with limited set of features.
...
$00000000 constant SPI4_SPI_RXDR_RXDR                               \ receive data register The register serves as an interface with RxFIFO. When it is read, RxFIFO is accessed. Note: In SPI mode, data is always right-aligned. Alignment of data at I2S mode depends on DATLEN and DATFMT setting. Unused bits are read as zero when the register is read. Writing to the register is ignored. Note: DR can be accessed byte-wise (8-bit access): in this case only one data-byte is read by single access halfword-wise (16 bit access) in this case 2 data-bytes or 1 halfword-data can be read by single access word-wise (32 bit access). In this case 4 data-bytes or 2 halfword-data or word-data can be read by single access. Read access of this register less than the configured data size is forbidden.
...
\ @brief SPI4 global interrupt
\
$40014c00 constant SPI4_SPI_CR1   \ offset: 0x00 : SPI/I2S control register 1
$40014c04 constant SPI4_SPI_CR2   \ offset: 0x04 : SPI/I2S control register 2
$40014c08 constant SPI4_SPI_CFG1  \ offset: 0x08 : SPI/I2S configuration register 1
$40014c0c constant SPI4_SPI_CFG2  \ offset: 0x0C : SPI/I2S configuration register 2
$40014c10 constant SPI4_SPI_IER   \ offset: 0x10 : SPI/I2S interrupt enable register
$40014c14 constant SPI4_SPI_SR    \ offset: 0x14 : SPI/I2S status register
$40014c18 constant SPI4_SPI_IFCR  \ offset: 0x18 : SPI/I2S interrupt/status flags clear register
$40014c20 constant SPI4_SPI_TXDR  \ offset: 0x20 : SPI/I2S transmit data register
$40014c30 constant SPI4_SPI_RXDR  \ offset: 0x30 : SPI/I2S receive data register
$40014c40 constant SPI4_SPI_CRCPOLY  \ offset: 0x40 : SPI/I2S polynomial register
$40014c44 constant SPI4_SPI_TXCRC  \ offset: 0x44 : SPI/I2S transmitter CRC register
$40014c48 constant SPI4_SPI_RXCRC  \ offset: 0x48 : SPI/I2S receiver CRC register
$40014c4c constant SPI4_SPI_UDRDR  \ offset: 0x4C : SPI/I2S underrun data register
$40014c50 constant SPI4_SPI_I2SCFGR  \ offset: 0x50 : SPI/I2S configuration register
