...
[ifndef] SAES_DEF
  ...
  [ifdef] SAES_SAES_CR_DEF
    ...
    $05 constant SAES_CHMOD1                    \ [0x05 : 2] Chaining mode selection This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when the BUSY flag of SAES_SR register is set, as well as when the EN bit of the SAES_CR register is set before the write access and it is not cleared by that write access.
    ...
    $10 constant SAES_CHMOD2                    \ [0x10] Chaining mode selection This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when the BUSY flag of SAES_SR register is set, as well as when the EN bit of the SAES_CR register is set before the write access and it is not cleared by that write access.
    ...
    $13 constant SAES_KEYPROT                   \ [0x13] Key protection When set hardware-based key protection is enabled. Attempts to write the bit are ignored when the BUSY flag of SAES_SR register is set, as well as when the EN bit of the SAES_CR register is set before the write access and it is not cleared by that write access.
  ...
  [ifdef] SAES_SAES_SR_DEF
    ...
    $03 constant SAES_BUSY                      \ [0x03] Busy This flag indicates whether SAES is idle or busy during GCM payload encryption phase: The flag is set upon SAES initialization, upon fetching random number from the RNG, or upon transferring a shared key to a target peripheral. When GCM encryption is selected, the flag must be at zero before selecting the GCM final phase.
  ...
  [ifdef] SAES_SAES_IVR0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant SAES_IVI                       \ [0x00 : 32] Initialization vector input, bits [31:0] Refer to for description of the IVI[127:0] bitfield. The initialization vector is only used in chaining modes other than ECB. The SAES_IVRx registers may be written only when the SAES peripheral is disabled
  [then]
  ...
  [ifdef] SAES_SAES_ICR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant SAES_CCF                       \ [0x00] Computation complete flag clear Application must set this bit to clear the CCF status bit in SAES_SR and SAES_ISR register.
    $01 constant SAES_RWEIF                     \ [0x01] Read or write error interrupt flag clear Application must set this bit to clear the RWEIF status bit in SAES_ISR register, and both RDERR and WRERR flags in SAES_SR register.
    $02 constant SAES_KEIF                      \ [0x02] Key error interrupt flag clear Application must set this bit to clear the KEIF status bit in SAES_ISR register.
  ...
  [ifdef] SAES_SAES_HWCFGR_DEF
    \
    \ @brief SAES hardware configuration register
    \ Address offset: 0x3F0
    \ Reset value: 0x00000112
    \
    $00 constant SAES_CFG1                      \ [0x00 : 4] HW Generic 1
    $04 constant SAES_CFG2                      \ [0x04 : 4] HW Generic 2
    $08 constant SAES_CFG3                      \ [0x08 : 4] HW Generic 3
  [then]


  [ifdef] SAES_SAES_VERR_DEF
    \
    \ @brief SAES version register
    \ Address offset: 0x3F4
    \ Reset value: 0x00000030
    \
    $00 constant SAES_MINREV                    \ [0x00 : 4] Minor revision This bitfield returns the SAES peripheral minor version (0x0).
    $04 constant SAES_MAJREV                    \ [0x04 : 4] Major revision This bitfield returns the SAES peripheral major version (0x3).
  [then]


  [ifdef] SAES_SAES_IPIDR_DEF
    \
    \ @brief SAES identification register
    \ Address offset: 0x3F8
    \ Reset value: 0x00170023
    \
    $00 constant SAES_ID                        \ [0x00 : 32] Identification code of the peripheral This bitfield returns the identification code of the SAES peripheral.
  [then]


  [ifdef] SAES_SAES_SIDR_DEF
    \
    \ @brief SAES size ID register
    \ Address offset: 0x3FC
    \ Reset value: 0xA3C5DD01
    \
    $00 constant SAES_SID                       \ [0x00 : 32] Size identification code This bitfield returns the size identification code of the SAES peripheral as defined below: Bits[31:8] = 0xA3C5DD (fixed code) Bits[7:0] = 0x01 (1 Kbyte address decoding)
  [then]
  ...
  $3F0 constant SAES_SAES_HWCFGR        \ SAES hardware configuration register
  $3F4 constant SAES_SAES_VERR          \ SAES version register
  $3F8 constant SAES_SAES_IPIDR         \ SAES identification register
  $3FC constant SAES_SAES_SIDR          \ SAES size ID register
