...
[ifndef] PKA_DEF
  ...
  [ifdef] PKA_PKA_CR_DEF
    ...
    \
    $00 constant PKA_EN                         \ [0x00] PKA enable. When an illegal operation is selected while EN=1 OPERRF bit is set in PKA_SR. See PKA_CR.MODE bitfield for details. When EN=0 PKA RAM can still be accessed by the application.
  ...
  [ifdef] PKA_PKA_HWCFGR_DEF
    \
    \ @brief PKA hardware configuration register
    \ Address offset: 0x1FF0
    \ Reset value: 0x00000011
    \
    $00 constant PKA_CFG1                       \ [0x00 : 4] HW Generic 2 This field returns the PKA64_SEL generic value (0x1)
    $04 constant PKA_CFG2                       \ [0x04 : 4] HW Generic 2 This field returns the DPAEN generic value (0x1)
  [then]


  [ifdef] PKA_PKA_VERR_DEF
    \
    \ @brief PKA version register
    \ Address offset: 0x1FF4
    \ Reset value: 0x00000010
    \
    $00 constant PKA_MINREV                     \ [0x00 : 4] Minor revision This field returns the PKA minor version.
    $04 constant PKA_MAJREV                     \ [0x04 : 4] Major revision This field returns the PKA major version.
  [then]


  [ifdef] PKA_PKA_IPIDR_DEF
    \
    \ @brief PKA identification register
    \ Address offset: 0x1FF8
    \ Reset value: 0x00170072
    \
    $00 constant PKA_ID                         \ [0x00 : 32] Identification Code This field returns the PKA identification code.
  [then]


  [ifdef] PKA_PKA_SIDR_DEF
    \
    \ @brief PKA size ID register
    \ Address offset: 0x1FFC
    \ Reset value: 0xA3C5DD08
    \
    $00 constant PKA_SID                        \ [0x00 : 32] Size Identification Code This field returns the size identification code of the PKA as defined below: Bits[31:8] = 0xA3C5DD (fixed code) Bits[7:0] = 0x08 (8 KB address decoding)
  [then]
  ...
  $1FF0 constant PKA_PKA_HWCFGR         \ PKA hardware configuration register
  $1FF4 constant PKA_PKA_VERR           \ PKA version register
  $1FF8 constant PKA_PKA_IPIDR          \ PKA identification register
  $1FFC constant PKA_PKA_SIDR           \ PKA size ID register
