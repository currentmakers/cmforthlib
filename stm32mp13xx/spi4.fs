...
$00001000 constant SPI4_SPI_CR1_SSI                                 \ internal SS signal input level This bit has an effect only when the SSM bit is set. The value of this bit is forced onto the peripheral SS input and the I/O value of the SS pin is ignored.
...
$0000001f constant SPI4_SPI_CFG1_DSIZE                              \ number of bits in at single SPI data frame ..... Note: The most significant bit at DSIZE bit field is reserved at the peripheral instances where data size is limited to 16-bit.
$000001e0 constant SPI4_SPI_CFG1_FTHLV                              \ FIFO threshold level Defines number of data frames at single data packet. The size of the packet must not exceed 1/2 of FIFO space. SPI interface is more efficient if configured packet sizes are aligned with data register access parallelism: If SPI data register is accessed as a 16-bit register and DSIZE<=8bit, better to select FTHLV=2, 4, 6 etc, If SPI data register is accessed as a 32-bit register and DSIZE>8bit, better to select FTHLV=2, 4, 6 etc, while if DSIZE<=8bit, better to select FTHLV=4, 8, 12 etc
$00000600 constant SPI4_SPI_CFG1_UDRCFG                             \ behavior of slave transmitter at underrun condition When slave is configured at transmit only mode (COMM[1:0]=01), slave repeats all zeros pattern at UDRCFG[1:0]=01 setting. For more details see underrun condition.
$00001800 constant SPI4_SPI_CFG1_UDRDET                             \ detection of underrun condition at slave transmitter For more details see underrun condition.
...
$70000000 constant SPI4_SPI_CFG1_MBR                                \ master baud rate Note: MBR setting is considered at slave working at TI mode, too (see mode).
...
$0000000f constant SPI4_SPI_CFG2_MSSI                               \ master SS idleness Specifies an extra delay, expressed in number of SPI clock cycle periods, inserted additionally between active edge of SS and first data of a session start in master mode when SSOE is enabled. ... Note: This feature is not supported in TI mode.
...
$00008000 constant SPI4_SPI_CFG2_IOSWP                              \ swap functionality of MISO and MOSI pins When this bit is set, the function of MISO and MOSI pins alternate functions are inverted. Original MISO pin becomes MOSI and original MOSI pin becomes MISO. Note: This bit can be also used in PCM and I2S modes.
...
$04000000 constant SPI4_SPI_CFG2_SSM                                \ software management of SS signal input When master uses hardware SS output (SSM=0 and SSOE=1), the SS signal input is forced to non active state internally to prevent master mode fault error.
$10000000 constant SPI4_SPI_CFG2_SSIOP                              \ SS input/output polarity
$20000000 constant SPI4_SPI_CFG2_SSOE                               \ SS output enable This bit is taken into account at master mode only
$40000000 constant SPI4_SPI_CFG2_SSOM                               \ SS output management in master mode This bit is taken into account in master mode when SSOE is enabled. It allows to configure SS output between two consecutive data transfers.
$80000000 constant SPI4_SPI_CFG2_AFCNTR                             \ alternate function GPIOs control This bit is taken into account when SPE=0 only When SPI master has to be disabled temporary for a specific configuration reason (e.g. CRC reset, CPHA or HDDIR change) setting this bit prevents any glitches on the associated outputs configured at alternate function mode by keeping them forced at state corresponding the current SPI configuration. This bit must be never used at slave mode as any slave transmitter must not force its MISO output once the SPI is disabled. Note: This bit can be also used in PCM and I2S modes.
...
$00000400 constant SPI4_SPI_IER_TSERFIE                             \ additional number of transactions reload interrupt enable
...
$00000001 constant SPI4_SPI_SR_RXP                                  \ Rx-packet available RXP flag is changed by hardware. It monitors number of overall data currently available at RxFIFO if SPI is enabled. It has to be checked once a data packet is completely read out from RxFIFO.
$00000002 constant SPI4_SPI_SR_TXP                                  \ Tx-packet space available TXP flag is changed by hardware. It monitors overall space currently available at TxFIFO if SPI is enabled. It has to be checked once a complete data packet is stored at TxFIFO.
$00000004 constant SPI4_SPI_SR_DXP                                  \ duplex packet The DXP flag is set whenever both TXP and RXP flags are set regardless the SPI mode.
...
$00000020 constant SPI4_SPI_SR_UDR                                  \ underrun at slave transmission mode This bit is cleared by write 1 to UDRC bit at SPI_IFCR Note: UDR flag applies to Slave mode only
$00000040 constant SPI4_SPI_SR_OVR                                  \ overrun This bit is cleared by write 1 to OVRC bit at SPI_IFCR
$00000080 constant SPI4_SPI_SR_CRCE                                 \ CRC error This bit is cleared by write 1 to CRCEC bit at SPI_IFCR
$00000100 constant SPI4_SPI_SR_TIFRE                                \ TI frame format error This bit is cleared by write 1 to TIFREC bit at SPI_IFCR
$00000200 constant SPI4_SPI_SR_MODF                                 \ mode fault This bit is cleared by write 1 to MODFC bit at SPI_IFCR. When MODF is set, the SPE and IOLOCK bits at the SPI_CR1 register are reset and their setting is blocked.
$00000400 constant SPI4_SPI_SR_TSERF                                \ additional number of SPI data to be transacted was reload This bit is cleared by write 1 to TSERFC bit at SPI_IFCR or by writing the TSER[15:0] (SPI_CR2) register
...
$ffff0000 constant SPI4_SPI_SR_CTSIZE                               \ number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus and at LP mode too.
...
$00000400 constant SPI4_SPI_IFCR_TSERFC                             \ TSERFC flag clear Writing a 1 into this bit clears TSERF flag in the SPI_SR register Note: TSERF is also reset by writing the TSER[15:0] (SPI_CR2) register
...
$00000000 constant SPI4_SPI_RXDR_RXDR                               \ receive data register The register serves as an interface with RxFIFO. When it is read, RxFIFO is accessed. Note: Data is always right-aligned. Unused bits are read as zero when the register is read. Writing to the register is ignored. Note: DR can be accessed byte-wise (8-bit access): in this case only one data-byte is read by single access. halfword-wise (16 bit access) in this case 2 data-bytes or 1 halfword-data can be read by single access word-wise (32 bit access). In this case 4 data-bytes or 2 halfword-data or word-data can be read by single access. Read access of this register less than the configured data size is forbidden.
...
$00000000 constant SPI4_SPI_UDRDR_UDRDR                             \ data at slave underrun condition The register is taken into account at slave mode and at underrun condition only. The number of bits considered depends on DSIZE bit settings at SPI_CFG1 register. Underrun condition handling depends on setting if UDRDET and UDRCFG bits at SPI_CFG1 register. Bits 31-16 are reserved at the peripheral instances with data size limited to 16-bit. There is no constrain when 32-bit access is applied at these addresses. Reserved bits 31-16 are always read zero while any write to them is ignored.
...
$00000030 constant SPI4_SPI_I2SCFGR_I2SSTD                          \ I2S standard selection For more details on I2S standards, refer to
$00000080 constant SPI4_SPI_I2SCFGR_PCMSYNC                         \ PCM frame synchronization
$00000300 constant SPI4_SPI_I2SCFGR_DATLEN                          \ data length to be transferred
$00000400 constant SPI4_SPI_I2SCFGR_CHLEN                           \ channel length (number of bits per audio channel) The bit write operation has a meaning only if DATLEN = 00 otherwise the channel length is fixed to 32-bit by hardware whatever the value filled in.
...
$00002000 constant SPI4_SPI_I2SCFGR_WSINV                           \ Word select inversion This bit is used to invert the default polarity of WS signal. WS is low. In PCM mode the start of frame is indicated by a rising edge. WS is high. In PCM mode the start of frame is indicated by a falling edge.
$00004000 constant SPI4_SPI_I2SCFGR_DATFMT                          \ data format
$00ff0000 constant SPI4_SPI_I2SCFGR_I2SDIV                          \ I2S linear prescaler I2SDIV can take any values except the value 1, when ODD is also equal to 1. Refer to for details
$01000000 constant SPI4_SPI_I2SCFGR_ODD                             \ odd factor for the prescaler Refer to for details
...
$0000000f constant SPI4_SPI_I2S_HWCFGR_TXFCFG                       \ TxFIFO size 0002: the FIFO size is 4 bytes 0003: the FIFO size is 8 bytes 0004: the FIFO size is 16 bytes 0005: the FIFO size is 32 bytes All the other combinations are reserved.
$000000f0 constant SPI4_SPI_I2S_HWCFGR_RXFCFG                       \ RxFIFO size 0002: the FIFO size is 4 bytes 0003: the FIFO size is 8 bytes 0004: the FIFO size is 16 bytes 0005: the FIFO size is 32 bytes All the other combinations are reserved.
$00000f00 constant SPI4_SPI_I2S_HWCFGR_CRCCFG                       \ CRC configuration for SPI All the other combinations are reserved.
$0000f000 constant SPI4_SPI_I2S_HWCFGR_I2SCFG                       \ I2S configuration All the other combinations are reserved.
$000f0000 constant SPI4_SPI_I2S_HWCFGR_DSCFG                        \ SPI data size configuration All the other combinations are reserved.
...
$0000000f constant SPI4_SPI_VERR_MINREV                             \ minor revision of the IP.
$000000f0 constant SPI4_SPI_VERR_MAJREV                             \ major revision of the IP.
...
$00000000 constant SPI4_SPI_IPIDR_ID                                \ IP identification MCD identification code which identifies the IP.
...
$00000000 constant SPI4_SPI_SIDR_SID                                \ size identification Bits[31:8]: fixed code 0xA3C5DD0 Bits[7:0]: range of address decoding boundary All the other combination are reserved.


\
\ @brief SPI4 global interrupt
\
$4c002000 constant SPI4_SPI_CR1   \ offset: 0x00 : SPI/I2S control register 1
$4c002004 constant SPI4_SPI_CR2   \ offset: 0x04 : SPI control register 2
$4c002008 constant SPI4_SPI_CFG1  \ offset: 0x08 : SPI configuration register 1
$4c00200c constant SPI4_SPI_CFG2  \ offset: 0x0C : SPI configuration register 2
$4c002010 constant SPI4_SPI_IER   \ offset: 0x10 : SPI/I2S interrupt enable register
$4c002014 constant SPI4_SPI_SR    \ offset: 0x14 : SPI/I2S status register
$4c002018 constant SPI4_SPI_IFCR  \ offset: 0x18 : SPI/I2S interrupt/status flags clear register
$4c002020 constant SPI4_SPI_TXDR  \ offset: 0x20 : SPI/I2S transmit data register
$4c002030 constant SPI4_SPI_RXDR  \ offset: 0x30 : SPI/I2S receive data register
$4c002040 constant SPI4_SPI_CRCPOLY  \ offset: 0x40 : SPI polynomial register
$4c002044 constant SPI4_SPI_TXCRC  \ offset: 0x44 : SPI transmitter CRC register
$4c002048 constant SPI4_SPI_RXCRC  \ offset: 0x48 : SPI receiver CRC register
$4c00204c constant SPI4_SPI_UDRDR  \ offset: 0x4C : SPI underrun data register
$4c002050 constant SPI4_SPI_I2SCFGR  \ offset: 0x50 : SPI/I2S configuration register
$4c0023f0 constant SPI4_SPI_I2S_HWCFGR  \ offset: 0x3F0 : SPI/I2S hardware configuration register
$4c0023f4 constant SPI4_SPI_VERR  \ offset: 0x3F4 : SPI/I2S version register
$4c0023f8 constant SPI4_SPI_IPIDR  \ offset: 0x3F8 : SPI/I2S identification register
$4c0023fc constant SPI4_SPI_SIDR  \ offset: 0x3FC : SPI/I2S size identification register
