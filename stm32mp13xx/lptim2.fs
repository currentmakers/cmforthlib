...
$00000001 constant LPTIM2_LPTIM_ISR_CMPM                            \ Compare match The CMPM bit is set by hardware to inform application that LPTIM_CNT register value reached the LPTIM_CMP registerâs value.
$00000002 constant LPTIM2_LPTIM_ISR_ARRM                            \ Autoreload match ARRM is set by hardware to inform application that LPTIM_CNT registerâs value reached the LPTIM_ARR registerâs value. ARRM flag can be cleared by writing 1 to the ARRMCF bit in the LPTIM_ICR register.
...
$00000001 constant LPTIM2_LPTIM_ICR_CMPMCF                          \ Compare match clear flag Writing 1 to this bit clears the CMPM flag in the LPTIM_ISR register
...
$0000ffff constant LPTIM2_LPTIM_CMP_CMP                             \ Compare value CMP is the compare value used by the LPTIM.
...
$0000ffff constant LPTIM2_LPTIM_ARR_ARR                             \ Auto reload value ARR is the autoreload value for the LPTIM. This value must be strictly greater than the CMP[15:0] value.
...
$00000003 constant LPTIM2_LPTIM_CFGR2_IN1SEL                        \ LPTIM input 1 selection The IN1SEL bits control the LPTIM Input 1 multiplexer, which connects LPTIM Input 1 to one of the available inputs. For connection details refer to . Note: If LPTIM does not implement external input clock, these bits are reserved. Please refer to .
...
$0000000f constant LPTIM2_LPTIM_VERR_MINREV                         \ Minor revision This is a read only bit-field and is read as MINREV[3:0] = 4.
$000000f0 constant LPTIM2_LPTIM_VERR_MAJREV                         \ Major revision This is read only bit-field and is read as MAJREV[7:4] = 1.
...
$00000000 constant LPTIM2_LPTIM_PIDR_P_ID                           \ Peripheral type identifier This a read only register and is always read as P_ID[31:0] = 0x00120011.
...
$00000000 constant LPTIM2_LPTIM_SIDR_S_ID                           \ Registers map size identifier This a read only register and is always read as S_ID[31:0] = 0xA3C5DD01. This register identifies the size of the memory region allocated to the LPTIM registers. The size of this memory region is 1Â KB.
