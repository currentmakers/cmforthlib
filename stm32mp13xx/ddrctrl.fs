...
[ifndef] DDRCTRL_DEF
  ...
  [ifdef] DDRCTRL_DDRCTRL_MRSTAT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_MR_WR_BUSY             \ [0x00] The SoC core may initiate a MR write operation only if this signal is low. This signal goes high in the clock after the DDRCTRL accepts the MRW/MRR request. It goes low when the MRW/MRR command is issued to the SDRAM. It is recommended not to perform MRW/MRR commands when 'MRSTAT.mr_wr_busy' is high. - 0 - Indicates that the SoC core can initiate a mode register write operation - 1 - Indicates that mode register write operation is in progress Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DERATEINT_DEF
    ...
    \
    $00 constant DDRCTRL_MR4_READ_INTERVAL      \ [0x00 : 32] Interval between two MR4 reads, used to derate the timing parameters. Present only in designs configured to support LPDDR2/LPDDR3/LPDDR4. This register must not be set to zero. Unit: DFI clock cycle. Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PWRCTL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_SELFREF_EN             \ [0x00] If true then the DDRCTRL puts the SDRAM into Self Refresh after a programmable number of cycles "maximum idle clocks before Self Refresh (PWRTMG.selfref_to_x32)". This register bit may be re-programmed during the course of normal operation. Programming mode: Dynamic
    $01 constant DDRCTRL_POWERDOWN_EN           \ [0x01] If true then the DDRCTRL goes into power-down after a programmable number of cycles "maximum idle clocks before power down" (PWRTMG.powerdown_to_x32). This register bit may be re-programmed during the course of normal operation. Programming mode: Dynamic
    $02 constant DDRCTRL_DEEPPOWERDOWN_EN       \ [0x02] When this is 1, DDRCTRL puts the SDRAM into deep power-down mode when the transaction store is empty. This register must be reset to '0' to bring DDRCTRL out of deep power-down mode. Controller performs automatic SDRAM initialization on deep power-down exit. Present only in designs configured to support mDDR or LPDDR2 or LPDDR3. For non-mDDR/non-LPDDR2/non-LPDDR3, this register should not be set to 1. For performance only Programming mode: Dynamic
    $03 constant DDRCTRL_EN_DFI_DRAM_CLK_DISABLE     \ [0x03] Enable the assertion of dfi_dram_clk_disable whenever a clock is not required by the SDRAM. If set to 0, dfi_dram_clk_disable is never asserted. Assertion of dfi_dram_clk_disable is as follows: In DDR2/DDR3, can only be asserted in Self Refresh. In DDR4, can be asserted in following: - in Self Refresh. - in Maximum Power Saving Mode In mDDR/LPDDR2/LPDDR3, can be asserted in following: - in Self Refresh - in Power Down - in Deep Power Down - during Normal operation (Clock Stop) In LPDDR4, can be asserted in following: - in Self Refresh Power Down - in Power Down - during Normal operation (Clock Stop) Programming mode: Dynamic
    $05 constant DDRCTRL_SELFREF_SW             \ [0x05] A value of 1 to this register causes system to move to Self Refresh state immediately, as long as it is not in INIT or DPD/MPSM operating_mode. This is referred to as Software Entry/Exit to Self Refresh. - 1 - Software Entry to Self Refresh - 0 - Software Exit from Self Refresh Programming mode: Dynamic
    $07 constant DDRCTRL_DIS_CAM_DRAIN_SELFREF  \ [0x07] Indicates whether skipping CAM draining is allowed when entering Self-Refresh. This register field cannot be modified while PWRCTL.selfref_sw==1. - 0 - CAMs must be empty before entering SR - 1 - CAMs are not emptied before entering SR (unsupported) Note, PWRCTL.dis_cam_drain_selfref=1 is unsupported in this release. PWRCTL.dis_cam_drain_selfref=0 is required. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PWRTMG_DEF
    ...
    \
    $00 constant DDRCTRL_POWERDOWN_TO_X32       \ [0x00 : 5] After this many clocks of the DDRC command channel being idle the DDRCTRL automatically puts the SDRAM into power-down. The DDRC command channel is considered idle when there are no HIF commands outstanding. This must be enabled in the PWRCTL.powerdown_en. Unit: Multiples of 32 DFI clocks For performance only. Programming mode: Quasi-dynamic Group 4
    $08 constant DDRCTRL_T_DPD_X4096            \ [0x08 : 8] Minimum deep power-down time. For mDDR, value from the JEDEC specification is 0 as mDDR exits from deep power-down mode immediately after PWRCTL.deeppowerdown_en is de-asserted. For LPDDR2/LPDDR3, value from the JEDEC specification is 500 us. Unit: Multiples of 4096 DFI clocks. Present only in designs configured to support mDDR, LPDDR2 or LPDDR3. For performance only. Programming mode: Quasi-dynamic Group 4
    $10 constant DDRCTRL_SELFREF_TO_X32         \ [0x10 : 8] After this many clocks of the DDRC command channel being idle the DDRCTRL automatically puts the SDRAM into Self Refresh. The DDRC command channel is considered idle when there are no HIF commands outstanding. This must be enabled in the PWRCTL.selfref_en. Unit: Multiples of 32 DFI clocks. For performance only. Programming mode: Quasi-dynamic Group 4
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_CRCPARCTL0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_DFI_ALERT_ERR_INT_EN   \ [0x00] Interrupt enable bit for DFI alert error. If this bit is set, any parity/CRC error detected on the dfi_alert_n input results in an interrupt being set on CRCPARSTAT.dfi_alert_err_int. Programming mode: Dynamic
    $01 constant DDRCTRL_DFI_ALERT_ERR_INT_CLR  \ [0x01] Interrupt clear bit for DFI alert error. If this bit is set, the alert error interrupt on CRCPARSTAT.dfi_alert_err_int is cleared. When the clear operation is complete, the DDRCTRL automatically clears this bit. Programming mode: Dynamic
    $02 constant DDRCTRL_DFI_ALERT_ERR_CNT_CLR  \ [0x02] DFI alert error count clear. Clear bit for DFI alert error counter. Asserting this bit, clears the DFI alert error counter, CRCPARSTAT.dfi_alert_err_cnt. When the clear operation is complete, the DDRCTRL automatically clears this bit. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_CRCPARSTAT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_DFI_ALERT_ERR_CNT      \ [0x00 : 16] DFI alert error count. If a parity/CRC error is detected on dfi_alert_n, this counter be incremented. This is independent of the setting of CRCPARCTL0.dfi_alert_err_int_en. It saturates at 0xFFFF, and can be cleared by asserting CRCPARCTL0.dfi_alert_err_cnt_clr. Programming mode: Static
    $10 constant DDRCTRL_DFI_ALERT_ERR_INT      \ [0x10] DFI alert error interrupt. If a parity/CRC error is detected on dfi_alert_n, and the interrupt is enabled by CRCPARCTL0.dfi_alert_err_int_en, this interrupt bit is set. It remains set until cleared by CRCPARCTL0.dfi_alert_err_int_clr Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_INIT0_DEF
    ...
    \
    $00 constant DDRCTRL_PRE_CKE_X1024          \ [0x00 : 12] Cycles to wait after reset before driving CKE high to start the SDRAM initialization sequence. Unit: 1024 DFI clock cycles. DDR2 specifications typically require this to be programmed for a delay of >= 200 us. LPDDR2/LPDDR3: tINIT1 of 100 ns (min) LPDDR4: tINIT3 of 2 ms (min) When the controller is operating in 1:2 frequency ratio mode, program this to JEDEC spec value divided by 2, and round it up to the next integer value. For DDR3/DDR4 RDIMMs, this should include the time needed to satisfy tSTAB Programming mode: Static
    $10 constant DDRCTRL_POST_CKE_X1024         \ [0x10 : 10] Cycles to wait after driving CKE high to start the SDRAM initialization sequence. Unit: 1024 DFI clock cycles. DDR2 typically requires a 400 ns delay, requiring this value to be programmed to 2 at all clock speeds. LPDDR2/LPDDR3 typically requires this to be programmed for a delay of 200 us. LPDDR4 typically requires this to be programmed for a delay of 2 us. When the controller is operating in 1:2 frequency ratio mode, program this to JEDEC spec value divided by 2, and round it up to the next integer value. Programming mode: Static
    $1e constant DDRCTRL_SKIP_DRAM_INIT         \ [0x1e : 2] If lower bit is enabled the SDRAM initialization routine is skipped. The upper bit decides what state the controller starts up in when reset is removed - 00 - SDRAM Initialization routine is run after power-up - 01 - SDRAM Initialization routine is skipped after power-up. Controller starts up in Normal Mode - 11 - SDRAM Initialization routine is skipped after power-up. Controller starts up in Self-refresh Mode - 10 - Reserved. Programming mode: Quasi-dynamic Group 2
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_INIT1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_PRE_OCD_X32            \ [0x00 : 4] Wait period before driving the OCD complete command to SDRAM. Unit: Counts of a global timer that pulses every 32 DFI clock cycles. There is no known specific requirement for this; it may be set to zero. Programming mode: Static
    $10 constant DDRCTRL_DRAM_RSTN_X1024        \ [0x10 : 9] Number of cycles to assert SDRAM reset signal during init sequence. This is only present for designs supporting DDR3, DDR4 or LPDDR4 devices. For use with a DDR PHY, this should be set to a minimum of 1. When the controller is operating in 1:2 frequency ratio mode, program this to JEDEC spec value divided by 2, and round it up to the next integer value. Unit: 1024 DFI clock cycles. Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_INIT2_DEF
    ...
    \
    $00 constant DDRCTRL_MIN_STABLE_CLOCK_X1    \ [0x00 : 4] Time to wait after the first CKE high, tINIT2. Present only in designs configured to support LPDDR2/LPDDR3. LPDDR2/LPDDR3 typically requires 5 x tCK delay. When the controller is operating in 1:2 frequency ratio mode, program this to JEDEC spec value divided by 2, and round it up to the next integer value. Unit: DFI clock cycles. Programming mode: Static
    $08 constant DDRCTRL_IDLE_AFTER_RESET_X32   \ [0x08 : 8] Idle time after the reset command, tINIT4. Present only in designs configured to support LPDDR2. When the controller is operating in 1:2 frequency ratio mode, program this to JEDEC spec value divided by 2, and round it up to the next integer value. Unit: 32 DFI clock cycles. Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_INIT4_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_EMR3                   \ [0x00 : 16] DDR2: Value to write to EMR3 register. DDR3/DDR4: Value to write to MR3 register mDDR/LPDDR2/LPDDR3: Unused LPDDR4: Value to write to MR13 register Programming mode: Quasi-dynamic Group 2 and Group 4
    $10 constant DDRCTRL_EMR2                   \ [0x10 : 16] DDR2: Value to write to EMR2 register. DDR3/DDR4: Value to write to MR2 register LPDDR2/LPDDR3/LPDDR4: Value to write to MR3 register mDDR: Unused Programming mode: Quasi-dynamic Group 4
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DRAMTMG8_DEF
    ...
    \
    $00 constant DDRCTRL_T_XS_X32               \ [0x00 : 7] tXS: Exit Self Refresh to commands not requiring a locked DLL. When the controller is operating in 1:2 frequency ratio mode, program this to the above value divided by 2 and round up to next integer value. Unit: Multiples of 32 clocks. Note: Used only for DDR2, DDR3 and DDR4 SDRAMs. Programming mode: Quasi-dynamic Group 2 and Group 4
    $08 constant DDRCTRL_T_XS_DLL_X32           \ [0x08 : 7] tXSDLL: Exit Self Refresh to the commands requiring a locked DLL. When the controller is operating in 1:2 frequency ratio mode, program this to the above value divided by 2 and round up to next integer value. Unit: Multiples of 32 clocks. Note: Used only for DDR2, DDR3 and DDR4 SDRAMs. Programming mode: Quasi-dynamic Group 2 and Group 4
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DRAMTMG14_DEF
    ...
    \
    $00 constant DDRCTRL_T_XSR                  \ [0x00 : 12] tXSR: Exit Self Refresh to any command. When the controller is operating in 1:2 frequency ratio mode, program this to the above value divided by 2 and round up to next integer value. Note: Used only for mDDR/LPDDR2/LPDDR3/LPDDR4 mode. Programming mode: Quasi-dynamic Group 2 and Group 4
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_ZQCTL2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_ZQ_RESET               \ [0x00] Setting this register bit to 1 triggers a ZQ Reset operation. When the ZQ Reset operation is complete, the DDRCTRL automatically clears this bit. It is recommended NOT to set this register bit if in Init, in Self-Refresh(except LPDDR4) or SR-Powerdown(LPDDR4) or Deep power-down operating modes. For Self-Refresh(except LPDDR4) or SR-Powerdown(LPDDR4) it is scheduled after SR(except LPDDR4) or SPRD(LPDDR4) has been exited. For Deep power down, it is not scheduled, although ZQSTAT.zq_reset_busy is de-asserted. This is only present for designs supporting LPDDR2/LPDDR3/LPDDR4 devices. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFILPCFG0_DEF
    ...
    \
    $00 constant DDRCTRL_DFI_LP_EN_PD           \ [0x00] Enables DFI Low Power interface handshaking during Power Down Entry/Exit. - 0 - Disabled - 1 - Enabled Programming mode: Static
    $04 constant DDRCTRL_DFI_LP_WAKEUP_PD       \ [0x04 : 4] Value in DFI clock cycles to drive on dfi_lp_wakeup signal when Power Down mode is entered. Determines the DFI's tlp_wakeup time: - 0x0 - 16 cycles - 0x1 - 32 cycles - 0x2 - 64 cycles - 0x3 - 128 cycles - 0x4 - 256 cycles - 0x5 - 512 cycles - 0x6 - 1024 cycles - 0x7 - 2048 cycles - 0x8 - 4096 cycles - 0x9 - 8192 cycles - 0xA - 16384 cycles - 0xB - 32768 cycles - 0xC - 65536 cycles - 0xD - 131072 cycles - 0xE - 262144 cycles - 0xF - Unlimited Programming mode: Static
    $08 constant DDRCTRL_DFI_LP_EN_SR           \ [0x08] Enables DFI Low Power interface handshaking during Self Refresh Entry/Exit. - 0 - Disabled - 1 - Enabled Programming mode: Static
    $0c constant DDRCTRL_DFI_LP_WAKEUP_SR       \ [0x0c : 4] Value in DFI clpck cycles to drive on dfi_lp_wakeup signal when Self Refresh mode is entered. Determines the DFI's tlp_wakeup time: - 0x0 - 16 cycles - 0x1 - 32 cycles - 0x2 - 64 cycles - 0x3 - 128 cycles - 0x4 - 256 cycles - 0x5 - 512 cycles - 0x6 - 1024 cycles - 0x7 - 2048 cycles - 0x8 - 4096 cycles - 0x9 - 8192 cycles - 0xA - 16384 cycles - 0xB - 32768 cycles - 0xC - 65536 cycles - 0xD - 131072 cycles - 0xE - 262144 cycles - 0xF - Unlimited Programming mode: Static
    $10 constant DDRCTRL_DFI_LP_EN_DPD          \ [0x10] Enables DFI Low-power interface handshaking during Deep Power Down Entry/Exit. - 0 - Disabled - 1 - Enabled This is only present for designs supporting mDDR or LPDDR2/LPDDR3 devices. Programming mode: Static.
    $14 constant DDRCTRL_DFI_LP_WAKEUP_DPD      \ [0x14 : 4] Value in DFI clock cycles to drive on dfi_lp_wakeup signal when Deep Power Down mode is entered. Determines the DFI's tlp_wakeup time: - 0x0 - 16 cycles - 0x1 - 32 cycles - 0x2 - 64 cycles - 0x3 - 128 cycles - 0x4 - 256 cycles - 0x5 - 512 cycles - 0x6 - 1024 cycles - 0x7 - 2048 cycles - 0x8 - 4096 cycles - 0x9 - 8192 cycles - 0xA - 16384 cycles - 0xB - 32768 cycles - 0xC - 65536 cycles - 0xD - 131072 cycles - 0xE - 262144 cycles - 0xF - Unlimited This is only present for designs supporting mDDR or LPDDR2/LPDDR3 devices. Programming mode: Static.
    $18 constant DDRCTRL_DFI_TLP_RESP           \ [0x18 : 5] Setting in DFI clock cycles for DFI's tlp_resp time. Same value is used for both Power Down, Self Refresh, Deep Power Down and Maximum Power Saving modes. DFI 2.1 specification onwards, recommends using a fixed value of 7 always. Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFIUPD0_DEF
    ...
    \
    $00 constant DDRCTRL_DFI_T_CTRLUP_MIN       \ [0x00 : 10] Specifies the minimum number of DFI clock cycles that the dfi_ctrlupd_req signal must be asserted. The DDRCTRL expects the PHY to respond within this time. If the PHY does not respond, the DDRCTRL de-asserts dfi_ctrlupd_req after dfi_t_ctrlup_min + 2 cycles. Lowest value to assign to this variable is 0x3. Programming mode: Static
    $10 constant DDRCTRL_DFI_T_CTRLUP_MAX       \ [0x10 : 10] Specifies the maximum number of DFI clock cycles that the dfi_ctrlupd_req signal can assert. Lowest value to assign to this variable is 0x40. Programming mode: Static
    $1d constant DDRCTRL_CTRLUPD_PRE_SRX        \ [0x1d] Selects dfi_ctrlupd_req requirements at SRX: -0: send ctrlupd after SRX -1: send ctrlupd before SRX If DFIUPD0.dis_auto_ctrlupd_srx=1, this register has no impact, because no dfi_ctrlupd_req is issued when SRX. Programming mode: Static
    $1e constant DDRCTRL_DIS_AUTO_CTRLUPD_SRX   \ [0x1e] When '1', disable the automatic dfi_ctrlupd_req generation by the DDRCTRL at self-refresh exit. When '0', DDRCTRL issues a dfi_ctrlupd_req before or after exiting self-refresh, depending on DFIUPD0.ctrlupd_pre_srx. Programming mode: Static
    $1f constant DDRCTRL_DIS_AUTO_CTRLUPD       \ [0x1f] When '1', disable the automatic dfi_ctrlupd_req generation by the DDRCTRL. The core must issue the dfi_ctrlupd_req signal using register DBGCMD.ctrlupd. When '0', DDRCTRL issues dfi_ctrlupd_req periodically. Programming mode: Quasi-dynamic Group 3
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFIUPD2_DEF
    ...
    \
    $1f constant DDRCTRL_DFI_PHYUPD_EN          \ [0x1f] Enables the support for acknowledging PHY-initiated updates: - 0 - Disabled - 1 - Enabled Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFIMISC_DEF
    ...
    \
    $00 constant DDRCTRL_DFI_INIT_COMPLETE_EN   \ [0x00] PHY initialization complete enable signal. When asserted the dfi_init_complete signal can be used to trigger SDRAM initialisation Programming mode: Quasi-dynamic Group 3
    $04 constant DDRCTRL_CTL_IDLE_EN            \ [0x04] Enables support of ctl_idle signal Programming mode: Static
    $05 constant DDRCTRL_DFI_INIT_START         \ [0x05] PHY init start request signal.When asserted it triggers the PHY init start request Programming mode: Quasi-dynamic Group 3
    $08 constant DDRCTRL_DFI_FREQUENCY          \ [0x08 : 5] Indicates the operating frequency of the system. The number of supported frequencies and the mapping of signal values to clock frequencies are defined by the PHY. Programming mode: Quasi-dynamic Group 1
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFISTAT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_DFI_INIT_COMPLETE      \ [0x00] The status flag register which announces when the DFI initialization has been completed. The DFI INIT triggered by dfi_init_start signal and then the dfi_init_complete flag is polled to know when the initialization is done. Programming mode: Dynamic
    $01 constant DDRCTRL_DFI_LP_ACK             \ [0x01] Stores the value of the dfi_lp_ack input to the controller. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DFIPHYMSTR_DEF
    ...
    \ Reset value: 0x00000001
    \
    $00 constant DDRCTRL_DFI_PHYMSTR_EN         \ [0x00] Enables the PHY Master Interface: - 0 - Disabled - 1 - Enabled Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_SCHED1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_PAGECLOSE_TIMER        \ [0x00 : 8] This field works in conjunction with SCHED.pageclose. It only has meaning if SCHED.pageclose==1. If SCHED.pageclose==1 and pageclose_timer==0, then an auto-precharge may be scheduled for last read or write command in the CAM with a bank and page hit. Note, sometimes an explicit precharge is scheduled instead of the auto-precharge. See SCHED.pageclose for details of when this may happen. If SCHED.pageclose==1 and pageclose_timer>0, then an auto-precharge is not scheduled for last read or write command in the CAM with a bank and page hit. Instead, a timer is started, with pageclose_timer as the initial value. There is a timer on a per bank basis. The timer decrements unless the next read or write in the CAM to a bank is a page hit. It gets reset to pageclose_timer value if the next read or write in the CAM to a bank is a page hit. Once the timer has reached zero, an explicit precharge is attempted to be scheduled. Programming mode: Static.
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PERFHPR1_DEF
    ...
    \
    $00 constant DDRCTRL_HPR_MAX_STARVE         \ [0x00 : 16] Number of DFI clocks that the HPR queue can be starved before it goes critical. The minimum valid functional value for this register is 0x1. Programming it to 0x0 disables the starvation functionality; during normal operation, this function must not be disabled as it causes excessive latencies. For performance only. Programming mode: Quasi-dynamic Group 3
    $18 constant DDRCTRL_HPR_XACT_RUN_LENGTH    \ [0x18 : 8] Number of transactions that are serviced once the HPR queue goes critical is the smaller of: - (a) This number - (b) Number of transactions available. Unit: Transaction. For performance only. Programming mode: Quasi-dynamic Group 3
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PERFLPR1_DEF
    ...
    \
    $00 constant DDRCTRL_LPR_MAX_STARVE         \ [0x00 : 16] Number of DFI clocks that the LPR queue can be starved before it goes critical. The minimum valid functional value for this register is 0x1. Programming it to 0x0 disables the starvation functionality; during normal operation, this function must be disabled as it causes excessive latencies. For performance only Programming mode: Quasi-dynamic Group 3
    $18 constant DDRCTRL_LPR_XACT_RUN_LENGTH    \ [0x18 : 8] Number of transactions that are serviced once the LPR queue goes critical is the smaller of: - (a) This number - (b) Number of transactions available. Unit: Transaction. For performance only Programming mode: Quasi-dynamic group 3
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PERFWR1_DEF
    ...
    \ Reset value: 0x0F00007F
    \
    $00 constant DDRCTRL_W_MAX_STARVE           \ [0x00 : 16] Number of DFI clocks that the WR queue can be starved before it goes critical. The minimum valid functional value for this register is 0x1. Programming it to 0x0 disables the starvation functionality; during normal operation, this function must not be disabled as it causes excessive latencies. For performance only. Programming mode: Quasi-dynamic Group 3.
    $18 constant DDRCTRL_W_XACT_RUN_LENGTH      \ [0x18 : 8] Number of transactions that are serviced once the WR queue goes critical is the smaller of: - (a) This number - (b) Number of transactions available. Unit: Transaction. For performance only. Programming mode: Quasi-dynamic Group 3.
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DBG0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_DIS_WC                 \ [0x00] When 1, disable write combine. FOR DEBUG ONLY Programming mode: Static
    $04 constant DDRCTRL_DIS_COLLISION_PAGE_OPT     \ [0x04] When this is set to '0', auto-precharge is disabled for the flushed command in a collision case. Collision cases are write followed by read to same address, read followed by write to same address, or write followed by write to same address with DBG0.dis_wc bit = 1 (where same address comparisons exclude the two address bits representing critical word). FOR DEBUG ONLY. Programming mode: Static
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DBGCAM_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_DBG_HPR_Q_DEPTH        \ [0x00 : 5] High priority read queue depth For debug only. Programming mode: Dynamic
    $08 constant DDRCTRL_DBG_LPR_Q_DEPTH        \ [0x08 : 5] Low priority read queue depth The last entry of Lpr queue is reserved for ECC SCRUB operation. This entry is not included in the calculation of the queue depth. For debug only. Programming mode: Dynamic
    $10 constant DDRCTRL_DBG_W_Q_DEPTH          \ [0x10 : 5] Write queue depth The last entry of WR queue is reserved for ECC SCRUB operation. This entry is not included in the calculation of the queue depth. For debug only. Programming mode: Dynamic
    $18 constant DDRCTRL_DBG_STALL              \ [0x18] Stall For debug only. Programming mode: Dynamic
    $19 constant DDRCTRL_DBG_RD_Q_EMPTY         \ [0x19] When 1, all the Read command queues and Read data buffers inside DDRC are empty. This register is to be used for debug purpose. An example use-case scenario: When Controller enters Self-Refresh using the Low-Power entry sequence, Controller is expected to have executed all the commands in its queues and the write and read data drained. Hence this register should be 1 at that time. For debug only. Programming mode: Dynamic
    $1a constant DDRCTRL_DBG_WR_Q_EMPTY         \ [0x1a] When 1, all the Write command queues and Write data buffers inside DDRC are empty. This register is to be used for debug purpose. An example use-case scenario: When Controller enters Self-Refresh using the Low-Power entry sequence, Controller is expected to have executed all the commands in its queues and the write and read data drained. Hence this register should be 1 at that time. For debug only. Programming mode: Dynamic.
    $1c constant DDRCTRL_RD_DATA_PIPELINE_EMPTY     \ [0x1c] This bit indicates that the read data pipeline on the DFI interface is empty. This register is intended to be polled at least twice after setting DBG1.dis_dq, to ensure that all remaining commands/data have completed. Programming mode: Dynamic.
    $1d constant DDRCTRL_WR_DATA_PIPELINE_EMPTY     \ [0x1d] This bit indicates that the write data pipeline on the DFI interface is empty. This register is intended to be polled at least twice after setting DBG1.dis_dq, to ensure that all remaining commands/data have completed. Programming mode: Dynamic.
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DBGCMD_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_RANK0_REFRESH          \ [0x00] Setting this register bit to 1 indicates to the DDRCTRL to issue a refresh to rank 0. Writing to this bit causes DBGSTAT.rank0_refresh_busy to be set. When DBGSTAT.rank0_refresh_busy is cleared, the command has been stored in DDRCTRL. For 3DS configuration, refresh is sent to rank index 0. This operation can be performed only when RFSHCTL3.dis_auto_refresh=1. It is recommended NOT to set this register bit if in Init or Deep power-down operating modes or Maximum Power Saving Mode. Programming mode: Dynamic.
    $04 constant DDRCTRL_ZQ_CALIB_SHORT         \ [0x04] Setting this register bit to 1 indicates to the DDRCTRL to issue a ZQCS (ZQ calibration short)/MPC(ZQ calibration) command to the SDRAM. When this request is stored in the DDRCTRL, the bit is automatically cleared. This operation can be performed only when ZQCTL0.dis_auto_zq=1. It is recommended NOT to set this register bit if in Init, in Self-Refresh(except LPDDR4) or SR-Powerdown(LPDDR4) or Deep power-down operating modes or maximum power saving mode. For self-refresh(except LPDDR4) or SR-powerdown(LPDDR4) it is scheduled after SR(except LPDDR4) or SPRD(LPDDR4) has been exited. For Deep power down and maximum power saving mode, it is not scheduled, although DBGSTAT.zq_calib_short_busy is de-asserted. Programming mode: Dynamic.
    $05 constant DDRCTRL_CTRLUPD                \ [0x05] Setting this register bit to 1 indicates to the DDRCTRL to issue a dfi_ctrlupd_req to the PHY. When this request is stored in the DDRCTRL, the bit is automatically cleared. This operation must only be performed when DFIUPD0.dis_auto_ctrlupd=1. Programming mode: Dynamic.
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_DBGSTAT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_RANK0_REFRESH_BUSY     \ [0x00] SoC core may initiate a rank0_refresh operation (refresh operation to rank 0) only if this signal is low. This signal goes high in the clock after DBGCMD.rank0_refresh is set to one. It goes low when the rank0_refresh operation is stored in the DDRCTRL. It is recommended not to perform rank0_refresh operations when this signal is high. - 0 - Indicates that the SoC core can initiate a rank0_refresh operation - 1 - Indicates that rank0_refresh operation has not been stored yet in the DDRCTRL Programming mode: Dynamic
    $04 constant DDRCTRL_ZQ_CALIB_SHORT_BUSY    \ [0x04] SoC core may initiate a ZQCS (ZQ calibration short) operation only if this signal is low. This signal goes high in the clock after the DDRCTRL accepts the ZQCS request. It goes low when the ZQCS operation is initiated in the DDRCTRL. It is recommended not to perform ZQCS operations when this signal is high. - 0 - Indicates that the SoC core can initiate a ZQCS operation - 1 - Indicates that ZQCS operation has not been initiated yet in the DDRCTRL Programming mode: Dynamic
    $05 constant DDRCTRL_CTRLUPD_BUSY           \ [0x05] SoC core may initiate a ctrlupd operation only if this signal is low. This signal goes high in the clock after the DDRCTRL accepts the ctrlupd request. It goes low when the ctrlupd operation is initiated in the DDRCTRL. It is recommended not to perform ctrlupd operations when this signal is high. - 0 - Indicates that the SoC core can initiate a ctrlupd operation - 1 - Indicates that ctrlupd operation has not been initiated yet in the DDRCTRL Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_SWCTL_DEF
    ...
    \ Reset value: 0x00000001
    \
    $00 constant DDRCTRL_SW_DONE                \ [0x00] Enable quasi-dynamic register programming outside reset. Program register to 0 to enable quasi-dynamic programming. Set back register to 1 once programming is done. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_POISONCFG_DEF
    ...
    \
    $00 constant DDRCTRL_WR_POISON_SLVERR_EN    \ [0x00] If set to 1, enables SLVERR response for write transaction poisoning Programming mode: Dynamic
    $04 constant DDRCTRL_WR_POISON_INTR_EN      \ [0x04] If set to 1, enables interrupts for write transaction poisoning Programming mode: Dynamic
    $08 constant DDRCTRL_WR_POISON_INTR_CLR     \ [0x08] Interrupt clear for write transaction poisoning. Allow 2/3 clock cycles for correct value to propagate to core logic and clear the interrupts. Programming mode: Dynamic
    $10 constant DDRCTRL_RD_POISON_SLVERR_EN    \ [0x10] If set to 1, enables SLVERR response for read transaction poisoning Programming mode: Dynamic
    $14 constant DDRCTRL_RD_POISON_INTR_EN      \ [0x14] If set to 1, enables interrupts for read transaction poisoning Programming mode: Dynamic
    $18 constant DDRCTRL_RD_POISON_INTR_CLR     \ [0x18] Interrupt clear for read transaction poisoning. Allow 2/3 clock cycles for correct value to propagate to core logic and clear the interrupts. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PSTAT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_RD_PORT_BUSY_0         \ [0x00] Indicates if there are outstanding reads for AXI port 0. Programming mode: Dynamic
    $10 constant DDRCTRL_WR_PORT_BUSY_0         \ [0x10] Indicates if there are outstanding writes for AXI port 0. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PCTRL_0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_PORT_EN                \ [0x00] Enables AXI port n. Programming mode: Dynamic
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PCFGQOS1_0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_RQOS_MAP_TIMEOUTB      \ [0x00 : 11] Specifies the timeout value for transactions mapped to the blue address queue. Programming mode: Quasi-dynamic Group 3.
    $10 constant DDRCTRL_RQOS_MAP_TIMEOUTR      \ [0x10 : 11] Specifies the timeout value for transactions mapped to the red address queue. With single read address queue, there is no red queue and this value has no effect. Programming mode: Quasi-dynamic Group 3
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PCFGWQOS0_0_DEF
    ...
    \
    $00 constant DDRCTRL_WQOS_MAP_LEVEL1        \ [0x00 : 4] Separation level indicating the end of region0 mapping; start of region0 is 0. Possible values for level1 are 0 to 13 which corresponds to awqos. Note that for PA, awqos values are used directly as port priorities, where the higher the value corresponds to higher port priority. All of the map_level* registers must be set to distinct values. Programming mode: Quasi-dynamic Group 3
    $08 constant DDRCTRL_WQOS_MAP_LEVEL2        \ [0x08 : 4] Separation level2 indicating the end of region1 mapping; start of region1 is (level1 + 1). Possible values for level2 are (level1 + 1) to 14 which corresponds to awqos. Region2 starts from (level2 + 1) up to 15. Note that for PA, awqos values are used directly as port priorities, where the higher the value corresponds to higher port priority. All of the map_level* registers must be set to distinct values. Programming mode: Quasi-dynamic Group 3
    $10 constant DDRCTRL_WQOS_MAP_REGION0       \ [0x10 : 2] This bitfield indicates the traffic class of region 0. Valid values are: When VPW support is disabled (UMCTL2_VPW_EN = 0) and traffic class of region 0 is set to 1 (VPW), VPW traffic is aliased to NPW traffic. Programming mode: Quasi-dynamic Group 3
    $14 constant DDRCTRL_WQOS_MAP_REGION1       \ [0x14 : 2] This bitfield indicates the traffic class of region 1. Valid values are: When VPW support is disabled (UMCTL2_VPW_EN = 0) and traffic class of region 1 is set to 1 (VPW), VPW traffic is aliased to NPW traffic. Programming mode: Quasi-dynamic Group 3
    $18 constant DDRCTRL_WQOS_MAP_REGION2       \ [0x18 : 2] This bitfield indicates the traffic class of region 2. Valid values are: When VPW support is disabled (UMCTL2_VPW_EN = 0) and traffic class of region 2 is set to 1 (VPW), VPW traffic is aliased to NPW traffic. Programming mode: Quasi-dynamic Group 3
  [then]
  ...
  [ifdef] DDRCTRL_DDRCTRL_PCFGWQOS1_0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant DDRCTRL_WQOS_MAP_TIMEOUT1      \ [0x00 : 11] Specifies the timeout value for write transactions in region 0 and 1. Programming mode: Quasi-dynamic Group 3.
    $10 constant DDRCTRL_WQOS_MAP_TIMEOUT2      \ [0x10 : 11] Specifies the timeout value for write transactions in region 2. Programming mode: Quasi-dynamic Group 3.
  [then]
  ...
  $10 constant DDRCTRL_DDRCTRL_MRCTRL0  \ DDRCTRL mode register read/write control register 0
  ...
  $36C constant DDRCTRL_DDRCTRL_POISONCFG    \ DDRCTRL AXI Poison configuration register
  ...
  $404 constant DDRCTRL_DDRCTRL_PCFGR_0 \ DDRCTRL port x configuration read register
  $408 constant DDRCTRL_DDRCTRL_PCFGW_0 \ DDRCTRL port x configuration write register
  $490 constant DDRCTRL_DDRCTRL_PCTRL_0 \ DDRCTRL port x control register
  $494 constant DDRCTRL_DDRCTRL_PCFGQOS0_0    \ DDRCTRL port x read Q0S configuration register 0
  $498 constant DDRCTRL_DDRCTRL_PCFGQOS1_0    \ DDRCTRL port x read Q0S configuration register 1
  $49C constant DDRCTRL_DDRCTRL_PCFGWQOS0_0    \ DDRCTRL port x write Q0S configuration register 0
  $4A0 constant DDRCTRL_DDRCTRL_PCFGWQOS1_0    \ DDRCTRL port x write Q0S configuration register 1
