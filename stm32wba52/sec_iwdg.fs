...
$00000fff constant SEC_IWDG_IWDG_EWCR_EWIT                          \ Watchdog counter window value These bits are write access protected (see Section 29.4.6). They are written by software to define at which position of the IWDCNT down-counter the early wakeup interrupt must be generated. The early interrupt is generated when the IWDCNT is lower or equal to EWIT[11:0] - 1. EWIT[11:0] must be bigger than 1. An interrupt is generated only if EWIE = 1. The EWU bit in the IWDG status register (IWDG_SR) must be reset to be able to change the reload value. Note: Reading this register returns the Early wakeup comparator value and the Interrupt enable bit from the VsubDD/sub voltage domain. This value may not be up to date/valid if a write operation to this register is ongoing, hence the value read from this register is valid only when the EWU bit in the IWDG status register (IWDG_SR) is reset.
$00004000 constant SEC_IWDG_IWDG_EWCR_EWIC                          \ Watchdog early interrupt acknowledge The software must write a 1 into this bit in order to acknowledge the early wakeup interrupt and to clear the EWIF flag. Writing 0 has not effect, reading this flag returns a 0.
$00008000 constant SEC_IWDG_IWDG_EWCR_EWIE                          \ Watchdog early interrupt enable Set and reset by software. The EWU bit in the IWDG status register (IWDG_SR) must be reset to be able to change the value of this bit.


\
\ @brief Independent watchdog
\
$50003000 constant SEC_IWDG_IWDG_KR  \ offset: 0x00 : IWDG key register
$50003004 constant SEC_IWDG_IWDG_PR  \ offset: 0x04 : IWDG prescaler register
$50003008 constant SEC_IWDG_IWDG_RLR  \ offset: 0x08 : IWDG reload register
$5000300c constant SEC_IWDG_IWDG_SR  \ offset: 0x0C : IWDG status register
$50003010 constant SEC_IWDG_IWDG_WINR  \ offset: 0x10 : IWDG window register
$50003014 constant SEC_IWDG_IWDG_EWCR  \ offset: 0x14 : IWDG early wakeup interrupt register
