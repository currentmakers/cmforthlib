...
[ifndef] AES_DEF
  ...
  [ifdef] AES_AES_CR_DEF
    ...
    \
    $00 constant AES_EN                         \ [0x00] Enable This bit enables/disables the AES peripheral. At any moment, clearing then setting the bit re-initializes the AES peripheral. This bit is automatically cleared by hardware upon the completion of the key preparation (MODE[1:0] at 01) and upon the completion of GCM/GMAC/CCM initialization phase. The bit cannot be set as long as KEYVALID=0.
    ...
    $03 constant AES_MODE                       \ [0x03 : 2] Operating mode This bitfield selects the AES operating mode: Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
    $05 constant AES_CHMOD                      \ [0x05 : 2] CHMOD[1:0]: Chaining mode This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
    $0b constant AES_DMAINEN                    \ [0x0b] DMA input enable When this bit is set, DMA requests are automatically generated by the peripheral during the input data phase. Setting this bit is ignored when MODE[1:0] is at 01 (key derivation).
    $0c constant AES_DMAOUTEN                   \ [0x0c] DMA output enable When this bit is set, DMA requests are automatically generated by the peripheral during the output data phase. Setting this bit is ignored when MODE[1:0] is at 01 (key derivation).
    $0d constant AES_GCMPH                      \ [0x0d : 2] GCM or CCM phase selection This bitfield selects the phase, applicable only with GCM, GMAC or CCM chaining modes. This bitfield has no effect if GCM, GMAC or CCM algorithm is not selected with CHMOD[2:0].
    $10 constant AES_CHMOD_1                    \ [0x10] CHMOD[2]
    $12 constant AES_KEYSIZE                    \ [0x12] Key size selection This bitfield defines the key length in bits of the key used by AES. Attempts to write the bit are ignored when BUSY is set, as well as when the EN is set before the write access and it is not cleared by that write access.
    ...
    $18 constant AES_KMOD                       \ [0x18 : 2] Key mode selection The bitfield defines how the AES key can be used by the application. KEYSIZE must be correctly initialized when setting KMOD[1:0] different from zero. Others: Reserved Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
  ...
  [ifdef] AES_AES_SR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $01 constant AES_RDERRF                     \ [0x01] Read error flag This bit is set when an unexpected read to the AES_DOUTR register occurred. When set RDERRF bit has no impact on the AES operations. This flag is cleared by setting the RWEIF bit in AES_ICR register. If RWEIE bit is set in AES_IER register an interrupt is generated when RDERRF is set. It is cleared by setting the RWEIF bit of the AES_ICR register.
    $02 constant AES_WRERRF                     \ [0x02] Write error flag This bit is set when an unexpected write to the AES_DINR register occurred. When set WRERRF bit has no impact on the AES operations. This flag is cleared by setting the RWEIF bit in AES_ICR register. If RWEIE bit is set in AES_IER register an interrupt is generated when WRERRF is set. It is cleared by setting the RWEIF bit of the AES_ICR register.
    $03 constant AES_BUSY                       \ [0x03] Busy This flag indicates whether AES is idle or busy. AES is flagged as idle when disabled or when the last processing is completed. AES is flagged as busy when processing a block data, preparing a key (ECB or CBC decryption only), or transferring a shared key from SAES peripheral. When GCM encryption is selected, this flag must be at zero before suspending current process to manage a higher-priority message.
  ...
  [ifdef] AES_AES_KEYR1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [63:32] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [95:64] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR3_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [127:96] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR4_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [159:128] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR5_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [191:160] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR6_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [223:192] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
  ...
  [ifdef] AES_AES_KEYR7_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant AES_KEY                        \ [0x00 : 32] Cryptographic key, bits [255:224] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
  [then]
