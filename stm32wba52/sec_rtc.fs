...
$00000010 constant SEC_RTC_RTC_CR_REFCKON                           \ RTC_REFIN reference clock detection enable (50 or 60Hz) Note: BIN must be 0x00 and PREDIV_S must be 0x00FF.
$00000020 constant SEC_RTC_RTC_CR_BYPSHAD                           \ Bypass the shadow registers Note: If the frequency of the APB clock is less than seven times the frequency of RTCCLK, BYPSHAD must be set to 1.
...
$00080000 constant SEC_RTC_RTC_CR_COSEL                             \ Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to Section74.3.18: Calibration clock output.
...
$80000000 constant SEC_RTC_RTC_CR_OUT2EN                            \ RTC_OUT2 output enable With this bit set, the RTC outputs can be remapped on RTC_OUT2 as follows: OUT2EN=0: RTC output 2 disable If OSEL different 00 or TAMPOE = 1: TAMPALRM is output on RTC_OUT1 If OSEL=00 and TAMPOE=0 and COE=1: CALIB is output on RTC_OUT1 OUT2EN = 1: RTC output 2 enable If (OSEL different 00 or TAMPOE = 1) and COE = 0: TAMPALRM is output on RTC_OUT2 If OSEL=00 and TAMPOE=0 and COE=1: CALIB is output on RTC_OUT2 If (OSEL different 00 or TAMPOE = 1) and COE = 1: CALIB is output on RTC_OUT2 and TAMPALRM is output on RTC_OUT1.
...
$000000ff constant SEC_RTC_RTC_WPR_KEY                              \ Write protection key This byte is written by software. Reading this byte always returns 0x00. Refer to RTC register write protection for a description of how to unlock RTC register write protection.
...
$000001ff constant SEC_RTC_RTC_CALR_CALM                            \ Calibration minus The frequency of the calendar is reduced by masking CALM out of 2sup20/sup RTCCLK pulses (32seconds if the input frequency is 32768Hz). This decreases the frequency of the calendar with a resolution of 0.9537ppm. To increase the frequency of the calendar, this feature should be used in conjunction with CALP. See Section74.3.16: RTC smooth digital calibration on page4278.
$00001000 constant SEC_RTC_RTC_CALR_LPCAL                           \ RTC low-power mode
$00002000 constant SEC_RTC_RTC_CALR_CALW16                          \ Use a 16-second calibration cycle period When CALW16 is set to 1, the 16-second calibration cycle period is selected. This bit must not be set to 1 if CALW8 = 1. Note: CALM[0] is stuck at 0 when CALW16 = 1. Refer to Section74.3.16: RTC smooth digital calibration.
$00004000 constant SEC_RTC_RTC_CALR_CALW8                           \ Use an 8-second calibration cycle period When CALW8 is set to 1, the 8-second calibration cycle period is selected. Note: CALM[1:0] are stuck at 00 when CALW8 = 1. Refer to Section74.3.16: RTC smooth digital calibration.
$00008000 constant SEC_RTC_RTC_CALR_CALP                            \ Increase frequency of RTC by 488.5ppm. This feature is intended to be used in conjunction with CALM, which lowers the frequency of the calendar with a fine resolution. if the input frequency is 32768 Hz, the number of RTCCLK pulses added during a 32 second window is calculated as follows: (512 * CALP) CALM. Refer to Section74.3.16: RTC smooth digital calibration.
...
$00000000 constant SEC_RTC_RTC_TSSSR_SS                             \ Subsecond value/synchronous binary counter values SS[31:0] is the value of the synchronous prescaler counter when the timestamp event occurred.
...
$00007fff constant SEC_RTC_RTC_ALRMASSR_SS                          \ Subseconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS[14:0] in the RTC_ALRMABINR, and so can also be read or written through RTC_ALRMABINR.
$3f000000 constant SEC_RTC_RTC_ALRMASSR_MASKSS                      \ Mask the most-significant bits starting at this bit ... From 32 to 63: All 32 SS bits are compared and must match to activate alarm. Note: In BCD mode (BIN=00) the overflow bits of the synchronous counter (bits 31:15) are never compared. These bits can be different from 0 only after a shift operation.
...
$00007fff constant SEC_RTC_RTC_ALRMBSSR_SS                          \ Subseconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm B is to be activated. Only bits 0 up to MASKSS-1 are compared. This field is the mirror of SS[14:0] in the RTC_ALRMBBINR, and so can also be read or written through RTC_ALRMBBINR.
$3f000000 constant SEC_RTC_RTC_ALRMBSSR_MASKSS                      \ Mask the most-significant bits starting at this bit ... From 32 to 63: All 32 SS bits are compared and must match to activate alarm. Note: In BCD mode (BIN=00)The overflow bits of the synchronous counter (bits 15) is never compared. This bit can be different from 0 only after a shift operation.
...
$00000008 constant SEC_RTC_RTC_SR_TSF                               \ Timestamp flag This flag is set by hardware when a timestamp event occurs. Note: TSF is not set if TAMPTS=1 and the tamper flag is read during the 3 ck_apre cycles following tamper event. Refer to Timestamp on tamper event for more details.
...
$00000008 constant SEC_RTC_RTC_MISR_TSMF                            \ Timestamp non-secure masked flag This flag is set by hardware when a timestamp non-secure interrupt occurs.
...
$00000008 constant SEC_RTC_RTC_SMISR_TSMF                           \ Timestamp interrupt secure masked flag This flag is set by hardware when a timestamp secure interrupt occurs.
...
$00000008 constant SEC_RTC_RTC_SCR_CTSF                             \ Clear timestamp flag Writing 1 in this bit clears the TSF bit in the RTC_SR register.
...
\ @brief Real-time clock
\
$56007800 constant SEC_RTC_RTC_TR  \ offset: 0x00 : RTC time register
$56007804 constant SEC_RTC_RTC_DR  \ offset: 0x04 : RTC date register
$56007808 constant SEC_RTC_RTC_SSR  \ offset: 0x08 : RTC subsecond register
$5600780c constant SEC_RTC_RTC_ICSR  \ offset: 0x0C : RTC initialization control and status register
$56007810 constant SEC_RTC_RTC_PRER  \ offset: 0x10 : RTC prescaler register
$56007814 constant SEC_RTC_RTC_WUTR  \ offset: 0x14 : RTC wakeup timer register
$56007818 constant SEC_RTC_RTC_CR  \ offset: 0x18 : RTC control register
$5600781c constant SEC_RTC_RTC_PRIVCFGR  \ offset: 0x1C : RTC privilege mode control register
$56007820 constant SEC_RTC_RTC_SECCFGR  \ offset: 0x20 : RTC secure configuration register
$56007824 constant SEC_RTC_RTC_WPR  \ offset: 0x24 : RTC write protection register
$56007828 constant SEC_RTC_RTC_CALR  \ offset: 0x28 : RTC calibration register
$5600782c constant SEC_RTC_RTC_SHIFTR  \ offset: 0x2C : RTC shift control register
$56007830 constant SEC_RTC_RTC_TSTR  \ offset: 0x30 : RTC timestamp time register
$56007834 constant SEC_RTC_RTC_TSDR  \ offset: 0x34 : RTC timestamp date register
$56007838 constant SEC_RTC_RTC_TSSSR  \ offset: 0x38 : RTC timestamp subsecond register
$56007840 constant SEC_RTC_RTC_ALRMAR  \ offset: 0x40 : RTC alarm A register
$56007844 constant SEC_RTC_RTC_ALRMASSR  \ offset: 0x44 : RTC alarm A subsecond register
$56007848 constant SEC_RTC_RTC_ALRMBR  \ offset: 0x48 : RTC alarm B register
$5600784c constant SEC_RTC_RTC_ALRMBSSR  \ offset: 0x4C : RTC alarm B subsecond register
$56007850 constant SEC_RTC_RTC_SR  \ offset: 0x50 : RTC status register
$56007854 constant SEC_RTC_RTC_MISR  \ offset: 0x54 : RTC non-secure masked interrupt status register
$56007858 constant SEC_RTC_RTC_SMISR  \ offset: 0x58 : RTC secure masked interrupt status register
$5600785c constant SEC_RTC_RTC_SCR  \ offset: 0x5C : RTC status clear register
$56007870 constant SEC_RTC_RTC_ALRABINR  \ offset: 0x70 : RTC alarm A binary mode register
$56007874 constant SEC_RTC_RTC_ALRBBINR  \ offset: 0x74 : RTC alarm B binary mode register
