...
$00000001 constant GPDMA_GPDMA_RCFGLOCKR_LOCK0                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000002 constant GPDMA_GPDMA_RCFGLOCKR_LOCK1                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000004 constant GPDMA_GPDMA_RCFGLOCKR_LOCK2                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000008 constant GPDMA_GPDMA_RCFGLOCKR_LOCK3                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000010 constant GPDMA_GPDMA_RCFGLOCKR_LOCK4                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000020 constant GPDMA_GPDMA_RCFGLOCKR_LOCK5                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000040 constant GPDMA_GPDMA_RCFGLOCKR_LOCK6                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
$00000080 constant GPDMA_GPDMA_RCFGLOCKR_LOCK7                      \ lock the configuration of GPDMA_SECCFGR.SECx and GPDMA_PRIVCFGR.PRIVx, until a global GPDMA reset This bit is cleared after reset and, once set, it cannot be reset until a global GPDMA reset.
...
$00000100 constant GPDMA_GPDMA_C0SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C0SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$0000003f constant GPDMA_GPDMA_C0TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C0TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C0TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C0TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C0TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C0TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C0LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C1SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C1SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$80000000 constant GPDMA_GPDMA_C1TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$0000003f constant GPDMA_GPDMA_C1TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C1TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C1TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger. If the channel x is enabled (GPDMA_CxCR.EN asserted) with TRIGPOL[1:0] = 00 or 11, these TRIGM[1:0] bits are ignored.
$001f0000 constant GPDMA_GPDMA_C1TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C1TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C1TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C1LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C2SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C2SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C2TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$80000000 constant GPDMA_GPDMA_C2TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$0000003f constant GPDMA_GPDMA_C2TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C2TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C2TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C2TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C2TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C2TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C2LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C3SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C3SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C3TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$0000003f constant GPDMA_GPDMA_C3TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C3TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C3TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C3TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C3TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C3TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C3LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C4SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C4SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C4TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$80000000 constant GPDMA_GPDMA_C4TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$00000400 constant GPDMA_GPDMA_C4TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C4TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C4TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C4TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C4TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C4LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C5SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C5SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C5TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$80000000 constant GPDMA_GPDMA_C5TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$0000003f constant GPDMA_GPDMA_C5TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C5TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C5TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C5TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C5TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C5TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C5LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
$00000100 constant GPDMA_GPDMA_C6SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C6SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C6TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$80000000 constant GPDMA_GPDMA_C6TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$0000003f constant GPDMA_GPDMA_C6TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C6TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C6TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C6TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C6TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C6TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$00000100 constant GPDMA_GPDMA_C7SR_TCF                             \ transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM[1:0]).
$00000200 constant GPDMA_GPDMA_C7SR_HTF                             \ half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT[15:0]/2) has been transferred to the destination.
...
$00008000 constant GPDMA_GPDMA_C7TR1_SSEC                           \ security attribute of the GPDMA transfer from the source If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx =1 . A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this SSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer from the source is non-secure.
...
$80000000 constant GPDMA_GPDMA_C7TR1_DSEC                           \ security attribute of the GPDMA transfer to the destination If GPDMA_SECCFGR.SECx = 1 and the access is secure: This is a secure register bit. This bit can only be read by a secure software. This bit must be written by a secure software when GPDMA_SECCFGR.SECx = 1. A secure write is ignored when GPDMA_SECCFGR.SECx = 0. When GPDMA_SECCFGR.SECx is deasserted, this DSEC bit is also deasserted by hardware (on a secure reconfiguration of the channel as non-secure), and the GPDMA transfer to the destination is non-secure.
...
$0000003f constant GPDMA_GPDMA_C7TR2_REQSEL                         \ GPDMA hardware request selection These bits are ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else, the selected hardware request is internally taken into account as per Section 16.3.3. The user must not assign a same input hardware request (same REQSEL[5:0] value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.
...
$00000400 constant GPDMA_GPDMA_C7TR2_DREQ                           \ destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else:
...
$0000c000 constant GPDMA_GPDMA_C7TR2_TRIGM                          \ trigger mode These bits define the transfer granularity for its conditioning by the trigger.
$001f0000 constant GPDMA_GPDMA_C7TR2_TRIGSEL                        \ trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 16.3.5), with an active trigger event if TRIGPOL[1:0] different 00.
$03000000 constant GPDMA_GPDMA_C7TR2_TRIGPOL                        \ trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL[4:0].
$c0000000 constant GPDMA_GPDMA_C7TR2_TCEM                           \ transfer complete event mode These bits define the transfer granularity for the transfer complete and half transfer complete events generation. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] = 0), then neither the complete transfer event nor the half transfer event is generated. Note: If the initial LLIsub0 /subdata transfer is null/void (i.e. directly programmed by the internal register file with GPDMA_CxBR1.BNDT[15:0] =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLIsub1/sub.
...
$20000000 constant GPDMA_GPDMA_C7LLR_UB1                            \ Update GPDMA_CxBR1 from memory This bit controls the update of GPDMA_CxBR1 from the memory during the link transfer. If UB1 = 0 and if GPDMA_CxLLR different 0, the linked-list is not completed. GPDMA_CxBR1.BNDT[15:0] is then restored to the programmed value after data transfer is completed and before the link transfer.
...
\ @brief General purpose direct memory access controller
\
$40020000 constant GPDMA_GPDMA_SECCFGR  \ offset: 0x00 : GPDMA secure configuration register
$40020004 constant GPDMA_GPDMA_PRIVCFGR  \ offset: 0x04 : GPDMA privileged configuration register
$40020008 constant GPDMA_GPDMA_RCFGLOCKR  \ offset: 0x08 : GPDMA configuration lock register
$4002000c constant GPDMA_GPDMA_MISR  \ offset: 0x0C : GPDMA non-secure masked interrupt status register
$40020010 constant GPDMA_GPDMA_SMISR  \ offset: 0x10 : GPDMA secure masked interrupt status register
$40020050 constant GPDMA_GPDMA_C0LBAR  \ offset: 0x50 : GPDMA channel 0 linked-list base address register
$4002005c constant GPDMA_GPDMA_C0FCR  \ offset: 0x5C : GPDMA channel 0 flag clear register
$40020060 constant GPDMA_GPDMA_C0SR  \ offset: 0x60 : GPDMA channel 0 status register
$40020064 constant GPDMA_GPDMA_C0CR  \ offset: 0x64 : GPDMA channel 0 control register
$40020090 constant GPDMA_GPDMA_C0TR1  \ offset: 0x90 : GPDMA channel 0 transfer register 1
$40020094 constant GPDMA_GPDMA_C0TR2  \ offset: 0x94 : GPDMA channel 0 transfer register 2
$40020098 constant GPDMA_GPDMA_C0BR1  \ offset: 0x98 : GPDMA channel 0 block register 1
$4002009c constant GPDMA_GPDMA_C0SAR  \ offset: 0x9C : GPDMA channel 0 source address register
$400200a0 constant GPDMA_GPDMA_C0DAR  \ offset: 0xA0 : GPDMA channel 0 destination address register
$400200cc constant GPDMA_GPDMA_C0LLR  \ offset: 0xCC : GPDMA channel 0 linked-list address register
$400200d0 constant GPDMA_GPDMA_C1LBAR  \ offset: 0xD0 : GPDMA channel 1 linked-list base address register
$400200dc constant GPDMA_GPDMA_C1FCR  \ offset: 0xDC : GPDMA channel 1 flag clear register
$400200e0 constant GPDMA_GPDMA_C1SR  \ offset: 0xE0 : GPDMA channel 1 status register
$400200e4 constant GPDMA_GPDMA_C1CR  \ offset: 0xE4 : GPDMA channel 1 control register
$40020110 constant GPDMA_GPDMA_C1TR1  \ offset: 0x110 : GPDMA channel 1 transfer register 1
$40020114 constant GPDMA_GPDMA_C1TR2  \ offset: 0x114 : GPDMA channel 1 transfer register 2
$40020118 constant GPDMA_GPDMA_C1BR1  \ offset: 0x118 : GPDMA channel 1 block register 1
$4002011c constant GPDMA_GPDMA_C1SAR  \ offset: 0x11C : GPDMA channel 1 source address register
$40020120 constant GPDMA_GPDMA_C1DAR  \ offset: 0x120 : GPDMA channel 1 destination address register
$4002014c constant GPDMA_GPDMA_C1LLR  \ offset: 0x14C : GPDMA channel 1 linked-list address register
$40020150 constant GPDMA_GPDMA_C2LBAR  \ offset: 0x150 : GPDMA channel 2 linked-list base address register
$4002015c constant GPDMA_GPDMA_C2FCR  \ offset: 0x15C : GPDMA channel 2 flag clear register
$40020160 constant GPDMA_GPDMA_C2SR  \ offset: 0x160 : GPDMA channel 2 status register
$40020164 constant GPDMA_GPDMA_C2CR  \ offset: 0x164 : GPDMA channel 2 control register
$40020190 constant GPDMA_GPDMA_C2TR1  \ offset: 0x190 : GPDMA channel 2 transfer register 1
$40020194 constant GPDMA_GPDMA_C2TR2  \ offset: 0x194 : GPDMA channel 2 transfer register 2
$40020198 constant GPDMA_GPDMA_C2BR1  \ offset: 0x198 : GPDMA channel 2 block register 1
$4002019c constant GPDMA_GPDMA_C2SAR  \ offset: 0x19C : GPDMA channel 2 source address register
$400201a0 constant GPDMA_GPDMA_C2DAR  \ offset: 0x1A0 : GPDMA channel 2 destination address register
$400201cc constant GPDMA_GPDMA_C2LLR  \ offset: 0x1CC : GPDMA channel 2 linked-list address register
$400201d0 constant GPDMA_GPDMA_C3LBAR  \ offset: 0x1D0 : GPDMA channel 3 linked-list base address register
$400201dc constant GPDMA_GPDMA_C3FCR  \ offset: 0x1DC : GPDMA channel 3 flag clear register
$400201e0 constant GPDMA_GPDMA_C3SR  \ offset: 0x1E0 : GPDMA channel 3 status register
$400201e4 constant GPDMA_GPDMA_C3CR  \ offset: 0x1E4 : GPDMA channel 3 control register
$40020210 constant GPDMA_GPDMA_C3TR1  \ offset: 0x210 : GPDMA channel 3 transfer register 1
$40020214 constant GPDMA_GPDMA_C3TR2  \ offset: 0x214 : GPDMA channel 3 transfer register 2
$40020218 constant GPDMA_GPDMA_C3BR1  \ offset: 0x218 : GPDMA channel 3 block register 1
$4002021c constant GPDMA_GPDMA_C3SAR  \ offset: 0x21C : GPDMA channel 3 source address register
$40020220 constant GPDMA_GPDMA_C3DAR  \ offset: 0x220 : GPDMA channel 3 destination address register
$4002024c constant GPDMA_GPDMA_C3LLR  \ offset: 0x24C : GPDMA channel 3 linked-list address register
$40020250 constant GPDMA_GPDMA_C4LBAR  \ offset: 0x250 : GPDMA channel 4 linked-list base address register
$4002025c constant GPDMA_GPDMA_C4FCR  \ offset: 0x25C : GPDMA channel 4 flag clear register
$40020260 constant GPDMA_GPDMA_C4SR  \ offset: 0x260 : GPDMA channel 4 status register
$40020264 constant GPDMA_GPDMA_C4CR  \ offset: 0x264 : GPDMA channel 4 control register
$40020290 constant GPDMA_GPDMA_C4TR1  \ offset: 0x290 : GPDMA channel 4 transfer register 1
$40020294 constant GPDMA_GPDMA_C4TR2  \ offset: 0x294 : GPDMA channel 4 transfer register 2
$40020298 constant GPDMA_GPDMA_C4BR1  \ offset: 0x298 : GPDMA channel 4 block register 1
$4002029c constant GPDMA_GPDMA_C4SAR  \ offset: 0x29C : GPDMA channel 4 source address register
$400202a0 constant GPDMA_GPDMA_C4DAR  \ offset: 0x2A0 : GPDMA channel 4 destination address register
$400202cc constant GPDMA_GPDMA_C4LLR  \ offset: 0x2CC : GPDMA channel 4 linked-list address register
$400202d0 constant GPDMA_GPDMA_C5LBAR  \ offset: 0x2D0 : GPDMA channel 5 linked-list base address register
$400202dc constant GPDMA_GPDMA_C5FCR  \ offset: 0x2DC : GPDMA channel 5 flag clear register
$400202e0 constant GPDMA_GPDMA_C5SR  \ offset: 0x2E0 : GPDMA channel 5 status register
$400202e4 constant GPDMA_GPDMA_C5CR  \ offset: 0x2E4 : GPDMA channel 5 control register
$40020310 constant GPDMA_GPDMA_C5TR1  \ offset: 0x310 : GPDMA channel 5 transfer register 1
$40020314 constant GPDMA_GPDMA_C5TR2  \ offset: 0x314 : GPDMA channel 5 transfer register 2
$40020318 constant GPDMA_GPDMA_C5BR1  \ offset: 0x318 : GPDMA channel 5 block register 1
$4002031c constant GPDMA_GPDMA_C5SAR  \ offset: 0x31C : GPDMA channel 5 source address register
$40020320 constant GPDMA_GPDMA_C5DAR  \ offset: 0x320 : GPDMA channel 5 destination address register
$4002034c constant GPDMA_GPDMA_C5LLR  \ offset: 0x34C : GPDMA channel 5 linked-list address register
$40020350 constant GPDMA_GPDMA_C6LBAR  \ offset: 0x350 : GPDMA channel 6 linked-list base address register
$4002035c constant GPDMA_GPDMA_C6FCR  \ offset: 0x35C : GPDMA channel 6 flag clear register
$40020360 constant GPDMA_GPDMA_C6SR  \ offset: 0x360 : GPDMA channel 6 status register
$40020364 constant GPDMA_GPDMA_C6CR  \ offset: 0x364 : GPDMA channel 6 control register
$40020390 constant GPDMA_GPDMA_C6TR1  \ offset: 0x390 : GPDMA channel 6 transfer register 1
$40020394 constant GPDMA_GPDMA_C6TR2  \ offset: 0x394 : GPDMA channel 6 transfer register 2
$40020398 constant GPDMA_GPDMA_C6BR1  \ offset: 0x398 : GPDMA channel 6 block register 1
$4002039c constant GPDMA_GPDMA_C6SAR  \ offset: 0x39C : GPDMA channel 6 source address register
$400203a0 constant GPDMA_GPDMA_C6DAR  \ offset: 0x3A0 : GPDMA channel 6 destination address register
$400203cc constant GPDMA_GPDMA_C6LLR  \ offset: 0x3CC : GPDMA channel 6 linked-list address register
$400203d0 constant GPDMA_GPDMA_C7LBAR  \ offset: 0x3D0 : GPDMA channel 7 linked-list base address register
$400203dc constant GPDMA_GPDMA_C7FCR  \ offset: 0x3DC : GPDMA channel 7 flag clear register
$400203e0 constant GPDMA_GPDMA_C7SR  \ offset: 0x3E0 : GPDMA channel 7 status register
$400203e4 constant GPDMA_GPDMA_C7CR  \ offset: 0x3E4 : GPDMA channel 7 control register
$40020410 constant GPDMA_GPDMA_C7TR1  \ offset: 0x410 : GPDMA channel 7 transfer register 1
$40020414 constant GPDMA_GPDMA_C7TR2  \ offset: 0x414 : GPDMA channel 7 transfer register 2
$40020418 constant GPDMA_GPDMA_C7BR1  \ offset: 0x418 : GPDMA channel 7 block register 1
$4002041c constant GPDMA_GPDMA_C7SAR  \ offset: 0x41C : GPDMA channel 7 source address register
$40020420 constant GPDMA_GPDMA_C7DAR  \ offset: 0x420 : GPDMA channel 7 destination address register
$4002044c constant GPDMA_GPDMA_C7LLR  \ offset: 0x44C : GPDMA channel 7 linked-list address register
