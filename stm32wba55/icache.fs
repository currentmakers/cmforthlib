...
[ifndef] ICACHE_DEF
  ...
  [ifdef] ICACHE_ICACHE_CRR0_DEF
    ...
    \
    $00 constant ICACHE_BASEADDR                \ [0x00 : 8] base address for region x This alias address is replaced by REMAPADDR field. The only useful bits are [28:RI], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see Section 18.4.7). If the programmed value has more LSBs, the useless bits are ignored.
    $09 constant ICACHE_RSIZE                   \ [0x09 : 3] size for region x
    $0f constant ICACHE_REN                     \ [0x0f] enable for region x
    $10 constant ICACHE_REMAPADDR               \ [0x10 : 11] remapped address for region x This field replaces the alias address defined by BASEADDR field. The only useful bits are [31:RI], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see Section 18.4.7). If the programmed value has more LSBs, the useless bits are ignored.
