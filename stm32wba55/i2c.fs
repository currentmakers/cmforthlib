...
[ifndef] I2C_DEF
  ...
  [ifdef] I2C_I2C_CR1_DEF
    ...
    $06 constant I2C_TCIE                       \ [0x06] Transfer Complete interrupt enable Note: Any of these events generate an interrupt: Note: Transfer Complete (TC) Note: Transfer Complete Reload (TCR)
    $07 constant I2C_ERRIE                      \ [0x07] Error interrupts enable Note: Any of these errors generate an interrupt: Note: Arbitration Loss (ARLO) Note: Bus Error detection (BERR) Note: Overrun/Underrun (OVR) Note: Timeout detection (TIMEOUT) Note: PEC error detection (PECERR) Note: Alert pin event detection (ALERT)
    ...
    $12 constant I2C_WUPEN                      \ [0x12] Wakeup from Stop mode enable Note: If the Wakeup from Stop mode feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation. Note: WUPEN can be set only when DNF = '0000'
    $13 constant I2C_GCEN                       \ [0x13] General call enable
    $14 constant I2C_SMBHEN                     \ [0x14] SMBus host address enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
    $15 constant I2C_SMBDEN                     \ [0x15] SMBus device default address enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
    $16 constant I2C_ALERTEN                    \ [0x16] SMBus alert enable Note: When ALERTEN=0, the SMBA pin can be used as a standard GPIO. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
    $17 constant I2C_PECEN                      \ [0x17] PEC enable Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
  ...
  [ifdef] I2C_I2C_OAR2_DEF
    ...
    $08 constant I2C_OA2MSK                     \ [0x08 : 3] Own Address 2 masks Note: These bits can be written only when OA2EN=0. Note: As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
  ...
  [ifdef] I2C_I2C_TIMINGR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant I2C_SCLL                       \ [0x00 : 8] SCL low period (master mode) This field is used to generate the SCL low period in master mode. tsubSCLL /sub= (SCLL+1) x tsubPRESC/sub Note: SCLL is also used to generate tsubBUF /suband tsubSU:STA /subtimings.
    $08 constant I2C_SCLH                       \ [0x08 : 8] SCL high period (master mode) This field is used to generate the SCL high period in master mode. tsubSCLH /sub= (SCLH+1) x tsubPRESC/sub Note: SCLH is also used to generate tsubSU:STO /suband tsubHD:STA /subtiming.
    $10 constant I2C_SDADEL                     \ [0x10 : 4] Data hold time This field is used to generate the delay tsubSDADEL /subbetween SCL falling edge and SDA edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tsubSDADEL/sub. tsubSDADEL/sub= SDADEL x tsubPRESC/sub Note: SDADEL is used to generate tsubHD:DAT /subtiming.
    $14 constant I2C_SCLDEL                     \ [0x14 : 4] Data setup time This field is used to generate a delay tsubSCLDEL /subbetween SDA edge and SCL rising edge. In master mode and in slave mode with NOSTRETCH = 0, the SCL line is stretched low during tsubSCLDEL/sub. tsubSCLDEL /sub= (SCLDEL+1) x tsubPRESC/sub Note: tsubSCLDEL/sub is used to generate tsubSU:DAT /subtiming.
    $1c constant I2C_PRESC                      \ [0x1c : 4] Timing prescaler This field is used to prescale i2c_ker_ck in order to generate the clock period tsubPRESC /subused for data setup and hold counters (refer to FMPI2C timings on page 1928) and for SCL high and low level counters (refer to FMPI2C master initialization on page 1951). tsubPRESC /sub= (PRESC+1) x tsubI2CCLK/sub
  [then]
  ...
  [ifdef] I2C_I2C_TIMEOUTR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant I2C_TIMEOUTA                   \ [0x00 : 12] Bus Timeout A This field is used to configure: The SCL low timeout condition tsubTIMEOUT/sub when TIDLE=0 tsubTIMEOUT/sub= (TIMEOUTA+1) x 2048 x tsubI2CCLK/sub The bus idle condition (both SCL and SDA high) when TIDLE=1 tsubIDLE/sub= (TIMEOUTA+1) x 4 x tsubI2CCLK/sub Note: These bits can be written only when TIMOUTEN=0.
  ...
  [ifdef] I2C_I2C_ISR_DEF
    ...
    $0c constant I2C_TIMEOUT                    \ [0x0c] Timeout or tsubLOW/sub detection flag This flag is set by hardware when a timeout or extended clock timeout occurred. It is cleared by software by setting the TIMEOUTCF bit. Note: This bit is cleared by hardware when PE = 0. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
  ...
  [ifdef] I2C_I2C_ICR_DEF
    ...
    $0b constant I2C_PECCF                      \ [0x0b] PEC Error flag clear Writing 1 to this bit clears the PECERR flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
    $0c constant I2C_TIMOUTCF                   \ [0x0c] Timeout detection flag clear Writing 1 to this bit clears the TIMEOUT flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
    $0d constant I2C_ALERTCF                    \ [0x0d] Alert flag clear Writing 1 to this bit clears the ALERT flag in the I2C_ISR register. Note: If the SMBus feature is not supported, this bit is reserved and forced by hardware to '0'. Refer to Section 52.3: FMPI2C implementation.
  [then]
  ...
  [ifdef] I2C_I2C_RXDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant I2C_RXDATA                     \ [0x00 : 8] 8-bit receive data Data byte received from the Isup2/supC bus
  [then]
  ...
  [ifdef] I2C_I2C_TXDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant I2C_TXDATA                     \ [0x00 : 8] 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1.
  [then]


  [ifdef] I2C_I2C_AUTOCR_DEF
    \
    \ @brief I2C Autonomous mode control register
    \ Address offset: 0x2C
    \ Reset value: 0x00000000
    \
    $06 constant I2C_TCDMAEN                    \ [0x06] DMA request enable on Transfer Complete event
    $07 constant I2C_TCRDMAEN                   \ [0x07] DMA request enable on Transfer Complete Reload event
    $10 constant I2C_TRIGSEL                    \ [0x10 : 4] Trigger selection (refer to Section 52.4.3: FMPI2C pins and internal signals I2C interconnections tables). ... Note: This bit can be written only when PE = 0
    $14 constant I2C_TRIGPOL                    \ [0x14] Trigger polarity Note: This bit can be written only when PE = 0
    $15 constant I2C_TRIGEN                     \ [0x15] Trigger enable When a trigger is detected, a START condition is sent and the transfer is launched as defined in I2C_CR2.
  [then]
  ...
  $2C constant I2C_I2C_AUTOCR           \ I2C Autonomous mode control register
