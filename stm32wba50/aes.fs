...
$00000001 constant AES_AES_CR_EN                                    \ Enable This bit enables/disables the AES peripheral. At any moment, clearing then setting the bit re-initializes the AES peripheral. This bit is automatically cleared by hardware upon the completion of the key preparation (MODE[1:0] at 01) and upon the completion of GCM/GMAC/CCM initialization phase. The bit cannot be set as long as KEYVALID=0.
...
$00000018 constant AES_AES_CR_MODE                                  \ Operating mode This bitfield selects the AES operating mode: Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
$00000060 constant AES_AES_CR_CHMOD                                 \ CHMOD[1:0]: Chaining mode This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
$00000800 constant AES_AES_CR_DMAINEN                               \ DMA input enable When this bit is set, DMA requests are automatically generated by the peripheral during the input data phase. Setting this bit is ignored when MODE[1:0] is at 01 (key derivation).
$00001000 constant AES_AES_CR_DMAOUTEN                              \ DMA output enable When this bit is set, DMA requests are automatically generated by the peripheral during the output data phase. Setting this bit is ignored when MODE[1:0] is at 01 (key derivation).
$00006000 constant AES_AES_CR_GCMPH                                 \ GCM or CCM phase selection This bitfield selects the phase, applicable only with GCM, GMAC or CCM chaining modes. This bitfield has no effect if GCM, GMAC or CCM algorithm is not selected with CHMOD[2:0].
$00010000 constant AES_AES_CR_CHMOD_1                               \ CHMOD[2]
$00040000 constant AES_AES_CR_KEYSIZE                               \ Key size selection This bitfield defines the key length in bits of the key used by AES. Attempts to write the bit are ignored when BUSY is set, as well as when the EN is set before the write access and it is not cleared by that write access.
...
$03000000 constant AES_AES_CR_KMOD                                  \ Key mode selection The bitfield defines how the AES key can be used by the application. KEYSIZE must be correctly initialized when setting KMOD[1:0] different from zero. Others: Reserved Attempts to write the bitfield are ignored when BUSY is set, as well as when EN is set before the write access and it is not cleared by that write access.
...
$00000002 constant AES_AES_SR_RDERRF                                \ Read error flag This bit is set when an unexpected read to the AES_DOUTR register occurred. When set RDERRF bit has no impact on the AES operations. This flag is cleared by setting the RWEIF bit in AES_ICR register. If RWEIE bit is set in AES_IER register an interrupt is generated when RDERRF is set. It is cleared by setting the RWEIF bit of the AES_ICR register.
$00000004 constant AES_AES_SR_WRERRF                                \ Write error flag This bit is set when an unexpected write to the AES_DINR register occurred. When set WRERRF bit has no impact on the AES operations. This flag is cleared by setting the RWEIF bit in AES_ICR register. If RWEIE bit is set in AES_IER register an interrupt is generated when WRERRF is set. It is cleared by setting the RWEIF bit of the AES_ICR register.
$00000008 constant AES_AES_SR_BUSY                                  \ Busy This flag indicates whether AES is idle or busy. AES is flagged as idle when disabled or when the last processing is completed. AES is flagged as busy when processing a block data, preparing a key (ECB or CBC decryption only), or transferring a shared key from SAES peripheral. When GCM encryption is selected, this flag must be at zero before suspending current process to manage a higher-priority message.
...
$00000000 constant AES_AES_KEYR1_KEY                                \ Cryptographic key, bits [63:32] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR2_KEY                                \ Cryptographic key, bits [95:64] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR3_KEY                                \ Cryptographic key, bits [127:96] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR4_KEY                                \ Cryptographic key, bits [159:128] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR5_KEY                                \ Cryptographic key, bits [191:160] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR6_KEY                                \ Cryptographic key, bits [223:192] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
...
$00000000 constant AES_AES_KEYR7_KEY                                \ Cryptographic key, bits [255:224] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield. Writing to this register has no effect when key size is 128-bit (KEYSIZE=0). When KEYVALID is set in AES_SR a write to this register clears KEYVALID if EN is cleared in AES_CR.
