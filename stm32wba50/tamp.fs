...
[ifndef] TAMP_DEF
  ...
  [ifdef] TAMP_TAMP_CR2_DEF
    ...
    $16 constant TAMP_BKBLOCK                   \ [0x16] Backup registers and device secretssup(1)/sup access blocked
    $17 constant TAMP_BKERASE                   \ [0x17] Backup registers and device secretssup(1)/sup erase Writing '1' to this bit reset the backup registers and device secretssup(1)/sup. Writing 0 has no effect. This bit is always read as 0.
  ...
  [ifdef] TAMP_TAMP_ATCR1_DEF
    ...
    $18 constant TAMP_ATPER                     \ [0x18 : 3] Active tamper output change period The tamper output is changed every CK_ATPER = (2supATPER /supx CK_ATPRE) cycles. Refer to Table713: Minimum ATPER value.
    $1e constant TAMP_ATOSHARE                  \ [0x1e] Active tamper output sharing TAMP_IN1 is compared with TAMPOUTSEL1 TAMP_IN2 is compared with TAMPOUTSEL2 TAMP_IN3 is compared with TAMPOUTSEL3 TAMP_IN4 is compared with TAMPOUTSEL4 TAMP_IN5 is compared with TAMPOUTSEL5 TAMP_IN6 is compared with TAMPOUTSEL6
  ...
  [ifdef] TAMP_TAMP_SECCFGR_DEF
    ...
    $1f constant TAMP_TAMPSEC                   \ [0x1f] Tamper protection (excluding monotonic counters and backup registers) Note: Refer to Section75.3.5: TAMP secure protection modes for details on the read protection.
  [then]
  ...
  [ifdef] TAMP_TAMP_PRIVCFGR_DEF
    ...
    $1f constant TAMP_TAMPPRIV                  \ [0x1f] Tamper privilege protection (excluding backup registers) Note: Refer to Section75.3.7: TAMP privilege protection modes for details on the read protection.
  [then]
  ...
  [ifdef] TAMP_TAMP_MISR_DEF
    ...
    $16 constant TAMP_ITAMP7MF                  \ [0x16] Internal tamper 7 tamper non-secure interrupt masked flag This flag is set by hardware when the internal tamper 7 non-secure interrupt is raised.
  ...
  [ifdef] TAMP_TAMP_SMISR_DEF
    ...
    $16 constant TAMP_ITAMP7MF                  \ [0x16] Internal tamper 7 secure interrupt masked flag This flag is set by hardware when the internal tamper 7 secure interrupt is raised.
  ...
  [ifdef] TAMP_TAMP_ERCFGR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $01 constant TAMP_ERCFG1                    \ [0x01] Configurable device secrets configuration
    $02 constant TAMP_ERCFG2                    \ [0x02] Configurable device secrets configuration
    $03 constant TAMP_ERCFG3                    \ [0x03] Configurable device secrets configuration
    $04 constant TAMP_ERCFG4                    \ [0x04] Configurable device secrets configuration
    $05 constant TAMP_ERCFG5                    \ [0x05] Configurable device secrets configuration
  [then]
