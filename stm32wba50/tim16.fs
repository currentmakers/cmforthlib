...
$00000300 constant TIM16_TIM16_CR1_CKD                              \ Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and the dead-time and sampling clock (tsubDTS/sub)used by the dead-time generators and the digital filters (tim_tix),
...
$0000000c constant TIM16_TIM16_CCMR1_IC1PSC                         \ Input capture 1 prescaler This bit-field defines the ratio of the prescaler acting on CC1 input (tim_ic1). The prescaler is reset as soon as CC1E='0' (TIMx_CCER register).
...
$00000003 constant TIM16_TIM16_CCMR1_ALTERNATE1_CC1S                \ Capture/Compare 1 selection This bit-field defines the direction of the channel (input/output) as well as the used input. Others: Reserved Note: CC1S bits are writable only when the channel is OFF (CC1E = '0' in TIMx_CCER).
$00000004 constant TIM16_TIM16_CCMR1_ALTERNATE1_OC1FE               \ Output Compare 1 fast enable This bit decreases the latency between a trigger event and a transition on the timer output. It must be used in one-pulse mode (OPM bit set in TIMx_CR1 register), to have the output pulse starting as soon as possible after the starting trigger.
$00000008 constant TIM16_TIM16_CCMR1_ALTERNATE1_OC1PE               \ Output Compare 1 preload enable Note: These bits can not be modified as long as LOCK level 3 has been programmed (LOCK bits in TIMx_BDTR register) and CC1S='00' (the channel is configured in output).
$00000070 constant TIM16_TIM16_CCMR1_ALTERNATE1_OC1M                \ OC1M[2:0]: Output Compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 and tim_oc1n are derived. tim_oc1ref is active high whereas tim_oc1 and tim_oc1n active level depends on CC1P and CC1NP bits. Others: Reserved Note: These bits can not be modified as long as LOCK level 3 has been programmed (LOCK bits in TIMx_BDTR register) and CC1S='00' (the channel is configured in output). Note: In PWM mode 1 or 2, the tim_oc1ref level changes only when the result of the comparison changes or when the output compare mode switches from 'frozen' mode to 'PWM' mode.
$00000080 constant TIM16_TIM16_CCMR1_ALTERNATE1_OC1CE               \ Output Compare 1 clear enable
...
$00000001 constant TIM16_TIM16_CCER_CC1E                            \ Capture/Compare 1 output enable When CC1 channel is configured as output, the OC1 level depends on MOE, OSSI, OSSR, OIS1, OIS1N and CC1NE bits, regardless of the CC1E bits state. Refer to Table 305 for details.
...
$0000000f constant TIM16_TIM16_TISEL_TI1SEL                         \ selects tim_ti1_in[15:0] input ... Refer to Section 30.3.2: TIM16/TIM17 pins and internal signals for interconnects list.
...
$00070000 constant TIM16_TIM16_AF2_OCRSEL                           \ tim_ocref_clr source selection These bits select the tim_ocref_clr input source. Refer to Section 30.3.2: TIM16/TIM17 pins and internal signals for product specific implementation. Note: These bits can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
...
$00000002 constant TIM16_TIM16_OR1_HSE32EN                          \ HSE Divided by 32 enable This bit enables the HSE divider by 32 for the tim_ti1_in3. See Table 296: Interconnect to the tim_ti1 input multiplexer for details.
...
$40014418 constant TIM16_TIM16_CCMR1  \ offset: 0x18 : TIM16 capture/compare mode register 1 [alternate]
$40014418 constant TIM16_TIM16_CCMR1_ALTERNATE1  \ offset: 0x18 : TIM16 capture/compare mode register 1 [alternate]
...
$40014468 constant TIM16_TIM16_OR1  \ offset: 0x68 : TIM16 option register 1
