...
$0000ffff constant OTG_HS_GLOBAL_OTG_DIEPTXF8_INEPTXSA              \ IN endpoint FIFOx transmit RAM start address This field contains the memory start address for IN endpoint transmit FIFOx. The address must be aligned with a 32-bit memory location.
$ffff0000 constant OTG_HS_GLOBAL_OTG_DIEPTXF8_INEPTXFD              \ IN endpoint Tx FIFO depth This value is in terms of 32-bit words. Minimum value is 16
...
$40040120 constant OTG_HS_GLOBAL_OTG_DIEPTXF8  \ offset: 0x120 : IN endpoint FIFOx transmit RAM start address This field contains the memory start address for IN endpoint transmit FIFOx. The address must be aligned with a 32-bit memory location.
