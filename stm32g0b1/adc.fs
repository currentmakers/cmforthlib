...
[ifndef] ADC_DEF
  ...
  [ifdef] ADC_ADC_AWD1TR_DEF
    ...
    \
    $00 constant ADC_LT1                        \ [0x00 : 12] Analog watchdog 1 lower threshold These bits are written by software to define the lower threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
    $10 constant ADC_HT1                        \ [0x10 : 12] Analog watchdog 1 higher threshold These bits are written by software to define the higher threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
  [then]
  ...
  [ifdef] ADC_ADC_AWD2TR_DEF
    ...
    \ Reset value: 0x0FFF0000
    \
    $00 constant ADC_LT2                        \ [0x00 : 12] Analog watchdog 2 lower threshold These bits are written by software to define the lower threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
    $10 constant ADC_HT2                        \ [0x10 : 12] Analog watchdog 2 higher threshold These bits are written by software to define the higher threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
  [then]
  ...
  [ifdef] ADC_ADC_AWD3TR_DEF
    ...
    \ Reset value: 0x0FFF0000
    \
    $00 constant ADC_LT3                        \ [0x00 : 12] Analog watchdog 3lower threshold These bits are written by software to define the lower threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
    $10 constant ADC_HT3                        \ [0x10 : 12] Analog watchdog 3 higher threshold These bits are written by software to define the higher threshold for the analog watchdog. Refer to ADC_AWDxTR) on page407.
  [then]
  ...
  [ifdef] ADC_ADC_DR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant ADC_DATA                       \ [0x00 : 16] Converted data These bits are read-only. They contain the conversion result from the last converted channel. The data are left- or right-aligned as shown in OVSE = 0) on page401. Just after a calibration is complete, DATA[6:0] contains the calibration factor.
  [then]
  ...
  $28 constant ADC_ADC_CHSELR           \ ADC channel selection register [alternate]
