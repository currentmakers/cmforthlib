...
[ifndef] FDCAN_DEF
  ...
  [ifdef] FDCAN_FDCAN_ENDN_DEF
    ...
    \
    $00 constant FDCAN_ETV                      \ [0x00 : 32] Endianness test value The endianness test value is 0x8765 4321.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_DBTP_DEF
    ...
    \
    $00 constant FDCAN_DSJW                     \ [0x00 : 4] Synchronization jump width Must always be smaller than DTSEG2, valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1: tSJW = (DSJW + 1) x tq.
    $04 constant FDCAN_DTSEG2                   \ [0x04 : 4] Data time segment after sample point Valid values are 0 to 15. The value used by the hardware is the one programmed, incremented by 1, i.e. tBS2 = (DTSEG2 + 1) x tq.
    $08 constant FDCAN_DTSEG1                   \ [0x08 : 5] Data time segment before sample point Valid values are 0 to 31. The value used by the hardware is the one programmed, incremented by 1, i.e. tBS1 = (DTSEG1 + 1) x tq.
    $10 constant FDCAN_DBRP                     \ [0x10 : 5] Data bit rate prescaler The value by which the oscillator frequency is divided to generate the bit time quanta. The bit time is built up from a multiple of this quanta. Valid values for the Baud Rate Prescaler are 0 to 31. The hardware interpreters this value as the value programmed plus 1.
  ...
  [ifdef] FDCAN_FDCAN_TEST_DEF
    ...
    $07 constant FDCAN_RX                       \ [0x07] Receive pin Monitors the actual value of pin FDCANx_RX
  [then]
  ...
  [ifdef] FDCAN_FDCAN_RWD_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_WDC                      \ [0x00 : 8] Watchdog configuration Start value of the message RAM watchdog counter. With the reset value of 00, the counter is disabled. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of FDCAN_CCCR register are set to 1.
    $08 constant FDCAN_WDV                      \ [0x08 : 8] Watchdog value Actual message RAM watchdog counter value.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_CCCR_DEF
    ...
    $02 constant FDCAN_ASM                      \ [0x02] ASM restricted operation mode The restricted operation mode is intended for applications that adapt themselves to different CAN bit rates. The application tests different bit rates and leaves the Restricted Operation Mode after it has received a valid frame. In the optional Restricted Operation Mode the node is able to transmit and receive data and remote frames and it gives acknowledge to valid frames, but it does not send active error frames or overload frames. In case of an error condition or overload condition, it does not send dominant bits, instead it waits for the occurrence of bus idle condition to resynchronize itself to the CAN communication. The error counters are not incremented. Bit ASM can only be set by software when both CCE and INIT are set to 1. The bit can be reset by the software at any time.
    $03 constant FDCAN_CSA                      \ [0x03] Clock stop acknowledge
    $04 constant FDCAN_CSR                      \ [0x04] Clock stop request
    $05 constant FDCAN_MON                      \ [0x05] Bus monitoring mode Bit MON can only be set by software when both CCE and INIT are set to 1. The bit can be reset by the Host at any time.
    ...
    $0f constant FDCAN_NISO                     \ [0x0f] Non ISO operation If this bit is set, the FDCAN uses the CAN FD frame format as specified by the Bosch CAN FD Specification V1.0.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_NBTP_DEF
    ...
    \
    $00 constant FDCAN_NTSEG2                   \ [0x00 : 7] Nominal time segment after sample point Valid values are 0 to 127. The actual interpretation by the hardware of this value is such that one more than the programmed value is used.
    $08 constant FDCAN_NTSEG1                   \ [0x08 : 8] Nominal time segment before sample point Valid values are 0 to 255. The actual interpretation by the hardware of this value is such that one more than the programmed value is used. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $10 constant FDCAN_NBRP                     \ [0x10 : 9] Bit rate prescaler Value by which the oscillator frequency is divided for generating the bit time quanta. The bit time is built up from a multiple of this quanta. Valid values are 0 to 511. The actual interpretation by the hardware of this value is such that one more than the value programmed here is used. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $19 constant FDCAN_NSJW                     \ [0x19 : 7] Nominal (re)synchronization jump width Valid values are 0 to 127. The actual interpretation by the hardware of this value is such that the used value is the one programmed incremented by one. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TOCC_DEF
    ...
    \
    $00 constant FDCAN_ETOC                     \ [0x00] Timeout counter enable This is a protected write (P) bit, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $01 constant FDCAN_TOS                      \ [0x01 : 2] Timeout select When operating in Continuous mode, a write to TOCV presets the counter to the value configured by TOCC[TOP] and continues down-counting. When the timeout counter is controlled by one of the FIFOs, an empty FIFO presets the counter to the value configured by TOCC[TOP]. Down-counting is started when the first FIFO element is stored. These are protected write (P) bits, write access is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $10 constant FDCAN_TOP                      \ [0x10 : 16] Timeout period Start value of the timeout counter (down-counter). Configures the timeout period.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_ECR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_TEC                      \ [0x00 : 8] Transmit error counter Actual state of the transmit error counter, values between 0 and 255. When CCCR.ASM is set, the CAN protocol controller does not increment TEC and REC when a CAN protocol error is detected, but CEL is still incremented.
    $08 constant FDCAN_REC                      \ [0x08 : 7] Receive error counter Actual state of the receive error counter, values between 0 and 127.
    $0f constant FDCAN_RP                       \ [0x0f] Receive error passive
    $10 constant FDCAN_CEL                      \ [0x10 : 8] CAN error logging The counter is incremented each time when a CAN protocol error causes the transmit error counter or the receive error counter to be incremented. It is reset by read access to CEL. The counter stops at 0xFF; the next increment of TEC or REC sets interrupt flag IR[ELO]. Access type is RX: reset on read.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_PSR_DEF
    ...
    \
    $00 constant FDCAN_LEC                      \ [0x00 : 3] Last error code The LEC indicates the type of the last error to occur on the CAN bus. This field is cleared to 0 when a message has been transferred (reception or transmission) without error. Access type is RS: set on read.
    $03 constant FDCAN_ACT                      \ [0x03 : 2] Activity Monitors the module's CAN communication state.
    $05 constant FDCAN_EP                       \ [0x05] Error passive
    $06 constant FDCAN_EW                       \ [0x06] Warning Sstatus
    $07 constant FDCAN_BO                       \ [0x07] Bus_Off status
    $08 constant FDCAN_DLEC                     \ [0x08 : 3] Data last error code Type of last error that occurred in the data phase of a FDCAN format frame with its BRS flag set. Coding is the same as for LEC. This field is cleared to 0 when a FDCAN format frame with its BRS flag set has been transferred (reception or transmission) without error. Access type is RS: set on read.
    $0b constant FDCAN_RESI                     \ [0x0b] ESI flag of last received FDCAN message This bit is set together with REDL, independent of acceptance filtering. Access type is RX: reset on read.
    $0c constant FDCAN_RBRS                     \ [0x0c] BRS flag of last received FDCAN message This bit is set together with REDL, independent of acceptance filtering. Access type is RX: reset on read.
    $0d constant FDCAN_REDL                     \ [0x0d] Received FDCAN message This bit is set independent of acceptance filtering. Access type is RX: reset on read.
    $0e constant FDCAN_PXE                      \ [0x0e] Protocol exception event
    $10 constant FDCAN_TDCV                     \ [0x10 : 7] Transmitter delay compensation value Position of the secondary sample point, defined by the sum of the measured delay from FDCAN_TX to FDCAN_RX and TDCR.TDCO. The SSP position is, in the data phase, the number of minimum time quanta (mtq) between the start of the transmitted bit and the secondary sample point. Valid values are 0 to 127 mtq.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TDCR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_TDCF                     \ [0x00 : 7] Transmitter delay compensation filter window length Defines the minimum value for the SSP position, dominant edges on FDCAN_RX that would result in an earlier SSP position are ignored for transmitter delay measurements. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $08 constant FDCAN_TDCO                     \ [0x08 : 7] Transmitter delay compensation offset Offset value defining the distance between the measured delay from FDCAN_TX to FDCAN_RX and the secondary sample point. Valid values are 0 to 127 mtq. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_ILS_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_RxFIFO0                  \ [0x00] RX FIFO bit grouping the following interruption RF0LL: Rx FIFO 0 message lost interrupt line RF0FL: Rx FIFO 0 full interrupt line RF0NL: Rx FIFO 0 new message interrupt line
    $01 constant FDCAN_RxFIFO1                  \ [0x01] RX FIFO bit grouping the following interruption RF1LL: Rx FIFO 1 message lost interrupt line RF1FL: Rx FIFO 1 full Interrupt line RF1NL: Rx FIFO 1 new message interrupt line
    $02 constant FDCAN_SMSG                     \ [0x02] Status message bit grouping the following interruption TCFL: Transmission cancellation finished interrupt line TCL: Transmission completed interrupt line HPML: High-priority message interrupt line
    $03 constant FDCAN_TFERR                    \ [0x03] Tx FIFO ERROR grouping the following interruption TEFLL: Tx event FIFO element lost interrupt line TEFFL: Tx event FIFO full interrupt line TEFNL: Tx event FIFO new entry interrupt line TFEL: Tx FIFO empty interrupt line
    $04 constant FDCAN_MISC                     \ [0x04] Interrupt regrouping the following interruption TOOL: Timeout occurred interrupt line MRAFL: Message RAM access failure interrupt line TSWL: Timestamp wraparound interrupt line
    $05 constant FDCAN_BERR                     \ [0x05] BERR
    $06 constant FDCAN_PERR                     \ [0x06] Protocol error grouping the following interruption ARAL: Access to reserved address line PEDL: Protocol error in data phase line PEAL: Protocol error in arbitration phase line WDIL: Watchdog interrupt line BOL: Bus_Off status EWL: Warning status interrupt line
  [then]
  ...
  [ifdef] FDCAN_FDCAN_RXGFC_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_RRFE                     \ [0x00] Reject remote frames extended These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $01 constant FDCAN_RRFS                     \ [0x01] Reject remote frames standard These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $02 constant FDCAN_ANFE                     \ [0x02 : 2] Accept non-matching frames extended Defines how received messages with 29-bit IDs that do not match any element of the filter list are treated. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $04 constant FDCAN_ANFS                     \ [0x04 : 2] Accept Non-matching frames standard Defines how received messages with 11-bit IDs that do not match any element of the filter list are treated. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $08 constant FDCAN_F1OM                     \ [0x08] FIFO 1 operation mode (overwrite or blocking) This is a protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $09 constant FDCAN_F0OM                     \ [0x09] FIFO 0 operation mode (overwrite or blocking) This is protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $10 constant FDCAN_LSS                      \ [0x10 : 5] List size standard >28: Values greater than 28 are interpreted as 28. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
    $18 constant FDCAN_LSE                      \ [0x18 : 4] List size extended >8: Values greater than 8 are interpreted as 8. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_HPMS_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_BIDX                     \ [0x00 : 3] Buffer index Index of Rx FIFO element to which the message was stored. Only valid when MSI[1] = 1.
    $06 constant FDCAN_MSI                      \ [0x06 : 2] Message storage indicator
    $08 constant FDCAN_FIDX                     \ [0x08 : 5] Filter index Index of matching filter element. Range is 0 to RXGFC[LSS] - 1 or RXGFC[LSE] - 1.
    $0f constant FDCAN_FLST                     \ [0x0f] Filter list Indicates the filter list of the matching filter element.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_RXF0S_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_F0FL                     \ [0x00 : 4] Rx FIFO 0 fill level Number of elements stored in Rx FIFO 0, range 0 to 3.
    $08 constant FDCAN_F0GI                     \ [0x08 : 2] Rx FIFO 0 get index Rx FIFO 0 read index pointer, range 0 to 2.
    $10 constant FDCAN_F0PI                     \ [0x10 : 2] Rx FIFO 0 put index Rx FIFO 0 write index pointer, range 0 to 2.
    $18 constant FDCAN_F0F                      \ [0x18] Rx FIFO 0 full
    $19 constant FDCAN_RF0L                     \ [0x19] Rx FIFO 0 message lost This bit is a copy of interrupt flag IR[RF0L]. When IR[RF0L] is reset, this bit is also reset.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_RXF1S_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_F1FL                     \ [0x00 : 4] Rx FIFO 1 fill level Number of elements stored in Rx FIFO 1, range 0 to 3.
    $08 constant FDCAN_F1GI                     \ [0x08 : 2] Rx FIFO 1 get index Rx FIFO 1 read index pointer, range 0 to 2.
    $10 constant FDCAN_F1PI                     \ [0x10 : 2] Rx FIFO 1 put index Rx FIFO 1 write index pointer, range 0 to 2.
    $18 constant FDCAN_F1F                      \ [0x18] Rx FIFO 1 full
    $19 constant FDCAN_RF1L                     \ [0x19] Rx FIFO 1 message lost This bit is a copy of interrupt flag IR[RF1L]. When IR[RF1L] is reset, this bit is also reset.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXBC_DEF
    ...
    \ Reset value: 0x00000000
    \
    $18 constant FDCAN_TFQM                     \ [0x18] Tx FIFO/queue mode This is a protected write (P) bit, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXFQS_DEF
    ...
    \
    $00 constant FDCAN_TFFL                     \ [0x00 : 3] Tx FIFO free level Number of consecutive free Tx FIFO elements starting from TFGI, range 0 to 3. Read as 0 when Tx queue operation is configured (TXBC[TFQM] = 1).
    $08 constant FDCAN_TFGI                     \ [0x08 : 2] Tx FIFO get index Tx FIFO read index pointer, range 0 to 3. Read as 0 when Tx queue operation is configured (TXBC.TFQM = 1)
    $10 constant FDCAN_TFQPI                    \ [0x10 : 2] Tx FIFO/queue put index Tx FIFO/queue write index pointer, range 0 to 3
  ...
  [ifdef] FDCAN_FDCAN_TXBTO_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_TO                       \ [0x00 : 3] Transmission occurred. Each Tx buffer has its own TO bit. The bits are set when the corresponding TXBRP bit is cleared after a successful transmission. The bits are reset when a new transmission is requested by writing a 1 to the corresponding bit of register TXBAR.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXBCF_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_CF                       \ [0x00 : 3] Cancellation finished Each Tx buffer has its own CF bit. The bits are set when the corresponding TXBRP bit is cleared after a cancellation was requested via TXBCR. In case the corresponding TXBRP bit was not set at the point of cancellation, CF is set immediately. The bits are reset when a new transmission is requested by writing a 1 to the corresponding bit of register TXBAR.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXBTIE_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_TIE                      \ [0x00 : 3] Transmission interrupt enable Each Tx buffer has its own TIE bit.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXBCIE_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_CFIE                     \ [0x00 : 3] Cancellation finished interrupt enable. Each Tx buffer has its own CFIE bit.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXEFS_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_EFFL                     \ [0x00 : 3] Event FIFO fill level Number of elements stored in Tx event FIFO, range 0 to 3.
    $08 constant FDCAN_EFGI                     \ [0x08 : 2] Event FIFO get index Tx Event FIFO read index pointer, range 0 to 3.
    $10 constant FDCAN_EFPI                     \ [0x10 : 2] Event FIFO put index Tx Event FIFO write index pointer, range 0 to 3.
    $18 constant FDCAN_EFF                      \ [0x18] Event FIFO full
    $19 constant FDCAN_TEFL                     \ [0x19] Tx Event FIFO element lost This bit is a copy of interrupt flag IR[TEFL]. When IR[TEFL] is reset, this bit is also reset. 0 No Tx event FIFO element lost 1 Tx event FIFO element lost, also set after write attempt to Tx Event FIFO of size 0.
  [then]
  ...
  [ifdef] FDCAN_FDCAN_TXEFA_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_EFAI                     \ [0x00 : 2] Event FIFO acknowledge index After the Host has read an element or a sequence of elements from the Tx event FIFO, it has to write the index of the last element read from Tx event FIFO to EFAI. This sets the Tx event FIFO get index TXEFS[EFGI] to EFAI + 1 and updates the FIFO 0 fill level TXEFS[EFFL].
  [then]
  ...
  [ifdef] FDCAN_FDCAN_CKDIV_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant FDCAN_PDIV                     \ [0x00 : 4] input clock divider The APB clock could be divided prior to be used by the CAN sub system. The rate must be computed using the divider output clock. These are protected write (P) bits, which means that write access by the bits is possible only when the bit 1 [CCE] and bit 0 [INIT] of CCCR register are set to 1.
  [then]
