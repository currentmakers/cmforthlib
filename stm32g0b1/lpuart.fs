...
[ifndef] LPUART_DEF
  ...
  [ifdef] LPUART_LPUART_CR1_enabled_DEF
    \
    ...
    $0b constant LPUART_WAKE                    \ [0x0b] Receiver wakeup method This bit determines the LPUART wakeup method from Mute mode. It is set or cleared by software. This bitfield can only be written when the LPUART is disabled (UE=0).
  ...
  [ifdef] LPUART_LPUART_CR1_disabled_DEF
    \
    \ @brief LPUART control register 1 [alternate]
    \ Address offset: 0x00
    \ Reset value: 0x00000000
  ...
  [ifdef] LPUART_LPUART_CR3_DEF
    ...
    $14 constant LPUART_WUS                     \ [0x14 : 2] Wakeup from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wakeup from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the LPUART does not support the wakeup from Stop feature, this bit is reserved and must be kept at reset value. Refer to .
    $16 constant LPUART_WUFIE                   \ [0x16] Wakeup from low-power mode interrupt enable This bit is set and cleared by software. Note: WUFIE must be set before entering in low-power mode. If the LPUART does not support the wakeup from Stop feature, this bit is reserved and must be kept at reset value. Refer to .
  ...
  [ifdef] LPUART_LPUART_BRR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART_BRR                     \ [0x00 : 20] LPUART baud rate
  [then]
  ...
  [ifdef] LPUART_LPUART_ISR_enabled_DEF
    \
    ...
    $0a constant LPUART_CTS                     \ [0x0a] CTS flag This bit is set/reset by hardware. It is an inverted copy of the status of the nCTS input pin. Note: If the hardware flow control feature is not supported, this bit is reserved and kept at reset value.
    ...
    $18 constant LPUART_RXFF                    \ [0x18] RXFIFO full This bit is set by hardware when the number of received data corresponds to RXFIFOsize+1 (RXFIFO full + 1 data in the LPUART_RDR register. An interrupt is generated if the RXFFIE bit =1 in the LPUART_CR1 register.
  ...
  [ifdef] LPUART_LPUART_ISR_disabled_DEF
    \
    \ @brief LPUART interrupt and status register [alternate]
    \ Address offset: 0x1C
    \ Reset value: 0x008000C0
  ...
  [ifdef] LPUART_LPUART_ICR_DEF
    ...
    $14 constant LPUART_WUCF                    \ [0x14] Wakeup from low-power mode clear flag Writing 1 to this bit clears the WUF flag in the LPUART_ISR register. Note: If the LPUART does not support the wakeup from Stop feature, this bit is reserved and kept at reset value. Refer to .
  [then]
  ...
  [ifdef] LPUART_LPUART_RDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART_RDR                     \ [0x00 : 9] Receive data value Contains the received data character. The RDR register provides the parallel interface between the input shift register and the internal bus (see ). When receiving with the parity enabled, the value read in the MSB bit is the received parity bit.
  [then]
  ...
  [ifdef] LPUART_LPUART_PRESC_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant LPUART_PRESCALER               \ [0x00 : 4] Clock prescaler The LPUART input clock can be divided by a prescaler: Remaining combinations: Reserved. Note: When PRESCALER is programmed with a value different of the allowed ones, programmed prescaler value is 1011 i.e. input clock divided by 256.
  [then]

  \
  \ @brief Low-power universal asynchronous receiver transmitter
  \
  $00 constant LPUART_LPUART_CR1_ENABLED    \ LPUART control register 1 [alternate]
  $00 constant LPUART_LPUART_CR1_DISABLED    \ LPUART control register 1 [alternate]
  ...
  $1C constant LPUART_LPUART_ISR_ENABLED    \ LPUART interrupt and status register [alternate]
  $1C constant LPUART_LPUART_ISR_DISABLED    \ LPUART interrupt and status register [alternate]
