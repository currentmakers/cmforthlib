...
[ifndef] SAI_DEF
  ...
  [ifdef] SAI_SAI_ACR1_DEF
    ...
    \
    $00 constant SAI_MODE                       \ [0x00 : 2] SAIx audio block mode immediately
    ...
    $0a constant SAI_SYNCEN                     \ [0x0a : 2] Synchronization enable. These bits are set and cleared by software. They must be configured when the audio sub-block is disabled. Note: The audio sub-block should be configured as asynchronous when SPDIF mode is enabled.
    ...
    $13 constant SAI_NOMCK                      \ [0x13] No divider
    $14 constant SAI_MCKDIV                     \ [0x14 : 4] Master clock divider. These bits are set and cleared by software. These bits are meaningless when the audio block operates in slave mode. They have to be configured when the audio block is disabled. Others: the master clock frequency is calculated accordingly to the following formula:
  ...
  [ifdef] SAI_SAI_ACR2_DEF
    ...
    $07 constant SAI_MUTECNT                    \ [0x07 : 6] Mute counter. These bits are set and cleared by software. They are used only in reception mode. The value set in these bits is compared to the number of consecutive mute frames detected in reception. When the number of mute frames is equal to this value, the flag MUTEDET will be set and an interrupt will be generated if bit MUTEDETIE is set. Refer to Section: Mute mode for more details.
  ...
  [ifdef] SAI_SAI_AFRCR_DEF
    ...
    \
    $00 constant SAI_FRL                        \ [0x00 : 8] Frame length. These bits are set and cleared by software. They define the audio frame length expressed in number of SCK clock cycles: the number of bits in the frame is equal to FRL[7:0] + 1. The minimum number of bits to transfer in an audio frame must be equal to 8, otherwise the audio block will behaves in an unexpected way. This is the case when the data size is 8 bits and only one slot 0 is defined in NBSLOT[4:0] of SAI_xSLOTR register (NBSLOT[3:0] = 0000). In master mode, if the master clock (available on MCLK_x pin) is used, the frame length should be aligned with a number equal to a power of 2, ranging from 8 to 256. When the master clock is not used (NODIV = 1), it is recommended to program the frame length to an value ranging from 8 to 256. These bits are meaningless and are not used in AC97 or SPDIF audio block configuration.
  ...
  [ifdef] SAI_SAI_ASR_DEF
    ...
    $10 constant SAI_FLVL                       \ [0x10 : 3] FIFO level threshold. This bit is read only. The FIFO level threshold flag is managed only by hardware and its setting depends on SAI block configuration (transmitter or receiver mode). If the SAI block is configured as transmitter: If SAI block is configured as receiver:
  [then]
  ...
  [ifdef] SAI_SAI_PDMCR_DEF
    ...
    $0a constant SAI_CKEN3                      \ [0x0a] Clock enable of bitstream clock number 3
    $0b constant SAI_CKEN4                      \ [0x0b] Clock enable of bitstream clock number 4
  [then]
  ...
  $0C constant SAI_SAI_AFRCR            \ This register has no meaning in AC97 and SPDIF audio protocol
  $10 constant SAI_SAI_ASLOTR           \ This register has no meaning in AC97 and SPDIF audio protocol
  $14 constant SAI_SAI_AIM              \ Interrupt mask register 2
  ...
  $2C constant SAI_SAI_BFRCR            \ This register has no meaning in AC97 and SPDIF audio protocol
  $30 constant SAI_SAI_BSLOTR           \ This register has no meaning in AC97 and SPDIF audio protocol
  $34 constant SAI_SAI_BIM              \ Interrupt mask register 2
