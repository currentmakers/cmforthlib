...
$0000000c constant GPIOB_PUPDR_PUPD1                                \ PUPD1: Port B configuration bits These bits are written by software to configure the I/O pull-up or pull-down 00: No pullup, pulldown 01: Pullup 10: Pulldown 11: Reserved
$00000030 constant GPIOB_PUPDR_PUPD2                                \ PUPD2: Port B configuration bits These bits are written by software to configure the I/O pullup or pulldown 00: No pullup, pulldown 01: Pullup 10: Pulldown 11: Reserved
...
$00000001 constant GPIOB_BSRR_BS0                                   \ BS0: Port B set bit 0 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000002 constant GPIOB_BSRR_BS1                                   \ BS1: Port B set bit 1 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000004 constant GPIOB_BSRR_BS2                                   \ BS2: Port B set bit 2 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000008 constant GPIOB_BSRR_BS3                                   \ BS3: Port B set bit 3 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000010 constant GPIOB_BSRR_BS4                                   \ BS4: Port B set bit 4 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000020 constant GPIOB_BSRR_BS5                                   \ BS5: Port B set bit 5 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000040 constant GPIOB_BSRR_BS6                                   \ BS6: Port B set bit 6 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00000080 constant GPIOB_BSRR_BS7                                   \ BS7: Port B set bit 7 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00001000 constant GPIOB_BSRR_BS12                                  \ BS12: Port B set bit 12 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00002000 constant GPIOB_BSRR_BS13                                  \ BS13: Port B set bit 13 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00004000 constant GPIOB_BSRR_BS14                                  \ BS14: Port B set bit 14 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00008000 constant GPIOB_BSRR_BS15                                  \ BS15: Port B set bit 15 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Sets the corresponding ODx bit
$00010000 constant GPIOB_BSRR_BR0                                   \ BR0: Port B reset bit 0 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00020000 constant GPIOB_BSRR_BR1                                   \ BR1: Port B reset bit 1 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00040000 constant GPIOB_BSRR_BR2                                   \ BR2: Port B reset bit 2 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00080000 constant GPIOB_BSRR_BR3                                   \ BR3: Port B reset bit 3 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00100000 constant GPIOB_BSRR_BR4                                   \ BR4: Port B reset bit 4 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00200000 constant GPIOB_BSRR_BR5                                   \ BR5: Port B reset bit 5 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00400000 constant GPIOB_BSRR_BR6                                   \ BR6: Port B reset bit 6 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$00800000 constant GPIOB_BSRR_BR7                                   \ BR7: Port B reset bit 7 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$10000000 constant GPIOB_BSRR_BR12                                  \ BR12: Port B reset bit 12 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$20000000 constant GPIOB_BSRR_BR13                                  \ BR13: Port B reset bit 13 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$40000000 constant GPIOB_BSRR_BR14                                  \ BR14: Port B reset bit 14 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
$80000000 constant GPIOB_BSRR_BR15                                  \ BR15: Port B reset bit 15 These bits are write-only. A read to these bits returns the value 0x0000. -0: No action on the corresponding ODx bit -1: Resets the corresponding ODx bit Note: If both BSx and BRx are set, BSx has priority
