...
[ifndef] MDIOS_DEF
  ...
  [ifdef] MDIOS_MDIOS_CR_DEF
    ...
    $01 constant MDIOS_WRIE                     \ [0x01] register write interrupt enable Note: When this bit is set, an interrupt is generated if any of the read flags (WRF[31:0] in the MDIOS_WRFR register) is set.
    $02 constant MDIOS_RDIE                     \ [0x02] register read interrupt enable Note: When this bit is set, an interrupt is generated if any of the read flags (RDF[31:0] in the MDIOS_RDFR register) is set.
    $03 constant MDIOS_EIE                      \ [0x03] error interrupt enable Note: When this bit is set, an interrupt is generated if any of the error flags (PERF, SERF, or TERF in the MDIOS_SR register) is set.
    $07 constant MDIOS_DPC                      \ [0x07] disable preamble check Note: When this bit is set, the application must be sure that no frame is currently in progress when the MDIOS is enabled. Otherwise, the MDIOS can become desynchronized with the master. Note: This bit cannot be changed unless EN = 0 (though it can be changed at the same time that EN is being set).
    $08 constant MDIOS_PORT_ADDRESS             \ [0x08 : 5] slave address Can be written only when the peripheral is disabled (EN = 0). If the address given by the MDIO master matches PORT_ADRESS[4:0], then the MDIOS services the frame. Otherwise the frame is ignored.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_CWRFR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_CWRF                     \ [0x00 : 32] clear the write flag Writing 1 to CWRFx clears the WRFx bit in the MDIOS_WRF register.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_CRDFR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_CRDF                     \ [0x00 : 32] clear the read flag Writing 1 to CRDFx clears the RDFx bit in the MDIOS_RDF register.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_SR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_PERF                     \ [0x00] preamble error flag Note: Writing 1 to CPERF (MDIOS_CLRFR) clears this bit. Note: This bit is not set if DPC (disable preamble check, MDIOS_CR[7]) is set.
    $01 constant MDIOS_SERF                     \ [0x01] start error flag Note: Writing 1 to CSERF (MDIOS_CLRFR) clears this bit.
    $02 constant MDIOS_TERF                     \ [0x02] turnaround error flag Note: Writing 1 to CTERF (MDIOS_CLRFR) clears this bit.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_CLRFR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_CPERF                    \ [0x00] clear the preamble error flag Writing 1 to this bit clears the PERF flag (in MDIOS_SR).
    $01 constant MDIOS_CSERF                    \ [0x01] clear the start error flag Writing 1 to this bit clears the SERF flag (in MDIOS_SR). When DPC = 1 (MDIOS_CR[7]), the SERF flag must be cleared only when there is not a frame already in progress.
    $02 constant MDIOS_CTERF                    \ [0x02] clear the turnaround error flag Writing 1 to this bit clears the TERF flag (in MDIOS_SR). When DPC = 1 (MDIOS_CR[7]), the TERF flag must be cleared only when there is not a frame already in progress.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_DINR0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_DIN                      \ [0x00 : 16] input data received from MDIO master during write frames This field is written by hardware with the 16-bit data received in a write frame which is addressed to MDIOS register x.
  [then]
  ...
  [ifdef] MDIOS_MDIOS_DOUTR0_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant MDIOS_DOUT                     \ [0x00 : 16] output data sent to MDIO Master during read frames This field is written by software. These 16 bits are serially output on the MDIO bus during read frames which address the MDIOS register x.
  [then]
