...
[ifndef] TIM4_DEF
  ...
  [ifdef] TIM4_TIM4_SMCR_DEF
    ...
    $03 constant TIM4_OCCS                      \ [0x03] OCREF clear selection This bit is used to select the OCREF clear source Note: If the OCREF clear selection feature is not supported, this bit is reserved and forced by hardware to 0. Section 65.3: TIM2/TIM3/TIM4/TIM5 implementation.
  ...
  [ifdef] TIM4_TIM4_CCMR1_OUTPUT_DEF
    ...
    $04 constant TIM4_OC1M                      \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 is derived. tim_oc1ref is active high whereas tim_oc1 active level depends on CC1P bit. Note: In PWM mode, the tim_ocref_clr level changes only when the result of the comparison changes or when the output compare mode switches from frozen mode to PWM mode.
  ...
  [ifdef] TIM4_TIM4_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_CNT                       \ [0x00 : 31] Least significant part of counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register holds the non-dithered part in CNT[30:0]. The fractional part is not available.
    $1f constant TIM4_UIFCPY_CNT                \ [0x1f] Value depends on IUFREMAP in TIMx_CR1. If UIFREMAP = 0 CNT[31]: Most significant bit of counter value If UIFREMAP = 1 UIFCPY: UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register
  [then]
  ...
  [ifdef] TIM4_TIM4_ECR_DEF
    ...
    $10 constant TIM4_PW                        \ [0x10 : 8] Pulse width This bitfield defines the pulse duration, as following: t<sub>PW</sub> = PW[7:0] x t<sub>PWG</sub>
    $18 constant TIM4_PWPRSC                    \ [0x18 : 3] Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: t<sub>PWG</sub> = (2<sup>(PWPRSC[2:0])</sup>) x t<sub>tim_ker_ck</sub>
  [then]
  ...
  [ifdef] TIM4_TIM4_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM4_TI1SEL                    \ [0x00 : 4] Selects tim_ti1[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $08 constant TIM4_TI2SEL                    \ [0x08 : 4] Selects tim_ti2[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $10 constant TIM4_TI3SEL                    \ [0x10 : 4] Selects tim_ti3[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
    $18 constant TIM4_TI4SEL                    \ [0x18 : 4] Selects tim_ti4[15:0] input ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
  ...
  [ifdef] TIM4_TIM4_AF1_DEF
    ...
    \ Reset value: 0x00000000
    \
    $0e constant TIM4_ETRSEL                    \ [0x0e : 4] etr_in source selection These bits select the etr_in input source. ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
  ...
  [ifdef] TIM4_TIM4_AF2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant TIM4_OCRSEL                    \ [0x10 : 3] ocref_clr source selection These bits select the ocref_clr input source. ... Refer to Section 65.4.2: TIM2/TIM3/TIM4/TIM5 pins and internal signals for product specific implementation.
  [then]
