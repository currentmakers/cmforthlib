...
[ifndef] CEC_DEF
  ...
  [ifdef] CEC_CEC_CFGR_DEF
    ...
    $07 constant CEC_BRDNOGEN                   \ [0x07] Avoid error-bit generation in broadcast The BRDNOGEN bit is set and cleared by software. error-bit on the CEC line. LBPE detection with LBPEGEN = 0 on a broadcast message generates an error-bit on the CEC line.
    $08 constant CEC_SFTOP                      \ [0x08] SFT option bit The SFTOPT bit is set and cleared by software.
  ...
  [ifdef] CEC_CEC_TXDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant CEC_TXD                        \ [0x00 : 8] Tx data TXD is a write-only register containing the data byte to be transmitted.
  [then]
  ...
  [ifdef] CEC_CEC_RXDR_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant CEC_RXD                        \ [0x00 : 8] Rx data RXD is read-only and contains the last data byte that has been received from the CEC line.
  [then]
  ...
  $04 constant CEC_CEC_CFGR             \ CEC configuration register
