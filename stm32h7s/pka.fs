...
$00000001 constant PKA_PKA_CR_EN                                    \ PKA enable. When an illegal operation is selected while EN=1 OPERRF bit is set in PKA_SR. See PKA_CR.MODE bitfield for details. Note: When EN=0 PKA RAM can still be accessed by the application.
...
\ @brief Public key accelerator
\
$48022000 constant PKA_PKA_CR     \ offset: 0x00 : PKA control register
$48022004 constant PKA_PKA_SR     \ offset: 0x04 : PKA status register
$48022008 constant PKA_PKA_CLRFR  \ offset: 0x08 : PKA clear flag register
