...
[ifndef] TIM17_DEF
  ...
  [ifdef] TIM17_TIM17_CR1_DEF
    ...
    $08 constant TIM17_CKD                      \ [0x08 : 2] Clock division This bit-field indicates the division ratio between the timer clock (tim_ker_ck) frequency and the dead-time and sampling clock (t<sub>DTS</sub>)used by the dead-time generators and the digital filters (tim_tix),
    $0b constant TIM17_UIFREMAP                 \ [0x0b] UIF status bit remapping
    $0c constant TIM17_DITHEN                   \ [0x0c] Dithering enable Note: The DITHEN bit can only be modified when CEN bit is reset.
  [then]
  ...
  [ifdef] TIM17_TIM17_CR2_DEF
    ...
    $08 constant TIM17_OIS1                     \ [0x08] Output Idle state 1 (tim_oc1 output) Note: This bit can not be modified as long as LOCK level 1, 2 or 3 has been programmed (LOCK bits in TIMx_BKR register).
    $09 constant TIM17_OIS1N                    \ [0x09] Output Idle state 1 (tim_oc1n output) Note: This bit can not be modified as long as LOCK level 1, 2 or 3 has been programmed (LOCK bits in TIMx_BKR register).
  [then]
  ...
  [ifdef] TIM17_TIM17_SR_DEF
    ...
    $05 constant TIM17_COMIF                    \ [0x05] COM interrupt flag This flag is set by hardware on a COM event (once the capture/compare control bits CCxE, CCxNE, OCxM have been updated). It is cleared by software.
    $07 constant TIM17_BIF                      \ [0x07] Break interrupt flag This flag is set by hardware as soon as the tim_brk input goes active. It can be cleared by software if the break input is not active.
  ...
  [ifdef] TIM17_TIM17_CCMR1_INPUT_DEF
    ...
    $02 constant TIM17_IC1PSC                   \ [0x02 : 2] Input capture 1 prescaler This bit-field defines the ratio of the prescaler acting on CC1 input (tim_ic1). The prescaler is reset as soon as CC1E=0 (TIMx_CCER register).
    $04 constant TIM17_IC1F                     \ [0x04 : 4] Input capture 1 filter This bit-field defines the frequency used to sample tim_ti1 input and the length of the digital filter applied to tim_ti1. The digital filter is made of an event counter in which N consecutive events are needed to validate a transition on the output:
  [then]
  ...
  [ifdef] TIM17_TIM17_CCMR1_OUTPUT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM17_CC1S                     \ [0x00 : 2] Capture/Compare 1 selection This bit-field defines the direction of the channel (input/output) as well as the used input. Note: CC1S bits are writable only when the channel is OFF (CC1E = 0 in TIM15_CCER).
    ...
    $03 constant TIM17_OC1PE                    \ [0x03] Output Compare 1 preload enable Note: These bits can not be modified as long as LOCK level 3 has been programmed (LOCK bits in TIM15_BDTR register) and CC1S=00 (the channel is configured in output).
    $04 constant TIM17_OC1M                     \ [0x04 : 3] OC1M[2:0]: Output compare 1 mode These bits define the behavior of the output reference signal tim_oc1ref from which tim_oc1 and tim_oc1n are derived. tim_oc1ref is active high whereas tim_oc1 and tim_oc1n active level depends on CC1P and CC1NP bits. Note: These bits can not be modified as long as LOCK level 3 has been programmed (LOCK bits in TIM15_BDTR register) and CC1S=00 (the channel is configured in output). Note: In PWM mode, the tim_ocxref level changes only when the result of the comparison changes or when the output compare mode switches from frozen mode to PWM mode. Note: On channels that have a complementary output, this bit field is preloaded. If the CCPC bit is set in the TIM15_CR2 register then the OC1M active bits take the new value from the preloaded bits only when a COM event is generated.
    $07 constant TIM17_OC1CE                    \ [0x07] Output compare 1 clear enable
    $08 constant TIM17_CC2S                     \ [0x08 : 2] Capture/Compare 2 selection This bit-field defines the direction of the channel (input/output) as well as the used input. Note: CC2S bits are writable only when the channel is OFF (CC2E = 0 in TIM15_CCER).
    $0a constant TIM17_OC2FE                    \ [0x0a] Output compare 2 fast enable
    $0b constant TIM17_OC2PE                    \ [0x0b] Output compare 2 preload enable
    $0c constant TIM17_OC2M                     \ [0x0c : 3] OC2M[2:0]: Output compare 2 mode
    $0f constant TIM17_OC2CE                    \ [0x0f] Output compare 2 clear enable
    $10 constant TIM17_OC1M_1                   \ [0x10] OC1M[3]
    $18 constant TIM17_OC2M_1                   \ [0x18] OC2M[3]
  [then]
  ...
  [ifdef] TIM17_TIM17_CCER_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM17_CC1E                     \ [0x00] Capture/Compare 1 output enable When CC1 channel is configured as output, the OC1 level depends on MOE, OSSI, OSSR, OIS1, OIS1N and CC1NE bits, regardless of the CC1E bits state. Refer to Table 680 for details.
  ...
  [ifdef] TIM17_TIM17_CNT_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM17_CNT                      \ [0x00 : 16] Counter value Non-dithering mode (DITHEN = 0) The register holds the counter value. Dithering mode (DITHEN = 1) The register only holds the non-dithered part in CNT[15:0]. The fractional part is not available.
    $1f constant TIM17_UIFCPY                   \ [0x1f] UIF Copy This bit is a read-only copy of the UIF bit of the TIMx_ISR register. If the UIFREMAP bit in TIMx_CR1 is reset, bit 31 is reserved.
  [then]
  ...
  [ifdef] TIM17_TIM17_BDTR_DEF
    ...
    $0c constant TIM17_BKE                      \ [0x0c] Break enable 1; Break inputs (tim_brk and tim_sys_brk event) enabled Note: This bit cannot be modified when LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Note: Any write operation to this bit takes a delay of 1 APB clock cycle to become effective.
    $0d constant TIM17_BKP                      \ [0x0d] Break polarity Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register). Note: Any write operation to this bit takes a delay of 1 APB clock cycle to become effective.
  ...
  [ifdef] TIM17_TIM17_TISEL_DEF
    ...
    \ Reset value: 0x00000000
    \
    $00 constant TIM17_TI1SEL                   \ [0x00 : 4] selects tim_ti1_in[15:0] input ... Refer to Section 68.4.2: TIM15/TIM16/TIM17 pins and internal signals for interconnects list.
  [then]
  ...
  [ifdef] TIM17_TIM17_AF1_DEF
    ...
    \
    $00 constant TIM17_BKINE                    \ [0x00] TIMx_BKIN input enable This bit enables the TIMx_BKIN alternate function input for the timers tim_brk input. TIMx_BKIN input is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $01 constant TIM17_BKCMP1E                  \ [0x01] tim_brk_cmp1 enable This bit enables the tim_brk_cmp1 for the timers tim_brk input. tim_brk_cmp1 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $02 constant TIM17_BKCMP2E                  \ [0x02] tim_brk_cmp2 enable This bit enables the tim_brk_cmp2 for the timers tim_brk input. tim_brk_cmp2 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $03 constant TIM17_BKCMP3E                  \ [0x03] tim_brk_cmp3 enable This bit enables the tim_brk_cmp3 for the timers tim_brk input. tim_brk_cmp3 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $04 constant TIM17_BKCMP4E                  \ [0x04] tim_brk_cmp4 enable This bit enables the tim_brk_cmp4 for the timers tim_brk input. tim_brk_cmp4 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $05 constant TIM17_BKCMP5E                  \ [0x05] tim_brk_cmp5 enable This bit enables the tim_brk_cmp5 for the timers tim_brk input. tim_brk_cmp5 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $06 constant TIM17_BKCMP6E                  \ [0x06] tim_brk_cmp6 enable This bit enables the tim_brk_cmp6 for the timers tim_brk input. tim_brk_cmp6 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $07 constant TIM17_BKCMP7E                  \ [0x07] tim_brk_cmp7 enable This bit enables the tim_brk_cmp7 for the timers tim_brk input. tim_brk_cmp7 output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $08 constant TIM17_BKCMP8E                  \ [0x08] tim_brk_cmp8 enable This bit enables the tim_brk_cmp8 for the timers tim_brk input. mdf_brkx output is ORed with the other tim_brk sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $09 constant TIM17_BKINP                    \ [0x09] TIMx_BKIN input polarity This bit selects the TIMx_BKIN alternate function input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $0a constant TIM17_BKCMP1P                  \ [0x0a] tim_brk_cmp1 input polarity This bit selects the tim_brk_cmp1 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $0b constant TIM17_BKCMP2P                  \ [0x0b] tim_brk_cmp2 input polarity This bit selects the tim_brk_cmp2 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $0c constant TIM17_BKCMP3P                  \ [0x0c] tim_brk_cmp3 input polarity This bit selects the tim_brk_cmp3 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    $0d constant TIM17_BKCMP4P                  \ [0x0d] tim_brk_cmp4 input polarity This bit selects the tim_brk_cmp4 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
  [then]
  ...
  [ifdef] TIM17_TIM17_AF2_DEF
    ...
    \ Reset value: 0x00000000
    \
    $10 constant TIM17_OCRSEL                   \ [0x10 : 3] tim_ocref_clr source selection These bits select the tim_ocref_clr input source. Refer to Section 68.4.2: TIM15/TIM16/TIM17 pins and internal signals for product specific implementation. Note: These bits can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
  [then]
  ...
  [ifdef] TIM17_TIM17_DCR_DEF
    ...
    $10 constant TIM17_DBSS                     \ [0x10 : 4] DMA burst source selection This bitfield defines the interrupt source that triggers the DMA burst transfers (the timer recognizes a burst transfer when a read or a write access is done to the TIMx_DMAR address). Other: reserved
  [then]
