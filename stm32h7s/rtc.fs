...
$00080000 constant RTC_RTC_CR_COSEL                                 \ Calibration output selection When COE = 1, this bit selects which signal is output on CALIB. These frequencies are valid for RTCCLK at 32.768 kHz and prescalers at their default values (PREDIV_A = 127 and PREDIV_S = 255). Refer to Section 45.3.17: Calibration clock output.
...
$80000000 constant RTC_RTC_CR_OUT2EN                                \ RTC_OUT2 output enable With this bit set, the RTC outputs can be remapped on RTC_OUT2 as follows: OUT2EN = 0: RTC output 2 disable If OSEL different from 00 or TAMPOE = 1: TAMPALRM is output on RTC_OUT1 If OSEL = 00 and TAMPOE = 0 and COE = 1: CALIB is output on RTC_OUT1 OUT2EN = 1: RTC output 2 enable If (OSEL different from 00 or TAMPOE = 1) and COE = 0: TAMPALRM is output on RTC_OUT2 If OSEL = 00 and TAMPOE = 0 and COE = 1: CALIB is output on RTC_OUT2 If (OSEL different from 00 or TAMPOE = 1) and COE = 1: CALIB is output on RTC_OUT2 and TAMPALRM is output on RTC_OUT1.
...
$000001ff constant RTC_RTC_CALR_CALM                                \ Calibration minus The frequency of the calendar is reduced by masking CALM out of 2<sup>20</sup> RTCCLK pulses (32 seconds if the input frequency is 32768 Hz). This decreases the frequency of the calendar with a resolution of 0.9537 ppm. To increase the frequency of the calendar, this feature should be used in conjunction with CALP. See Section 45.3.15: RTC smooth digital calibration on page 2349.
$00001000 constant RTC_RTC_CALR_LPCAL                               \ RTC low-power mode
$00002000 constant RTC_RTC_CALR_CALW16                              \ Use a 16-second calibration cycle period When CALW16 is set to 1, the 16-second calibration cycle period is selected. This bit must not be set to 1 if CALW8 = 1. Note: CALM[0] is stuck at 0 when CALW16 = 1. Refer to Section 45.3.15: RTC smooth digital calibration.
$00004000 constant RTC_RTC_CALR_CALW8                               \ Use an 8-second calibration cycle period When CALW8 is set to 1, the 8-second calibration cycle period is selected. Note: CALM[1:0] are stuck at 00 when CALW8 = 1. Refer to Section 45.3.15: RTC smooth digital calibration.
...
$58004008 constant RTC_RTC_SSR    \ offset: 0x08 : RTC subsecond register
...
$5800401c constant RTC_RTC_PRIVCFGR  \ offset: 0x1C : RTC privilege mode control register
...
$58004038 constant RTC_RTC_TSSSR  \ offset: 0x38 : RTC timestamp subsecond register
$58004040 constant RTC_RTC_ALRMAR  \ offset: 0x40 : RTC alarm A register
$58004044 constant RTC_RTC_ALRMASSR  \ offset: 0x44 : RTC alarm A subsecond register
$58004048 constant RTC_RTC_ALRMBR  \ offset: 0x48 : RTC alarm B register
$5800404c constant RTC_RTC_ALRMBSSR  \ offset: 0x4C : RTC alarm B subsecond register
...
$58004070 constant RTC_RTC_ALRABINR  \ offset: 0x70 : RTC alarm A binary mode register
$58004074 constant RTC_RTC_ALRBBINR  \ offset: 0x74 : RTC alarm B binary mode register
