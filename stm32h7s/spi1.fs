...
$0000001f constant SPI1_SPI_CFG1_DSIZE                              \ number of bits in at single SPI data frame ..... Maximum data size can be limited up to 16-bits at some instances. At instances with limited set of features, DSIZE[2:0] bits are reserved and must be kept at reset state. DSIZE[4:3] bits then control next settings of data size: 00xxx: 8-bits 01xxx: 16-bits 10xxx: 24-bits Note: 11xxx: 32-bits.
...
$00000200 constant SPI1_SPI_CFG1_UDRCFG                             \ behavior of slave transmitter at underrun condition For more details see Figure 962: Optional configurations of slave detecting underrun condition.
...
$70000000 constant SPI1_SPI_CFG1_MBR                                \ master baud rate prescaler setting Note: MBR setting is considered at slave working at TI mode, too (see Section 80.5.1: TI mode).
...
$00002000 constant SPI1_SPI_CFG2_RDIOM                              \ RDY signal input/output management Note: When DSIZE at the SPI_CFG1 register is configured shorter than 8-bit, the RDIOM bit must be kept at zero.
$00004000 constant SPI1_SPI_CFG2_RDIOP                              \ RDY signal input/output polarity
$00008000 constant SPI1_SPI_CFG2_IOSWP                              \ swap functionality of MISO and MOSI pins When this bit is set, the function of MISO and MOSI pins alternate functions are inverted. Original MISO pin becomes MOSI and original MOSI pin becomes MISO. Note: This bit can be also used in PCM and I2S modes to swap SDO and SDI pins.
...
$00000001 constant SPI1_SPI_SR_RXP                                  \ Rx-packet available The flag is changed by hardware. It monitors the total number of data currently available at RxFIFO if SPI is enabled. RXP value depends on the FIFO threshold (FTHLV[3:0]), data frame size (DSIZE[4:0] in SPI mode and DATLEN[1:0] in I2S/PCM mode), and actual communication flow. If the data packet is read by performing consecutive read operations from SPI_RXDR, RXP flag must be checked again once a complete data packet is read out from RxFIFO.
$00000002 constant SPI1_SPI_SR_TXP                                  \ Tx-packet space available TXP flag can be changed only by hardware. Its value depends on the physical size of the FIFO and its threshold (FTHLV[3:0]), data frame size (DSIZE[4:0] in SPI mode and respective DATLEN[1:0] in I2S/PCM mode), and actual communication flow. If the data packet is stored by performing consecutive write operations to SPI_TXDR, TXP flag must be checked again once a complete data packet is stored at TxFIFO. TXP is set despite SPI TxFIFO becomes inaccessible when SPI is reset or disabled.
...
$00000020 constant SPI1_SPI_SR_UDR                                  \ underrun This bit is cleared when SPI is re-enabled or by writing 1 to UDRC bit of SPI_IFCR optionally. Note: In SPI mode, the UDR flag applies to Slave mode only. In I2S/PCM mode, (when available) this flag applies to Master and Slave mode
$00000040 constant SPI1_SPI_SR_OVR                                  \ overrun This bit is cleared when SPI is re-enabled or by writing 1 to OVRC bit of SPI_IFCR optionally.
$00000080 constant SPI1_SPI_SR_CRCE                                 \ CRC error This bit is cleared when SPI is re-enabled or by writing 1 to CRCEC bit of SPI_IFCR optionally.
$00000100 constant SPI1_SPI_SR_TIFRE                                \ TI frame format error This bit is cleared by writing 1 to TIFREC bit of SPI_IFCR exclusively.
$00000200 constant SPI1_SPI_SR_MODF                                 \ mode fault When MODF is set, SPE and IOLOCK bits of SPI_CR1 register are reset and setting SPE again is blocked until MODF is cleared. This bit is cleared by writing 1 to MODFC bit of SPI_IFCR exclusively.
...
$ffff0000 constant SPI1_SPI_SR_CTSIZE                               \ number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus . Note: CTSIZE[15:0] bits are not available in instances with limited set of features.
...
$00000000 constant SPI1_SPI_RXDR_RXDR                               \ receive data register The register serves as an interface with RxFIFO. When it is read, RxFIFO is accessed. Note: In SPI mode, data is always right-aligned. Alignment of data at I2S mode depends on DATLEN and DATFMT setting. Unused bits are read as zero when the register is read. Writing to the register is ignored. Note: DR can be accessed byte-wise (8-bit access): in this case only one data-byte is read by single access Note: halfword-wise (16 bit access) in this case 2 data-bytes or 1 halfword-data can be read by single access Note: word-wise (32 bit access). In this case 4 data-bytes or 2 halfword-data or word-data can be read by single access. Note: Read access of this register less than the configured data size is forbidden.
...
$00000030 constant SPI1_SPI_I2SCFGR_I2SSTD                          \ I<sup>2</sup>S standard selection For more details on I<sup>2</sup>S standards, refer to Section 80.9.5: Supported audio protocols
$00000080 constant SPI1_SPI_I2SCFGR_PCMSYNC                         \ PCM frame synchronization
$00000300 constant SPI1_SPI_I2SCFGR_DATLEN                          \ data length to be transferred. Data width of 24 and 32 bits are not always supported, (DATLEN = 01 or 10), refer to Section 80.3: SPI implementation to check the supported data size.
$00000400 constant SPI1_SPI_I2SCFGR_CHLEN                           \ channel length (number of bits per audio channel)
...
\ @brief Serial peripheral interface
\
$42003000 constant SPI1_SPI_CR1   \ offset: 0x00 : SPI/I2S control register 1
$42003004 constant SPI1_SPI_CR2   \ offset: 0x04 : SPI/I2S control register 2
$42003008 constant SPI1_SPI_CFG1  \ offset: 0x08 : SPI/I2S configuration register 1
$4200300c constant SPI1_SPI_CFG2  \ offset: 0x0C : SPI/I2S configuration register 2
$42003010 constant SPI1_SPI_IER   \ offset: 0x10 : SPI/I2S interrupt enable register
$42003014 constant SPI1_SPI_SR    \ offset: 0x14 : SPI/I2S status register
$42003018 constant SPI1_SPI_IFCR  \ offset: 0x18 : SPI/I2S interrupt/status flags clear register
$42003020 constant SPI1_SPI_TXDR  \ offset: 0x20 : SPI/I2S transmit data register
$42003030 constant SPI1_SPI_RXDR  \ offset: 0x30 : SPI/I2S receive data register
$42003040 constant SPI1_SPI_CRCPOLY  \ offset: 0x40 : SPI/I2S polynomial register
$42003044 constant SPI1_SPI_TXCRC  \ offset: 0x44 : SPI/I2S transmitter CRC register
$42003048 constant SPI1_SPI_RXCRC  \ offset: 0x48 : SPI/I2S receiver CRC register
$4200304c constant SPI1_SPI_UDRDR  \ offset: 0x4C : SPI/I2S underrun data register
$42003050 constant SPI1_SPI_I2SCFGR  \ offset: 0x50 : SPI/I2S configuration register
