...
$00000001 constant AES_AES_CR_EN                                    \ Enable This bit enables/disables the AES peripheral. At any moment, clearing then setting the bit re-initializes the AES peripheral. This bit is automatically cleared by hardware upon the completion of the key preparation (MODE[1:0] at 0x1) and upon the completion of GCM/GMAC/CCM initialization phase. The bit cannot be set as long as KEYVALID1is cleared
$00000006 constant AES_AES_CR_DATATYPE                              \ Data type This bitfield defines the format of data written in the AES_DINR register or read from the AES_DOUTR register, through selecting the mode of data swapping. This swapping is defined in Section121.4.14: AES data registers and data swapping. Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
$00000018 constant AES_AES_CR_MODE                                  \ Operating mode This bitfield selects the AES operating mode: Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
$00000060 constant AES_AES_CR_CHMOD                                 \ CHMOD[1:0]: Chaining mode This bitfield selects the AES chaining mode: others: Reserved Attempts to write the bitfield are ignored when EN is set before the write access and it is not cleared by that write access.
$00000800 constant AES_AES_CR_DMAINEN                               \ DMA input enable This bit enables automatic generation of DMA requests during the data phase, for incoming data transfers to AES via DMA. Setting this bit is ignored when MODE[1:0] is at 0x1 (key derivation).
$00001000 constant AES_AES_CR_DMAOUTEN                              \ DMA output enable This bit enables automatic generation of DMA requests during the data phase, for outgoing data transfers from AES via DMA. Setting this bit is ignored when MODE[1:0] is at 0x1 (key derivation).
$00006000 constant AES_AES_CR_GCMPH                                 \ GCM or CCM phase selection This bitfield selects the phase, applicable only with GCM, GMAC or CCM chaining modes.
$00010000 constant AES_AES_CR_CHMOD_1                               \ CHMOD[2]
$00040000 constant AES_AES_CR_KEYSIZE                               \ Key size selection This bitfield defines the key length in bits of the key used by AES. Attempts to write the bit are ignored when the EN is set before the write access and it is not cleared by that write access.
$00f00000 constant AES_AES_CR_NPBLB                                 \ Number of padding bytes in last block This padding information must be filled by software before processing the last block of GCM payload encryption or CCM payload decryption, otherwise authentication tag computation is incorrect. ...
...
$00000002 constant AES_AES_SR_RDERRF                                \ Read error flag This bit is set when an unexpected read to the AES_DOUTR register occurred. When set RDERRF bit has no impact on the AES operations. The flag setting generates an interrupt if the RWEIE bit of the AES_IER register is set. The flag is cleared by setting the RWEIF bit of the AES_ICR register.
$00000004 constant AES_AES_SR_WRERRF                                \ Write error flag This bit is set when an unexpected write to the AES_DINR register occurred. When set WRERRF bit has no impact on the AES operations. The flag setting generates an interrupt if the RWEIE bit of the AES_IER register is set. The flag is cleared by setting the RWEIF bit of the AES_ICR register.
$00000008 constant AES_AES_SR_BUSY                                  \ Busy This flag indicates whether AES is idle or busy. AES is flagged as idle when disabled (when EN is low) or when the last processing is completed. AES is flagged as busy when processing a block data, preparing a key (ECB or CBC decryption only). When GCM encryption is selected, this flag must be at zero before suspending current process to manage a higher-priority message.
...
$00000000 constant AES_AES_KEYR1_KEY                                \ Cryptographic key, bits [63:32] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR2_KEY                                \ Cryptographic key, bits [95:64] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR3_KEY                                \ Cryptographic key, bits [127:96] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR4_KEY                                \ Cryptographic key, bits [159:128] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR5_KEY                                \ Cryptographic key, bits [191:160] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR6_KEY                                \ Cryptographic key, bits [223:192] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_KEYR7_KEY                                \ Cryptographic key, bits [255:224] Refer to the AES_KEYR0 register for description of the KEY[255:0] bitfield and for information relative to writing AES_KEYRx registers.
...
$00000000 constant AES_AES_SUSPR7_SUSP                              \ Suspend data AES_SUSPRx registers contain the complete internal register states of the AES when the GCM, GMAC or CCM processing of the current task is suspended to process a higher-priority task. Refer to Section121.4.8: AES suspend and resume operations for more details. Read to this register returns zero when EN bit is cleared in AES_SR register. AES_SUSPRx registers are not used in other chaining modes than GCM, GMAC or CCM.
...
$00000001 constant AES_AES_ICR_CCF                                  \ Computation complete flag clear Setting this bit clears the CCF status bit of the AES_ISR register.
$00000002 constant AES_AES_ICR_RWEIF                                \ Read or write error interrupt flag clear Setting this bit clears the RWEIF status bit of the AES_ISR register, and clears both RDERRF and WRERRF flags in the AES_SR register.
...
$40026040 constant AES_AES_SUSPR0  \ offset: 0x40 : AES suspend registers
$40026044 constant AES_AES_SUSPR1  \ offset: 0x44 : AES suspend registers
$40026048 constant AES_AES_SUSPR2  \ offset: 0x48 : AES suspend registers
$4002604c constant AES_AES_SUSPR3  \ offset: 0x4C : AES suspend registers
$40026050 constant AES_AES_SUSPR4  \ offset: 0x50 : AES suspend registers
$40026054 constant AES_AES_SUSPR5  \ offset: 0x54 : AES suspend registers
$40026058 constant AES_AES_SUSPR6  \ offset: 0x58 : AES suspend registers
$4002605c constant AES_AES_SUSPR7  \ offset: 0x5C : AES suspend registers
$40026300 constant AES_AES_IER    \ offset: 0x300 : AES interrupt enable register
$40026304 constant AES_AES_ISR    \ offset: 0x304 : AES interrupt status register
$40026308 constant AES_AES_ICR    \ offset: 0x308 : AES interrupt clear register
